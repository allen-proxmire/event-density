...
## Two further weapon findings (2026-07-24, real-source checks)

- **The second-harmonic "fingerprint" is NOT cleanly distinctive as stated — likely confounded by conventional interferometer optics.** The proposal claims "3–6% second harmonic; decoherence predicts none (pure first harmonic)." But Fein 2019 uses a **Kapitza-Dirac-Talbot-Lau** interferometer whose middle grating is a standing-light-wave *phase grating*, and Talbot-Lau/KDTL interferometers **conventionally produce higher harmonics** (documented: "high visibility at the fifth and sixth spatial harmonics" — quantumnano.at; Nimmrichter–Hornberger KDTL theory, arXiv:0902.0234). So a second harmonic is *expected* from the optics — finding one would not confirm ED. The "decoherence predicts none" baseline is naive. A real ED test would require **excess** second harmonic beyond the full Nimmrichter–Hornberger KDTL prediction, using raw fringe data (Fein 2019 publishes only *fitted visibilities*, not raw fringes or a harmonic decomposition — so it can't be checked from that paper). **Correction to a prior recommendation:** this is NOT an easy near-term test; it has a distinctiveness problem and needs the full KDTL model + raw data.
- **ED does not out-predict Penrose on the gravitational collapse rate.** Both give `τ ≈ ℏ/E_G`; ED *reproduces* Penrose's scaling but leaves the coefficient and energy functional **open** ("`E_G` identified with Penrose's answer, not computed" — `Paper_ED_StateReduction_vs_CollapseModels.md`), so ED is *less* predictive than Penrose there (Penrose computes `E_G` parameter-free; ED borrows it). Penrose's parameter-free version was already tested and the simplest form **excluded** by the 2020 Gran Sasso spontaneous-radiation experiment; ED survives that bound (discrete channel-projector collapse doesn't radiate that way) — but the corpus's own verdict is "immunity, not a win; standard QM predicts the same null; ED's distinction is structural, not a new observable." **Net: the collapse-rate turf is Penrose's, not ED's; ED's only-ED lane remains the arrangement-dependent multiplicity wall (map-gated), not the collapse rate.**
