...
> **ATTEMPTED SAME DAY — AND NOT MET.** `Note_wN_Attempt_2026-09-06.md`. **No mechanism computes `w(N)`.** Three candidates were fixed from the primitives before computing — simplex sharing (P04 + `P-Locus-Bandwidth-Bound`), random-phase superposition (P11 at the limit), and maximum entropy — and **all three fail, in complementary ways**: the simplex gives real `N`-dependence in the right direction but reaches `CV = 1` only as `N → ∞`; the exponential gives `CV = 1` **exactly**, the lepton value, but is `N`-independent and so predicts `w = √2` for every family, **falsified by the quarks**. **One supplies the value and precludes the variation; the other supplies the variation and precludes the value.** **So this line still has an ORDERING and has NOT been upgraded to a value — do not describe it as though it had.**
