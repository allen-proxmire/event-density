...
**Status:** Scoping memo + one candidate analytic result, 2026-07-05. Target = the corpus dive's #2 cross-arc open item: the V5 cross-chain kernel (`Paper_090`) is postulated everywhere and derived nowhere; it is the single load-bearer under entanglement's Tsirelson/PR-box exclusion (`Paper_069` P-V5-Hilbert-Constraint), Schmidt-genericity (063/064), monogamy's budget (065), q-compute's M_cap/N_V5, and the SCBU multiplicity anchor. **Discipline note:** every load-bearing claim below is quoted from its source; the one new move (the reduction in §3) is labeled a CANDIDATE and uses only standard math (Tsirelson 1980) on structures the corpus itself already commits to. Nothing is folded back into Paper_069 yet; this memo is offered for the adversarial cross-check first.
...
## 3. The candidate reduction (the new move — CANDIDATE, not yet a result)
