# Paper outline — The Relational Tick: Mass as Carried Memory, and V5 as the Finite-Reach Binder of Local Proper Time
...
## Preamble — what this paper does NOT claim (write first, reconcile the abstract against it)

1. It does **not** derive the value of any mass, coupling, or timescale. All are form-level results with inherited values (the standing ED pattern).
2. It does **not** derive, from the 13 primitives, that the substrate must carry the V5 cross-chain coupling. V5 is implemented as an honestly-named structural addition, faithful to Paper_090's stated form (finite reach, finite memory, retarded, gauge-covariant phase), but the forward primitive derivation of V5 remains OPEN. This paper characterizes what V5 *does* and what structure it *must* have, not that the primitives *force* it.
3. It does **not** claim the memory channel or the V5 coupling are already present in the certified reference substrate. Both are additions the certified base lacks (the certified base is ballistic-or-extinct, Sigma-driven by rho only, with no chain-specific memory and no cross-chain coupling). Their necessity is argued from the physics they produce, not derived from primitives.
4. It does **not** claim a Higgs mechanism, EWSB, gauge-boson mass, or a condensate. The mass result here is intrinsic-inertia-like (a chain's own carried memory slows it), not the SM Higgs sector.
5. It does **not** claim to reproduce quantitative stellar pulsation, quantitative time dilation, or the quantitative Zeno rate. The connections to those are structural (same mechanism, right qualitative behavior), not fits.
...
- Source: `Dwell_Intrinsic_Memory_Results.md`, `dwell_channel_mass_probe.py`, `dwell_intrinsic_memory_probe.py`.
- The certified substrate is ballistic-or-extinct: a front advances one hop or dies, no dispersion (slowed-but-surviving) mode. This is why the two live Higgs candidates (H1, H2) both failed (E1 result; H1_Leg_Scoping).
- The missing ingredient (diagnosed): the certified substrate has no chain-specific state that is both carried with the chain AND read by its dynamics (orientation is carried but Sigma-blind; rho is read but chain-anonymous).
- Add exactly that: a chain-carried "history" value that the movement rule reads. With a fading (finite-window) update, it produces a **stable, bounded, surviving sub-ballistic rate** — a real intrinsic-inertia / dispersion-gap signature — with no external reference position (intrinsic to the chain, as mass should be).
- Tier: CANDIDATE, positive, confirmed over long runs. Requires the added memory channel (honestly named). Not FORCED, not the SM Higgs.
- Honest audit table row: the memory channel = P (structural addition); the resulting sub-ballistic rate = D-conditional-on-the-channel.

## §3. The memory parameter, and where it bottoms out

- Sources: `Dwell_Intrinsic_Memory_Results.md` (passes 5-7), the ħ / G-formula dimensional check.
- The mechanism has two parameters (coupling strength, fade rate); shown they are not independent (only their product is constrained) — effectively one free quantity.
- The fade rate is not a new unknown: it is V1's own already-inherited memory time (Paper_089's kernel is literally a chain's response to its own earlier state), now attached to working code for the first time.
- The book-level framing "ħ = memory" is consistent with the closed result G = c³ℓ_P²/ℏ read as (speed of influence)·(grain)²/(memory). Dimensional check (verified): c, ℓ_P, ℏ alone cannot form a pure number; a fourth ingredient (a rate) is required. The natural candidate, the commitment rate Γ_commit, reduces to c/ℓ_P × k₁₁ — not dimensionally independent; the sole remaining unknown is the O(1) factor k₁₁ that recurs across mass, GR α₁, QC decoherence, and QFT vertices.
- Tier: form-FORCED, value-INHERITED (the standing pattern). The reduction to a single O(1) unknown (k₁₁) is a real STRUCTURAL narrowing, not a closure.
- Cross-connection (CANDIDATE, `Mass_GR_SparseCommitment_CrossConnection.md`): the mass-memory fade rate and the GR sparse-commitment parameter share the same commitment-reserve band, the same k₁₁, the same ρ_event/ρ_Planck scaling — plausibly two faces of one underived quantity. The GR side's band dependency was checked and found canonical (P02-vs-P11), not archived.

## §4. Persistence is rhythmic: finite memory gives a real tick

- Sources: `Dwell_Memory_Rhythm_Results.md`, `dwell_memory_rhythm_probe.py`.
- A single chain cannot oscillate spatially (P11 forbids reversal), so the testable core is temporal. Finite memory produces a **quantized, tunable, noise-robust hop-stride** (exact period 2 or 3; survives noise to 60% of signal scale before a sharp order-disorder transition).
...
- Sources: `V5_Synchronization_Characterization.md` (phase model + real-substrate confirmation), `v5_synchronization_probe.py`, `v5_substrate_coupling_probe.py`.
- V5's known structure (Paper_090): retarded, finite cross-chain reach ℓ_V5, finite memory τ_V5, gauge-covariant relative phase. Finite reach is the defining feature.
- **Result (phase model, then confirmed in the real certified substrate sim):** with finite reach smaller than the inter-group distance, nearby chains with genuinely different natural rates LOCK to a shared rate (local proper time, a composite rest frame — within-cluster spread collapses ~99% in the real sim), WHILE distant groups keep their different rates (time dilation preserved, cross-group rate-gap intact). With reach at or beyond the separation, everything locks and the rate-gap collapses to zero — a universal clock, forbidden by §1.
- **So V5's finiteness is load-bearing for reality:** it is exactly what gives local clocks (bound composites) without a universal one. A bound cluster sharing a proper time IS a composite rest frame — which connects §6 back to §2: mass (carried-memory inertia, intrinsic) and composite rest frames (V5-bound shared clocks) are two faces of the same "chains acquiring a shared local tick."
...
- The forward primitive derivation of V5 (why the substrate must carry it) — the hard open core.
- Whether the memory channel and V5 coupling can be reduced to existing primitives rather than added (both are honest additions here).
- The k₁₁ O(1) factor: derive it once, it cascades across mass, GR α₁, QC decoherence, QFT vertices.
- Full phase-wave / de Broglie behavior (phase is Sigma-blind in the certified substrate; the rhythm is temporal only).
- The mass↔GR sparse-commitment identity (CANDIDATE: same band, same k₁₁, same scaling; not proven a single scalar).
- Collective pulsation in an OPEN/throughput system (a star is open; the closed substrate saturates) — untested.

---

## Load-bearing step audit (to complete when drafting)
...
| 5 | Finite memory gives a quantized noise-robust temporal rhythm | DERIVED-in-model | §4 |
| 6 | Native coupling does not globally synchronize (correct: no universal tick) | MEASURED + STRUCTURAL | §5 |
| 7 | V5 finite reach gives local proper time + preserved time dilation (real sim) | STRUCTURAL / reverse-char, confirmed in sim | §6 |
| 8 | V5 must be finite-reach + attractive (two derived constraints) | DERIVED (backward) | §6 |
| 9 | Forward primitive derivation of V5 | OPEN | §8 |

## Drafting notes

- Reference example for house template/tone: `Paper_CommonCauseNotChannel_A1.md`.
- Keep cross-references minimal (cold-reader standalone); inline the needed corpus facts.
- Every "added structure" (memory channel, V5 coupling) must be flagged as an addition, not a primitive result, at first mention and in the audit table.
- Build the PDF per the standard pandoc+xelatex recipe when the draft is complete.
