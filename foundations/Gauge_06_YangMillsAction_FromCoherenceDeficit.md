...
> **⚠️ PRIMITIVE-FIDELITY CORRECTION (2026-07-09).** Every "(P08)" below (e.g. "indistinguishable (P08)", "the SU(N)-giving primitive (P08)", "uniform by P08", "P08 fiber-average") should read **P07** — channel structure/multiplicity is P07; **P08 is the substrate scale ℓ_ED**. Also, §4.5's "the only U(N)-invariant distribution is uniform" is a non-sequitur — the fiber-average `∫|ψ⟩⟨ψ|dψ ∝ 𝟙` follows by **Schur's lemma** for *any* U(N)-invariant distribution, not only the uniform one (the conclusion `𝟙/N` is unchanged). And the Wilson trace being the *matter-averaged* per-chain deficit rests on the effective-action prescription (an input). "Indistinguishable" = dynamical (no accessible intra-family label). Full note in `Gauge_01` and `Paper_GaugeStructure_FromChannelTransport.md` §4.
