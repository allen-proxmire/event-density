...
> **⚠️ PRIMITIVE-FIDELITY CORRECTION (2026-07-09, after adversarial review of the derived paper `Paper_GaugeStructure_FromChannelTransport.md`).** Three attribution errors run through this doc and the sibling gauge notes; read the derivation with these corrections:
> 1. **Channel multiplicity is P07, NOT P08.** Every "(P08)" / "multiplicity (P08)" / "the SU(N)-giving primitive (P08)" / "P08 §7.5" below should read **P07** (channel structure). In canonical Paper_087, **P08 is the substrate scale ℓ_ED**, not multiplicity. This is the stale-numbering trap flagged in memory `feedback_canonical_primitive_sourcing`.
> 2. **P04 is not "conserved."** "Bandwidth is conserved (P04)" over-reads canonical P04, which states bandwidth is **non-negative and additive** only. *Conservation under transport* (the `Σ|ψ_i|²`-preserving isometry) is a **P05-transport (isometry)** property, not P04. Read "conserved (P04)" as "preserved by P05 transport as an isometry; P04 supplies additivity/non-negativity."
> 3. **"Indistinguishable" = DYNAMICAL, not ontological.** Canonical P07 says channels are *distinguishable* carriers with intrinsic identities. The load-bearing premise here is **dynamical indistinguishability**: distinct tokens, but no substrate *observable* resolves channel `i` vs `j` within a same-rule-type family (only bandwidth + rule-type label are accessible). And note the honest consequence (made explicit in the paper): dynamical indistinguishability *alone* gives only the permutation group `S_N`; the continuous `U(N)` additionally needs the ℂ-amplitude structure. So the group form is **derived-conditional**, not forced.
>
> The corrected framing lives in `Paper_GaugeStructure_FromChannelTransport.md` §2. The physics below is otherwise unchanged; only the primitive labels and the "forced" → "derived-conditional" tier are corrected.
