...
- **The coupling model is uniform-random, not the geometrically local coupling V5 actually describes.** V5 correlates chains within a finite spatial and temporal reach (σ within ℓ_V5, time within τ_V5) — a proximity structure, not "any two chains anywhere, with equal probability." A proximity-based (geometric) coupling graph generally needs a higher density than a uniform-random one to reach the same minor-containment threshold, because local structure is less efficient at building far-reaching interconnections. This check likely overstates how easy intrinsic linking is to reach under ED's real coupling geometry.
