...
- **The one genuine open premise (narrow):** that ED's dynamics actually hold committed order by the linking its structure demonstrably supports. Well-posed, not yet shown; the prior "failures" against it were tooling, not substrate.
...
Not more toys, and not another operational-probe round with better tooling (though that would help settle the one premise). The decisive closes are: (i) show, from the certified dynamics, that committed order is *carried* by cross-chain linking rather than merely by P11's plain sequential index (settles the premise); and (ii) the shared curvature-emergence derivation (settles the bridge, and would turn "3D is where a link holds" into "ED forces 3D"). Both are real programs, not tidy-ups. Until then the honest label is **structural bridge, one open premise, one shared open bridge** — which is a strong, well-supported place, and should have been stated as such rather than as recurring insecurity.
