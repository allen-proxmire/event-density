...
> **⚠️ PRIMITIVE-FIDELITY CORRECTION (2026-07-09).** Read "bandwidth conservation (P04)" below as "**preserved by P05 transport as an isometry**; canonical P04 supplies only additivity/non-negativity, not conservation-under-transport." And "N indistinguishable channels" = **dynamically** indistinguishable (P07 gives distinct tokens with no accessible intra-family label), which alone gives only `S_N`; the continuous `U(N)` additionally needs the ℂ-amplitude. Full note in `Gauge_01` and `Paper_GaugeStructure_FromChannelTransport.md` §2.
