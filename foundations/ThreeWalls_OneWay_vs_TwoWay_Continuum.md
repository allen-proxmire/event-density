...
- **#5c (Gaussianity): does NOT flip — stays a real wall.** Tested at sparse seeding (`grf_gaussian_decomp.py`): removing the filament cells drives skew→0 but kurtosis→−1 — you never reach Gaussian; under the filaments is a **flat/sub-Gaussian background, not a Gaussian**. The field is *flat background + super-Gaussian filaments*, **neither Gaussian** — there is no Gaussian signal to recover. The GRF "no" is a genuine ED prediction (non-Gaussian participation field), not a measurement error.
- **#3 (diffusion): not yet re-checked** — the swing vote (does the coherent/mean density obey the PDE, fluctuations as entropy?).

So the corrected framing is **not "three walls are windows"** but the sharper, more useful one: **some coarse-graining failures are measurement errors (a real continuum signal masked by entropy — #2), others are genuine features (no signal there — #5c).** The discriminator: is there a coherent signal under the disorder? That distinction is the result. The "ED goes one way" content below still holds as the *reason the entropy is there*; what's corrected is that the entropy doesn't always *block* a signal — sometimes (e.g. #2) the signal is intact underneath, sometimes (e.g. #5c) there is no signal. Read the rest of this note as the original (over-strong) framing, kept for the record.
