...
- **#5c (Gaussianity): does NOT flip — stays a real wall, now PROVEN by the decisive test.** First via `grf_gaussian_decomp.py` (removing filament cells drives skew→0 but kurtosis→−1; under the filaments is a flat/sub-Gaussian background, not a Gaussian). Then **decisively via the Fourier-phase test** (`fourier_phase_test.py`, route 3 — cosmology's own non-Gaussianity test): the phase-randomized surrogate (ED's exact power spectrum, random phases) is **perfectly Gaussian (skew 0.003)** while ED's field is strongly non-Gaussian (skew 0.96) → **100% of the non-Gaussianity is in the PHASES.** A GRF has random phases (structureless); ED has **correlated phases = structure (filaments).** So *"ED non-Gaussian" = "ED builds structure / commits, doesn't mix"* — the same statement, the distributional face of the arrow. The GRF "no" is a real internal finding (the coarse field is non-Gaussian), **not promoted to an external/official prediction** (notebook documentation only).
