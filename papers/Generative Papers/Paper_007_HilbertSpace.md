...
**UPDATE 2026-07-02:** Paper_004 is now partially successful — one of its two blocking postulates (P-Gleason-Compatibility) is derived in a bookkeeping sense, from P02+P04 alone (`foundations/Gleason_Rehabilitation_Attempt.md`). The other (P-Channel-Orthogonality) remains open, and is now understood as a regime-conditional approximation (valid where cross-channel coherence vanishes) rather than a universal primitive-forced fact. **The inner-product row stays I** — the full derivation needs both postulates, and orthogonality is still the blocking one.
