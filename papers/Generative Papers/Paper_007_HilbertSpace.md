...
| Sesquilinear inner product $\langle\Psi|\Phi\rangle$ | **I (Paper #3 inheritance)** — status unchanged, see 2026-07-02 note | Inner-product structure not derived in this paper; inherited from Paper #3 pre-pivot work. **UPDATE 2026-07-02:** Paper_004 has now partially closed one of its two blocking postulates (P-Gleason-Compatibility, bookkeeping sense — derived from P02+P04). The other, P-Channel-Orthogonality, remains open with no working regime-bound (a candidate one was tried and withdrawn after direct checking, `foundations/Gleason_Rehabilitation_Attempt.md` §3). **This row stays I** — both postulates are required for the I → D relabel, and orthogonality is still the blocking one. |
...
**Forward-pointer:** Paper_004 (Gleason-type uniqueness) is the candidate paper that would derive the inner product from canonical 13 primitives + Paper_001 amplitude content alone via a substrate-level Gleason-analog uniqueness argument. If Paper_004 succeeds at this derivation, the inner-product row in §2.5 (and the load-bearing chain in §5.2) would relabel from **I → D**, strengthening the substrate-framework's self-containment.

**UPDATE 2026-07-02:** Paper_004 is now partially successful — one of its two blocking postulates (P-Gleason-Compatibility) is derived in a bookkeeping sense, from P02+P04 alone (`foundations/Gleason_Rehabilitation_Attempt.md`). The other (P-Channel-Orthogonality) remains open. A candidate regime-conditional account was tried and withdrawn the same session after a direct check of the corpus's own composition-rule source found it backwards and likely conflating two different objects (see `Gleason_Rehabilitation_Attempt.md` §3) — no working regime-bound exists. **The inner-product row stays I** — the full derivation needs both postulates, and orthogonality is still the blocking one.
