...
**P-Channel-Orthogonality remains fully open**, now understood more precisely: three independent derivation routes (P04 additivity, P07 distinctness, P11 phase-randomization) were tried directly and each fails or applies to the wrong layer, confirming this paper's own §3.4 diagnosis independently. A candidate regime-conditional account (channel-orthogonality as an approximation valid where cross-channel coherence vanishes) was tried and **withdrawn within the same session** after a direct check of the corpus's own composition-rule source found the claim backwards (the cited coherence parameter is large, not small, in the regime it was claimed to vanish in) and likely conflating two different objects (interference visibility vs. channel-basis orthogonality) — see `foundations/Gleason_Rehabilitation_Attempt.md` §3 for the full correction. No working regime-bound exists; this remains a clean structural negative, not a regime-conditional approximation.
...
| Distinct channels orthogonal $\langle K|L\rangle = \delta_{KL}$ | **P (P-Channel-Orthogonality)** — three routes tried 2026-07-02, all fail; a candidate regime-conditional account was tried and withdrawn (checked, does not hold) | §2 substrate-level commitment beyond P07; orthogonality is added structural content; see Update above |
