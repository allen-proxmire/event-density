...
- **Primitive 04 (bandwidth):** a non-negative additive scalar field $b_K(u) \geq 0$ on the event manifold (canonical P04, `Paper_087`). *Arc-M refinement (NOT canonical P04):* a four-band decomposition $b_K = b_K^{\mathrm{int}} + b_K^{\mathrm{adj}} + b_K^{\mathrm{env}} + b_K^{\mathrm{com}}$ is used below as a modeling convenience (the σ_τ summation index). **Correction 2026-07-05:** the four-band partition is not part of canonical P04 — it derives from the archived M-series paper `paper_M2_primitive_forcing_P04_four_band_partition.md` ("do not build new work on its framework"). No Arc-M result depends on it: σ_τ, Theorems M1/M2, and the MR-P/MR-R massless slots all stand on the canonical single scalar (collapse the band sum to one term; only the vacuous SC4 and the non-load-bearing M-Order-2 candidate use the bands). See `theory/Higgs_Emergence/FourBand_Dependency_Check.md`.
