...
\chapter{The Substrate Ontology: Primitives P1–P13}
\label{chap:1}
...
\subsection{1.2.2 Why these primitives, not more or fewer}
...
\subsection{1.2.3 The form-FORCED / value-INHERITED methodology starts here}

The program's signature methodological pattern — \textit{form-FORCED at the structural level, value-INHERITED at the numerical level} — has its origin point in the substrate primitives. The primitives commit the framework to specific \textit{forms} (events are discrete, commitments are irreversible, chains carry bandwidth) without committing to specific \textit{values} (the precise substrate length scale, the V1 kernel functional shape, the bandwidth-capacity coefficient). Values inherit from substrate constants whose closed-form derivation lives downstream of the primitive layer or is acknowledged as open work. Every chapter respects this distinction; every theorem in the program is form-FORCED at the structural level, with specific numerical thresholds explicitly identified as INHERITED.
...
\section{1.3 Event-Structural Primitives}

\subsection{1.3.1 P01 — Event discreteness}
...
\textbf{Structural role.} P01 is load-bearing for several downstream results. It supplies substrate-level UV-finiteness in form-level QFT (Chapter 6): because events are discrete, there is a smallest length scale built into the substrate, and the divergent loop integrals that motivate renormalization in standard QFT do not have substrate-level analogues. P01 is also load-bearing for the no-singularity result in black-hole architecture (Chapter 13): one of the three substrate constraints that jointly forbid divergent curvature is the discreteness of micro-events, which makes any continuum singularity statement that requires unboundedly many infinitely small events at one point inadmissible at the substrate level.

\textbf{Downstream consequences.} UV-finiteness in QFT (Chapter 6); no-singularity in BH-3 (Chapter 13); the discrete-event structure of P11 commitment (downstream); the cross-domain echo with the matter-wave Q-C boundary's mass-cap interpretation in Chapter 7 (large molecules accumulate too many activated internal events to sustain the unresolved regime).

\subsection{1.3.2 P02 — Chain worldline structure}

Events are not free-floating; they are linked into \textit{chains} that carry persistent identity. A chain is a sequence of events linked by participation relations; chains have proper-time-ordered structure (handed off to P13 below) and they are the substrate-level object that, after coarse-graining, produces the appearance of particle worldlines.

\textbf{Structural role.} P02 supplies the chain forward-propagator structure used in Theorem 18's derivation of the kernel-level arrow of time (Chapter 4). The forward-propagator $U_K(n,m)$ between two chain commitments is identically zero for $n < m$; this property is what makes the chain-contribution sum produce a forward-cone-only kernel, ruling out symmetric, advanced, and hybrid kernels at the primitive level.

\textbf{Downstream consequences.} Theorem 18 retardation (Chapter 4); the substrate-level identification of "particle" as a chain pattern (rather than a fundamental object) used throughout Parts II and III; the chain identity that makes commitment-irreversibility (P11) propagate consistently across substrate dynamics.

\section{1.4 Participation and Bandwidth Primitives}

\subsection{1.4.1 P04 — Bandwidth update rule}
...
\section{1.6 Temporal Structure}

\subsection{1.6.1 P13 — Proper-time ordering}
...
\subsection{1.7.1 Finite-kernel commitments}
...
\textbf{Phase-1 closure of QM (T1–T16, Chapter 5):} primarily P04 (bandwidth conservation for Born; finite-bandwidth for Heisenberg) plus P11 (measurement) plus P01 (UV-finiteness contribution). Mathematical bridges via Gleason's theorem (1957) and Stone's theorem (1932).

\textbf{Theorem 17 — Gauge-Field-as-Rule-Type (Chapter 6):} label-carrying-rule-type structure plus P04 plus DCGT (Chapter 3) coarse-graining of charged-chain populations.

\textbf{Theorem N1 + Theorem 18 (Chapter 4):} P02 + P04 + P11 + P13 + finite-width-kernel commitments + substrate-locality. The forcing chain propagates from P11 (forward-only commitments along chains) to spacetime-retarded V1 (forward-cone-only support).

\textbf{T19 + T20 + ECR + T21 (Chapter 11):} participation/multiplicity primitives + DCGT + cumulative-strain mechanism + holographic participation-count bound (which is itself a structural commitment, not a separate primitive, since it follows from substrate locality applied to participation-count surfaces).

\textbf{DCGT (Chapter 3):} P01 + P04 + finite-kernel commitments + substrate-locality + scale-separation hypothesis $\ell_P \ll R_\mathrm{cg} \ll L_\mathrm{flow}$.

\textbf{UR-1 (Chapter 7):} P04 (multiplicity bound and bandwidth conservation) + P11 (commitment-injection failure mode) + DCGT (cross-endpoint connectivity from substrate-locality + gradient sparsity).
...
\section{1.9 The Substrate Boundary}

A central methodological task of this chapter is to make explicit the boundary between \textit{substrate ontology} (everything in this chapter and the primitives within it) and \textit{emergent continuum physics} (everything in subsequent chapters). The boundary is not negotiable in the program: substrate-level claims live below the boundary; coarse-grained continuum claims live above it; the bridge between them is DCGT (Chapter 3) and its non-Abelian generalization (Chapter 9).

\subsection{1.9.1 What is substrate-ontological}

\begin{itemize}
\item The thirteen primitives.
\item The substrate-level objects defined directly from them: events, chains, participation channels, commitment events, substrate gradients of $\rho$, finite-width kernels.
\item The substrate-level scalar quantities derived without coarse-graining: $\rho$ (participation density), $\nabla\rho$, $\sigma$, $\mathcal{M}$, $\Gamma_\mathrm{cross}$ (substrate-level cross-bandwidth before continuum interpretation), $\Lambda$ (commitment-injection rate at endpoints), $\mathcal{U}$ (participation-rule unresolvedness in UR-1).
\end{itemize}

\subsection{1.9.2 What is emergent}

\begin{itemize}
\item Spacetime as a smooth manifold (Chapter 12 makes this explicit at acoustic-metric class).
\item Particles as persistent stable participation patterns (Phase-1 closure, Chapter 5).
\item Fields as coarse-grained averages over participation events (Chapter 6 develops the gauge-field instance).
\item Continuum equations: the Schrödinger equation (Chapter 5), Maxwell's equations (Chapter 9), the Yang–Mills equation (Chapter 9), the Navier–Stokes equation (Chapter 8), the modified Poisson equation (Chapter 11), the acoustic-metric scalar-tensor covariantization (Chapter 12), and so on. Every continuum equation in the program is reached from substrate primitives through DCGT or one of its generalizations.
\item "Decoherence" as standardly understood: a coarse-grained signature of substrate-level commitment events, not a fundamental coupling-rate to environment (Chapter 7 develops this reframing for quantum computing).
\end{itemize}

\subsection{1.9.3 Why the boundary matters}
...
\section{1.10 Form-FORCED vs. Value-INHERITED at the Primitive Level}

\subsection{1.10.1 What is form-FORCED at this layer}

The thirteen primitives commit the framework to specific structural properties:
\begin{itemize}
\item Events are discrete (P01).
\item Events live on chains with proper-time ordering (P02 + P13).
\item Participation channels have bounded bandwidth (P04).
\item Commitments are irreversible (P11).
\item Substrate response kernels (V1, V5) are finite-width.
\item Substrate dynamics are local in the causal-adjacency sense.
\end{itemize}

These commitments are form-FORCED in the sense that they fix the \textit{shape} of substrate-level statements that any downstream theorem can reach. They do not commit the framework to specific numerical values for any substrate constant.

\subsection{1.10.2 What is value-INHERITED at this layer}

The primitives do not specify:
\begin{itemize}
\item The numerical scale of the substrate's smallest length (identified later, via T19 Newton-recovery, with the Planck length $\ell_P$).
\item The specific functional shape of the V1 vacuum kernel (Theorem N1 establishes finite-width; the closed-form shape is INHERITED).
\item The specific numerical values of any substrate-determined dimensionless threshold ($\beta_\mathrm{crit}$, $\mathcal{M}_\mathrm{crit}$, $\Gamma_\mathrm{min}$, etc., introduced in subsequent chapters).
\item The specific numerical values of the substrate inputs ($\hbar$, $c$, $H_0$) used in derivations like T19 ($G = c^3\ell_P^2/\hbar$) and T20 ($a_0 = c\,H_0/(2\pi)$). These are taken as substrate inputs in the program; their specific values are calibrated empirically.
\end{itemize}
...
\subsection{1.10.3 The methodological consistency}

The form-FORCED / value-INHERITED pattern starts at the primitive layer and propagates through every chapter. Each downstream theorem inherits the pattern: T17 derives the \textit{form} of gauge fields without committing to which specific gauge group nature realizes; T19 derives the \textit{form} of Newton's law from substrate primitives with the Planck length identified as the substrate length scale through Newton-recovery; UR-1 derives the \textit{form} of three substrate conditions for the unresolved regime with specific numerical thresholds INHERITED. The methodology's consistency across nine sectors of substrate-level architectural closure is itself a structural signature of the framework, addressed at length in Chapter 14.

\section{1.11 Dependencies}

\subsection{1.11.1 Upstream dependencies}
...
\subsection{1.11.2 Downstream readers}
...
\begin{itemize}
\item \textbf{Chapter 2 — Load-Bearing Invariants.} P01, P02, P04, P11, P13, finite-kernel and substrate-locality primitives. The chapter constructs the load-bearing invariants ($\mathcal{M}$, $\sigma$, $\Gamma_\mathrm{cross}$, V1, P11-as-direction-primitive) directly from the primitives.
\item \textbf{Chapter 3 — DCGT.} P01, P04, finite-kernel, substrate-locality. The hydrodynamic-window scale separation $\ell_P \ll R_\mathrm{cg} \ll L_\mathrm{flow}$ rests on substrate-locality and on the Planck-scale event discreteness contributed by P01.
\item \textbf{Chapter 4 — Theorem N1 + Theorem 18.} P02, P04, P11, P13, finite-kernel, substrate-locality. The forcing chain explicitly cites P11 + P02 + P04 + P13 + Theorem N1 + Q.8 effective-vacuum factorisation.
\item \textbf{Chapter 5 — Phase-1 closure.} P01, P04, P11. Plus mathematical bridges via Gleason's theorem and Stone's theorem.
\item \textbf{Chapter 6 — Form-Level QFT and ED-I-13.} P01, P04, P13, label-carrying-rule-type structure (a structural commitment within the participation-channel primitive group).
\item \textbf{Chapter 7 — Quantum Computation.} P04 (multiplicity bound), P11 (commitment-injection), substrate-locality (cross-endpoint connectivity), DCGT (Chapter 3) for cross-bandwidth structure.
\item \textbf{Chapter 8 — Navier–Stokes.} P04 + finite-kernel (V1) + DCGT.
\item \textbf{Chapter 9 — MHD and Yang–Mills.} Chapter 8's content + T17 (Chapter 6) + DCGT non-Abelian extension.
\item \textbf{Chapter 10 — Soft-Matter Mobility.} P04 + V5 finite-width kernel + DCGT.
\item \textbf{Chapter 11 — Substrate Gravity.} Participation-density primitives + cumulative-strain mechanism + holographic participation-count bound (substrate-locality applied to surfaces) + DCGT.
\item \textbf{Chapter 12 — Curvature Emergence.} Chapter 11 + the substrate cumulative-strain four-index object as load-bearing curvature degree of freedom.
\item \textbf{Chapter 13 — Black-Hole Architecture.} P01 + P04 + P11 + finite-kernel + DCGT + the single substrate condition $|\nabla\rho|\,\ell_P^2/\rho_\mathrm{local} \gtrsim \log(R_\mathrm{cg}/\ell_P)$.
\item \textbf{Chapters 14–15.} Cross-platform unifications and empirical synthesis. Inherit dependencies from all preceding chapters.
\end{itemize}
...
\section{1.12 Canonical Sources}
...
\begin{itemize}
\item \texttt{papers/Event\_Density\_Ontology\_and\_Axioms/}
\item \texttt{papers/Foundations\_of\_Event\_Density/}
\item \texttt{papers/ED\_One\_Substrate\_Three\_Domains/} (orientation context for the program-level scope of the substrate ontology)
\end{itemize}
...
The two regimes:
\begin{itemize}
\item $\mathcal{M} \to 1$: only one viable participation pathway is locally accessible. Hyper-coherent regime. The substrate's gradient structure has collapsed to admit a single coherent channel; charge-related ED-flow is forced into laminar, unified motion. This is the substrate reading of bulk superconductivity (ED-I-01).
\item $\mathcal{M} \to \infty$: arbitrarily many distinct pathways are locally accessible. Classical thermal regime. The substrate supports branching, scattering, and decoherence; ED-flow proliferates across many pathways simultaneously.
\end{itemize}

\subsection{2.3.2 Why multiplicity is the entropy analogue}
...
\subsection{2.3.3 Multiplicity is per-region and per-substrate-timescale}

Multiplicity is defined per substrate region per substrate-timescale. A composite system has a \textit{system multiplicity} that depends both on the multiplicity of its constituent regions and on the substrate connectivity between them. This is the structural reason a multi-qubit array's effective multiplicity is not the sum of single-qubit multiplicities: cross-qubit pathway count contributes additively at the substrate level even when individual qubit multiplicities are held low by structural commitment.
...
\subsection{2.3.4 Downstream consequences}

\begin{itemize}
\item \textbf{Phase-1 closure (Chapter 5):} $\mathcal{M}$'s relationship to bandwidth conservation feeds the Born-rule derivation through Gleason's theorem.
\item \textbf{Form-level QFT (Chapter 6):} The structural status of multiplicity-counting at substrate level supports the gauge-field-as-rule-type interpretation.
\item \textbf{Quantum computing (Chapter 7):} UR-1 condition (i) is the explicit multiplicity bound $\mathcal{M}_i \leq \mathcal{M}_\mathrm{crit}$ at every endpoint; the multiplicity-cap function $M$'s Class A static-failure branch is set by $\mathcal{M}_\mathrm{floor}(\mathcal{S}) = \mathcal{M}_\mathrm{crit}$.
\item \textbf{MHD architectural classification (Chapter 9):} Multiplicity governs whether a continuum content item is canonical-ED (low-$\mathcal{M}$ structural) or transport-kinematic (multiplicity-proliferating).
\item \textbf{Soft-matter mobility (Chapter 10):} The mobility-capacity bound from P04 produces $M(\rho) = M_0(1-\rho/\rho_\mathrm{max})^\beta$ via the substrate's multiplicity-suppression mechanism near packing.
\item \textbf{Black-hole entropy (Chapter 13):} $S \propto A/\ell_P^2 \cdot \log g$ is the count of viable commitment histories at the saturated participation surface — a multiplicity count.
\end{itemize}

\section{2.4 Gradient Sparsity $\sigma$}

\subsection{2.4.1 Substrate-level definition}
...
Two regimes:
\begin{itemize}
\item $\sigma \ll 1$: the substrate is locally smooth at the Planck scale. Continuum approximations are well-controlled; ordinary hydrodynamic-window coarse-graining proceeds without obstruction.
\item $\sigma \gtrsim \log(R_\mathrm{cg}/\ell_P)$: the substrate has steepened to the threshold where the multi-scale expansion's small parameter ceases to be small. This is the \textit{decoupling-surface threshold}. At and above this threshold, the substrate region cannot be treated as locally smooth at the coarse-graining scale, and several substrate quantities (cross-bandwidth, the acoustic-metric reading) lose their continuum-approximation validity.
\end{itemize}

\subsection{2.4.2 Why sparsity, not gradient}
...
\subsection{2.4.3 The decoupling-surface threshold}

The threshold $\sigma \gtrsim \log(R_\mathrm{cg}/\ell_P)$ is the substrate condition that recurs across multiple closed sectors:

\begin{itemize}
\item \textbf{BH-2 horizon formation (Chapter 13):} A black-hole event horizon is the surface where $\sigma$ exceeds the threshold. The horizon is not a geometric primitive; it is a substrate-level statistical feature of $\sigma$ crossing.
\item \textbf{BH-3 singularity replacement (Chapter 13):} The acoustic-metric reading breaks down when $\sigma$ exceeds the threshold; the substrate's saturated participation zone replaces the singular endpoint.
\item \textbf{QC condition (ii) failure (Chapter 7):} Cross-bandwidth between rule-spanning endpoints collapses when $\sigma$ along the connecting pathway crosses the threshold. This is the structural reason MQT in a Josephson junction (Chapter 7) and horizon formation in a black hole (Chapter 13) share the same substrate mechanism.
\end{itemize}
...
\subsection{2.4.4 Downstream consequences}

\begin{itemize}
\item \textbf{DCGT (Chapter 3):} The cross-bandwidth structure $\Gamma_\mathrm{cross} \sim \exp[-\alpha\sigma]$ uses $\sigma$ as the substrate-scale exponent.
\item \textbf{Substrate gravity (Chapter 11):} The cumulative-strain mechanism that produces Newton's law is built on integrated substrate gradients, with $\sigma$ controlling the substrate-scale contribution to the strain field.
\item \textbf{Black-hole architecture (Chapter 13):} The single substrate condition unifying horizon formation, interior saturation, information blocking, participation-capacity saturation, and strong-curvature scattering region is a $\sigma$-threshold condition.
\item \textbf{Quantum computing (Chapter 7):} $\sigma$ along participation-rule-spanning pathways controls the cross-bandwidth $\gamma_{ij}$ that enters UR-1 condition (ii).
\end{itemize}

\section{2.5 Cross-Bandwidth $\Gamma_\mathrm{cross}$}

\subsection{2.5.1 Substrate-level definition}

Cross-bandwidth $\Gamma_\mathrm{cross}(\mathbf{x}_1, \mathbf{x}_2)$ is the substrate-mediated rate at which participation events at $\mathbf{x}_1$ and $\mathbf{x}_2$ exchange correlated content. It is the substrate-scale measure of how strongly two regions participate in each other's substrate dynamics. Cross-bandwidth differs from local participation density: $\rho$ measures the participation-channel availability \textit{at} a region; $\Gamma_\mathrm{cross}$ measures the substrate-mediated \textit{exchange} between regions.
...
\subsection{2.5.3 Downstream consequences}

\begin{itemize}
\item \textbf{DCGT (Chapter 3):} $\Gamma_\mathrm{cross}$ is the central substrate-to-continuum bridge quantity.
\item \textbf{Quantum computing (Chapter 7):} UR-1 condition (ii) is $\gamma_{ij} \geq \Gamma_\mathrm{min}$ along every rule-spanning pathway.
\item \textbf{Black-hole architecture (Chapter 13):} Horizon formation is precisely the surface where $\Gamma_\mathrm{cross}$ falls below hydrodynamic-window resolution. The same substrate mechanism that produces horizons at gravitational-collapse scales produces QC condition (ii) failure at engineered-system scales (Chapter 14 develops this cross-domain identity).
\item \textbf{Form-level QFT (Chapter 6):} The substrate origin of vacuum-response analytic structure passes through $\Gamma_\mathrm{cross}$ via the V1 kernel mediation.
\end{itemize}

\subsection{2.5.4 The cross-domain echo}
...
\section{2.6 The V1 Finite-Width Vacuum Kernel}

\subsection{2.6.1 Substrate-level role}
...
\subsection{2.6.2 Theorem N1: V1 finite-width vacuum kernel}
...
\subsection{2.6.3 Theorem 18: V1 kernel retardation}
...
\subsection{2.6.4 Downstream consequences}

\begin{itemize}
\item \textbf{Phase-1 closure (Chapter 5):} V1 mediates the substrate-level analogue of vacuum-response correlations that enter the Born-rule and Schrödinger derivations.
\item \textbf{Form-level QFT (Chapter 6):} V1 is the substrate-level origin of the standard Wightman correlator and the retarded Green's function structure of QFT. The continuum-approximation structure inherits from V1's substrate-level form.
\item \textbf{Theorem 18 and the kernel-level arrow (Chapter 4):} V1's forward-cone-only support is the primitive-level statement of the arrow of time.
\item \textbf{Substrate-cutoff regularization R1 (Chapter 8):} V1's finite width produces the hyperviscous correction term $-\kappa\mu_\mathrm{V1}\ell_P^2 \nabla^4\mathbf{v}$ in Navier–Stokes through DCGT coarse-graining.
\item \textbf{Yang–Mills mass-gap mechanism (Chapter 9):} V1's second-moment expansion produces the substrate-level mass-gap mechanism.
\item \textbf{Black-hole area-law entropy (Chapter 13):} V1's per-patch substrate temporal width sets the per-patch motif alphabet $g$ in the BH entropy expression.
\end{itemize}

\section{2.7 P11 Commitment-Irreversibility}

\subsection{2.7.1 The framework's only direction-bearing primitive}
...
\subsection{2.7.2 P11 as substrate-level seed}

P11's role across the program:

\begin{itemize}
\item \textbf{Theorem 18 (Chapter 4):} P11 plus chain structure (P02) plus bandwidth update (P04) plus proper-time ordering (P13) plus V1 finite-width plus substrate locality jointly force the V1 kernel to forward-cone-only support. The kernel-level arrow of time is FORCED at the primitive level.
\item \textbf{Phase-1 measurement rule (Chapter 5):} Wavefunction collapse is identified as the coarse-grained continuum signature of substrate-level commitment events. There is no separate measurement postulate in the QM emergence sector; P11 supplies the substrate-level mechanism.
\item \textbf{BH-4 information-non-paradox (Chapter 13):} Committed structure cannot un-commit, so committed structure cannot cross a horizon and re-emerge. Entanglement (uncommitted structure) can straddle. The information paradox does not arise because the four assumptions that generate it (global unitarity, global Cauchy data, sharp geometric boundary, monogamy-at-boundary) are not imposed at the substrate level — and P11 is the primitive that makes the substrate-level account internally consistent.
\item \textbf{UR-1 condition (iii) (Chapter 7):} Each P11 event at a system endpoint contributes to a Poisson-class accumulation $\int_0^t \Lambda_\mathcal{S}(t')\,dt'$ that defines the third UR-1 condition's failure timescale. The QC operating window's commitment-injection-bounded condition is a direct P11 consequence.
\item \textbf{Cross-domain unification (Chapter 14):} P11 is the common substrate-level direction-bearing commitment that unifies the kernel-level arrow of time, the measurement rule, evaporation as participation re-routing, and the QC commitment-injection failure mode under one substrate ontology.
\end{itemize}

\subsection{2.7.3 Why P11 is treated as a primitive rather than derived}
...
\section{2.8 The Five Invariants Together: A Structural Map}
...
\section{2.9 The Five Invariants in the Closed-Arc Inventory}
...
\section{2.10 Notation Conventions}
...
\begin{itemize}
\item $\mathcal{M}(\mathcal{S})$ — multiplicity at substrate region $\mathcal{S}$.
\item $\rho(\mathbf{x})$ — local participation density.
\item $\nabla\rho$ — participation density gradient.
\item $\sigma(\mathbf{x}) = |\nabla\rho|\,\ell_P^2/\rho_\mathrm{local}$ — substrate-scale gradient sparsity.
\item $\Gamma_\mathrm{cross}(\mathbf{x}_1, \mathbf{x}_2)$ — substrate-mediated cross-bandwidth.
\item V1 — finite-width vacuum response kernel (Theorem N1).
\item V5 — finite-width cross-chain memory kernel (Chapter 10 develops the soft-matter use).
\item P11 — substrate primitive of commitment-irreversibility.
\item $\ell_P$ — Planck length / substrate length scale (identified with the standard Planck length via T19 Newton-recovery; Chapter 11).
\item $R_\mathrm{cg}$ — coarse-graining length scale.
\item $L_\mathrm{flow}$ — continuum-flow length scale.
\item $\alpha$ — DCGT prefactor in the cross-bandwidth exponential structure.
\end{itemize}

\section{2.11 Form-FORCED vs Value-INHERITED at the Invariant Layer}

\subsection{2.11.1 What is form-FORCED at this layer}

\begin{itemize}
\item The existence and structural definition of multiplicity $\mathcal{M}$ as the count of viable distinct ED-gradient pathways available locally.
\item The dimensionless form $\sigma = |\nabla\rho|\,\ell_P^2/\rho_\mathrm{local}$ for substrate-scale gradient sparsity.
\item The exponential form $\Gamma_\mathrm{cross} \sim \exp[-\alpha\int_\mathrm{path}\sigma\,d\ell]$ for substrate-mediated cross-bandwidth (formally derived in Chapter 3 via DCGT).
\item The existence and finite-width character of the V1 vacuum response kernel.
\item The existence and direction-bearing character of P11 commitment-irreversibility.
\item The substrate-natural threshold scale $\sigma \gtrsim \log(R_\mathrm{cg}/\ell_P)$ for decoupling-surface formation.
\end{itemize}

\subsection{2.11.2 What is value-INHERITED at this layer}

\begin{itemize}
\item The specific functional shape of V1 (finite-width is FORCED; closed-form shape is INHERITED).
\item The numerical value of the prefactor $\alpha$ in the cross-bandwidth exponential.
\item The numerical value of the dimensionless coefficient in the decoupling-surface threshold (the prefactor on $\log(R_\mathrm{cg}/\ell_P)$, here written as $\beta_\mathrm{crit}$ in subsequent chapters).
\item The numerical scale of $\ell_P$ (taken as substrate input; identified with the standard Planck length via T19 Newton-recovery).
\item The specific numerical thresholds $\mathcal{M}_\mathrm{crit}$, $\Gamma_\mathrm{min}$, and related quantities introduced in later chapters; these are calibrated against empirical anchors (Chapter 7's matter-wave Q-C boundary at 140–250 kDa is the canonical anchor for $\mathcal{M}_\mathrm{crit}$).
\end{itemize}

\subsection{2.11.3 Why this matters at the invariant layer}
...
\section{2.12 Dependencies}

\subsection{2.12.1 Upstream}

\begin{itemize}
\item \textbf{Chapter 1.} All thirteen substrate primitives. The five load-bearing invariants are constructed from subsets of the primitives: $\mathcal{M}$ from P04 plus participation primitives; $\sigma$ from participation density primitives plus the substrate length scale; $\Gamma_\mathrm{cross}$ from substrate locality plus V1 plus integrated $\sigma$; V1 from the finite-kernel commitment; P11 from itself.
\end{itemize}

\subsection{2.12.2 Downstream}

\begin{itemize}
\item \textbf{Chapter 3 (DCGT).} Constructs the explicit form $\Gamma_\mathrm{cross} \sim \exp[-\alpha\sigma]$ from the multi-scale expansion using the invariants of this chapter.
\item \textbf{Chapter 4 (Theorem N1 + Theorem 18).} Establishes V1's finite-width and forward-cone-only character formally; uses P11 + V1 as central components.
\item \textbf{Chapter 5 (Phase-1 closure).} Uses $\mathcal{M}$ in the Born-rule derivation through bandwidth conservation; uses P11 in the measurement rule.
\item \textbf{Chapter 6 (Form-level QFT).} Uses V1 for substrate-level vacuum response; uses $\mathcal{M}$ in gauge-field-as-rule-type interpretation.
\item \textbf{Chapter 7 (Quantum computing).} All five invariants. Multiplicity bounds UR-1 condition (i); cross-bandwidth and gradient sparsity govern UR-1 condition (ii); commitment-injection (P11-driven) governs UR-1 condition (iii); V1 enters through DCGT.
\item \textbf{Chapter 8 (Navier–Stokes).} V1's finite width produces the substrate-cutoff R1 hyperviscous term.
\item \textbf{Chapter 9 (MHD and Yang–Mills).} $\mathcal{M}$ governs the canonical/non-canonical content classification; V1's second-moment produces the YM mass-gap mechanism.
\item \textbf{Chapter 10 (Soft-matter mobility).} $\mathcal{M}$ enters the Universal Mobility Law; V5 (the V1-companion finite-width kernel) produces Maxwell viscoelasticity through DCGT.
\item \textbf{Chapter 11 (Substrate gravity).} $\sigma$ enters the cumulative-strain mechanism.
\item \textbf{Chapter 12 (Curvature emergence).} $\sigma$-structure persists to the covariantization.
\item \textbf{Chapter 13 (Black-hole architecture).} All five invariants. The single substrate condition unifying horizon formation, interior saturation, information blocking, participation-capacity saturation, and strong-curvature scattering is a $\sigma$-threshold condition; $\Gamma_\mathrm{cross}$ collapses at the surface; $\mathcal{M}$ saturates in the interior; V1 sets the per-patch motif alphabet; P11 supplies the information-non-paradox.
\item \textbf{Chapter 14 (Cross-platform unifications).} The cross-domain identity between BH horizon formation and QC condition (ii) failure rests on the same $\Gamma_\mathrm{cross}$ structure; the matter-wave ↔ qubit-system identity rests on the same $\mathcal{M}_\mathrm{crit}$ threshold.
\item \textbf{Chapter 15 (Public test inventory).} Empirical anchors calibrate the value-INHERITED thresholds attached to these invariants.
\end{itemize}

\section{2.13 Canonical Sources}

\begin{itemize}
\item \texttt{papers/Event\_Density\_Ontology\_and\_Axioms/}
\item ED-I-01 (Superconductivity, Feb 2026; multiplicity-as-entropy; bulk SC as low-$\mathcal{M}$ regime; $T_c$ as multiplicity-overwhelming-symmetry threshold).
\item ED-I-23 (Josephson Junctions, Mar 2026; multiplicity at engineered low-$\mathcal{M}$ regions; JJ as deliberately engineered ED-bottleneck; macroscopic quantum coherence as preservation of low-multiplicity geometry).
\item ED-I-29 (Tunneling, Mar 2026; sparse-$\sigma$ regions as global participation-rule reconfiguration; tunneling reframed as substrate global reconfiguration across $\sigma$-gap rather than as motion through a forbidden region).
\end{itemize}
...
\subsection{3.2.2 What standard physics does instead}
...
\subsection{3.2.3 What DCGT must deliver}

For DCGT to close the substrate/continuum gap, it must do four things:

\begin{enumerate}
\item Establish a controlled scale separation under which substrate dynamics admit a multi-scale expansion. (Section 3.3.)
\item Produce the substrate-level cross-bandwidth structure $\Gamma_\mathrm{cross}$ used throughout the program. (Section 3.4.)
\item Deliver leading-order coarse-grained continuum content covering at minimum scalar diffusion, directional viscosity, substrate-cutoff regularization, viscoelastic memory, and T17 minimal coupling. (Section 3.5.)
\item Generalize to the non-Abelian content needed for Yang–Mills (Chapter 9) and to the gradient-source content needed for substrate gravity (Chapter 11). (Section 3.7.)
\end{enumerate}

DCGT delivers all four. The chapter establishes each in turn.

\section{3.3 The Hydrodynamic Window}

\subsection{3.3.1 The scale separation}

DCGT operates within a \textit{hydrodynamic window} — a range of length scales between the substrate scale and the continuum-flow scale where substrate dynamics admit a controlled multi-scale expansion. The window is bounded below by the substrate's irreducible length scale $\ell_P$ (the Planck length, identified with the substrate length scale through T19 Newton-recovery in Chapter 11) and bounded above by the characteristic length scale $L_\mathrm{flow}$ of the continuum theory being derived. Within the window, the coarse-graining length scale $R_\mathrm{cg}$ provides a controlled intermediate scale at which substrate dynamics can be averaged without losing structural content:
...
\subsection{3.3.2 Why scale separation is required}
...
The leading-order content delivers the \textit{standard} continuum equations — Newton's law of viscosity in NS, Maxwell's equations in MHD, the modified Poisson equation in substrate gravity, and so on. The first subleading correction delivers substrate-cutoff regularization terms, of which the canonical example is the R1 hyperviscous term $-\kappa\mu_\mathrm{V1}\ell_P^2 \nabla^4\mathbf{v}$ in NS (Chapter 8). The substrate-cutoff terms are small at ordinary continuum-flow scales — suppressed by powers of $\ell_P/R_\mathrm{cg}$ — but they are structurally inevitable consequences of the substrate's finite-width kernels.

\subsection{3.4.2 What the expansion produces at each level}
...
\subsection{3.4.3 The coarse-graining-invariance condition}
...
\subsection{3.5.2 Why the form is structural rather than coincidental}
...
\section{3.6 The Five Leading-Order Consequences}
...
\subsection{3.6.1 Scalar diffusion}
...
\subsection{3.6.2 Directional viscosity (the NS viscous sector)}
...
This is the substrate-to-continuum bridge for the viscous sector of Navier–Stokes (Chapter 8). The advective sector $(\mathbf{v}\cdot\nabla)\mathbf{v}$ is \textit{not} DCGT-derived; it is a frame-kinematic artifact of writing fluid dynamics in laboratory-frame coordinates. The pressure and incompressibility terms are continuum-imposed constraints. Of the four kinds of structure in NS, two (viscous diffusion and the substrate-cutoff R1 below) are DCGT-derived; the other two are not.

\subsection{3.6.3 V1 → R1 substrate-cutoff regularization}
...
\subsection{3.6.4 V5 → Maxwell viscoelastic memory}
...
\subsection{3.6.5 T17 minimal coupling (Lorentz force)}
...
\subsection{3.6.6 The five together}
...
\section{3.7 The Non-Abelian Generalization}

\subsection{3.7.1 What changes for non-Abelian content}
...
\subsection{3.7.2 Why the generalization works}

The structural reason DCGT generalizes cleanly to non-Abelian content is that the substrate ontology does not commit to which gauge group nature realizes. T17 establishes gauge fields as participation measures of structural rule-types; the \textit{form} of gauge field content is FORCED by the substrate, but the specific gauge group (Abelian U(1) vs non-Abelian SU(2), SU(3), etc.) is empirical input. DCGT's machinery is therefore agnostic about gauge-group choice: it produces the appropriate continuum Yang–Mills equation for whichever compact-simple-group structure the rule-type carries.

The form-FORCED status of DCGT's non-Abelian generalization means: the framework predicts the \textit{form} of the Yang–Mills equation (the $D_\mu F^{\mu\nu} = J^\nu$ structure) and the \textit{form} of the mass-gap mechanism, without committing to specific values for gauge couplings or to specific gauge groups. The Standard Model gauge group remains empirical input; the form of Yang–Mills equations is FORCED.

\section{3.8 DCGT's Role Across the Closed-Arc Inventory}
...
\section{3.9 The Substrate-to-Continuum Mapping}

\subsection{3.9.1 What DCGT delivers in plain structural terms}

DCGT takes substrate-level inputs:
\begin{itemize}
\item The substrate primitives of Chapter 1.
\item The load-bearing invariants of Chapter 2.
\item A system characterized by a continuum-flow scale $L_\mathrm{flow}$ that admits a hydrodynamic window $\ell_P \ll R_\mathrm{cg} \ll L_\mathrm{flow}$.
\end{itemize}

And produces continuum-level outputs:
\begin{itemize}
\item Leading-order continuum equations governing the system's coarse-grained dynamics.
\item First-subleading-order substrate-cutoff corrections that distinguish ED from purely phenomenological continuum theories.
\item The cross-bandwidth structure $\Gamma_\mathrm{cross}$ governing two-region exchange.
\item Coarse-graining-invariant content that does not depend on the choice of $R_\mathrm{cg}$ within the window.
\end{itemize}

\subsection{3.9.2 The form-FORCED / value-INHERITED content of DCGT}

The form of DCGT's output is FORCED:
\begin{itemize}
\item The hydrodynamic-window scale separation requirement.
\item The multi-scale expansion structure.
\item The exponential form of $\Gamma_\mathrm{cross}$ in integrated $\sigma$.
\item The five leading-order consequences (scalar diffusion, directional viscosity, V1→R1, V5→Maxwell, T17 minimal coupling).
\item The non-Abelian generalization structure for Yang–Mills.
\end{itemize}

The value of DCGT's output is INHERITED:
\begin{itemize}
\item The specific dimensionless prefactor $\alpha$ in the cross-bandwidth exponential.
\item The specific diffusion coefficient in scalar diffusion.
\item The specific viscosity coefficient in directional viscosity.
\item The specific R1 cutoff coefficient $\kappa\mu_\mathrm{V1}\ell_P^2$.
\item The specific Maxwell relaxation time $\tau_R$ (V5 first temporal moment).
\item The specific gauge-coupling values in Yang–Mills (gauge-group-specific empirical input).
\end{itemize}

\subsection{3.9.3 What DCGT does \textit{not} deliver}

DCGT does not deliver:
\begin{itemize}
\item The advective sector of Navier–Stokes (a frame-kinematic artifact, not a substrate consequence).
\item The pressure and incompressibility constraints in NS (continuum-imposed constraints, not DCGT outputs).
\item Einstein's general relativity (Chapter 12 is honest that the acoustic-metric covariantization is kinematic-class, not full GR).
\item A constructive proof of the Clay Yang–Mills problem (Chapter 9 gives a structural-positive verdict at the substrate-suggestive level, which is a form-level claim, not a constructive theorem at Streater–Wightman / OS-axiom rigor).
\item Specific numerical values for any of the substrate constants.
\end{itemize}
...
\section{3.10 The Coarse-Graining Bridge as Audit Trail}

\subsection{3.10.1 Auditability is a methodological commitment}
...
\begin{enumerate}
\item Identify the substrate primitives consumed.
\item Identify the load-bearing invariants used.
\item Specify the hydrodynamic window for the system being analyzed.
\item Apply the DCGT multi-scale expansion.
\item Read off the leading-order continuum equation as form-FORCED output.
\item Read off the first-subleading substrate-cutoff correction as a separate form-FORCED output.
\item Identify which numerical values in the output are INHERITED.
\item Identify which content in the original continuum theory (if any) is frame-kinematic or continuum-imposed and therefore not DCGT-derived.
\end{enumerate}
...
\subsection{3.10.2 The audit at the boundaries of DCGT applicability}
...
\begin{itemize}
\item The matter-wave Q-C boundary at 140–250 kDa molecular mass (Chapter 7): the molecule's effective system multiplicity has crossed the substrate threshold, and the DCGT-derived unresolved-rule regime ceases to be sustainable.
\item The BH-3 saturated participation zone (Chapter 13): substrate gradients have steepened to the saturation regime where the acoustic-metric reading breaks down, and the coarse-grained continuum content is replaced by a substrate-level finite-thickness saturated zone.
\item The deep-MOND superluminal scalar propagation in Arc ED-10 (Chapter 12): the acoustic-metric scalar-tensor covariantization produces a kinematic-class reading whose superluminality is a structurally-FORCED consequence of producing the substrate-derived MOND behavior without introducing additional dynamical fields.
\end{itemize}
...
\section{3.11 Form-FORCED vs Value-INHERITED at the Coarse-Graining Bridge}

\subsection{3.11.1 What is form-FORCED at this layer}

\begin{itemize}
\item The hydrodynamic-window scale separation requirement $\ell_P \ll R_\mathrm{cg} \ll L_\mathrm{flow}$.
\item The multi-scale expansion structure with the small parameter $\ell_P/R_\mathrm{cg}$.
\item The exponential structure $\Gamma_\mathrm{cross} \sim \exp[-\alpha\int_\mathrm{path}\sigma\,d\ell]$ for substrate-mediated cross-region exchange.
\item The five leading-order coarse-grained consequences (scalar diffusion, directional viscosity, V1→R1, V5→Maxwell, T17 minimal coupling).
\item The non-Abelian generalization to Yang–Mills.
\item The coarse-graining-invariance condition for leading-order outputs.
\item The substrate-cutoff structure of first-subleading corrections.
\item The boundaries of DCGT applicability (matter-wave Q-C boundary, BH saturated zone, deep-MOND superluminality regime).
\end{itemize}

\subsection{3.11.2 What is value-INHERITED at this layer}

\begin{itemize}
\item The specific functional shape of the V1 finite-width vacuum kernel (the existence and finite-width are FORCED; the closed-form shape is INHERITED).
\item The specific dimensionless prefactor $\alpha$ in the cross-bandwidth exponential structure.
\item The specific diffusion coefficient in scalar diffusion (set by substrate-channel statistics; INHERITED).
\item The specific viscosity coefficient in directional viscosity (INHERITED).
\item The specific R1 cutoff coefficient combination $\kappa\mu_\mathrm{V1}\ell_P^2$ (INHERITED; appears as the small substrate-cutoff prefactor).
\item The specific Maxwell relaxation time $\tau_R$ (INHERITED from molecular or substrate physics).
\item The specific T17 minimal-coupling charge values (INHERITED from gauge-group empirical input).
\item The specific numerical thresholds at the boundaries of DCGT applicability (the matter-wave boundary value of 140–250 kDa, the BH-3 saturation threshold $\beta_\mathrm{crit}$, the deep-MOND transition acceleration $a_0$).
\end{itemize}

\subsection{3.11.3 Methodological consistency at the coarse-graining bridge}
...
\section{3.12 Dependencies}

\subsection{3.12.1 Upstream}

\begin{itemize}
\item \textbf{Chapter 1.} P01 event discreteness (sets the substrate length scale floor); P04 bandwidth update (controls the substrate's bounded-bandwidth content); finite-kernel commitments (V1, V5 finite-width); substrate-locality commitments (controls the path-integration structure of $\Gamma_\mathrm{cross}$).
\item \textbf{Chapter 2.} Load-bearing invariants — particularly $\sigma$ (entering the cross-bandwidth exponent), V1 (entering the substrate-cutoff structure), and the $\Gamma_\mathrm{cross}$ definition that DCGT formalizes.
\end{itemize}

\subsection{3.12.2 Downstream}

\begin{itemize}
\item \textbf{Chapter 4 (Theorem N1 + Theorem 18).} Theorem N1 establishes V1 as a finite-width chain-sourced response kernel, formally completing the V1 commitment that DCGT consumes. Theorem 18 propagates P11 + chain structure through V1 to forward-cone-only support, formally completing the kernel-arrow content that DCGT inherits.
\item \textbf{Chapter 5 (Phase-1 closure).} DCGT's V1 mediation enters the Born-rule and Schrödinger-equation derivations indirectly through bandwidth-conservation structure.
\item \textbf{Chapter 6 (Form-level QFT).} DCGT's T17 minimal-coupling content is the substrate-to-continuum bridge for gauge fields; vacuum-response structure passes through DCGT's V1 content.
\item \textbf{Chapter 7 (Quantum computing).} The cross-bandwidth structure $\Gamma_\mathrm{cross}$ enters UR-1's condition (ii). The multi-scale expansion underlies the unresolvedness $\mathcal{U}$'s three-factor product structure.
\item \textbf{Chapter 8 (Navier–Stokes).} DCGT directly produces the viscous sector and the V1→R1 substrate-cutoff regularization.
\item \textbf{Chapter 9 (MHD and Yang–Mills).} DCGT's T17 minimal coupling produces the Lorentz force; the non-Abelian DCGT generalization produces Yang–Mills; V1's second-moment produces the YM mass-gap.
\item \textbf{Chapter 10 (Soft-matter mobility).} DCGT's scalar-diffusion content produces the Universal Mobility Law; V5→Maxwell produces viscoelasticity.
\item \textbf{Chapter 11 (Substrate gravity).} DCGT's cumulative-strain coarse-graining is the substrate-to-continuum bridge for the modified Poisson equation.
\item \textbf{Chapter 12 (Curvature emergence).} DCGT's acoustic-metric kinematic-summary content underlies the scalar-tensor covariantization.
\item \textbf{Chapter 13 (Black-hole architecture).} DCGT's cross-bandwidth structure governs horizon formation; V1's per-patch motif content governs the area-law entropy form.
\item \textbf{Chapter 14 (Cross-platform unifications).} The cross-domain identity between BH horizon formation and QC condition (ii) failure rests on the same DCGT-derived $\Gamma_\mathrm{cross}$ structure.
\item \textbf{Chapter 15 (Public test inventory).} Empirical anchors calibrate the value-INHERITED coefficients in DCGT's leading-order and substrate-cutoff outputs.
\end{itemize}

The graph is comprehensive. Every chapter from Chapter 4 onward depends on DCGT directly or through a chapter that does.

\section{3.13 Canonical Sources}

\begin{itemize}
\item Arc D memos in \texttt{theory/Arc\_D/}
\item NS Synthesis Paper Appendix D — \texttt{papers/Navier Stokes\_Synthesis\_Paper/}
\item ED-QFT Unified Overview Paper — \texttt{papers/ED\_QFT\_Overview/}
\end{itemize}
...
\item \textbf{The cosmological arrow} says the universe is expanding, and forward in time is the direction in which the cosmos gets bigger. Also correct, but the expansion itself is a cosmic boundary condition rather than a derivation.
\item \textbf{The measurement arrow} says wavefunction collapse is irreversible. The collapse is a postulate of standard quantum mechanics, not a structural consequence of the rest of the theory.
\item \textbf{The radiation arrow} says Maxwell's equations are time-symmetric, but only the retarded Green's function is used in practice. The physicists pick the retarded solution because the advanced solution gives anti-causal absurdities. The arrow here is a solution-selection rule applied externally to a time-symmetric equation — it works, but it is a choice rather than a derivation.
\end{itemize}

Each account is correct as far as it goes. None of them locates the arrow \textit{inside} the laws. Each either passes the buck (to a boundary condition, to a postulate, to a solution-selection rule) or hangs the arrow on something else that is itself unjustified.

\subsection{4.2.2 ED's structural position}
...
\subsection{4.2.3 What the kernel-level arrow is, exactly}
...
This is not the same as the thermodynamic arrow (entropy increase), the cosmological arrow (cosmic expansion direction), the measurement arrow (wavefunction collapse), or the radiation arrow (retarded Green's function selection). It is the \textit{substrate-level} arrow at the layer of vacuum response. Standard physics has this at the level of a postulate (the iε prescription, upper-half-plane analyticity, retarded Green's function selection) or as a solution-selection rule applied externally. ED \textit{derives} it.

\section{4.3 Theorem N1: The V1 Finite-Width Vacuum Kernel}

\subsection{4.3.1 What N1 establishes}
...
\begin{enumerate}
\item \textbf{Existence.} V1 exists as a substrate-level vacuum response kernel. It mediates participation events — a perturbation at one chain endpoint produces response at other endpoints through the V1 kernel.
\item \textbf{Finite width.} V1 has nonzero temporal width at the substrate scale. It is not an instantaneous delta function. The width is INHERITED — Theorem N1 establishes that the width is finite and substrate-determined, but the specific functional shape of the kernel is not closed-form derived from any other primitive.
\item \textbf{Chain-sourced.} V1 is sourced by chain-level participation events. The kernel is not a free-standing vacuum object that exists independently of the chain ensemble; it is constructed from the contributions of chain dynamics. This is the structural content that Theorem 18 will use to propagate P11-irreversibility through the kernel.
\end{enumerate}

\subsection{4.3.2 Why finite-width matters structurally}
...
\begin{itemize}
\item \textbf{Theorem 18 retardation (Section 4.4).} The forward-cone-only support derived in Theorem 18 requires V1's finite width as a structural input. A delta-function kernel would not admit the chain-summed structure that Theorem 18 uses.
\item \textbf{R1 substrate-cutoff regularization in Navier–Stokes (Chapter 8).} V1's finite width produces, at first subleading order in the DCGT multi-scale expansion, the hyperviscous correction term $-\kappa\mu_\mathrm{V1}\ell_P^2 \nabla^4\mathbf{v}$. This is the R1 mechanism. Without finite-width V1, the substrate-cutoff regularization that supplies the Clay-NS-relevant Intermediate Path C verdict would not exist.
\item \textbf{Yang–Mills mass-gap mechanism (Chapter 9).} V1's second-moment expansion under DCGT non-Abelian generalization produces the substrate-level mass-gap. The second-moment is well-defined precisely because V1 has finite width.
\item \textbf{Black-hole area-law motif alphabet (Chapter 13).} V1's per-patch substrate temporal width sets the per-patch motif alphabet $g$ that enters the BH area-law entropy expression $S = (A/\ell_P^2)\log g$.
\end{itemize}

\subsection{4.3.3 What N1 does \textit{not} establish}
...
\subsection{4.3.4 N1-E directionality}
...
The strengthening is \textit{over-determined-forced}: N1-E directionality is reachable through two independent paths (cascade-from-V1 and R1-bypass redundancy), so it is forced even in regimes where one of the paths fails. Section 4.6 develops this over-determined-forced status as a structural strengthening of the kernel-level arrow.

\section{4.4 Theorem 18: V1 Kernel Retardation}

\subsection{4.4.1 The theorem statement}

Theorem 18 states: \textit{given the ED primitive set, P11 commitment-irreversibility (taken as primitive), the chain worldline structure (P02), the bandwidth update rule (P04), the proper-time ordering (P13), Theorem N1 (V1 finite-width vacuum kernel), and the effective-vacuum factorisation (Q.8), the V1 vacuum response kernel is uniquely forced at primitive level to have support restricted to the forward causal cone; no symmetric, advanced, or hybrid kernel is constructible at primitive level; the microscopic arrow of time is structurally FORCED at the kernel level; form FORCED (retardation), value INHERITED (specific kernel functional form $G(\sigma)$, continuum-approximation propagator-boundary-condition specifics).}

The theorem's structure has three parts:
\begin{enumerate}
\item \textbf{Inputs.} A specified subset of the substrate primitives, plus Theorem N1, plus the effective-vacuum factorisation (Q.8) that connects V1 to the chain-contribution sum.
\item \textbf{Forcing chain.} A constructive argument showing that the inputs jointly force V1 to have forward-cone-only support.
\item \textbf{Refutation sweep.} A demonstration that none of the alternative kernels (symmetric, advanced, hybrid variants) is constructible from the same inputs. This is what makes the theorem \textit{forcing} rather than merely \textit{consistent} — alternatives are not just refuted, they are non-constructible.
\end{enumerate}

\subsection{4.4.2 The forcing chain in one breath}
...
\subsection{4.4.3 The refutation sweep}

Theorem 18's refutation sweep dispatches the alternative kernel candidates by \textit{non-construction} rather than by external constraint:

\begin{itemize}
\item \textbf{Symmetric V1 (BC3).} A symmetric vacuum response kernel would require the chain-summed kernel to have backward-cone support as well as forward-cone. But there are no chain contributions with backward-cone support, because every chain's forward-propagator vanishes for backward chain-proper-time arguments (P11 + P02 + P04 + P13). The chain-summed kernel therefore cannot acquire symmetric content; it has no source. \textbf{REFUTED-by-non-construction.}
\item \textbf{Advanced V1.} An advanced kernel would require everything to run backward — P11 inverted, P13 reversed, P07 vacuous, N1-E cascade conflict. Multiple substrate primitives would have to fail simultaneously. \textbf{REFUTED-by-multi-front-contradiction.}
\item \textbf{Hybrid variants.} Several hybrid candidates were considered: H1 (forward + ε backward), H2 (angular-restricted backward), H3 (weighted symmetric), H4 (symmetric core × envelope). Each requires backward-cone support in some weighting, and each runs into the same non-construction argument as the symmetric kernel. \textbf{All REFUTED by the same non-construction structure.}
\item \textbf{PDE T-symmetry cancellation.} A standard objection is that the underlying continuum PDE (the wave equation, the Schrödinger equation) is time-symmetric, so a symmetric kernel might emerge from the symmetric PDE despite primitive-level asymmetry. This objection is \textbf{REFUTED}: the PDE is direction-neutral, but it does not override the kernel-construction selection at the primitive level. The PDE describes solutions; the kernel is a specific construction from the chain ensemble, and the chain ensemble is not direction-neutral.
\end{itemize}

The refutation sweep's structural significance is that the alternative kernels are not merely inconsistent with observation; they are \textit{non-constructible} from the substrate primitives. There is no weighted average, no perturbative correction, no boundary-condition adjustment that produces them. The forward-cone-only kernel is the only kernel a chain ensemble can produce.

\subsection{4.4.4 The structural status of the result}

Theorem 18 belongs to the program's \textit{forced theorems} — results whose conclusions are not chosen but are uniquely produced by the substrate inputs. The forced-theorem inventory at Theorem 18's closure stands at eighteen: T1 through T16 from Phase-1 closure of QM, T17 (gauge-field-as-rule-type), and Theorem 18 itself.
...
\section{4.5 Distinguishing Response Kernel from Correlator: The CR Framing}

\subsection{4.5.1 The Wightman correlator is a different object}
...
The objection rests on a conflation. The Wightman correlator and the response kernel are \textit{different objects}:

\begin{itemize}
\item \textbf{The Wightman correlator} measures intrinsic vacuum fluctuations — what the vacuum looks like to itself, in equilibrium, with no perturbation applied.
\item \textbf{The response kernel} measures something different: how the vacuum \textit{responds} to a perturbation that arrives from somewhere.
\end{itemize}
...
\subsection{4.5.2 The continuum-approximation framing}

The framework formalizes this distinction as the \textit{CR (continuum-approximation) framing}:

\begin{itemize}
\item \textbf{Primitive level.} V1 is the substrate-level response kernel, forced by Theorem 18 to have forward-cone-only support.
\item \textbf{Continuum approximation.} Standard QFT objects (the Feynman propagator $G_F$, the retarded Green's function $G_R$, the advanced Green's function $G_A$, the Wightman correlator $W$) are continuum-level effective machinery, each playing distinct physical roles.
\item \textbf{Correspondence.} The natural correspondence is primitive-level retarded V1 ↔ continuum-approximation $G_R$. The other continuum objects ($G_F$, $G_A$, $W$) remain admissible at the continuum level as effective machinery for distinct physical roles (time-ordered amplitudes for $G_F$; certain scattering-theory uses for $G_A$; intrinsic vacuum fluctuations for $W$).
\end{itemize}
...
\begin{itemize}
\item \textbf{Cascade-from-V1.} N1-E inherits its forward-only chain-time character from P11 + P02 + P04 + P13 propagated through the chain-summed kernel construction (Theorem 18's primary forcing chain).
\item \textbf{R1-bypass redundancy.} Even in regimes where the chain-summed-kernel argument is weakened (e.g., extreme coarse-graining, marginal scale separation), N1-E's directional content is reachable through the R1 substrate-cutoff content and its first-subleading-order propagation in DCGT (Chapter 3).
\end{itemize}

The two paths are independent in the sense that they use different intermediate substrate machinery to reach the same conclusion. Either path alone suffices to force N1-E forward-only directionality; the existence of two paths makes the directionality \textit{over-determined-forced}.

\subsection{4.6.2 Why over-determination is structurally significant}
...
\subsection{4.6.3 Implications for downstream results}

The over-determined-forced status of N1-E feeds into several downstream chapters:

\begin{itemize}
\item \textbf{Phase-3 GR1 (curved-spacetime extension).} When V1 is extended to curved spacetime via Hadamard-parametrix causal-future restriction with Synge world function, the V1 retarded support is preserved. The over-determined-forced status of N1-E is what makes this preservation robust: even in curved-spacetime regimes where some substrate machinery is more delicate, the directionality survives.
\item \textbf{T17 vacuum coupling refinement.} T17's clauses C5–C7 are refined to forward-only directionality within sectors, inheriting the kernel-level arrow.
\item \textbf{Q.5 vacuum-polarisation analytic structure.} Standard QFT's upper-half-plane analyticity for vacuum-polarisation diagrams is \textit{postulated} in conventional treatments; Theorem 18 supplies the substrate-level structural foundation for it. The over-determined-forced status means this foundation is robust.
\end{itemize}

\section{4.7 The Forcing Chain in Detail}

\subsection{4.7.1 Step 1: V1 is a response kernel sourced by chains}
...
\subsection{4.7.2 Step 2: chain forward-propagator vanishes for backward arguments}
...
\begin{itemize}
\item \textbf{P11 commitment-irreversibility.} Once a chain commits at proper-time position $m$, the commitment cannot affect earlier chain-proper-time positions. There is no commitment-erasing or commitment-undoing dynamics.
\item \textbf{P02 chain worldline structure.} Chains are coherent worldlines with persistent identity. A chain at chain-proper-time $m$ is structurally connected to its earlier and later chain-proper-time positions through the chain identity.
\item \textbf{P04 bandwidth update rule.} The chain's bandwidth update at each proper-time step is forward-only — it updates the current chain state from its previous state, not the other way around.
\item \textbf{P13 proper-time ordering.} Each chain carries an intrinsic proper-time order; this ordering is what makes "earlier" and "later" meaningful at the chain level.
\end{itemize}
...
\subsection{4.7.3 Step 3: chain-summed kernel has forward-cone-only support}
...
\subsection{4.7.4 Step 4: alternative kernels are non-constructible}

The final step is the refutation sweep: alternative kernels (symmetric, advanced, hybrid) are not just inconsistent with observation; they are \textit{non-constructible} from the substrate inputs. There is no recipe to build them from chain contributions, because the chain contributions do not have the backward-cone content that the alternatives would require.
...
\section{4.8 What Is and Is Not Solved}

\subsection{4.8.1 The kernel-level arrow is solved}
...
\subsection{4.8.2 The other arrows are out of scope}

Four other arrows of time studied in standard physics are explicitly \textit{out of scope} for Theorem 18:
...
\item \textbf{The cosmological arrow.} Why is the universe expanding? The framework treats this as substrate-level open work (Arc COSMO, flagged in Chapter 15). The cosmic horizon enters through T20 (Chapter 11) and Arc ED-10 (Chapter 12), but the cosmological arrow itself — the directionality of cosmic expansion — has not been derived from the substrate.
\item \textbf{The measurement arrow.} Why is wavefunction collapse irreversible? The Phase-1 closure (Chapter 5) reframes this as the coarse-grained signature of substrate-level commitment events through P11. In this sense the measurement arrow inherits Theorem 18's substrate-level foundation directly: P11's irreversibility, propagated through the kernel-level arrow, supplies the structural origin of measurement irreversibility.
\item \textbf{The radiation arrow.} Why do we use the retarded Green's function? Theorem 18 supplies the structural answer: the substrate-level kernel V1 is uniquely forced to be retarded, and the continuum-approximation $G_R$ inherits this character. This is the framework's substrate-level resolution of the radiation arrow's standard-physics solution-selection rule.
\end{itemize}

The kernel-level arrow is therefore the \textit{structural prerequisite} for the other arrows. Each downstream arrow either inherits Theorem 18 as its kernel-level foundation (measurement, radiation) or remains substrate-level open work (thermodynamic, cosmological) that would build on Theorem 18's content if pursued.

\subsection{4.8.3 The structural significance of the kernel-level placement}
...
\section{4.9 Theorem 18's Cross-Arc Strengthening}

\subsection{4.9.1 Strengthening Phase-3 GR1}
...
\subsection{4.9.2 Strengthening T17 vacuum coupling}
...
\subsection{4.9.3 Strengthening Q.5 vacuum-polarisation analytic structure}
...
This is one of Theorem 18's most consequential downstream applications. Standard QFT has been using upper-half-plane analyticity as a postulate for decades; ED supplies the structural foundation. The framework does not change any standard-QFT calculation, but it changes the \textit{story underneath} — the analyticity is no longer a postulate floating above the laws, it is the continuum-level reflection of the substrate-level kernel arrow.

\subsection{4.9.4 Strengthening Arc M F-M8 mass-form mediation}
...
\subsection{4.9.5 Preservation of direction-neutral substrate}
...
\begin{itemize}
\item \textbf{Arc R Cl(3,1) signature.} The substrate-level signature on the spinor bundle remains direction-neutral.
\item \textbf{R.2.5 spin-statistics.} The substrate-level spin-statistics theorem is preserved.
\item \textbf{R.3 Dirac equation.} The Dirac equation's substrate-level derivation remains direction-aligned with retarded V1.
\item \textbf{ED-Phys-10 acoustic-metric guardrails.} The acoustic-metric kinematic-summary content of Chapter 12 inherits Theorem 18's directionality but does not require additional refinement.
\end{itemize}

\section{4.10 Form-FORCED vs Value-INHERITED at the Kernel-Level Arrow}

\subsection{4.10.1 What is form-FORCED at this layer}

\begin{itemize}
\item \textbf{V1's existence as a substrate-level vacuum response kernel} (Theorem N1).
\item \textbf{V1's finite-width character at the substrate scale} (Theorem N1).
\item \textbf{V1's chain-sourced construction} (Theorem N1 + Q.8 effective-vacuum factorisation).
\item \textbf{V1's forward-cone-only support} (Theorem 18).
\item \textbf{The non-constructibility of symmetric, advanced, and hybrid kernel alternatives} (Theorem 18 refutation sweep).
\item \textbf{The over-determined-forced status of N1-E directionality} (cascade-from-V1 + R1-bypass redundancy).
\item \textbf{The CR framing distinguishing primitive-level retarded V1 from continuum-level Wightman correlator} (an interpretive/structural framing, but the framing's content — that the two are different objects — is FORCED by the structure of the substrate-to-continuum bridge).
\end{itemize}

\subsection{4.10.2 What is value-INHERITED at this layer}

\begin{itemize}
\item \textbf{The specific functional shape $G(\sigma)$ of the V1 kernel.} Finite-width is FORCED; closed-form shape is INHERITED.
\item \textbf{Continuum-approximation propagator-boundary-condition specifics} (the iε prescription's specific small parameter, the contour-deformation conventions in standard QFT). These are continuum-level conventions that inherit consistency with substrate-level V1 retardation but are not closed-form derived from substrate primitives.
\item \textbf{Specific numerical thresholds} for testing kernel-arrow-discriminating signatures (the energy or length scales at which substrate-level deviations from standard-QFT causal structure could in principle become observable). These would be calibrated against any future near-substrate-scale experiments.
\end{itemize}

\subsection{4.10.3 What this means for empirical posture}
...
\section{4.11 Dependencies}

\subsection{4.11.1 Upstream}

\begin{itemize}
\item \textbf{Chapter 1 (substrate primitives).} Especially P11 commitment-irreversibility (the only direction-bearing primitive); P02 chain worldline structure; P04 bandwidth update rule; P13 proper-time ordering; finite-kernel commitment (V1); substrate locality.
\item \textbf{Chapter 2 (load-bearing invariants).} V1 as load-bearing invariant; gradient sparsity $\sigma$ as the substrate-scale measure entering the cross-bandwidth structure that is mediated by V1 in some downstream applications.
\item \textbf{Chapter 3 (DCGT as continuum-approximation bridge).} The CR framing that distinguishes primitive-level retarded V1 from continuum-level effective propagators relies on DCGT's substrate-to-continuum bridge to make the two-layer correspondence precise.
\end{itemize}

\subsection{4.11.2 Downstream}

\begin{itemize}
\item \textbf{Chapter 5 (Phase-1 closure).} The measurement rule emerges from P11 commitment-irreversibility; Theorem 18 supplies the kernel-level structural foundation for the directional content of measurement.
\item \textbf{Chapter 6 (Form-level QFT).} T17 vacuum coupling is refined to forward-only directionality. Q.5 vacuum-polarisation analytic structure inherits Theorem 18's structural foundation.
\item \textbf{Chapter 7 (Quantum computing).} UR-1 condition (iii) commitment-injection-bounded inherits P11's irreversibility through Theorem 18's kernel-level arrow. The Poisson-class commitment-event accumulation that defines the third UR-1 condition is the QC-platform-specific reading of the kernel-level arrow.
\item \textbf{Chapter 8 (Navier–Stokes).} R1 substrate-cutoff regularization arises from V1's finite width; Theorem 18 supplies the directional content of how V1 couples to the dynamics.
\item \textbf{Chapter 9 (MHD and Yang–Mills).} YM mass-gap mechanism arises from V1's second-moment expansion; the mass-gap inherits V1's directional character.
\item \textbf{Chapter 10 (Soft-matter mobility).} V5 cross-chain memory under DCGT produces Maxwell viscoelasticity; the directional character of memory is supplied by Theorem 18-class arguments applied to V5.
\item \textbf{Chapter 11 (Substrate gravity).} Theorem 18's kernel-level arrow lifts to curved-spacetime backgrounds via Phase-3 GR1; substrate-gravity content inherits the directionality.
\item \textbf{Chapter 12 (Curvature emergence).} Acoustic-metric kinematic content respects ED-Phys-10 guardrails and is consistent with Theorem 18's kernel-arrow structure.
\item \textbf{Chapter 13 (Black-hole architecture).} BH-4 information-non-paradox uses P11 commitment-irreversibility directly; Theorem 18 supplies the kernel-level structural foundation for the substrate-level information architecture (committed structure cannot un-commit, so it cannot cross a horizon and re-emerge).
\item \textbf{Chapter 14 (Cross-platform unifications).} The form-FORCED methodology, including the kernel-level arrow as a substrate signature, is a cross-domain consistency check.
\end{itemize}

\section{4.12 Canonical Sources}

\begin{itemize}
\item \texttt{papers/Time\_Arrow\_Theorem\_18/}
\item Theorem N1 paper (in V1 vacuum kernel inventory)
\item \texttt{arcs/arc-B/} (Arc B closure memos: arc\_b\_scoping, arrow\_catalogue, arrow\_forced, arrow\_refuted, arrow\_implications, arc\_b\_synthesis)
\end{itemize}

The Time\_Arrow\_Theorem\_18 paper presents Theorem 18 in publication-grade form with the formal forcing chain and the refutation sweep. The Theorem N1 paper presents the V1 finite-width vacuum kernel theorem. The Arc B closure memos in \texttt{arcs/arc-B/} contain the six-substage Arc B trajectory (B.0 scoping → B.1 catalogue → B.2 forced-evaluation → B.3 refuted-evaluation → B.4 cross-arc implications → B.5 synthesis) that produced the closure of Theorem 18.
...
\begin{itemize}
\item \textbf{The Born rule.} Probability of an outcome equals the squared amplitude. Why squared? The textbooks do not say.
\item \textbf{The Schrödinger equation.} Wavefunctions evolve in time according to $i\hbar\partial_t |\psi\rangle = \hat H |\psi\rangle$. Why this specific functional form — linearity, first-order in time, Hamiltonian content? The textbooks do not say.
\item \textbf{The Heisenberg uncertainty principle.} $\Delta x \cdot \Delta p \geq \hbar/2$. Why $\hbar/2$? Why this specific lower bound? The textbooks do not say.
\item \textbf{The measurement rule.} When a quantum system is measured, the wavefunction "collapses" into a single outcome, and the collapse is irreversible. Why is collapse irreversible? Why is it a separate postulate from unitary evolution? The textbooks do not say.
\end{itemize}
...
\section{5.3 The Born Rule: Substrate Non-Contextuality + Gleason 1957}

\subsection{5.3.1 The structural question}

Why is probability the \textit{squared} amplitude rather than the linear amplitude, the fourth power, or the absolute value? Standard QM treats this as a postulate; the squared form is an empirical fact that is built into the theory's axioms.

ED reframes the question. The "amplitude" in standard QM is the participation-bandwidth content of a substrate region distributed across the orthogonal decomposition associated with a measurement basis. The probability of an outcome is the bandwidth that \textit{commits} to that outcome under P11. The structural question becomes: how is bandwidth distributed across an orthogonal decomposition, and what functional form does the distribution take?

\subsection{5.3.2 The substrate inputs}

Two substrate-level commitments are load-bearing:

\begin{itemize}
\item \textbf{Substrate non-contextuality.} The bandwidth assigned to a measurement outcome cannot depend on which other outcomes are simultaneously being considered (i.e., which orthogonal decomposition the outcome appears in). The substrate's bandwidth distribution is a property of the outcome itself relative to the substrate state, not a property of the measurement context. This is a direct substrate-ontological consequence of P04 (bandwidth update rule applied at chain endpoints) plus the participation-channel structure committed by the participation primitives.
\item \textbf{Bandwidth conservation across orthogonal decompositions.} The total bandwidth across any complete orthogonal decomposition equals the substrate region's total bandwidth. This is the substrate-level non-contextuality condition combined with bandwidth boundedness from P04.
\end{itemize}
...
\subsection{5.3.3 The mathematical bridge: Gleason 1957}
...
\subsection{5.3.4 Why the squared form is FORCED}
...
\subsection{5.3.5 Born-Gleason and the U1 paper}

The full derivation of the Born rule from substrate non-contextuality plus Gleason's theorem is presented in \texttt{papers/Born\_Gleason/} and in \texttt{papers/U1\_Participation\_Measure/}. The U1 paper establishes the substrate participation measure as the substrate-level analogue of the projective measure on Hilbert space; the Born-Gleason paper completes the bridge to the squared probability form.

\section{5.4 The Schrödinger Equation: Time-Translation Symmetry + Stone 1932}

\subsection{5.4.1 The structural question}
...
\subsection{5.4.2 The substrate inputs}

The substrate-level inputs:

\begin{itemize}
\item \textbf{Substrate time-translation symmetry.} The substrate's dynamical rules do not depend on which moment is "now." This is a substrate-ontological commitment that follows from the participation primitives plus P13 proper-time ordering (the ordering is intrinsic to chains, not to an external time coordinate that breaks time-translation symmetry).
\item \textbf{Bandwidth conservation under unitary evolution.} In the thin-participation regime (where commitments have not yet occurred), substrate dynamics preserve total bandwidth. This is the substrate-level analogue of probability conservation in standard QM, derived from P04 plus the absence of commitment events in the thin regime.
\item \textbf{Continuity of the evolution rule.} The substrate's time-evolution generates a one-parameter family of unitary transformations that depends continuously on the time parameter. This is a substrate-level regularity condition that the substrate's chain dynamics satisfy.
\end{itemize}

\subsection{5.4.3 The mathematical bridge: Stone 1932}
...
\subsection{5.4.4 Why the Schrödinger form is FORCED}

Each structural commitment in the Schrödinger equation is forced:

\begin{itemize}
\item \textbf{Existence.} The substrate-level evolution exists because substrate dynamics evolve in time (this is what time-translation symmetry says).
\item \textbf{Uniqueness.} Stone's theorem produces a unique generator $H$ for the one-parameter family.
\item \textbf{Linearity.} Unitary evolution is linear by construction; the substrate-level bandwidth conservation under unitary evolution forces linearity.
\item \textbf{First-order-in-time.} Stone's theorem produces a first-order ODE in $t$; second-order or higher would be a stronger continuity condition, but Stone's continuity hypothesis is the minimal condition for substrate time-translation symmetry to apply.
\item \textbf{Hamiltonian content.} Stone's generator $H$ is Hermitian; this is the substrate's Hamiltonian content, identified through the U3 and U4 paper chain.
\end{itemize}

The full derivation is presented in \texttt{papers/U3\_Time\_Translation\_Schrodinger/} and \texttt{papers/U4\_Hamiltonian\_Form/}. The U2 paper (\texttt{papers/U2\_Inner\_Product/}) establishes the substrate-level inner-product structure that makes Hilbert space the appropriate target for Stone's theorem; the U5 paper (\texttt{papers/U5\_Translation\_Momentum/}) extends the construction to spatial translations and identifies the substrate-level momentum content.

\section{5.5 The Heisenberg Uncertainty Principle: Substrate Finite-Bandwidth}

\subsection{5.5.1 The structural question}
...
\subsection{5.5.2 The substrate inputs}

The substrate-level inputs:

\begin{itemize}
\item \textbf{P04 bandwidth update rule.} Participation channels carry bounded bandwidth; the bandwidth update rule constrains how the substrate's bandwidth distributes across complementary observables (position and momentum being one canonical pair).
\item \textbf{Substrate position-momentum complementarity.} Position and momentum at the substrate level are complementary participation channels — sharper localization in position requires broader distribution in momentum, by a substrate-level bandwidth-conservation argument.
\item \textbf{Bandwidth quantization.} Bandwidth at chain endpoints is quantized in units of $\hbar/2$. This is the substrate-natural quantization unit; its specific value $\hbar$ is INHERITED, but the factor of $1/2$ is structurally fixed.
\end{itemize}

\subsection{5.5.3 The structural argument}
...
\subsection{5.5.4 Why the $\hbar/2$ factor is FORCED}

The $\hbar/2$ factor is the same factor that appears in:

\begin{itemize}
\item \textbf{The Born rule derivation} (Section 5.3): bandwidth-conservation across orthogonal decompositions is normalized by the same substrate quantum that fixes the Heisenberg lower bound.
\item \textbf{The Schrödinger equation's kinetic-energy term} (Section 5.4): the substrate-natural coupling between participation density and chain proper-time evolution is governed by the same substrate quantum.
\end{itemize}
...
\section{5.6 The Measurement Rule: P11 Commitment-Irreversibility}

\subsection{5.6.1 The structural question}
...
\subsection{5.6.2 The substrate-level mechanism}

The substrate-level analogue of measurement:

\begin{itemize}
\item \textbf{The substrate region representing the system} has its participation channels distributed across the available outcomes — the substrate analogue of the wavefunction's superposition over measurement-basis states.
\item \textbf{The measurement apparatus}, via its substrate coupling to the system, applies a structural constraint (effectively, an environmental ED-injection event) that triggers commitment events at the system's chain endpoints.
\item \textbf{P11 commitment-irreversibility} says that once a substrate event commits, the commitment cannot be reversed. The system's chain endpoints commit to a specific outcome; the commitment is irreversible.
\item \textbf{The commitment events}, when coarse-grained to the continuum level via DCGT (Chapter 3), produce the appearance of wavefunction collapse: the substrate-level participation across multiple outcomes is replaced by participation concentrated at a single committed outcome.
\end{itemize}
...
\section{5.7 The Sixteen-Theorem Inventory}

\subsection{5.7.1 The Phase-1 closure structure}
...
\begin{itemize}
\item \textbf{T-Born group.} Substrate participation measure (U1); Hilbert-space inner-product structure (U2); substrate non-contextuality and bandwidth-conservation arguments; Gleason 1957 application; Born rule's squared form. The U1 paper supplies the substrate-level participation measure; U2 supplies the inner-product structure; Born\_Gleason completes the bridge.
\item \textbf{T-Schrödinger group.} Substrate time-translation symmetry; one-parameter unitary family; Stone 1932 application; Schrödinger equation in canonical form; Hamiltonian content (U4); spatial translations and momentum content (U5). The U3 paper supplies the time-translation closure; U4 supplies the Hamiltonian form; U5 extends to momentum.
\item \textbf{T-Uncertainty group.} Substrate finite-bandwidth; complementary-observable structure; bandwidth-quantization at chain endpoints; Heisenberg lower bound with the $\hbar/2$ factor; canonical commutation relations as substrate-level structural consequence.
\item \textbf{T-Measurement group.} P11 commitment-irreversibility; chain-endpoint commitment events; coarse-graining to continuum collapse; Theorem 18 kernel-level arrow connection; no separate collapse postulate.
\end{itemize}

The full sixteen-theorem inventory is presented in \texttt{papers/Phase\_1/} and \texttt{papers/QM\_Emergence\_Structural\_Completion/}. The inventory's structural significance is that the four postulates of QM are each closed at the substrate level — none requires axiomatic status; each is a derived theorem.

\subsection{5.7.2 The forced-theorem methodology}

Each of T1–T16 is a \textit{forced theorem} in the program's methodological sense: its conclusion is uniquely produced by the substrate inputs combined with mathematical structure. Forced theorems differ from ordinary theorems in that:

\begin{itemize}
\item \textbf{The conclusion is not chosen.} Alternative conclusions are not just refuted by external constraint; they are non-constructible from the inputs.
\item \textbf{The forcing chain is auditable.} Each step in the derivation is identified, and the substrate inputs at each step are listed.
\item \textbf{The form-FORCED / value-INHERITED distinction is explicit.} Each theorem identifies which content is structurally fixed by the substrate (form-FORCED) and which content inherits from substrate constants whose specific values come from outside the primitive layer (value-INHERITED).
\end{itemize}
...
\section{5.8 Extensions: Spin-Statistics, Dirac, $g=2$, Canonical Commutation}
...
\subsection{5.8.1 Spin-statistics}
...
\subsection{5.8.2 The Dirac equation}
...
\subsection{5.8.3 The $g=2$ electron magnetic moment}
...
\subsection{5.8.4 Canonical commutation relations}
...
\subsection{5.8.5 What the extensions establish}
...
\section{5.9 What Phase-1 Changes About Quantum Mechanics}

\subsection{5.9.1 No experiment runs differently}

Standard QM's empirical content is preserved exactly. The Born rule is squared; the Schrödinger equation is unchanged; the Heisenberg bound is the same; measurement is irreversible. Every experiment that has ever validated quantum mechanics continues to validate it. The framework changes the \textit{story underneath} the postulates, not the postulates' empirical content.

\subsection{5.9.2 The interpretive frame shifts}

Three structural changes:

\begin{itemize}
\item \textbf{The four postulates are no longer postulates.} They are derived theorems with substrate-level structural origin.
\item \textbf{The measurement problem dissolves.} Wavefunction collapse is identified as the coarse-grained signature of substrate-level P11 commitment events; there is no separate collapse postulate, and no observer dependence is required.
\item \textbf{Quantum mechanics becomes a coarse-grained reading of substrate dynamics in the thin-participation regime.} Standard QM is what substrate dynamics look like when commitment has not yet occurred and substrate dynamics admit unitary, reversible evolution. Departures from standard QM (decoherence, measurement, classical limits) are the substrate-level signatures of commitment events at chain endpoints.
\end{itemize}

\subsection{5.9.3 The relationship to interpretations of quantum mechanics}
...
\subsection{5.9.4 The transition from substrate to QM}

The transition is auditable:

\begin{enumerate}
\item \textbf{Substrate primitives} (Chapter 1).
\item \textbf{Load-bearing invariants} (Chapter 2).
\item \textbf{DCGT coarse-graining bridge} (Chapter 3).
\item \textbf{Theorem N1 + Theorem 18 kernel-level arrow} (Chapter 4).
\item \textbf{Phase-1 closure of QM postulates} (Chapter 5, this chapter).
\item \textbf{Form-level QFT and quantum information} (Chapter 6).
\item \textbf{Quantum computation architecture} (Chapter 7).
\end{enumerate}
...
\section{5.10 Form-FORCED vs Value-INHERITED at the Phase-1 Closure}

\subsection{5.10.1 What is form-FORCED}

The structural content of T1–T16 is form-FORCED by the substrate inputs combined with the mathematical bridges:

\begin{itemize}
\item \textbf{The Born rule's squared form.} Forced by substrate non-contextuality + bandwidth conservation + Gleason 1957.
\item \textbf{The Schrödinger equation's structural content} (existence, uniqueness, linearity, first-order-in-time, Hamiltonian content). Forced by substrate time-translation symmetry + bandwidth conservation + continuity + Stone 1932.
\item \textbf{The Heisenberg lower bound's functional form.} Forced by substrate finite-bandwidth + complementary-observable structure + bandwidth quantization at $\hbar/2$.
\item \textbf{The measurement rule's irreversibility.} Forced by P11 commitment-irreversibility + the kernel-level arrow of Theorem 18.
\item \textbf{The spin-statistics theorem at substrate level.} Forced by substrate chain-statistics structure.
\item \textbf{The Dirac equation form.} Forced by substrate relativistic structure on the spinor bundle (Cl(3,1) signature derived from R.2.4).
\item \textbf{The $g=2$ leading-order electron magnetic moment.} Forced by substrate-level spinor structure plus Dirac-equation derivation.
\item \textbf{The canonical commutation relations.} Forced by substrate-level position-momentum complementarity.
\end{itemize}

\subsection{5.10.2 What is value-INHERITED}

Specific numerical values that the framework does not derive in closed form at the primitive layer:

\begin{itemize}
\item \textbf{The numerical value of $\hbar$.} Substrate input; not derived from other primitives. The factor of $1/2$ in $\hbar/2$ is form-FORCED; the value of $\hbar$ is INHERITED.
\item \textbf{The specific functional shape of the V1 vacuum kernel.} Used in Phase-1 indirectly through the substrate-level vacuum-response structure that enters Born and Schrödinger arguments. Finite-width is FORCED; closed-form shape is INHERITED.
\item \textbf{Particle-specific masses and coupling constants.} These appear in the Dirac equation and in the $g=2$ corrections; their specific values inherit from particle physics empirical input, not from substrate primitives.
\item \textbf{The Standard Model gauge group.} Phase-1 establishes the form of QM; T17 (Chapter 6) establishes the form of gauge fields; the specific gauge group nature realizes is INHERITED empirical input. The framework derives the \textit{form} of gauge invariance and gauge-field structure without committing to which specific group nature realizes.
\end{itemize}

\subsection{5.10.3 The methodological consistency}
...
\section{5.11 Dependencies}

\subsection{5.11.1 Upstream}

\begin{itemize}
\item \textbf{Chapter 1.} Substrate primitives, especially P04 (bandwidth update rule, foundational for Born and Heisenberg derivations) and P11 (commitment-irreversibility, foundational for the measurement rule). Also P01 (event discreteness, contributing to UV-finiteness in Chapter 6 extensions).
\item \textbf{Chapter 2.} Load-bearing invariants. Multiplicity $\mathcal{M}$ enters the Born-rule derivation through bandwidth-conservation structure. The V1 kernel mediates substrate-level vacuum response that enters the Schrödinger argument.
\item \textbf{Chapter 4.} Theorem N1 + Theorem 18 kernel-level arrow. Supplies the substrate-level structural foundation for the directional content of measurement (through P11 + V1 forward-only support).
\end{itemize}

\subsection{5.11.2 Downstream}

\begin{itemize}
\item \textbf{Chapter 6 (Form-level QFT and Quantum Information).} Builds on Phase-1's QM closure to derive T17 (gauge-field-as-rule-type), UV-finiteness, and the channel-geometry account of quantum information (ED-I-13). Phase-1 supplies the substrate-level QM foundation; Chapter 6 extends to QFT-territory form-level content.
\item \textbf{Chapter 7 (Quantum Computation).} Identifies quantum computation as the engineered occupation of the thin-participation regime that Phase-1 characterizes. Phase-1's substrate-level account of QM is the foundation on which UR-1's three substrate conditions are formulated.
\item \textbf{Chapters 8, 9, 10, 11, 12, 13.} Each consumes Phase-1's QM closure indirectly through DCGT coarse-graining, since the substrate-to-continuum bridge produces standard QM in the thin-participation regime as a baseline.
\item \textbf{Chapter 14 (Cross-platform unifications).} The form-FORCED / value-INHERITED methodology, including Phase-1's pattern of deriving QM postulates from substrate primitives, is one of the cross-domain consistency signatures developed in Chapter 14.
\end{itemize}

\section{5.12 Canonical Sources}

\begin{itemize}
\item \texttt{papers/Phase\_1/}
\item \texttt{papers/QM\_Emergence\_Structural\_Completion/}
\item \texttt{papers/Born\_Gleason/}
\item \texttt{papers/U1\_Participation\_Measure/}
\item \texttt{papers/U2\_Inner\_Product/}
\item \texttt{papers/U3\_Time\_Translation\_Schrodinger/}
\item \texttt{papers/U4\_Hamiltonian\_Form/}
\item \texttt{papers/U5\_Translation\_Momentum/}
\end{itemize}

The Phase\_1 paper presents the sixteen-theorem closure in publication-grade form. The QM\_Emergence\_Structural\_Completion paper integrates the closure with extensions (spin-statistics, Dirac, $g=2$, canonical commutation relations). The Born\_Gleason paper develops the Born-rule derivation. The U1–U5 paper chain develops the substrate-level construction underlying the Schrödinger equation: U1 (participation measure), U2 (inner-product structure), U3 (time-translation closure), U4 (Hamiltonian form), U5 (translation/momentum content).
...
\begin{itemize}
\item \textbf{Where do gauge fields come from?} Standard QFT treats gauge invariance as a fundamental principle: pick a symmetry group, require local invariance, and dynamical gauge fields appear as a consequence. Why this principle? Why these specific gauge groups? Why does nature use gauge fields at all? Standard QFT does not say.
\item \textbf{Why doesn't QFT diverge?} Loop integrals in standard QFT are divergent, and the divergences are absorbed by renormalization. Why does the universe avoid the divergences in practice? Why is renormalization needed in the formalism but not in nature? Standard QFT treats this as a feature of effective-theory machinery rather than as a structural property of reality.
\end{itemize}
...
\subsection{6.2.2 The third question: what is quantum information?}
...
\subsection{6.2.3 What this chapter delivers structurally}

Three closures, each at form level:

\begin{enumerate}
\item \textbf{Gauge structure closure.} T17 establishes the form of gauge fields and gauge invariance from substrate primitives. The Standard Model gauge group remains empirical input; the structural form is FORCED.
\item \textbf{UV-finiteness closure.} Substrate primitives P01 + P13 + P04 deliver UV-finiteness as a substrate property. Renormalization is reframed as continuum-approximation machinery; it is not load-bearing at the substrate level.
\item \textbf{Quantum information closure.} ED-I-13 reinterprets the five landmark QI results as substrate channel-geometry manipulations. Quantum information acquires a substrate-level structural foundation.
\end{enumerate}
...
\section{6.3 Theorem 17: Gauge-Field-as-Rule-Type}

\subsection{6.3.1 The structural question}
...
The structural commitment: T17 is form-FORCED for any compact-simple-group rule-type structure. It does not commit to which specific group nature realizes; the Standard Model's $SU(3) \times SU(2) \times U(1)$ structure is empirical input. What T17 commits to is the \textit{form} — that gauge fields are interface measures of rule-types, that gauge invariance is the rule-type label-relabeling symmetry, and that this form covers Abelian and non-Abelian structures alike.

\subsection{6.3.4 What T17 does and does not derive}

T17 derives:
\begin{itemize}
\item The structural form of gauge fields (as participation measures of rule-types).
\item The structural form of gauge invariance (as interface property under rule-type relabeling).
\item The coverage of Abelian and non-Abelian compact-simple-group structures.
\item The substrate-level mechanism by which gauge fields mediate interactions between label-carrying participation events.
\item The connection to T17-vacuum-coupling-clauses C5–C7, which Theorem 18 (Chapter 4) refines to forward-only directionality.
\end{itemize}

T17 does not derive:
\begin{itemize}
\item Which specific gauge group nature realizes. The Standard Model's $SU(3) \times SU(2) \times U(1)$ structure is empirical input.
\item The specific values of gauge couplings (the fine structure constant $\alpha$, the strong coupling $\alpha_s$, the weak coupling, the Higgs vacuum expectation value). These are INHERITED from particle-physics empirical input.
\item The Higgs sector's specific structure or mass. Empirical input.
\item The number of generations of fundamental fermions or the fermion mass spectrum. Empirical input.
\end{itemize}
...
\subsection{6.3.5 The substrate-level mechanism for charge and label conservation}
...
\section{6.4 Substrate-Level UV-Finiteness}

\subsection{6.4.1 The structural question}
...
\subsection{6.4.2 The substrate-level answer}

ED supplies a structural answer through three substrate primitives:

\begin{itemize}
\item \textbf{P01 event discreteness.} Substrate events are discrete; there is a smallest length scale built into the substrate. The continuum limit, where loop integrals run over arbitrary high momentum, does not have a substrate-level counterpart — substrate events do not exist at sub-Planckian length scales because the substrate's irreducible length is the Planck length (identified through T19 in Chapter 11).
\item \textbf{P13 finite proper-time intervals.} The substrate's proper-time ordering on chains uses finite intervals; there are no infinitesimal proper-time steps at the substrate level. This bounds substrate dynamics' temporal resolution and prevents infinitesimal-time-step divergences from arising.
\item \textbf{P04 bounded bandwidth.} Participation channels carry bounded bandwidth; the substrate cannot support unbounded high-frequency content at chain endpoints. This caps the substrate's high-frequency dynamics and prevents the integrand of substrate-level loop analogues from diverging at high momentum.
\end{itemize}
...
\subsection{6.4.3 Renormalization reframed}
...
\subsection{6.4.4 The structural payoff}
...
\section{6.5 ED-I-13: Quantum Information as Channel Geometry}

\subsection{6.5.1 The structural question}
...
\subsection{6.5.2 The substrate-level vocabulary for QI}

ED-I-13 introduces the substrate-level vocabulary in which quantum information lives:

\begin{itemize}
\item \textbf{Multiplicity.} The count of viable distinct ED-gradient pathways available locally, as introduced in Chapter 2. High-multiplicity channels support many simultaneous participation rules; minimal channels enforce strict alignment.
\item \textbf{Channels.} Substrate-level participation pathways. A channel is defined by its rule-type, its multiplicity, and its endpoint structure. Channels are the substrate-level analogue of "qubits" or "modes" in standard QI.
\item \textbf{Identity alignment.} A chain's substrate-level identity is encoded in its participation rule. Identity alignment is the substrate-level structure that makes "the same chain" coherent across multiple substrate locations. Operations on identity alignment underlie teleportation in the standard formalism.
\item \textbf{Global participation.} High-multiplicity channels can carry rules whose content is \textit{global} — properties of the entire channel's geometry rather than properties of any single endpoint. Global participation underlies Deutsch and Deutsch–Jozsa in the standard formalism.
\item \textbf{Rewrite-on-measurement.} Minimal-multiplicity channels enforce that a measurement event triggers a substrate-level rewrite of the channel's participation rule. Rewrite-on-measurement underlies BB84 cryptography in the standard formalism.
\end{itemize}
...
\subsection{6.5.5 ED-I-13's open predictions}

ED-I-13 predicts directions for future quantum-information work that follow from the substrate-level reframing:

\begin{itemize}
\item \textbf{New algorithmic classes} based on symmetry extraction and global constraint resolution. The substrate-level vocabulary identifies dimensions of channel geometry that standard quantum-information theory has not systematically exploited.
\item \textbf{Structural limits on measurement disturbance} determined by channel multiplicity. This is one of the predictive contents that Chapter 7 develops in the UR-1 architecture and the multiplicity-cap function $M$.
\item \textbf{New cryptographic primitives} based on channel incompatibility rather than no-cloning.
\item \textbf{Reinterpretation of entanglement as undeveloped identity} rather than stored correlation. Identity alignment across unresolved channels is the structural object; "entanglement" is the standard-QI label for it.
\item \textbf{Error correction via distributed alignment} rather than redundancy of states. This is the structural content that Chapter 7 develops as Class C high-multiplicity-redundancy architecture.
\item \textbf{Computation as manipulation of participation geometry} as a unifying design principle. This is the structural framing that Chapter 7's quantum computing architecture develops formally.
\end{itemize}
...
\section{6.6 The Form-Level QFT and QI Closure as a Whole}

\subsection{6.6.1 What is closed at form level}

The chapter closes form-level structural content for QFT and QI:

\begin{itemize}
\item \textbf{Gauge structure form-closure.} T17 establishes gauge fields as participation measures of structural rule-types; gauge invariance as interface property; coverage of Abelian and non-Abelian compact-simple-group structures.
\item \textbf{UV-finiteness substrate-closure.} Substrate UV-finiteness from P01 + P13 + P04; renormalization reframed as continuum-approximation tool.
\item \textbf{Quantum information channel-geometry-closure.} ED-I-13 reinterpretation of five landmark QI results as substrate channel-geometry manipulations; substrate-level vocabulary for QI (multiplicity, channels, identity alignment, global participation, rewrite-on-measurement).
\end{itemize}

What is \textit{not} closed at form level: the specific gauge group (Standard Model gauge group is empirical input); specific coupling constants, particle masses, and Higgs sector structure (all INHERITED); a constructive proof of QFT existence at Streater–Wightman / Osterwalder–Schrader rigor (Yang–Mills constructive existence is structurally addressed in Chapter 9 at the same Path-C-style verdict); specific quantum algorithms exploiting the substrate-level vocabulary (downstream open work).

\subsection{6.6.2 What this enables for Chapter 7 (Quantum Computation)}
...
\begin{itemize}
\item \textbf{(i) Multiplicity bounded.} $\mathcal{M}_i \leq \mathcal{M}_\mathrm{crit}$ at every endpoint. ED-I-13's multiplicity vocabulary supplies the substrate-level definition.
\item \textbf{(ii) Cross-endpoint connectivity sustained.} $\gamma_{ij} \geq \Gamma_\mathrm{min}$ along every rule-spanning pathway. ED-I-13's channel and rule-spanning vocabulary supplies the substrate-level definition.
\item \textbf{(iii) Commitment-injection bounded.} Inherits from P11 commitment-irreversibility (Chapter 1) and Theorem 18 (Chapter 4).
\end{itemize}
...
\subsection{6.6.3 The position of form-level QFT closure in the program}
...
\section{6.7 The CR Framing Applied to Form-Level QFT}
...
\begin{itemize}
\item \textbf{Primitive level.} T17 gauge-field-as-rule-type; substrate UV-finiteness from P01 + P13 + P04; ED-I-13 substrate channel-geometry vocabulary. These are substrate-level structural commitments.
\item \textbf{Continuum approximation.} Standard QFT machinery — gauge fields as dynamical fields, gauge invariance as a fundamental principle, renormalization as the technical apparatus for absorbing divergences, Hilbert-space quantum-information theory as the standard formalism for QI. This is continuum-level effective machinery.
\end{itemize}
...
The CR framing's value is that it preserves the empirical content of standard QFT and standard QI exactly. ED does not change any standard QFT calculation or any standard QI prediction; it changes the \textit{story underneath}. Form-level QFT closure is a structural-foundational claim, not a discriminating-from-standard-physics claim. The empirical posture is consistent with Chapter 4's empirical posture for Theorem 18.

\section{6.8 Form-FORCED vs Value-INHERITED}

\subsection{6.8.1 What is form-FORCED}

\begin{itemize}
\item \textbf{T17's structural form} of gauge fields as participation measures of structural rule-types.
\item \textbf{T17's coverage} of Abelian and non-Abelian compact-simple-group structures.
\item \textbf{The interface-property origin} of gauge invariance.
\item \textbf{Substrate-level UV-finiteness} from P01 + P13 + P04.
\item \textbf{The reframing of renormalization} as a continuum-approximation tool rather than a substrate-level requirement.
\item \textbf{ED-I-13's substrate channel-geometry vocabulary} for quantum information (multiplicity, channels, identity alignment, global participation, rewrite-on-measurement).
\item \textbf{The substrate-level reinterpretation of the five landmark QI results} as channel-geometry manipulations.
\item \textbf{The structural unification} of QI under substrate channel geometry (replacing the standard treatment as five distinct Hilbert-space tricks).
\end{itemize}

\subsection{6.8.2 What is value-INHERITED}

\begin{itemize}
\item \textbf{The specific gauge group} nature realizes (Standard Model's $SU(3) \times SU(2) \times U(1)$ structure). Empirical input.
\item \textbf{Specific gauge couplings} (the fine structure constant $\alpha$, the strong coupling $\alpha_s$, the weak couplings, the Higgs vacuum expectation value). Empirical input.
\item \textbf{Specific particle masses} in the Standard Model. Empirical input.
\item \textbf{The Higgs sector's specific structure}. Empirical input.
\item \textbf{The number of generations} of fundamental fermions. Empirical input.
\item \textbf{Specific quantum algorithms} beyond the five landmark results (depending on substrate channel-geometry properties yet to be systematically exploited).
\item \textbf{Specific cryptographic primitives} based on substrate channel incompatibility.
\item \textbf{Specific error-correction schemes} based on distributed alignment.
\end{itemize}

\subsection{6.8.3 What is open}

Several open extensions are flagged for downstream work:

\begin{itemize}
\item \textbf{Closed-form derivation of substrate constants} entering gauge couplings. This is part of the broader closed-form-substrate-constants program (alongside $\mathcal{M}_\mathrm{crit}$ in Chapter 7, $\log g$ in Chapter 13, and $\kappa/|\hat{N}'|$ in the ED-SC arc).
\item \textbf{Constructive QFT existence} at Streater–Wightman / OS-axiom rigor. Yang–Mills (Chapter 9) gives a structural-positive verdict at substrate-suggestive level; full constructive existence remains open.
\item \textbf{Substrate-level account of the Higgs mechanism}. The framework treats this as out of scope for the form-level closure.
\item \textbf{Generation structure} of fundamental fermions. The framework does not derive the number of generations or the mass spectrum.
\end{itemize}

\section{6.9 Dependencies}

\subsection{6.9.1 Upstream}

\begin{itemize}
\item \textbf{Chapter 1.} Substrate primitives, especially P01 (event discreteness, foundational for UV-finiteness), P04 (bandwidth update rule, foundational for UV-finiteness and rule-type label structure), and P13 (proper-time ordering with finite intervals, foundational for UV-finiteness). Plus the participation primitives that supply rule-type label structure.
\item \textbf{Chapter 2.} Load-bearing invariants. Multiplicity $\mathcal{M}$ enters ED-I-13's substrate channel-geometry vocabulary directly. The V1 kernel mediates substrate-level vacuum response that enters T17 vacuum-coupling content.
\item \textbf{Chapter 4.} Theorem 18 kernel-level arrow. T17 vacuum-coupling clauses C5–C7 are refined to forward-only directionality through Theorem 18. ED-I-13 inherits the kernel-level arrow indirectly through the substrate-level structural foundation.
\item \textbf{Chapter 5.} Phase-1 closure. The form-level QFT closure of this chapter extends the QM closure of Chapter 5 to relativistic and gauge-theoretic territory. ED-I-13 builds on Phase-1's substrate-level account of quantum systems.
\end{itemize}

\subsection{6.9.2 Downstream}

\begin{itemize}
\item \textbf{Chapter 7 (Quantum Computation).} UR-1's substrate-level conditions use the channel-geometry vocabulary established in ED-I-13. The three architectural classes (engineered-low-multiplicity, global-geometric-rigidity, high-multiplicity-redundancy) inherit substrate-level mechanisms from this chapter. The multiplicity-cap function $M$ is the QC-architecture-specific reading of substrate channel-geometry constraints.
\item \textbf{Chapter 8 (Navier–Stokes).} Substrate UV-finiteness (specifically through V1's finite width producing R1 substrate-cutoff regularization) is the substrate-level content that connects to the NS architectural decomposition.
\item \textbf{Chapter 9 (MHD and Yang–Mills).} T17 minimal coupling provides the substrate-to-continuum bridge for the Lorentz force; T17's non-Abelian extension produces the Yang–Mills equation; substrate UV-finiteness produces the Yang–Mills mass-gap mechanism through V1's second-moment expansion.
\item \textbf{Chapter 12 (Curvature Emergence).} ED-Phys-10 acoustic-metric guardrails are consistent with the form-level QFT closure of this chapter; the substrate-level structural commitments (UV-finiteness, no-singularity through P01) are preserved across the gravity sector.
\item \textbf{Chapter 13 (Black-Hole Architecture).} The cross-domain consistency between substrate UV-finiteness and the no-singularity result of BH-3 inherits from this chapter; both are substrate-level structural consequences of P01 plus related primitives.
\item \textbf{Chapter 14 (Cross-Platform Unifications).} The form-FORCED methodology, including form-level QFT closure as an example of the program's structural reach, is one of the cross-domain consistency signatures.
\end{itemize}

\section{6.10 Canonical Sources}

\begin{itemize}
\item \texttt{papers/Gauge\_Fields\_Theorem\_17/}
\item ED-I-13 (Quantum Information: Channel Geometry, Feb 2026)
\item \texttt{papers/Arc\_Q/}
\item \texttt{papers/Arc\_R/}
\end{itemize}

The Gauge\_Fields\_Theorem\_17 paper presents Theorem 17 in publication-grade form with the formal derivation of gauge-field-as-rule-type structure. ED-I-13 (the canonical source for the substrate channel-geometry framework) presents the reinterpretation of the five landmark QI results. Arc\_Q contains the closure memos for the Arc Q gauge-sector closure that produced T17. Arc\_R contains the relativistic-extension memos covering Cl(3,1) signature on the spinor bundle, R.2.5 spin-statistics, and R.3 Dirac equation derivation that connect to T17's relativistic content.
...
This framing is empirically successful but structurally incomplete. It does not say \textit{why} certain platforms hit walls at certain scales. It does not unify the matter-wave quantum-classical boundary (a foundational-physics observation about giant molecules) with qubit-system scaling limits (an engineering question about coherence preservation in arrays). It treats different platforms (superconducting, ion, photonic, topological, bosonic) as different problems requiring different solutions, when the substrate-level analysis says they are the same problem viewed from different architectural angles.

\subsection{7.2.2 The substrate variable}

ED reframes the central organizing variable. \textbf{Multiplicity} $\mathcal{M}$ — the count of viable distinct ED-gradient pathways available locally, established in Chapter 2 as the substrate analogue of entropy — is the upstream substrate-state quantity. Decoherence in the standard sense is the \textit{coarse-grained signature} of substrate-level individuation events firing at endpoints whose multiplicity has risen above a substrate-determined threshold. The standard variable is real but downstream; the substrate variable is upstream and structurally prior.

The reframing has direct structural consequences:

\begin{itemize}
\item \textbf{Coherence is not a state property.} It is the integrity of an \textit{unresolved participation rule} spanning designated endpoints (the substrate-level analogue of the wavefunction superposition).
\item \textbf{Decoherence is not a coupling rate.} It is the substrate-level event of P11 commitment occurring before the protocol intends. P11 commitment is the substrate-level mechanism; decoherence rate is the coarse-grained measurement of how often the mechanism fires.
\item \textbf{Engineering effort is not "reducing decoherence."} It is \textit{holding the substrate region in a low-multiplicity, well-connected, low-commitment-rate configuration}, against the substrate's natural tendency to proliferate gradients and force individuation.
\end{itemize}
...
\begin{itemize}
\item \textbf{Multiplicity} $\mathcal{M}(\mathcal{S})$ at substrate region $\mathcal{S}$. The count of viable distinct ED-gradient pathways available locally.
\item \textbf{Gradient sparsity} $\sigma(\mathbf{x}) = |\nabla\rho|\,\ell_P^2/\rho_\mathrm{local}$ at substrate point $\mathbf{x}$. The substrate-scale dimensionless steepness of participation density.
\item \textbf{Cross-bandwidth} $\gamma_{ij}$ along a rule-spanning pathway between endpoints $e_i$ and $e_j$. The substrate-mediated rate at which the two endpoints exchange correlated participation events.
\item \textbf{Commitment-injection rate} $\Lambda_\mathcal{S}(t) = \Lambda_\mathrm{env}(t) + \Lambda_\mathrm{int}(t)$. The rate at which P11 commitment events fire at $\mathcal{S}$'s endpoints, sourced by environmental ED-injection plus internal participation dynamics.
\item \textbf{Unresolvedness} $\mathcal{U}(\mathcal{S}, t) \in [0, 1]$. The integrity of an unresolved participation rule across the designated endpoints of $\mathcal{S}$ at substrate time $t$. $\mathcal{U} \to 1$ when the rule is fully unresolved; $\mathcal{U} \to 0$ when full individuation has been reached.
\end{itemize}
...
\subsection{7.3.2 The unresolvedness function}
...
\begin{itemize}
\item \textbf{Multiplicity headroom.} Whether each endpoint's local multiplicity is bounded below the substrate-determined critical value $\mathcal{M}_\mathrm{crit}$.
\item \textbf{Rule-spanning connectivity.} Whether each rule-spanning pathway maintains cross-bandwidth above the substrate-determined minimum $\Gamma_\mathrm{min}$.
\item \textbf{Commitment survival.} Whether the cumulative commitment-injection rate has not yet driven the rule to individuation.
\end{itemize}

The product structure is form-FORCED: the rule fails to remain unresolved if \textit{any} of the three factors collapses. The functional shapes $\mu$ and $\kappa$ are form-FORCED to be monotone with specific limiting behavior; their specific functional forms are INHERITED from the substrate-level V1-kernel and DCGT machinery.

\subsection{7.3.3 The relation set $\mathcal{R}$}

The relation set $\mathcal{R}$ identifies which endpoint pairs the rule spans. For a single qubit, $\mathcal{R}$ is the set of designated $\{|0\rangle, |1\rangle\}$ endpoints. For a multi-qubit entangled register, $\mathcal{R}$ extends across all qubits' designated endpoints. For a matter-wave interferometer, $\mathcal{R}$ is the pair of arms. The structural content of $\mathcal{R}$ is that it specifies \textit{which} substrate endpoints the rule must remain unresolved across; $\mathcal{R}$ is platform-specific but the UR-1 structural form is platform-independent.

\section{7.4 UR-1: The Unresolved-Regime Characterization Theorem}

\subsection{7.4.1 The theorem statement}

\textbf{UR-1 (Unresolved-Regime Characterization Theorem).} Let $\mathcal{S}$ be a substrate region with designated participation-rule relation set $\mathcal{R}$ over endpoints $\{e_1, \ldots, e_n\}$. Let $\mathcal{U}_\mathrm{min} \in (0, 1)$ be a target unresolvedness threshold. Then $\mathcal{U}(\mathcal{S}, t) \geq \mathcal{U}_\mathrm{min}$ holds at substrate time $t$ if and only if:

\begin{itemize}
\item \textbf{(i) Multiplicity bounded.} For every endpoint $e_i$, $\mathcal{M}_i(t) \leq \mathcal{M}_\mathrm{crit}$.
\item \textbf{(ii) Cross-endpoint connectivity sustained.} For every pair $(i, j) \in \mathcal{R}$, $\gamma_{ij}(t) \geq \Gamma_\mathrm{min}$.
\item \textbf{(iii) Commitment-injection bounded.} $\int_0^t \Lambda_\mathcal{S}(t')\,dt' \leq \ln(1/\mathcal{U}_\mathrm{min})$ (with appropriate corrections from the per-condition negative-log deficits).
\end{itemize}
...
\subsection{7.4.2 Why three conditions, not more or fewer}
...
\begin{itemize}
\item \textbf{Local multiplicity proliferation} at any endpoint of the rule. If the count of viable pathways at one endpoint exceeds $\mathcal{M}_\mathrm{crit}$, the rule cannot remain coherently spread across that endpoint.
\item \textbf{Cross-endpoint bandwidth collapse} along any rule-spanning pathway. If the substrate-mediated cross-bandwidth between two endpoints falls below $\Gamma_\mathrm{min}$, the rule cannot remain coherent across that pair.
\item \textbf{Commitment-event accumulation} at any endpoint of the rule. Each P11 commitment event at an endpoint is an irreversible individuation; cumulative commitment-injection drives $\mathcal{U}$ to zero exponentially.
\end{itemize}
...
\subsection{7.4.3 Independence and substrate-geometric coupling}

UR-1's three conditions are \textbf{independent in form} (each can fail alone, each is independently necessary, the proof establishes that violation of any one drops $\mathcal{U}$ below threshold even with the other two satisfied) but \textbf{coupled through substrate geometry} (a physical substrate region cannot independently dial $\mathcal{M}$, $\sigma$, $\Lambda$ to arbitrary values; substrate constraints link them). For Block-B failure-mode rate analysis (Section 7.5) and Block-D multiplicity-cap-function construction (Section 7.7), each condition is treated independently. For Block-C architectural classification (Section 7.6), the geometric coupling matters — different architectures use different substrate-geometric tactics to satisfy multiple conditions simultaneously, and the substrate-coupling structure is what distinguishes one architecture class from another.

\subsection{7.4.4 Reduction to known regimes}

UR-1 reproduces three known regimes cleanly, each with a different binding condition:

\begin{itemize}
\item \textbf{Bulk superconductor (ED-I-01).} Below $T_c$, conditions (i) and (ii) hold robustly; condition (iii) holds because thermal ED-injection rate is below the substrate individuation threshold. Above $T_c$, thermal injection drives condition (iii) failure; $\mathcal{U}$ drops; resistance reappears. \textbf{$T_c$ is identified as the substrate temperature at which condition (iii) crosses its threshold.}
\item \textbf{Josephson junction (ED-I-23).} Two low-$\mathcal{M}$ SC electrodes separated by a thin barrier. Conditions (i) and (iii) hold; condition (ii) is the \textit{load-bearing} condition by design — the barrier creates a high-$\sigma$ region with $\gamma_{LR}$ engineered just above $\Gamma_\mathrm{min}$. The JJ lives at the (ii) boundary. \textbf{Macroscopic quantum tunneling (Devoret-Martinis-Clarke 1985) corresponds to dynamical fluctuations at the (ii) boundary} — the rule globally reconfigures across the gap when $\gamma_{LR}$ momentarily drops below threshold. The MQT rate's WKB exponential structure is recovered directly from DCGT $\kappa$-function evaluation.
\item \textbf{Free atomic-scale matter wave.} The molecule is a low-$\mathcal{M}$ ED-object whose internal $\mathcal{M}$ scales with mass (rotational + vibrational + electronic DOF activation). For low molecular mass, $\mathcal{M} \ll \mathcal{M}_\mathrm{crit}$ and condition (i) holds. For high mass (140–250 kDa), internal $\mathcal{M}$ rises and crosses $\mathcal{M}_\mathrm{crit}$. \textbf{The matter-wave Q-C boundary is identified as the molecular mass at which condition (i) crosses threshold.}
\end{itemize}

Three different empirical phenomena, three different UR-1 conditions, one substrate framework.

\section{7.5 Failure-Mode Rates and the QC Operating Window}

\subsection{7.5.1 Three substrate-level dynamics}

Each UR-1 condition admits a substrate-level dynamical equation governing the timescale at which it crosses threshold.

\textbf{Condition (i) multiplicity dynamics.} The substrate-level evolution of $\mathcal{M}$ is
...
with $\alpha_\mathrm{env}\Lambda_\mathrm{env}$ environmental ED-injection driving pathway-count growth, $\alpha_\mathrm{act}S_\mathrm{int}$ internal pathway-activation rate from thermal or operational stimulation, and $A_\mathcal{S}(\mathcal{M} - \mathcal{M}_\mathrm{floor})$ architectural restoring rate returning $\mathcal{M}$ to the architecturally-imposed floor. Two failure modes: \textbf{static} when $\mathcal{M}_\mathrm{floor} \geq \mathcal{M}_\mathrm{crit}$ (the system cannot be operated regardless of timescale; this is the matter-wave Q-C boundary regime) and \textbf{dynamic} when $\mathcal{M}_\mathrm{floor} < \mathcal{M}_\mathrm{crit}$ but environmental injection drives the steady state above threshold.

\textbf{Condition (ii) cross-bandwidth dynamics.} The cross-bandwidth $\gamma_{ij}(t)$ evolves under substrate gradient erosion plus architectural pumping, with the form $d\gamma_{ij}/dt = -(\gamma_{ij} - \gamma_\mathrm{floor})/\tau_\mathrm{dec}^{(\mathrm{ii})} + \xi(t)$ where $\xi(t)$ is stochastic substrate perturbation. Two regimes: \textbf{sustained-above-threshold} ($\gamma_\mathrm{floor} > \Gamma_\mathrm{min}$ with margin; $\tau_{(\mathrm{ii})}$ exponentially long via Kramers-class escape) and \textbf{near-threshold} ($\gamma_\mathrm{floor} \approx \Gamma_\mathrm{min}$ by design, as in JJ; fluctuations push below threshold; each crossing produces global rule reconfiguration with WKB MQT rate $\sim \omega_0 \exp[-\alpha\int_\mathrm{barrier}\sigma\,d\ell]$).

\textbf{Condition (iii) commitment-injection dynamics.} The commitment-survival factor crosses threshold at
...
\subsection{7.5.2 The QC operating window}
...
\textbf{Different architectures bind on different conditions.} Each architectural class is characterized not by which timescale is \textit{longest} but by which is \textit{shortest} — i.e., which UR-1 condition is the architecture's binding constraint. The class taxonomy of Section 7.6 pivots on this observation.

\textbf{The operating window is a substrate observable.} $\tau_\mathrm{QC}$ corresponds to what experimentalists call coherence time, T₁/T₂, fault-tolerance-headroom, or maximum coherent flight time. Cross-platform measurements are evaluations of $\tau_\mathrm{QC}$ for different binding conditions.

\textbf{Architectural design is the optimization of $\min(\cdot)$.} Improving the longest of the three timescales is irrelevant; only improvement of the \textit{binding} timescale extends the window. The substrate provides the structural reason for what platform-development teams empirically know.

\subsection{7.5.3 Empirical anchor reductions}
...
\section{7.6 The Three-Class Architectural Taxonomy}

\subsection{7.6.1 Three substrate-allowed protection strategies}
...
\begin{itemize}
\item \textbf{System} $\mathcal{S}$: the protected region's configuration, including endpoints, $\mathcal{M}_\mathrm{floor}$, $\gamma_\mathrm{floor}$, and any architectural redundancy count.
\item \textbf{Class} $K \in \{A, B, C\}$: the architectural-class assignment.
\item \textbf{Environment} $\mathcal{E}$: $\Lambda_\mathrm{env}$, perturbation rates, temperature.
\item \textbf{Meta-architecture overlay} $\mathcal{O}$: dynamical decoupling, reservoir engineering, error correction, hybrid composition (any combination).
\end{itemize}
...
\subsection{7.7.2 The class projections}

\textbf{$M_A$ — Class A projection.} The $\tau_{(\mathrm{i})}^{(A)}$ is split into static-failure and dynamic-failure branches; static failure occurs if $\mathcal{M}_\mathrm{floor}(\mathcal{S}) \geq \mathcal{M}_\mathrm{crit}$. The $\tau_{(\mathrm{ii})}^{(A)}$ is Kramers-class escape from engineered low-$\sigma$ pathways. The $\tau_{(\mathrm{iii})}^{(A)}$ is environmental binding from Purcell + dielectric + quasiparticle channels. JJ-class engineering at the (ii) boundary recovers WKB MQT structure directly from DCGT.

\textbf{$M_B$ — Class B projection.} All three timescales are gap-suppressed by $\exp(\Delta_\mathrm{top}/T_\mathrm{eff})$. Binding shifts from environmental rate to topology-perturbation rate $\tau_\mathrm{gap-stab}(\mathcal{T})$, set by macroscopic substrate quality (sample homogeneity, edge purity, defect density, anyonic fusion-error rates), not by local environmental coupling. For Majorana platforms with $\Delta_\mathrm{top} \sim 0.1$ meV at $T_\mathrm{eff} \sim 20$ mK, the gap factor is order $10^{25}$. Class B can in principle exceed Class A coherence by exponential factors at the cost of much harder topology engineering.

\textbf{$M_C$ — Class C projection.} Three redundancy modifiers apply: $g(N)$ suppresses effective $\mathcal{M}$ (Class C \textit{relaxes} condition (i)); $h(N)$ enhances effective $\Gamma_\mathrm{cross}$; $c(N) \sim \exp(-N/N_\mathrm{corr})$ suppresses effective $\Lambda$ until correlation budget saturates. Class C ceiling: $\tau_\mathrm{QC}^{C, \mathrm{ceil}} = \tau_\mathrm{QC}^\mathrm{single} \cdot e$ asymptotically. Class C is \textbf{bounded-redundancy-advantage} — it cannot exceed single-pathway ceilings without bound; its role is extending the useful operating window before saturation.

\subsection{7.7.3 Meta-architectural overlays}

Meta-architectures appear as boundary-condition modifiers, not new projections:

\begin{itemize}
\item \textbf{Dynamical decoupling.} Replaces $\Lambda_\mathrm{env}^{(K)}$ with a temporally-averaged $\Lambda_\mathrm{env}^\mathrm{DD}(\omega_\mathrm{decoup})$.
\item \textbf{Reservoir engineering.} Increases $A_\mathcal{S}$ in the multiplicity dynamics via structured environmental coupling.
\item \textbf{Error correction.} Recursive overlay: $\tau_\mathrm{QC}^\mathrm{logical}$ is derived from $\tau_\mathrm{QC}^\mathrm{phys}$ via code distance and threshold. The meta-architecture takes the physical $M$ and applies a \textit{recursive overlay} at the logical level, outputting the logical $M$.
\item \textbf{Hybrid composition.} $\min$ over subsystems' projections plus losses at handoff boundaries; no new projection.
\end{itemize}

\subsection{7.7.4 OQ closure status}
...
\begin{itemize}
\item $M$ is one substrate object with three class-projections plus meta-architectural overlays, not three independent functions. (OQ-4 closed.)
\item The matter-wave Q-C boundary at 140–250 kDa is in $M_A$'s $\tau_{(\mathrm{i})}$ static-failure branch. The qubit-system multiplicity walls of various platforms lie at corresponding evaluation points of $M_A$ and the meta-architectures. The cross-platform identity is a substrate-level structural consequence. (OQ-5 closed.)
\item Specific functional shapes of $\mu, \kappa, g/h/c(N)$ are INHERITED at the same level as $\log g$ in BH-5 (Chapter 13). (OQ-7 closed to INHERITED level.)
\end{itemize}

\section{7.8 Cross-Platform Unification: Matter-Wave ↔ Qubit-System Identity}

\subsection{7.8.1 The strongest output}

The framework's strongest cross-platform result is the substrate-level identity between two empirically distinct phenomena: the matter-wave quantum-classical boundary at 140–250 kDa molecular mass and the qubit-system multiplicity walls of various Class A platforms. These are not analogous phenomena; they are the \textit{same} substrate-determined boundary projected onto two different platforms via shared substrate constants.
...
\subsection{7.8.2 Implications}

The identity has direct consequences for the QC roadmap:

\begin{itemize}
\item \textbf{Matter-wave interferometry becomes a direct probe of QC ceilings.} The Vienna and Basel matter-wave-interferometry programs are reframed as the cleanest available probe of the substrate constant $\mathcal{M}_\mathrm{crit}$ that determines qubit-array walls. Investment in matter-wave-boundary measurement (mass-matched isomer experiments, ultra-cold-vacuum extension, internal-DOF-controlled molecular probes) is investment in characterizing the limits of the qubit-based QC roadmap.
\item \textbf{The two communities study the same substrate phenomenon.} The matter-wave-interferometry program (foundational physics, no commercial QC application) and the SC-qubit-scaling program (commercial QC, no foundational-physics application) are studying the same substrate-determined boundary from two empirical directions.
\item \textbf{Cross-platform calibration is structurally valuable.} A national or institutional research program coordinating matter-wave-boundary measurement with qubit-platform multiplicity tomography produces consistency checks unavailable in either community alone.
\end{itemize}

\subsection{7.8.3 Sharp predictions}

The cross-platform identity supports sharp predictions:

\begin{itemize}
\item \textbf{Class A → Class C transition is mandatory at the wall.} Pure Class A scaling cannot pass the substrate-determined system-multiplicity wall. The path past the wall is composition with logical-qubit encoding (Class C); error correction is the standard meta-architectural composition that achieves this.
\item \textbf{Class B exponential coherence advantage.} Topological qubits can outrun Class A coherence by an exponential factor $\exp(\Delta_\mathrm{top}/T_\mathrm{eff})$ in the topological gap, but only if topology engineering can deliver stable gaps. The binding shifts from isolation engineering to topology engineering.
\item \textbf{Class C correlation-budget plateau.} Redundancy-based architectures saturate at a substrate-determined correlation budget $N_\mathrm{corr}$. Beyond this, additional redundancy stops paying.
\item \textbf{Cross-class architectural transitions.} A → C mandatory at the wall; B overtakes A at $\Delta/T \gtrsim 10$–20; C saturates at $N_\mathrm{corr}$.
\end{itemize}
...
\section{7.9 What QC in ED Does and Does Not Deliver}

\subsection{7.9.1 What is delivered}

\begin{itemize}
\item A substrate-level account of quantum computation grounded in UR-1.
\item Three exhaustive architectural classes (A, B, C) characterizing protection strategies.
\item The multiplicity-cap function $M$ as the substrate-level cross-platform unifier.
\item Cross-platform unification between the matter-wave Q-C boundary and qubit-system multiplicity walls.
\item Falsifiable predictive content with explicit sharp/structural/inherited demarcation.
\item Cross-domain consistency with all prior closed arcs (Phase-1, T17, T18, DCGT, Arc SG, Arc BH).
\item Six explicit OPEN extensions named for follow-on work (closed-form $\mathcal{M}_\mathrm{crit}$; architecture-to-platform calibration; closed-form $g/h/c(N)$; topology-stability theorem for Class B; surface-code recursive-overlay derivation; hybrid-architecture handoff dynamics).
\end{itemize}

\subsection{7.9.2 What is not delivered}

\begin{itemize}
\item Closed-form values for INHERITED parameters ($\mathcal{M}_\mathrm{crit}$, $N_\mathrm{corr}$, $\Delta_\mathrm{top}^{\max}$, etc.). These are calibrated empirically; the closed-form-substrate-constants program is downstream open work.
\item A constructive recipe for fault-tolerant quantum computing. The arc is structural, not engineering. Engineering against the substrate-level binding constraints is exactly the right work in the appropriate regime; the framework changes the \textit{interpretive frame} under which engineering progress is evaluated, not the engineering itself.
\item Specific numerical predictions for any specific QC platform. Architecture-to-platform calibration (O-QC-2) is the engineering-tractable program that converts the framework's structural predictions into platform-specific numerical bounds.
\item A claim that practical-scale QC is achievable or impossible. The framework predicts \textit{which substrate-level wall is binding at what scale} and \textit{which architectures can outrun which walls} — not whether the engineering effort to reach a given scale will or will not succeed.
\end{itemize}

\subsection{7.9.3 The decisive falsifier}
...
\section{7.10 Form-FORCED vs Value-INHERITED at the QC Architecture}

\subsection{7.10.1 What is form-FORCED}

\begin{itemize}
\item \textbf{UR-1's three-condition characterization} of the unresolved-rule regime.
\item \textbf{The three-factor product form of $\mathcal{U}$} (multiplicity headroom × rule-spanning connectivity × commitment-survival).
\item \textbf{The QC operating window structure} $\tau_\mathrm{QC} = \min(\tau_{(\mathrm{i})}, \tau_{(\mathrm{ii})}, \tau_{(\mathrm{iii})})$.
\item \textbf{The three-class architectural exhaustiveness} at substrate-level base.
\item \textbf{The reframing of meta-architectures} (error correction, dynamical decoupling, reservoir engineering, hybrid composition) as compositions or techniques rather than new substrate-level strategies.
\item \textbf{The multiplicity-cap function $M$} as one substrate object with three projections plus meta-architectural overlays.
\item \textbf{The static-failure branch} of $\tau_{(\mathrm{i})}$ at $\mathcal{M}_\mathrm{floor} = \mathcal{M}_\mathrm{crit}$.
\item \textbf{The exponential gap-suppression} of all three timescales in Class B by $\exp(\Delta_\mathrm{top}/T_\mathrm{eff})$.
\item \textbf{The correlation-budget plateau} in Class C with bounded redundancy advantage.
\item \textbf{The cross-platform identity} between matter-wave Q-C boundary and qubit-system multiplicity walls via shared $\mathcal{M}_\mathrm{crit}$.
\item \textbf{The WKB-form recovery of MQT} from DCGT $\kappa$-function evaluation at engineered JJ barriers.
\item \textbf{Three cross-class architectural transitions} (A → C mandatory, B overtakes A, C saturation).
\end{itemize}

\subsection{7.10.2 What is value-INHERITED}

\begin{itemize}
\item \textbf{$\mathcal{M}_\mathrm{crit}$.} Anchored empirically by matter-wave Q-C boundary at 140–250 kDa. Closed-form derivation is open (O-QC-1).
\item \textbf{$\Gamma_\mathrm{min}$.} Minimum cross-bandwidth for hydrodynamic-window resolution.
\item \textbf{$\Lambda_{V1}$.} V1 vacuum residual injection rate; bounds Class A perfect-isolation ceiling.
\item \textbf{$\beta_\mathrm{crit} \sim \log(R_\mathrm{cg}/\ell_P)$.} Critical gradient threshold for decoupling-surface formation; substrate-determined dimensionless number.
\item \textbf{$N_\mathrm{corr}$.} Correlation budget for Class C platforms; INHERITED, platform-specific.
\item \textbf{$\Delta_\mathrm{top}^{\max}$.} Maximum stable topological gap for Class B platforms.
\item \textbf{$T_\mathrm{eff}^{\min}$.} Minimum substrate-equivalent perturbation temperature.
\item \textbf{Specific shapes of $\mu, \kappa, g(N), h(N), c(N)$.} Form-FORCED to be monotone with specific limits; specific functional shapes INHERITED.
\item \textbf{Architecture-specific scaling functions} $f_\mathrm{sys}^{(A)}(\mathcal{S})$. INHERITED from architecture-to-platform calibration (O-QC-2).
\item \textbf{Platform-specific $N_\star$ values.} Determined by architecture-specific $f_\mathrm{sys}^{(A)}$ + the substrate-shared $\mathcal{M}_\mathrm{crit}$.
\end{itemize}

\subsection{7.10.3 Six open extensions}

Six O-QC items carried forward as named open extensions:

\begin{itemize}
\item \textbf{O-QC-1.} Closed-form derivation of $\mathcal{M}_\mathrm{crit}$ from V1 kernel and ED-I-01 substrate constants. Structurally similar in difficulty to closed-form $\log g$ (O2 from Arc BH).
\item \textbf{O-QC-2.} Architecture-to-platform calibration program. Map $\mathcal{M}_\mathrm{floor}^{(A)}(\mathcal{S})$ for canonical qubit platforms (transmon, fluxonium, trapped ion, photonic) and predict $N_\star$ values via the cross-platform identity with the matter-wave anchor.
\item \textbf{O-QC-3.} Closed-form $g(N), h(N), c(N)$ for Class C platforms. Currently INHERITED; closed-form derivation requires extending DCGT to multi-axis redundancy structure.
\item \textbf{O-QC-4.} Topology-stability theorem for Class B. Substrate-level account of $\tau_\mathrm{gap-stab}(\mathcal{T})$ for canonical topological structures (Majorana, Fibonacci anyons, Chern bands).
\item \textbf{O-QC-5.} Surface-code logical-qubit recursive-overlay derivation. Specific scaling of $\tau_\mathrm{QC}^\mathrm{logical}$ with code distance + threshold from the substrate-level error-correction-as-meta-architecture analysis.
\item \textbf{O-QC-6.} Hybrid-architecture handoff dynamics. SC-photonic interconnects, topological-SC platforms, and other hybrid architectures' handoff boundaries as substrate-geometric transitions.
\end{itemize}

\section{7.11 Dependencies}

\subsection{7.11.1 Upstream}

\begin{itemize}
\item \textbf{Chapter 1.} All thirteen substrate primitives. Especially: P04 bandwidth update rule (multiplicity bound and bandwidth conservation underlying UR-1 condition (i)); P11 commitment-irreversibility (commitment-injection underlying UR-1 condition (iii)); P01 event discreteness, P02 chain worldline structure, P13 proper-time ordering, finite-kernel and substrate-locality commitments.
\item \textbf{Chapter 2.} Load-bearing invariants — multiplicity $\mathcal{M}$, gradient sparsity $\sigma$, cross-bandwidth $\Gamma_\mathrm{cross}$, V1 finite-width vacuum kernel, P11 commitment-irreversibility. All five enter the QC architectural reading.
\item \textbf{Chapter 3.} DCGT cross-bandwidth structure $\Gamma_\mathrm{cross} \sim \exp[-\alpha\sigma]$ underlies UR-1 condition (ii). The hydrodynamic-window scale separation is the substrate-level prerequisite for the QC operating regime.
\item \textbf{Chapter 4.} Theorem 18 kernel-level arrow. Provides the substrate-level structural foundation for P11's directional content as it enters UR-1 condition (iii) commitment-injection accumulation.
\item \textbf{Chapter 5.} Phase-1 closure. Identifies QC as the engineered occupation of the thin-participation regime characterized by Phase-1; supplies the substrate-level QM foundation on which UR-1's three substrate conditions are formulated.
\item \textbf{Chapter 6.} T17 gauge-field-as-rule-type and ED-I-13 channel-geometry quantum information. T17 supports the substrate-level vocabulary for participation rules with rule-type label structure; ED-I-13 supplies the multiplicity-as-channel-geometry vocabulary that UR-1's three substrate conditions inherit.
\end{itemize}

\subsection{7.11.2 Downstream}

\begin{itemize}
\item \textbf{Chapter 13 (Black-Hole Architecture).} Cross-domain $\Gamma_\mathrm{cross}$ collapse mechanism shared with QC condition (ii) failure at scales separated by ~50 orders of magnitude. The substrate condition $|\nabla\rho|\,\ell_P^2/\rho_\mathrm{local} \gtrsim \log(R_\mathrm{cg}/\ell_P)$ unifies horizon formation in BH-2 with $\gamma_{ij}$ collapse in QC condition (ii).
\item \textbf{Chapter 14 (Cross-Platform Unifications).} The cross-platform identity between matter-wave Q-C boundary and qubit-system multiplicity walls is the strongest cross-platform claim in the program; Chapter 14 develops it as the central output of the cross-domain unification program.
\item \textbf{Chapter 15 (Public Test Inventory).} Empirical anchors for the QC sector — matter-wave Q-C boundary, JJ MQT, SC qubit T₁/T₂, Hafezi multi-timescale, topological-gap suppression — are catalogued in Chapter 15 with status taxonomy (PASSED / ANCHORED / IN PROGRESS / ACTIVE / OPEN).
\end{itemize}

\section{7.12 Canonical Sources}

\begin{itemize}
\item \texttt{papers/Quantum\_Computing\_Foundations/}
\item Arc Q-COMPUTE memos in \texttt{theory/Quantum\_Computing/}
\item ED-I-23 (Josephson Junctions, Mar 2026)
\item ED-I-14 (Topological Effects, Feb 2026)
\item ED-I-18 (Multi-Timescale Photonics, Feb 2026)
\item ED-I-12 (Photonics, Feb 2026)
\end{itemize}

The Quantum\_Computing\_Foundations paper presents the publication-grade architectural account of the QC sector, including UR-1, the three-class taxonomy, the multiplicity-cap function $M$, cross-platform unification, FORCED/INHERITED/OPEN classification, and the conceptual-shift framing (decoherence-centric → multiplicity-centric). Arc Q-COMPUTE memos (seven memos: Opening, UR-1 derivation, failure-mode rates, architectural-class audit, multiplicity-cap function $M$, predictive content, synthesis) develop each component in detail.
...
\chapter{Navier–Stokes Architectural Foundations: Form-FORCED, R1, Path C}
\label{chap:8}

\section{8.1 Chapter Overview}

The Navier–Stokes equation is structurally a hybrid. Its content does not partition into a single category at the substrate level; rather, the equation packages four kinds of structure that have four distinct origins. Two of the four — the viscous-diffusion sector and the substrate-cutoff hyperviscous regularization R1 — are \textit{substrate-derived} through the Diffusion Coarse-Graining Theorem applied to vector-valued participation transport. One — advection $(\mathbf{v}\cdot\nabla)\mathbf{v}$ — is a \textit{frame-kinematic} artifact of writing fluid dynamics in laboratory-frame coordinates rather than a substrate-level force. One — the pressure term plus the incompressibility constraint — is a \textit{continuum-imposed constraint} set by external assumption rather than derived. Standard fluid mechanics treats all four kinds of structure as phenomenological; the Event Density framework treats them as architecturally distinct.
...
\subsection{8.2.3 The structural questions standard fluid mechanics does not answer}

Three questions that the standard fluid-mechanics package leaves open:

\begin{itemize}
\item \textbf{Why this exact equation?} The momentum equation has its specific functional form: this combination of viscous diffusion, advection, pressure, and incompressibility. Other functional forms are mathematically possible. Why does nature pick this one?
\item \textbf{Why is 2D smooth and 3D open?} Mathematicians proved decades ago that 2D Navier–Stokes admits global smooth solutions for any reasonable initial condition. The 3D smoothness question is the Clay Millennium Problem; nobody has cracked it. Why the dimensional asymmetry?
\item \textbf{Why does turbulence have no canonical structural account?} Every theoretical approach to turbulence — Kolmogorov's energy cascade, Kraichnan's 2D inverse cascade, Renormalization Group methods, lattice-Boltzmann simulations — captures part of the phenomenon but not the whole. Why has no single architectural template been found?
\end{itemize}
...
\section{8.3 The Substrate-Level Decomposition}

\subsection{8.3.1 The DCGT-derivable sector}

The substrate-level analysis through DCGT (Chapter 3) produces \textit{two} of the four NS structures as form-FORCED outputs.

\textbf{Viscous diffusion ($\mu\nabla^2\mathbf{v}$).} DCGT's leading-order multi-scale expansion, applied to vector-valued participation transport, produces Newton's law of viscosity at leading order. This is one of DCGT's five canonical leading-order consequences (Chapter 3, Section 3.6.2). The viscous-diffusion term in standard Navier–Stokes is the substrate-to-continuum bridge for vector momentum transport in the hydrodynamic window. The viscosity coefficient $\mu$ inherits its specific value from substrate-channel statistics; the \textit{form} — Laplacian of velocity, multiplied by a positive coefficient — is FORCED.

\textbf{Substrate-cutoff hyperviscous regularization R1.} DCGT's first subleading order, applied to V1's finite-width kernel content (Theorem N1, Chapter 4), produces a hyperviscous correction term

\[
-\kappa\mu_{\mathrm{V1}}\,\ell_P^2\,\nabla^4\mathbf{v}.
\]

This is the R1 term. It is small at ordinary flow scales — suppressed by $\ell_P^2$ — but it is structurally inevitable. V1's finite-width is a substrate-ontological commitment (Chapter 1's finite-kernel primitive, Theorem N1's formal closure); the first-subleading-order DCGT expansion produces R1 directly. The R1 coefficient combination $\kappa\mu_{\mathrm{V1}}\ell_P^2$ is INHERITED; the \textit{form} — fourth-order spatial derivative of velocity, multiplied by a small substrate-cutoff coefficient — is FORCED.
...
\subsection{8.3.2 The frame-kinematic sector}

\textbf{Advection $(\mathbf{v}\cdot\nabla)\mathbf{v}$.} DCGT's multi-scale expansion does \textit{not} produce the advective term. The advective term arises as bookkeeping when fluid dynamics is written in laboratory-frame coordinates rather than co-moving-with-the-fluid coordinates. The fluid itself, as the substrate sees it, does not have an advective term; only the lab-frame description of it does. Advection is therefore a \textit{frame-kinematic} artifact, not a substrate-level force.
...
This identification is consistent across three independent angles, developed in Section 8.7 as the \textbf{three-angle convergence on advection-as-non-ED}.

\subsection{8.3.3 The continuum-imposed sector}
...
\subsection{8.3.4 The four-fold decomposition}
...
\subsection{8.3.5 The structural reading under ED-I-06}
...
\section{8.4 The R1 Substrate-Cutoff Regularization Term}

\subsection{8.4.1 R1's structural form}
...
\subsection{8.4.2 Why R1 is form-FORCED}
...
The framework does \textit{not} commit to the specific value of the prefactor $\kappa\mu_{\mathrm{V1}}$. That value inherits from V1's specific functional shape, which is INHERITED at the level of Theorem N1. R1's \textit{existence} and its \textit{form} (fourth-order spatial derivative, multiplied by a small substrate-cutoff coefficient) are FORCED; the \textit{coefficient value} is INHERITED.

\subsection{8.4.3 Why R1 matters for Clay-NS}
...
\subsection{8.4.4 R1's relation to Wilsonian effective-theory machinery}
...
\section{8.5 The Intermediate Path C Verdict on Clay-NS}

\subsection{8.5.1 What an Intermediate Path C verdict is}

The framework distinguishes three classes of verdict on a Clay-class problem:

\begin{itemize}
\item \textbf{Path A: full constructive solution.} A complete proof at Clay-rigorous level (in the NS case, global existence and smoothness of strong solutions for arbitrary smooth initial data in $\mathbb{R}^3$, satisfying the standard energy bounds).
\item \textbf{Path B: refutation.} A demonstration that the conjecture is false, e.g., a finite-time singularity construction.
\item \textbf{Path C: structural identification of the obstruction.} A substrate-grounded analysis that supplies a real regularizing mechanism but explicitly identifies what the substrate does \textit{not} control. This verdict is between Path A (full solution) and Path B (refutation): the framework supplies real Clay-relevant content without claiming to close the problem at constructive-rigorous level.
\end{itemize}

The framework's verdict on Clay-NS is \textbf{Intermediate Path C}: the substrate-level analysis supplies R1 as a real regularizing mechanism, but the residual obstruction lives in the frame-kinematic sector (advection) that the substrate does not natively control.

\subsection{8.5.2 The structural argument}

The argument has three steps:

\begin{enumerate}
\item \textbf{R1 produces strict monotone decay of the gradient-norm Lyapunov in any version of the equation that drops the advective term.} This is a substrate-level claim: R1's $-\nabla^4\mathbf{v}$ operator is strictly negative-definite when integrated against the gradient-norm Lyapunov; the leading viscous term $\mu\nabla^2\mathbf{v}$ is also negative-definite; pressure and incompressibility are sign-definite; only the advective vortex-stretching term has indefinite sign. The argument identifies advective vortex-stretching as the unique non-sign-definite contribution.
\end{enumerate}

\begin{enumerate}
\item \textbf{Advective vortex-stretching is the structural source of the Clay-NS difficulty.} The vortex-stretching term in the 3D vorticity equation $(\boldsymbol\omega\cdot\nabla)\mathbf{v}$ — the curl of the advective term — can amplify vorticity magnitudes in 3D. Whether it actually drives finite-time blow-up is the open Clay question. The framework's claim is \textit{not} that vortex-stretching does drive blow-up; the claim is that vortex-stretching is the unique structural channel through which blow-up could occur.
\end{enumerate}
...
\subsection{8.5.3 What Path C delivers and does not deliver}

Path C \textbf{delivers}:

\begin{itemize}
\item A real Clay-NS-relevant regularizing mechanism (R1).
\item A structural identification of where the obstruction lives (the frame-kinematic advective sector).
\item A structural explanation for why standard NS analyses have not closed the Clay problem despite decades of effort (the obstruction is not in the part of the equation that admits substrate-level control).
\item A reproduction of the dimensional asymmetry between 2D and 3D NS (Section 8.6).
\item A reproduction of the empirical observation that turbulence has no canonical structural template (Section 8.7).
\end{itemize}

Path C \textbf{does not deliver}:

\begin{itemize}
\item A constructive proof of global existence and smoothness for 3D NS at Clay-rigorous level.
\item A refutation of the smoothness conjecture (e.g., a finite-time singularity construction).
\item A solution to the Clay Millennium Problem.
\end{itemize}
...
\section{8.6 The Dimensional-Structure Account: 2D Smooth, 3D Obstruction}

\subsection{8.6.1 The 2D / 3D asymmetry as a substrate-level result}
...
\begin{itemize}
\item \textbf{In 2D, vortex-stretching identically vanishes.} The vorticity in 2D is a scalar (conserved along streamlines, with the vortex-stretching term $(\boldsymbol\omega\cdot\nabla)\mathbf{v}$ identically zero because there is no $z$-component for the gradient to act through). With no vortex-stretching, no obstruction; smooth solutions follow by standard energy methods.
\item \textbf{In 3D, vortex-stretching is structurally non-trivial.} The vorticity is a vector, and the analog of "conserved along streamlines" fails. The vortex-stretching term $(\boldsymbol\omega\cdot\nabla)\mathbf{v}$ is the unique non-sign-definite contribution to the gradient-norm Lyapunov balance; it can in principle drive finite-time singularities; whether it actually does is the Clay-open question.
\end{itemize}

The substrate-level reading of this asymmetry: the obstruction in 3D NS is structurally \textit{real} because the frame-kinematic advective sector produces a vector-valued nonlinear term that the substrate does not natively control. In 2D the same frame-kinematic sector produces a scalar nonlinear term whose curl identically vanishes; the obstruction is structurally \textit{absent} because vortex-stretching has no vector to operate on.

\subsection{8.6.2 Why this matters structurally}

Two structural consequences:

\begin{itemize}
\item \textbf{2D NS smoothness is a substrate-level structural consequence of the dimensional structure.} The framework's analysis recovers the 1960s mathematical result that 2D NS admits global smooth solutions, but it identifies the structural reason as the absence of vortex-stretching in 2D rather than as a delicate energy-method argument.
\item \textbf{3D NS obstruction is structurally real but located in the non-substrate-derived sector.} The Clay-NS difficulty is not a calculational artifact; it reflects a real structural feature of the 3D advective sector. The framework's Path C verdict says: this is where the obstruction lives, and it lives in a sector the substrate does not natively control.
\end{itemize}
...
\section{8.7 Three-Angle Convergence on Advection-as-Non-ED}

\subsection{8.7.1 What three-angle convergence means}
...
\section{8.8 Turbulence as the Dynamical Signature of Non-Substrate-Derived Content}

\subsection{8.8.1 Why turbulence has no canonical substrate-level template}
...
\begin{itemize}
\item \textbf{Standard physics's many incomplete approaches to turbulence} are each capturing some aspect of the advective dynamics in laboratory-frame coordinates. Their incompleteness is structural rather than methodological; no single approach captures the whole because the dynamics they describe lives in a sector with no substrate-level architectural template.
\item \textbf{Future turbulence theory should focus on the advective sector specifically.} The substrate-derived sectors (viscous, R1) are well-understood; the cascade dynamics live in the advective sector. The framework's contribution is to localize the open-problem space.
\end{itemize}
...
\subsection{8.8.3 The H1/H2/H3 turbulence hypotheses}

Three turbulence-related hypotheses appear in the program's NS-Turb material:

\begin{itemize}
\item \textbf{H1} — viscous-sector content has a structurally clean coarse-grained reading. \textbf{Holds.} The viscous sector is substrate-derived through DCGT.
\item \textbf{H2} — substrate-cutoff R1 content has a structurally clean coarse-grained reading. \textbf{Holds.} R1 is substrate-derived through DCGT's first subleading order.
\item \textbf{H3} — cascade-class advective content has a substrate-derivable canonical template. \textbf{Fails substantively.} The substrate does not contain the advective term, so no substrate-level canonical template for the cascade is forthcoming.
\end{itemize}
...
\section{8.9 What This Changes (And What It Doesn't)}

\subsection{8.9.1 What does not change}
...
\subsection{8.9.2 What does change}

Three structural shifts:

\begin{itemize}
\item \textbf{The Navier–Stokes equation's specific form is identified as a hybrid.} Viscous diffusion is substrate-derived; R1 substrate-cutoff is substrate-derived; advection is frame-kinematic; pressure and incompressibility are continuum-imposed constraints. Standard fluid mechanics treats all four as phenomenological; the substrate-level reading distinguishes them architecturally.
\item \textbf{The Clay-NS smoothness obstruction is structurally located.} It lives in the frame-kinematic advective sector, not in the substrate-derived sectors. R1 supplies a real Clay-relevant regularizing mechanism; the framework's contribution to Clay-NS is \textit{Intermediate Path C} — substrate-grounded structural decomposition with explicit identification of what is helped and what is not.
\item \textbf{Turbulence has no substrate-level canonical template} because the advective sector is non-substrate. The empirical observation that turbulence has stayed phenomenologically untamed for a century has a structural explanation: the cascade dynamics live in a sector outside the substrate's reach.
\end{itemize}

These three shifts do not change any laboratory prediction. They change the conceptual placement of the open problems.

\section{8.10 Form-FORCED vs Value-INHERITED at the NS Sector}

\subsection{8.10.1 What is form-FORCED}

\begin{itemize}
\item \textbf{The four-fold architectural decomposition} of NS into viscous-diffusion (substrate-derived), R1 substrate-cutoff (substrate-derived), advection (frame-kinematic), pressure + incompressibility (continuum-imposed).
\item \textbf{R1 substrate-cutoff form} as the first-subleading-order DCGT output: $-\kappa\mu_{\mathrm{V1}}\ell_P^2\nabla^4\mathbf{v}$.
\item \textbf{The Intermediate Path C verdict} on Clay-NS: real regularizing mechanism + structural identification of the obstruction.
\item \textbf{The dimensional-structure account}: 2D smoothness (vortex-stretching identically vanishes); 3D obstruction (vortex-stretching is the unique non-sign-definite contribution).
\item \textbf{Three-angle convergence} on advection-as-non-ED (architectural / dynamical / spectral).
\item \textbf{Turbulence as the dynamical signature of the non-substrate-derived advective sector.}
\item \textbf{The H1/H2/H3 status} (H1 holds, H2 holds, H3 fails substantively).
\end{itemize}

\subsection{8.10.2 What is value-INHERITED}

\begin{itemize}
\item \textbf{The viscosity coefficient $\mu$.} Set by substrate-channel statistics; value INHERITED.
\item \textbf{The R1 coefficient combination $\kappa\mu_{\mathrm{V1}}$.} Set by V1's specific functional shape; value INHERITED.
\item \textbf{The substrate length scale $\ell_P$ entering R1's coefficient.} Identified with the Planck length through T19 Newton-recovery (Chapter 11); value INHERITED.
\item \textbf{Specific turbulence cascade exponents} (Kolmogorov $-5/3$, Kraichnan $-3$, etc.). These are properties of the non-substrate-derived advective sector; values are INHERITED from fluid-mechanics empirical input rather than derived from substrate primitives.
\item \textbf{Critical Reynolds numbers} for transitions (laminar-to-turbulent transition, etc.). INHERITED from fluid-mechanics empirical input.
\end{itemize}

\subsection{8.10.3 What is open}

\begin{itemize}
\item \textbf{A constructive Clay-NS proof.} Path A is open. The framework's Intermediate Path C verdict does not provide a Clay-rigorous existence-and-smoothness proof.
\item \textbf{A canonical structural account of turbulence.} Open at substrate level; the framework predicts no substrate-level template will be forthcoming because turbulence lives in a non-substrate sector.
\item \textbf{A constructive analysis of the advective vortex-stretching dynamics} at substrate level. Substrate analysis does not natively reach this sector; whether a substrate-level extension could control vortex-stretching is open work.
\end{itemize}

\section{8.11 Dependencies}

\subsection{8.11.1 Upstream}

\begin{itemize}
\item \textbf{Chapter 1.} Substrate primitives, especially P04 bandwidth update rule (foundational for participation transport), the finite-kernel commitment for V1 (foundational for R1's substrate-cutoff structure), and substrate-locality (foundational for the lab-frame-versus-co-moving-frame distinction underlying advection-as-frame-kinematic).
\item \textbf{Chapter 2.} Load-bearing invariants. V1 finite-width vacuum kernel is the substrate-level object that produces R1 through DCGT first subleading order; gradient sparsity $\sigma$ enters substrate-level transport content indirectly.
\item \textbf{Chapter 3.} DCGT. The leading-order viscous-diffusion content and the first-subleading-order R1 content are the two DCGT outputs that produce the substrate-derived sectors of NS. The hydrodynamic-window scale separation $\ell_P \ll R_\mathrm{cg} \ll L_\mathrm{flow}$ is the substrate-level prerequisite for the multi-scale expansion that produces them.
\end{itemize}

\subsection{8.11.2 Downstream}

\begin{itemize}
\item \textbf{Chapter 9 (MHD and Yang–Mills).} Extends the NS architectural decomposition to MHD by adding T17 minimal coupling on charged-chain populations (producing the Lorentz force and Maxwell's equations). The MHD content classification inherits the canonical-ED-versus-non-ED boundary developed in this chapter. Yang–Mills uses the non-Abelian DCGT generalization that is structurally parallel to NS but applied to compact-simple-group rule-types.
\item \textbf{Chapter 10 (Soft-Matter Mobility).} Uses DCGT scalar-diffusion content (the substrate-to-continuum bridge analogous to the viscous-diffusion content of NS) plus V5→Maxwell viscoelasticity. The substrate-level decomposition methodology is consistent with the four-fold NS decomposition of this chapter.
\item \textbf{Chapter 11 (Substrate Gravity).} The cumulative-strain coarse-graining that produces the modified Poisson equation in the substrate-gravity sector is structurally parallel to the DCGT machinery that produces NS's substrate-derived sectors. The form-FORCED / value-INHERITED methodology developed in this chapter propagates through Chapter 11.
\item \textbf{Chapter 14 (Cross-Platform Unifications).} The kinematic-coupling-pattern boundary (canonical ED via T17 minimal coupling versus transport-kinematic non-ED via advection / induction kinematic / Ohm kinematic) is a cross-arc structural finding that this chapter establishes for NS and Chapter 9 extends to MHD.
\end{itemize}

\section{8.12 Canonical Sources}

\begin{itemize}
\item \texttt{papers/Navier Stokes\_Synthesis\_Paper/} (main + Appendix C [MHD architectural classification + ED-I-06] + Appendix D [DCGT] + Appendix E [Yang–Mills])
\item ED-I-06 (Fields and Forces in Event Density, Feb 2026; ontological roof for fluid mechanics, MHD, and substrate gravity)
\end{itemize}
...
\section{9.2 Why MHD and Yang–Mills Sit Together}

\subsection{9.2.1 The shared substrate machinery}
...
The substrate-level analysis identifies a structural connection that the standard separation hides. Both MHD and Yang–Mills are continuum theories that emerge from the substrate through the \textit{same} substrate-to-continuum machinery applied to different content channels:

\begin{itemize}
\item \textbf{DCGT} (Chapter 3) supplies the substrate-to-continuum bridge for both. MHD uses DCGT's leading-order viscous-diffusion content (the Navier–Stokes baseline of Chapter 8) plus the substrate-to-continuum bridge for charged-chain populations. Yang–Mills uses the non-Abelian generalization of DCGT applied to compact-simple-group rule-types.
\item \textbf{T17 minimal coupling} (Chapter 6) supplies the gauge-field content for both. MHD uses T17's Abelian (U(1)) content to produce the Lorentz force and Maxwell's equations. Yang–Mills uses T17's non-Abelian generalization to produce the Yang–Mills equation.
\item \textbf{V1 finite-width kernel} (Chapter 4) supplies the substrate-cutoff content for both. MHD inherits R1 (as in NS, Chapter 8). Yang–Mills uses V1's second-moment expansion to produce a substrate-level mass-gap mechanism.
\end{itemize}
...
\subsection{9.2.2 The structural gain of the joint treatment}

Three structural gains follow from treating MHD and Yang–Mills together:

\begin{itemize}
\item \textbf{The kinematic-coupling-pattern boundary becomes visible.} In both MHD and Yang–Mills, content items partition into substrate-derived (canonical-ED, reachable through DCGT plus T17 minimal coupling) and non-substrate (frame-kinematic or constraint-class). The boundary between these two classes is the same boundary that appeared in NS (Chapter 8); the joint treatment makes the boundary's universality visible.
\item \textbf{The mass-gap mechanism inherits from finite-width V1.} The substrate-level mass-gap in Yang–Mills uses V1's finite-width content (Theorem N1, Chapter 4), the same substrate object that produces the R1 hyperviscous term in NS and MHD. The structural reason MHD does not have a mass gap (it does not require non-Abelian quartic stabilization) and Yang–Mills does (it requires non-Abelian quartic stabilization for the gap to survive continuum-limit flow) is exposed.
\item \textbf{The Clay Yang–Mills verdict mirrors the Clay-NS verdict.} Both Clay-class Millennium Problems receive Intermediate Path C verdicts at structural-suggestive level. The structural parallels and differences are exposed through the joint chapter.
\end{itemize}

\subsection{9.2.3 What the chapter does not deliver}

The chapter does not deliver:

\begin{itemize}
\item \textbf{A constructive Yang–Mills existence-and-mass-gap proof at Streater–Wightman / OS-axiom rigor.} The Clay Yang–Mills problem demands a proof at constructive-rigorous level. The framework's verdict is at structural-suggestive level — a substrate-grounded structural-positive content with explicit identification of what the substrate does not constructively close.
\item \textbf{Specific values of the gauge couplings.} The fine structure constant $\alpha$, the strong coupling $\alpha_s$, the weak couplings — all are INHERITED from particle-physics empirical input.
\item \textbf{The Standard Model gauge group.} $SU(3) \times SU(2) \times U(1)$ is empirical input; the framework derives the form of gauge structure but not which group nature realizes (this is the same value-INHERITED status established in Chapter 6).
\item \textbf{Specific MHD coefficients.} Magnetic diffusivity, conductivity, and other MHD-specific empirical coefficients are INHERITED from plasma-physics empirical input.
\end{itemize}
...
\section{9.3 MHD as NS Plus T17 Minimal Coupling}

\subsection{9.3.1 The structural extension}
...
\begin{enumerate}
\item \textbf{NS as baseline.} The four-fold NS decomposition of Chapter 8 is the pure-fluid baseline: viscous diffusion (substrate-derived), R1 substrate-cutoff (substrate-derived), advection (frame-kinematic), pressure plus incompressibility (continuum-imposed).
\item \textbf{T17 minimal coupling on charged-chain populations.} T17 (Chapter 6) supplies the substrate-level structure for gauge fields as participation measures of structural rule-types. Applied to charged-chain populations, T17 minimal coupling produces the Lorentz force on charged participation patterns and Maxwell's equations as the substrate-to-continuum bridge for the gauge-field content.
\end{enumerate}
...
\subsection{9.3.2 The Lorentz force from T17 minimal coupling}
...
as the leading-order effective force on a charged participation pattern moving with velocity $\mathbf{v}$ through the U(1) gauge field $\mathbf{E}$, $\mathbf{B}$. The Lorentz force is \textit{derived} rather than postulated. The structural content:

\begin{itemize}
\item \textbf{The substrate-level origin} is T17 minimal coupling on charged-chain populations.
\item \textbf{The continuum-level form} is the standard Lorentz force.
\item \textbf{The DCGT bridge} (Chapter 3, Section 3.6.5) is the leading-order coarse-grained content.
\end{itemize}
...
\subsection{9.3.3 Maxwell's equations from T17 gauge content}
...
\subsection{9.3.4 The MHD content classification}
...
\textbf{Six canonical-ED items (substrate-derived).} The six items that are substrate-derived through DCGT plus T17 minimal coupling:

\begin{enumerate}
\item Viscous diffusion ($\mu\nabla^2\mathbf{v}$). Inherited from NS (Chapter 8, Section 8.3.1).
\item Magnetic diffusion. Substrate-derived through DCGT applied to magnetic-field participation density.
\item Lorentz force ($q(\mathbf{E} + \mathbf{v}\times\mathbf{B})$). Substrate-derived through T17 minimal coupling on charged-chain populations.
\item R1 substrate-cutoff ($-\kappa\mu_{\mathrm{V1}}\ell_P^2\nabla^4\mathbf{v}$). Inherited from NS (Chapter 8, Section 8.4.1).
\item $\partial_t\mathbf{B}$ time-evolution. Substrate-derived through T17 gauge-field dynamics.
\item $\nabla\cdot\mathbf{B} = 0$ no-monopole condition. Substrate-derived through T17's specific U(1) rule-type structure.
\end{enumerate}

\textbf{Two continuum-imposed constraints.} The two items that are continuum-imposed (the same class as in NS):
...
\textbf{Three transport-kinematic non-ED items.} The three items that are frame-kinematic artifacts, parallel to NS's advective term but in distinct content channels:

\begin{enumerate}
\item Advection ($(\mathbf{v}\cdot\nabla)\mathbf{v}$). Frame-kinematic artifact of laboratory-frame coordinates. Inherited from NS (Chapter 8, Section 8.3.2).
\item Induction kinematic. The induction-equation kinematic content corresponding to advective transport of magnetic field with the fluid. Frame-kinematic at substrate level.
\item Ohm kinematic. The kinematic part of Ohm's law (relating current to electric-plus-velocity-cross-magnetic field structure). Frame-kinematic at substrate level.
\end{enumerate}
...
\subsection{9.3.5 The H1/H2/H3 status for MHD}

The three turbulence-related hypotheses developed in the NS chapter (Chapter 8, Section 8.8.3) extend to MHD:

\begin{itemize}
\item \textbf{H1} — viscous-sector and magnetic-diffusion-sector content has structurally clean coarse-grained reading. \textbf{Holds.} Both are substrate-derived through DCGT.
\item \textbf{H2} — substrate-cutoff R1 content has structurally clean coarse-grained reading. \textbf{Holds.} Inherited from NS.
\item \textbf{H3} — cascade-class advective and induction-kinematic content has substrate-derivable canonical template. \textbf{Holds in modified form for MHD.} MHD turbulence cascades (Alfvén-wave cascades, Iroshnikov-Kraichnan, Goldreich-Sridhar) operate in a sector that includes some substrate-derivable content (magnetic diffusion, Lorentz force coupling) plus the non-substrate transport-kinematic sector. The substrate-level template covers more of the cascade than in pure NS but does not cover all of it.
\end{itemize}
...
\subsection{9.3.6 The kinematic-coupling-pattern as the canonical boundary}

The MHD classification exposes a structural finding that generalizes beyond the MHD sector itself: the canonical/non-canonical boundary in continuum field theory is the \textbf{kinematic-coupling-pattern boundary}.

Velocity-dependence in continuum equations splits cleanly into:
\begin{itemize}
\item \textbf{Minimal-coupling-derived content} (canonical ED via T17 — Lorentz force, magnetic-field coupling to charged participation). This is substrate-derived through T17 minimal coupling on label-carrying participation events.
\item \textbf{Transport-kinematic content} (non-ED — advection, induction kinematic, Ohm kinematic). This is frame-kinematic, arising from coordinate-frame choices rather than from substrate-level forces.
\end{itemize}
...
\section{9.4 Yang–Mills as the Non-Abelian DCGT Output}

\subsection{9.4.1 The non-Abelian DCGT generalization}
...
\begin{itemize}
\item The hydrodynamic-window scale separation $\ell_P \ll R_\mathrm{cg} \ll L_\mathrm{flow}$ is unchanged.
\item The multi-scale expansion structure is unchanged.
\item The participation-channel content is generalized to carry non-Abelian labels.
\item T17 minimal coupling is generalized to non-Abelian rule-types.
\end{itemize}
...
\subsection{9.4.2 What is form-FORCED in Yang–Mills}

The substrate-level analysis FORCES:

\begin{itemize}
\item \textbf{The Yang–Mills equation form} $D_\mu F^{\mu\nu} = J^\nu$ at leading order.
\item \textbf{The non-Abelian gauge-covariant derivative} structure $D_\mu = \partial_\mu - igA_\mu$ with $A_\mu$ taking values in the gauge-group's Lie algebra.
\item \textbf{The field-strength tensor} $F_{\mu\nu} = \partial_\mu A_\nu - \partial_\nu A_\mu - ig[A_\mu, A_\nu]$ with the non-Abelian commutator term.
\item \textbf{The gauge-invariance interface property} under local non-Abelian transformations $A_\mu \to U A_\mu U^\dagger + (i/g) U\partial_\mu U^\dagger$.
\item \textbf{The classical action} $S = -\frac{1}{4}\int d^4x\,\mathrm{tr}(F_{\mu\nu}F^{\mu\nu}) + \text{matter terms}$ at leading order.
\end{itemize}

\subsection{9.4.3 What is value-INHERITED in Yang–Mills}

The substrate-level analysis does not derive:

\begin{itemize}
\item \textbf{The specific gauge group} nature realizes (the Standard Model's $SU(3) \times SU(2) \times U(1)$ structure). Empirical input.
\item \textbf{The gauge couplings} $g$ for each gauge group (the strong coupling $\alpha_s$, the weak couplings, the electroweak mixing angle). Empirical input.
\item \textbf{The specific matter content} (number of quark flavors, lepton families, Higgs sector). Empirical input.
\item \textbf{Particle masses} in the matter sector. Empirical input.
\end{itemize}
...
\section{9.5 The Substrate-Level Mass-Gap Mechanism}

\subsection{9.5.1 The Yang–Mills mass-gap problem}

The Clay Yang–Mills Millennium Problem asks for a constructive proof that pure non-Abelian Yang–Mills theory on $\mathbb{R}^4$ exists at Streater–Wightman / Osterwalder–Schrader rigor \textit{and} that it has a positive mass gap — there is no zero-mass excitation in the spectrum despite the gauge bosons being massless at the classical level. The mass gap is the structural feature that makes confinement possible in QCD; without it, color-singlet hadrons would not exist.
...
The framework's claim: the mass gap survives under continuum-limit flow \textit{conditional} on V1's kernel profile rescaling appropriately. Specifically, if V1's second-moment scales correctly relative to the gauge-field rescaling under the flow, the mass-gap contribution remains finite at the continuum scale; if V1's rescaling produces a different scaling exponent, the gap could either grow or shrink relative to the gauge-field scale.

The structural status: the mass-gap mechanism is form-FORCED at substrate scale; survival under continuum-limit flow is \textit{conditional} on V1's kernel-profile rescaling, which is INHERITED from V1's specific functional shape. Closed-form derivation of V1's kernel profile is downstream open work.

\subsection{9.5.4 The OS-positivity audit}
...
\section{9.6 The Gribov-Class Gauge-Fixing Obstruction}

\subsection{9.6.1 What the Gribov obstruction is}
...
\subsection{9.6.3 What the reframing solves and does not solve}

The reframing \textbf{does}:
\begin{itemize}
\item Identify the Gribov ambiguity at substrate level as a feature of the gauge-quotient topology rather than as a quantization obstruction.
\item Suggest a constructive path through working with substrate-level gauge-equivalence classes directly.
\item Connect the Gribov question to T17's substrate-level structural commitments rather than to specific gauge-fixing technicalities.
\end{itemize}

The reframing \textbf{does not}:
\begin{itemize}
\item Constructively resolve the Gribov ambiguity at Clay-rigorous level.
\item Produce a complete constructive Yang–Mills existence proof on $\mathbb{R}^4$.
\item Replace lattice gauge theory or other partial-rigor frameworks at the level of empirical content.
\end{itemize}
...
\section{9.7 The Conditional-Positive Verdict on Clay Yang–Mills}

\subsection{9.7.1 Path-C-style verdict for Yang–Mills}

The framework's verdict on the Clay Yang–Mills problem is structurally parallel to the Clay-NS verdict (Chapter 8, Section 8.5): \textbf{Intermediate Path C} at substrate-suggestive level. The components:

\begin{itemize}
\item \textbf{Real Clay-relevant content.} The substrate-level mass-gap mechanism (V1 second-moment + non-Abelian quartic stabilization) is real substrate-derivable content for the Clay-NS-class question of mass-gap existence.
\item \textbf{Substrate-suggestive positive content.} The OS-positivity audit at channel-by-channel level produces structural-suggestive positive content. The gauge-quotient reframing of the Gribov obstruction produces structural-suggestive positive content.
\item \textbf{Explicit identification of what is not constructively closed.} The conditional survival of the mass gap under continuum-limit flow (conditional on V1's kernel-profile rescaling); the partial reframing of the Gribov obstruction (suggestive but not constructive); the absence of a full Streater–Wightman / OS-axiom rigor proof.
\end{itemize}
...
\subsection{9.7.2 What the verdict delivers}

\begin{itemize}
\item A substrate-derived continuum Yang–Mills equation at leading order.
\item A substrate-level mass-gap mechanism at first subleading order.
\item A substrate-suggestive positive OS-positivity content.
\item A substrate-level reframing of the Gribov gauge-fixing obstruction.
\item A clear identification of which Clay-rigorous components remain open.
\end{itemize}

\subsection{9.7.3 What the verdict does not deliver}

\begin{itemize}
\item A constructive Yang–Mills existence proof at Streater–Wightman / OS-axiom rigor on $\mathbb{R}^4$.
\item A constructive resolution of the Gribov ambiguity at Clay-rigor.
\item A closed-form proof of mass-gap survival under continuum-limit flow.
\item A solution to the Clay Yang–Mills Millennium Problem.
\end{itemize}
...
\section{9.8 Cross-Arc Structural Findings}

\subsection{9.8.1 The kinematic-coupling-pattern as a cross-arc finding}
...
\begin{itemize}
\item \textbf{In NS (Chapter 8).} The canonical/non-canonical split is viscous + R1 (substrate-derived) versus advection (frame-kinematic) versus pressure + incompressibility (constraint).
\item \textbf{In MHD (this chapter).} The 6:2:3 split with strictly-more-canonical content on the EM side. Same boundary structure as NS, with T17 minimal coupling supplying the additional canonical content.
\item \textbf{In Yang–Mills (this chapter).} The Yang–Mills equation $D_\mu F^{\mu\nu} = J^\nu$ is fully substrate-derived (gauge-covariant); the matter-coupling sector inherits T17 minimal coupling. The non-Abelian generalization preserves the kinematic-coupling-pattern boundary but moves more content into the canonical-ED class.
\end{itemize}
...
\subsection{9.8.3 Yang–Mills as fully canonical-ED on the gauge sector}
...
\section{9.9 What This Changes (And What It Doesn't)}

\subsection{9.9.1 What does not change}
...
\subsection{9.9.2 What does change}

Three structural shifts:

\begin{itemize}
\item \textbf{The Lorentz force is derived rather than postulated.} Standard physics treats the Lorentz force as fundamental; ED derives it from T17 minimal coupling on charged-chain populations.
\item \textbf{The Yang–Mills equation is derived rather than postulated.} Standard physics establishes Yang–Mills theory through the gauge-invariance principle; ED derives the equation from DCGT non-Abelian generalization plus T17 generalized minimal coupling.
\item \textbf{The mass gap has a substrate-level mechanism.} Standard physics establishes the mass gap empirically; ED supplies a substrate-level mechanism (V1 second-moment + non-Abelian quartic stabilization), conditional on V1's kernel-profile rescaling under continuum-limit flow.
\end{itemize}

These three shifts do not change any laboratory prediction. They change the structural origin of the equations.

\section{9.10 Form-FORCED vs Value-INHERITED at MHD and Yang–Mills}

\subsection{9.10.1 What is form-FORCED}

\begin{itemize}
\item \textbf{The MHD content classification} (6:2:3 partition into canonical-ED, continuum-imposed, transport-kinematic non-ED).
\item \textbf{The Lorentz force} as T17 minimal coupling on charged-chain populations.
\item \textbf{Maxwell's equations} as the substrate-to-continuum bridge for U(1) gauge content via T17.
\item \textbf{The Yang–Mills equation form} $D_\mu F^{\mu\nu} = J^\nu$ at leading order from DCGT non-Abelian generalization plus T17 generalized minimal coupling.
\item \textbf{The substrate-level mass-gap mechanism form} (V1 second-moment + non-Abelian quartic stabilization).
\item \textbf{The OS-positivity substrate-suggestive positive verdict} at channel-by-channel level.
\item \textbf{The gauge-quotient reframing of the Gribov obstruction} at substrate level.
\item \textbf{The Intermediate Path C verdict structure} for Clay Yang–Mills.
\item \textbf{The kinematic-coupling-pattern boundary} as the canonical/non-canonical distinction in continuum field theory.
\end{itemize}

\subsection{9.10.2 What is value-INHERITED}

\begin{itemize}
\item \textbf{The specific gauge group} (Standard Model's $SU(3) \times SU(2) \times U(1)$). Empirical input.
\item \textbf{The gauge couplings} ($\alpha$, $\alpha_s$, the weak couplings). Empirical input.
\item \textbf{The specific matter content} (quark flavors, lepton families, Higgs sector). Empirical input.
\item \textbf{Particle masses} in the matter sector. Empirical input.
\item \textbf{The mass-gap value} in pure Yang–Mills (e.g., the lowest-glueball mass in QCD). Conditional on V1's kernel-profile rescaling under continuum-limit flow; INHERITED.
\item \textbf{MHD-specific coefficients} (magnetic diffusivity, conductivity). INHERITED from plasma-physics empirical input.
\item \textbf{The R1 coefficient} in MHD's hyperviscous content. INHERITED, parallel to NS.
\end{itemize}

\subsection{9.10.3 What is open}

\begin{itemize}
\item \textbf{A constructive Yang–Mills existence-and-mass-gap proof at Clay-rigor.} Path A is open. The framework's substrate-suggestive positive verdict does not provide a Streater–Wightman / OS-axiom rigorous proof.
\item \textbf{A constructive resolution of the Gribov ambiguity at Clay-rigor.} Reframed at substrate level but not constructively closed.
\item \textbf{Closed-form derivation of V1's kernel-profile rescaling under continuum-limit flow.} Determines whether the mass gap survives unchanged or is renormalized; downstream open work.
\item \textbf{Closed-form values of gauge couplings.} Part of the broader closed-form-substrate-constants program (alongside $\mathcal{M}_\mathrm{crit}$ in Chapter 7, $\log g$ in Chapter 13, and $\kappa/|\hat{N}'|$ in the ED-SC arc).
\end{itemize}

\section{9.11 Dependencies}

\subsection{9.11.1 Upstream}

\begin{itemize}
\item \textbf{Chapter 1.} Substrate primitives. Especially: P04 bandwidth update rule (foundational for charge-carrying participation transport); P11 commitment-irreversibility (foundational for the kernel-level arrow that V1 inherits); the finite-kernel commitment for V1 (foundational for the mass-gap mechanism); substrate-locality (foundational for the MHD lab-frame-versus-co-moving-frame distinction underlying advection-as-frame-kinematic).
\item \textbf{Chapter 2.} Load-bearing invariants. V1 finite-width vacuum kernel produces both R1 substrate-cutoff (in MHD, inherited from NS) and the mass-gap mechanism in Yang–Mills.
\item \textbf{Chapter 3.} DCGT. The leading-order viscous-diffusion content (in MHD, inherited from NS), the first-subleading-order R1 content (in MHD), and the non-Abelian generalization (in Yang–Mills) are all DCGT outputs. The hydrodynamic-window scale separation $\ell_P \ll R_\mathrm{cg} \ll L_\mathrm{flow}$ is the substrate-level prerequisite.
\item \textbf{Chapter 6.} T17 minimal coupling on charged-chain populations is the substrate-level mechanism for the Lorentz force and Maxwell's equations in MHD; T17's non-Abelian generalization is the substrate-level mechanism for the matter-coupling sector of Yang–Mills.
\item \textbf{Chapter 8.} NS architectural decomposition. MHD inherits NS's pure-fluid baseline (viscous diffusion, R1, advection, pressure, incompressibility). The 6:2:3 MHD classification extends NS's four-fold decomposition by adding T17-derived EM content.
\end{itemize}

\subsection{9.11.2 Downstream}

\begin{itemize}
\item \textbf{Chapter 14 (Cross-Platform Unifications).} The kinematic-coupling-pattern boundary (canonical ED via T17 minimal coupling versus transport-kinematic non-ED) is a cross-arc structural finding developed in this chapter and Chapter 8 and synthesized in Chapter 14 as one of the program's structural signatures.
\item \textbf{Chapter 15 (Public Test Inventory).} Empirical anchors for MHD and Yang–Mills sectors are catalogued in Chapter 15.
\end{itemize}

\section{9.12 Canonical Sources}

\begin{itemize}
\item \texttt{papers/Navier Stokes\_Synthesis\_Paper/} Appendix C (MHD architectural classification) and Appendix E (Yang–Mills Clay-relevance verdict)
\item Arc YM memos in \texttt{theory/Yang\_Mills/}
\item \texttt{papers/Gauge\_Fields\_Theorem\_17/}
\end{itemize}

The Navier–Stokes Synthesis Paper Appendix C presents the publication-grade MHD content classification (6:2:3 partition, canonical-ED-versus-non-ED boundary, kinematic-coupling-pattern as the canonical boundary). Appendix E presents the publication-grade Yang–Mills Clay-relevance verdict (substrate-derived continuum equation, mass-gap mechanism, OS-positivity audit, Gribov-reframing, Intermediate Path C verdict). Arc YM memos in \texttt{theory/Yang\_Mills/} (six memos: YM-1 compact-simple-group via P09 amendment + T17 strengthening, YM-2 substrate→continuum limit, YM-3 mass gap from $\ell_P$ substrate cutoff, YM-4 axiom verification, YM-5 OS-positivity audit, YM-6 synthesis) develop each component in detail. The Gauge\_Fields\_Theorem\_17 paper presents T17 (Chapter 6's content) used here as substrate-level mechanism for the Lorentz force and the Yang–Mills matter coupling.
...
\section{9.13 Optional Figures}


\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_49_mhd_content_classification.pdf}
\caption*{Figure 9.1 --- The MHD content classification. A 11-row diagram with each MHD content item in a row, color-coded by class: six canonical-ED items in green (substrate-derived through DCGT plus T17 minimal coupling); two continuum-imposed items in blue (pressure, incompressibility); three transport-kinematic non-ED items in red (advection, induction kinematic, Ohm kinematic). Annotations identify the substrate-level origin of each item.}
\label{fig:fig_49_mhd_content_classification}
\end{figure}


\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_50_ns_to_mhd_via_t17.pdf}
\caption*{Figure 9.2 --- From NS to MHD via T17. A flow diagram showing the structural extension. Left column: NS four-fold decomposition (viscous-diffusion, R1, advection, pressure + incompressibility). Center: T17 minimal coupling on charged-chain populations. Right column: MHD 6:2:3 decomposition with the EM-side canonical content (magnetic diffusion, Lorentz force, $\partial_t\mathbf{B}$, $\nabla\cdot\mathbf{B} = 0$) added to the NS baseline.}
\label{fig:fig_50_ns_to_mhd_via_t17}
\end{figure}


\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_51_kinematic_coupling_pattern_boundary.pdf}
\caption*{Figure 9.3 --- The kinematic-coupling-pattern boundary. A two-column diagram. Left column ("Canonical ED via T17 minimal coupling"): viscous diffusion, R1, magnetic diffusion, Lorentz force, $\partial_t\mathbf{B}$, $\nabla\cdot\mathbf{B} = 0$, Yang–Mills equation. Right column ("Transport-kinematic non-ED"): advection, induction kinematic, Ohm kinematic. The boundary between the columns is annotated as "the canonical/non-canonical boundary in continuum field theory: minimal-coupling-derived versus frame-kinematic."}
\label{fig:fig_51_kinematic_coupling_pattern_boundary}
\end{figure}


\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_52_ym_dcgt_nonabelian_output.pdf}
\caption*{Figure 9.4 --- Yang–Mills as DCGT non-Abelian output. A flow diagram. DCGT (Chapter 3) generalizes from Abelian to non-Abelian rule-types (T17 generalized minimal coupling). The leading-order output is the Yang–Mills continuum equation $D_\mu F^{\mu\nu} = J^\nu$. The first-subleading-order output is the substrate-level mass-gap mechanism (V1 second-moment + non-Abelian quartic stabilization). Survival under continuum-limit flow is conditional on V1's kernel-profile rescaling.}
\label{fig:fig_52_ym_dcgt_nonabelian_output}
\end{figure}


\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_53_ym_mass_gap_mechanism.pdf}
\caption*{Figure 9.5 --- The mass-gap mechanism. A two-panel diagram. Left panel: substrate-level structure showing V1's second-moment expansion contributing to the gauge field's effective dispersion. Right panel: non-Abelian quartic self-interaction from $-ig[A_\mu, A_\nu]$ contributing to the mass-like dispersion correction. The combination produces the substrate-level mass gap at first subleading order. A note observes that survival under continuum-limit flow is conditional on V1's kernel-profile rescaling.}
\label{fig:fig_53_ym_mass_gap_mechanism}
\end{figure}


\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_54_gribov_gauge_quotient.pdf}
\caption*{Figure 9.6 --- Gribov-reframing through gauge-quotient. A two-panel diagram. Left panel: standard gauge-fixing approach with multiple Gribov copies in the configuration space and an obstruction to unique gauge-fixing. Right panel: substrate-level gauge-quotient identification, where the substrate dynamics live on the configuration-space-modulo-gauge-group quotient directly, reframing the Gribov ambiguity as a feature of the quotient topology rather than as a quantization obstruction.}
\label{fig:fig_54_gribov_gauge_quotient}
\end{figure}


\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_55_ym_path_c_verdict.pdf}
\caption*{Figure 9.7 --- The Intermediate Path C verdict structure for Yang–Mills. A diagram showing the structural parallel to the Clay-NS Path C verdict. Real Clay-relevant content (mass-gap mechanism, OS-positivity substrate-suggestive verdict, gauge-quotient Gribov-reframing) on one side; what is not constructively closed (Streater–Wightman rigor, full Gribov resolution, V1 kernel-profile closed form) on the other side. A note observes that the verdict is structural-positive at substrate-suggestive level, parallel to Clay-NS and to the BH 1/4-coefficient INHERITED verdict.}
\label{fig:fig_55_ym_path_c_verdict}
\end{figure}


\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_56_mhd_ym_form_value_tagging.pdf}
\caption*{Figure 9.8 --- Form-FORCED vs Value-INHERITED at MHD and Yang–Mills. A two-column diagram. Left column ("Form-FORCED"): MHD 6:2:3 classification, Lorentz force from T17, Maxwell's equations, Yang–Mills equation form, mass-gap mechanism form, OS-positivity verdict, gauge-quotient reframing, kinematic-coupling-pattern boundary. Right column ("Value-INHERITED"): Standard Model gauge group, gauge couplings, particle masses, mass-gap numerical value, MHD-specific coefficients, R1 prefactor.}
\label{fig:fig_56_mhd_ym_form_value_tagging}
\end{figure}


\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_57_mhd_ym_in_program_context.pdf}
\caption*{Figure 9.9 --- MHD and Yang–Mills in the program's broader closed-arc context. A horizontal layout showing the position of MHD and Yang–Mills in the program's continuum-theory inventory: NS (Chapter 8); MHD and Yang–Mills (this chapter); soft-matter mobility (Chapter 10); substrate gravity (Chapter 11). The figure makes visible the shared substrate machinery (DCGT, T17, V1) across the four continuum sectors.}
\label{fig:fig_57_mhd_ym_in_program_context}
\end{figure}

\newpage


\chapter{Soft-Matter Mobility and Non-Newtonian Rheology: UDM, P4-NN, V5}
\label{chap:10}
...
This chapter establishes the substrate-level architectural unification of soft-matter rheology. Two structural commitments produce all five canonical families. \textbf{The Universal Mobility Law (UDM)} establishes that mobility in concentrated soft-matter systems takes the form $M(\rho) = M_0(1-\rho/\rho_\mathrm{max})^\beta$ with $\beta \approx 2$, derived from substrate primitive P04 (the bandwidth update rule, applied as the mobility-capacity bound). \textbf{The P4-NonNewtonian extension (P4-NN)} generalizes the same architectural principle from concentration-driven mobility to flow-state-variable mobility, producing four of the five families through different applications of P04 to different state variables. The fifth family — Maxwell-class viscoelasticity — comes from \textbf{V5 cross-chain memory} under DCGT coarse-graining, producing the standard $\tau_R \dot\sigma + \sigma = 2\mu S$ Maxwell equation with the relaxation time $\tau_R$ identified as V5's first temporal moment.
...
\section{10.2 The Soft-Matter Rheology Inventory}

\subsection{10.2.1 The five canonical families}
...
\begin{itemize}
\item \textbf{Krieger–Dougherty divergence.} For suspensions of solid particles in fluid, viscosity diverges as the particle volume fraction approaches a packing limit:
\end{itemize}
\[
...
\begin{itemize}
\item \textbf{Discontinuous shear-thickening (DST).} Some suspensions (cornstarch in water, certain silica suspensions, body-armor fluids) show a sharp, discontinuous jump in viscosity above a critical shear rate. Below the critical rate the fluid is liquid-like; above it, viscosity rises by orders of magnitude.
\item \textbf{Cross-class shear-thinning.} Many polymer solutions, ketchup, paint, and biological fluids show monotone decrease of viscosity with shear rate. Cross's phenomenological model fits this with a high-viscosity plateau at low shear, transitioning to a low-viscosity tail at high shear.
\item \textbf{Mixed regimes.} Some fluids show non-monotone viscosity-shear-rate curves: thinning then thickening, or thickening then thinning. Mixed-regime models are typically constructed by combining Cross-class and DST-class machinery with material-specific transition shapes.
\item \textbf{Maxwell-class viscoelasticity.} Silly putty, long-chain polymer solutions, and biological gels respond elastically to fast deformations and viscously to slow deformations. The standard Maxwell equation $\tau_R\dot\sigma + \sigma = 2\mu S$ captures this with one parameter (the relaxation time $\tau_R$) that varies by orders of magnitude across systems.
\end{itemize}
...
\subsection{10.2.2 What is missing in the standard treatment}

Three structural questions that the standard soft-matter rheology framework does not answer:

\begin{itemize}
\item \textbf{Why these five families and not others?} Why does the empirical inventory cluster into Krieger–Dougherty, DST, Cross-class, mixed, and Maxwell rather than into some other classification? Standard rheology treats this as an empirical question without structural content.
\item \textbf{Why do diffusion, suspension viscosity, and shear-thickening share an exponent class?} Empirical exponents in concentration-driven diffusion suppression, Krieger–Dougherty viscosity divergence, and DST critical-rate behavior all cluster near $\beta \approx 2$. Standard rheology treats this as coincidence; the substrate-level analysis does not.
\item \textbf{What is the structural origin of the relaxation time in Maxwell-class viscoelasticity?} The standard Maxwell model treats $\tau_R$ as a phenomenological fitting parameter. Standard rheology does not derive $\tau_R$ from a substrate-level mechanism.
\end{itemize}
...
\section{10.3 Substrate Inputs: P04 and V5}

\subsection{10.3.1 P04 as the mobility-capacity bound}

Substrate primitive P04 (Chapter 1, Section 1.4.1) is the bandwidth update rule: participation channels carry bounded bandwidth, and the bandwidth update constrains how much of a chain's available participation capacity can be active at any substrate moment. Applied to soft-matter systems where the substrate is densely populated by participation events, P04 supplies a \textit{mobility-capacity bound}: the rate at which a tagged participation event can move through the system is bounded by the available substrate-channel capacity at each transit step.
...
\subsection{10.3.2 V5 as the cross-chain memory kernel}
...
\subsection{10.3.3 Together: P04 + V5 produce all five families}
...
\section{10.4 The Universal Mobility Law (UDM)}

\subsection{10.4.1 The structural derivation}
...
\begin{enumerate}
\item \textbf{A tagged participation event} in a soft-matter system (a tracer molecule, a colloidal particle, a polymer segment) moves through a substrate populated by other participation events.
\item \textbf{The tagged event's mobility} is bounded above by the available substrate-channel capacity at each transit step.
\item \textbf{As local participation density $\rho$ rises}, the available channel capacity falls; mobility decreases.
\item \textbf{The functional form of the mobility-capacity bound} is FORCED by P04's structural content: a smooth power-law approach to zero as $\rho \to \rho_\mathrm{max}$:
\end{enumerate}
\[
M(\rho) = M_0\left(1 - \frac{\rho}{\rho_\mathrm{max}}\right)^\beta.
\]
...
\subsection{10.4.2 Empirical validation across ten systems}
...
\begin{itemize}
\item PMMA colloids (hard-sphere suspensions)
\item Hard-sphere colloids (additional samples)
\item Sucrose–water solutions
\item Glycerol–water solutions
\item BSA protein solutions
\item Lysozyme protein solutions
\item PEG (polyethylene glycol) in water
\item Dextran (polysaccharide) in water
\item Casein micelles
\item Ludox silica suspensions
\end{itemize}
...
\subsection{10.4.3 The mechanism-clustered scatter}
...
Systems where the saturation mechanism involves \textit{cooperative} network effects — hydrogen-bonding viscosity in glycerol, hydrodynamic crowding in dextran, certain protein-solution structures — cluster around $\bar\beta \approx 2.31$. \textit{Above} the canonical value.

Systems where the saturation mechanism is \textit{steric} — hard-sphere colloids, ordinary polymer suspensions, the textbook hard-sphere cases — cluster around $\bar\beta \approx 1.76$. Right at the canonical value.

Systems where the saturation mechanism is \textit{gradual and less cooperative} — polymer entanglement at moderate concentration, electrostatic effects in dilute regime — cluster around $\bar\beta \approx 1.38$. \textit{Below} the canonical value.

The clustering by mechanism is structurally consistent with the substrate-level account. The substrate-natural prediction is that more-cooperative networks should saturate sharper (higher $\beta$) and less-cooperative ones should saturate gentler (lower $\beta$). The empirical data delivers this pattern across the ten-system range. The framework's claim is at the level of \textit{structural coherence} rather than first-principles numerical prediction; the canonical $\beta = 2$ value plus mechanism-clustered scatter is what the substrate-level account predicts and the data confirms.

\subsection{10.4.4 What UDM does and does not deliver}

UDM \textbf{delivers}:
\begin{itemize}
\item The form-FORCED functional shape of mobility-saturation in concentrated soft-matter systems.
\item The substrate-natural exponent $\beta = 2$ as the canonical value.
\item A structural account of why the exponent class recurs across chemically unrelated systems (the same P04 bound applied to different microscopic physics).
\item Empirical validation across ten systems with $\beta = 1.72 \pm 0.37$ and $R^2 > 0.986$.
\item Mechanism-clustered scatter consistent with substrate-level predictions about cooperativity.
\end{itemize}

UDM \textbf{does not deliver}:
\begin{itemize}
\item Specific numerical predictions of $M_0$ or $\rho_\mathrm{max}$ for any given material. These are INHERITED from material physics.
\item A first-principles closed-form derivation of the precise empirical $\beta$ for any single system. The substrate-natural value is 2; the empirical scatter is consistent but not predicted in closed form.
\item A complete microscopic theory of any specific soft-matter system. The framework supplies the substrate-level architectural skeleton; the microscopic content is in standard soft-matter physics.
\end{itemize}

\section{10.5 The P4-NonNewtonian Extension (P4-NN)}

\subsection{10.5.1 The generalization principle}

UDM applies P04's mobility-capacity bound to \textit{concentration-driven} mobility — the bound on mobility as a function of local participation density $\rho$. The P4-NonNewtonian extension generalizes the same architectural principle to \textit{flow-state-variable} mobility — the bound on mobility as a function of dynamical state variables like strain rate, applied stress, or shear deformation.
...
The structural content: \textit{one substrate-level bound, multiple state-variable applications, multiple canonical rheology families}. The generalization is form-FORCED by P04's structural content.

\subsection{10.5.2 Krieger–Dougherty divergence (concentration-driven)}
...
\subsection{10.5.3 Discontinuous shear-thickening (strain-rate-driven)}
...
\subsection{10.5.4 Cross-class shear-thinning (strain-rate-driven, monotone-decreasing)}

For shear-thinning fluids, the saturation variable is again strain rate, but the substrate-level mobility-capacity is approached from the \textit{opposite} direction. At low strain rates the substrate-level participation network is in a high-multiplicity configuration (entangled, structured); applied strain rate breaks down the multiplicity, and the mobility \textit{increases} as the strain rate disrupts the network. The result is monotone-decreasing viscosity with shear rate.
...
\subsection{10.5.5 Mixed regimes}
...
\subsection{10.5.6 The structural unification of four families}
...
\begin{itemize}
\item \textbf{Same substrate primitive} (P04) in all four cases.
\item \textbf{Different state variables} (volume fraction for KD, strain rate for DST and Cross-class, multiple state variables for mixed regimes).
\item \textbf{Different functional structures} at the continuum level (divergence for KD, discontinuous jump for DST, monotone decrease for Cross-class, non-monotone for mixed).
\item \textbf{Common exponent class} $\beta \approx 2$ across diffusion, suspension viscosity, and shear-thickening (when the substrate-level bound applies in directly comparable forms).
\end{itemize}
...
\section{10.6 Maxwell-Class Viscoelasticity from V5}

\subsection{10.6.1 The fifth family and its substrate-level mechanism}
...
\subsection{10.6.2 The structural origin of the relaxation time}
...
\subsection{10.6.3 Why this matters structurally}

Two structural shifts:

\begin{itemize}
\item \textbf{The Maxwell equation is derived rather than postulated.} Standard rheology treats the Maxwell equation as a phenomenological model (a spring-and-dashpot mechanical analogue). The substrate-level account derives it from V5 cross-chain memory through DCGT coarse-graining.
\item \textbf{The relaxation time has substrate-level structural origin.} $\tau_R$ is V5's first temporal moment, not just a fitting parameter. The substrate-level structural origin connects the relaxation time to the substrate's memory kernel rather than to any specific microscopic mechanism.
\end{itemize}

The empirical content of Maxwell-class viscoelasticity does not change; the structural origin of the equation does.

\section{10.7 The Unified Soft-Matter Architecture}

\subsection{10.7.1 Two substrate commitments, five canonical families}
...
\subsection{10.7.2 The shared exponent class}

The empirical $\beta \approx 2$ exponent appears in three distinct empirical contexts:

\begin{itemize}
\item \textbf{Concentration-driven diffusion suppression} in the ten UDM systems ($\beta = 1.72 \pm 0.37$).
\item \textbf{Krieger–Dougherty viscosity divergence} in suspensions ($\beta_\mathrm{KD}$ near 2).
\item \textbf{Discontinuous shear-thickening critical-rate behavior} in DST systems (saturation exponent near 2).
\end{itemize}
...
\section{10.8 The Universal Mobility Law as Empirical Anchor}

\subsection{10.8.1 The PASSED status}
...
\begin{itemize}
\item \textbf{The form-FORCED structural content} (the functional form $M(\rho) = M_0(1-\rho/\rho_\mathrm{max})^\beta$ across ten chemically unrelated systems) is empirically confirmed.
\item \textbf{The substrate-natural exponent $\beta = 2$} is consistent with empirical data within one standard deviation.
\item \textbf{The mechanism-clustered scatter} (cooperative-network/steric/gradual-electrostatic clustering around 2.31/1.76/1.38) is consistent with the substrate-level account's prediction about cooperativity.
\end{itemize}

\subsection{10.8.2 What the empirical anchor is and is not}

The empirical anchor \textbf{is}:
\begin{itemize}
\item A cross-system test of the form-FORCED structural content across ten systems with different microscopic physics.
\item A quantitatively-tested validation of the substrate-natural exponent class.
\item An identification of mechanism-clustered scatter consistent with the substrate-level account's structural predictions.
\end{itemize}

The empirical anchor \textbf{is not}:
\begin{itemize}
\item A first-principles derivation of $\beta = 1.72$ from substrate primitives in closed form. The canonical value is $\beta = 2$; the empirical value is consistent with it but not predicted in closed form.
\item A complete substrate-level theory of any single soft-matter system. The framework supplies the architectural skeleton; the microscopic content is in standard soft-matter physics.
\item A solution to all open soft-matter rheology problems. The framework explicitly leaves yield-stress fluids and deeply entangled polymer melts as open extensions.
\end{itemize}

\subsection{10.8.3 The FRAP-related extension}
...
\section{10.9 What This Changes (And What It Doesn't)}

\subsection{10.9.1 What does not change}
...
\subsection{10.9.2 What does change}

Three structural shifts:

\begin{itemize}
\item \textbf{Five separately-modeled rheology families share two substrate commitments.} The standard treatment of KD, DST, Cross-class, mixed, and Maxwell as five separate phenomena is reframed as four projections of P04 plus one projection of V5.
\item \textbf{The shared $\beta \approx 2$ exponent has a substrate-level explanation.} The recurrence of the exponent class across diffusion, suspension viscosity, and shear-thickening reflects the same P04 mechanism applied to different state variables.
\item \textbf{The Maxwell relaxation time has a substrate-level origin.} $\tau_R$ is V5's first temporal moment, not a phenomenological fitting parameter.
\end{itemize}
...
\section{10.10 Form-FORCED vs Value-INHERITED at Soft-Matter Mobility}

\subsection{10.10.1 What is form-FORCED}

\begin{itemize}
\item \textbf{The Universal Mobility Law functional form} $M(\rho) = M_0(1-\rho/\rho_\mathrm{max})^\beta$.
\item \textbf{The substrate-natural exponent} $\beta = 2$.
\item \textbf{The four-family P4-NonNewtonian unification} (KD, DST, Cross-class, mixed regimes from P04 applied to different state variables).
\item \textbf{The Maxwell-equation form} $\tau_R\dot\sigma + \sigma = 2\mu S$ from V5 cross-chain memory under DCGT.
\item \textbf{The identification of $\tau_R$ as V5's first temporal moment.}
\item \textbf{The structural exhaustiveness} of UDM + P4-NN + V5 over the five canonical non-Newtonian rheology families.
\item \textbf{The mechanism-clustered scatter pattern} (cooperative networks → higher $\beta$; steric → near $\beta = 2$; gradual/electrostatic → lower $\beta$).
\item \textbf{The substrate origin of the shared $\beta \approx 2$ exponent class} across diffusion, suspension viscosity, and shear-thickening as the same P04 bound applied to different state variables.
\end{itemize}

\subsection{10.10.2 What is value-INHERITED}

\begin{itemize}
\item \textbf{The specific empirical $\beta$ for any individual system.} Empirical: $\beta = 1.72 \pm 0.37$ across ten systems.
\item \textbf{The packing limit $\rho_\mathrm{max}$ for any individual system.} Material physics.
\item \textbf{The unconstrained mobility $M_0$ for any individual system.} Material physics.
\item \textbf{The critical strain rate} for any individual DST system.
\item \textbf{The viscosity coefficients} in any specific Cross-class fit.
\item \textbf{The relaxation time $\tau_R$} for any individual Maxwell-class system. INHERITED from V5's specific functional shape and from molecular physics.
\item \textbf{Specific transition shapes} in mixed-regime fluids.
\item \textbf{Microscopic-level molecular details} for any specific soft-matter system.
\end{itemize}

\subsection{10.10.3 What is open}

\begin{itemize}
\item \textbf{Yield-stress fluids.} Bingham plastics, Herschel-Bulkley fluids, and other yield-stress phenomena are not covered by the UDM + P4-NN + V5 unification. Whether an additional substrate commitment closes them or whether they have a different substrate-level origin is open.
\item \textbf{Deeply entangled polymer melts.} The Doi-Edwards reptation regime is not directly addressed by UDM. Whether it fits the substrate-level architecture is open.
\item \textbf{Closed-form derivation of mechanism-clustered scatter.} The empirical clustering pattern is consistent with substrate-level cooperativity predictions, but a closed-form derivation of the specific cluster values is downstream open work.
\item \textbf{FRAP-BSA empirical confirmation.} The pre-registered protocol's empirical results are pending.
\end{itemize}

\section{10.11 Dependencies}

\subsection{10.11.1 Upstream}

\begin{itemize}
\item \textbf{Chapter 1.} Substrate primitives, especially P04 (mobility-capacity bound, the load-bearing primitive for UDM and the four P4-NN families) and the finite-kernel commitment for V5 (the load-bearing primitive for Maxwell-class viscoelasticity).
\item \textbf{Chapter 2.} Load-bearing invariants. Multiplicity $\mathcal{M}$ enters the mechanism-clustered-scatter analysis (cooperative networks have higher cooperativity at the multiplicity level than steric or electrostatic mechanisms). V5 finite-width memory kernel is established as a load-bearing invariant.
\item \textbf{Chapter 3.} DCGT. The substrate-to-continuum bridge for V5→Maxwell viscoelasticity is one of DCGT's five leading-order consequences (Chapter 3, Section 3.6.4). The scalar-diffusion content of DCGT (Chapter 3, Section 3.6.1) is the substrate-to-continuum bridge for UDM.
\end{itemize}

\subsection{10.11.2 Downstream}

\begin{itemize}
\item \textbf{Chapter 14 (Cross-Platform Unifications).} UDM's $\beta \approx 2$ result and the soft-matter architectural unification are part of the program's cross-domain consistency signatures. The form-FORCED / value-INHERITED methodology developed here propagates through Chapter 14.
\item \textbf{Chapter 15 (Public Test Inventory).} UDM PASSED status across ten systems is one of the program's three PASSED-status empirical anchors, catalogued in Chapter 15. FRAP-BSA IN PROGRESS status is also catalogued.
\end{itemize}

\section{10.12 Canonical Sources}

\begin{itemize}
\item \texttt{papers/Universal\_Mobility\_Law/}
\item \texttt{papers/P4\_NonNewtonian\_Paper\_Draft/}
\end{itemize}

The Universal\_Mobility\_Law paper presents the publication-grade empirical validation across ten systems, with the canonical form $M(\rho) = M_0(1-\rho/\rho_\mathrm{max})^\beta$ derived from P04 mobility-capacity bound and the empirical results $\beta = 1.72 \pm 0.37$, $R^2 > 0.986$, plus the mechanism-clustered scatter analysis. The P4\_NonNewtonian\_Paper\_Draft presents the architectural extension covering Krieger–Dougherty, DST, Cross-class, mixed regimes, and Maxwell-class viscoelasticity through the unified P04 + V5 substrate commitments.
...
\section{10.13 Optional Figures}


\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_58_five_non_newtonian_families.pdf}
\caption*{Figure 10.1 --- The five canonical non-Newtonian rheology families. A five-row diagram. Each row corresponds to one family (Krieger–Dougherty, DST, Cross-class, mixed regimes, Maxwell-class viscoelasticity). Columns: (i) Standard constitutive equation; (ii) Substrate-level mechanism (P04 mobility-capacity bound applied to which state variable, or V5 cross-chain memory); (iii) Empirical example. The figure makes visible the substrate-level unification of the five families.}
\label{fig:fig_58_five_non_newtonian_families}
\end{figure}


\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_59_udm_ten_systems.pdf}
\caption*{Figure 10.2 --- UDM across ten systems. A scatter plot with mobility ratio on the y-axis and $\rho/\rho_\mathrm{max}$ on the x-axis. Ten curves, one for each empirical system, all fitting the same functional form $M(\rho) = M_0(1-\rho/\rho_\mathrm{max})^\beta$ with system-specific empirical $\beta$ values clustered around $\beta = 1.72 \pm 0.37$. The canonical $\beta = 2$ curve is shown for reference.}
\label{fig:fig_59_udm_ten_systems}
\end{figure}


\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_60_beta_mechanism_clustered_scatter.pdf}
\caption*{Figure 10.3 --- Mechanism-clustered scatter in $\beta$. A horizontal axis labeled with system mechanisms: cooperative-networks ($\bar\beta \approx 2.31$), steric ($\bar\beta \approx 1.76$), gradual/electrostatic ($\bar\beta \approx 1.38$). The canonical $\beta = 2$ value is marked. The figure makes visible the structural pattern that more-cooperative networks have higher $\beta$ and less-cooperative have lower $\beta$.}
\label{fig:fig_60_beta_mechanism_clustered_scatter}
\end{figure}


\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_61_p4nn_one_principle_four_apps.pdf}
\caption*{Figure 10.4 --- P4-NN as one substrate principle, four state-variable applications. A radial diagram with P04 at the center and four spokes radiating outward to: (i) Krieger–Dougherty (volume fraction); (ii) DST (strain rate, saturating); (iii) Cross-class (strain rate, disruption); (iv) mixed regimes (multiple state variables). Each spoke is annotated with the state variable used and the resulting constitutive structure. The figure makes visible the single-substrate-principle architecture covering four canonical families.}
\label{fig:fig_61_p4nn_one_principle_four_apps}
\end{figure}


\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_62_v5_to_maxwell_dcgt.pdf}
\caption*{Figure 10.5 --- V5 to Maxwell viscoelasticity through DCGT. A flow diagram showing V5 cross-chain memory at the substrate level, DCGT coarse-graining, and the Maxwell equation $\tau_R\dot\sigma + \sigma = 2\mu S$ at the continuum level. The relaxation time $\tau_R$ is identified as V5's first temporal moment.}
\label{fig:fig_62_v5_to_maxwell_dcgt}
\end{figure}


\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_63_softmatter_unification.pdf}
\caption*{Figure 10.6 --- The full architectural unification. A 2x5 grid: left column shows the five canonical families; right column shows the substrate mechanism (P04 for the first four; V5 for the fifth). A central note observes that two substrate commitments produce all five canonical non-Newtonian rheology families.}
\label{fig:fig_63_softmatter_unification}
\end{figure}


\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_64_softmatter_form_value_tagging.pdf}
\caption*{Figure 10.7 --- Form-FORCED vs Value-INHERITED at soft-matter mobility. A two-column diagram. Left column ("Form-FORCED"): UDM functional form, substrate-natural $\beta = 2$, four-family P4-NN unification, Maxwell-equation form, $\tau_R$ as V5 first temporal moment, exhaustiveness of UDM + P4-NN + V5 over five canonical families. Right column ("Value-INHERITED"): specific empirical $\beta$ for individual systems, packing limits $\rho_\mathrm{max}$, unconstrained mobilities $M_0$, critical strain rates, viscosity coefficients, relaxation times $\tau_R$.}
\label{fig:fig_64_softmatter_form_value_tagging}
\end{figure}


\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_65_udm_passed_anchor.pdf}
\caption*{Figure 10.8 --- The PASSED-status empirical anchor. A simplified view of UDM's empirical validation: ten chemically unrelated systems, all fitting $M(\rho) = M_0(1-\rho/\rho_\mathrm{max})^\beta$ with $\beta = 1.72 \pm 0.37$, $R^2 > 0.986$. A note observes that this is the most quantitatively-tested structural prediction in the Event Density program.}
\label{fig:fig_65_udm_passed_anchor}
\end{figure}

\newpage


\part{Part IV — Gravity and Cosmology}
...
\chapter{Substrate Gravity at Galactic Scale: T19, T20, ECR, T21}
\label{chap:11}

\section{11.1 Chapter Overview}
...
This chapter establishes the substrate-gravity sector of the Event Density framework: the entire flat-spacetime gravitational physics of galactic scales is derived from substrate primitives without free parameters. Four results combine to form the closure: \textbf{Theorem T19} derives Newton's gravitational constant $G = c^3\ell_P^2/\hbar$ from substrate cumulative-strain mechanism plus holographic participation-count bound; \textbf{Theorem T20} derives the MOND transition acceleration $a_0 = c\,H_0/(2\pi)$ from the cosmic horizon's dipole-projection contribution to a local accelerating object's substrate environment; \textbf{the ED Combination Rule (ECR)} establishes $a = \sqrt{a_N \cdot a_0}$ as the substrate-derived multiplicative-participation rule for the deep-galactic regime; and \textbf{Theorem T21} derives the slope-4 baryonic Tully–Fisher relation $v^4 = G\,M\,a_0$ from T19, T20, and ECR, with the proportionality constant in substrate quantities and zero intrinsic scatter predicted in the asymptotic deep-MOND regime. Flat rotation curves follow as a corollary.
...
\section{11.2 The Standard Cast of Characters in Galactic Gravity}

\subsection{11.2.1 The empirical regularities}

Standard galactic gravity has five canonical empirical regularities that any structural account must reproduce:

\begin{itemize}
\item \textbf{Newton's law in the solar system.} $a = GM/R^2$, with $G \approx 6.674 \times 10^{-11}$ m³ kg⁻¹ s⁻². Universal gravitational acceleration from a point mass; works to extraordinary precision at solar-system scales.
\item \textbf{Flat rotation curves.} Stars in the outer regions of spiral galaxies orbit at speeds that asymptote to a finite, mass-determined value as a function of radius, rather than falling off as $1/\sqrt{R}$ as Newton's law would predict for a Keplerian system.
\item \textbf{The MOND transition acceleration.} $a_0 \approx 1.2 \times 10^{-10}$ m/s². The empirical scale below which galactic dynamics depart from Newton's law. In MOND, this is a phenomenological constant; standard physics has no first-principles account of its specific value.
\item \textbf{The deep-MOND combination rule.} $a = \sqrt{a_N \cdot a_0}$. The empirical pattern connecting Newton-class and MOND-class accelerations in the regime well below $a_0$. In MOND, this is a phenomenological functional form.
\item \textbf{The slope-4 baryonic Tully–Fisher relation.} $v^4 \propto M$, observed empirically across hundreds of galaxies, with the proportionality constant set by data. In MOND, the slope-4 follows from the combination rule applied to the orbital-balance equation; in dark-matter cosmology, it emerges from a chain of empirical assumptions about halo profiles.
\end{itemize}
...
\subsection{11.2.3 The substrate-level alternative}
...
\section{11.3 The Substrate Inputs for Galactic Gravity}

\subsection{11.3.1 Three pieces of substrate machinery}
...
\subsection{11.3.2 The substrate inputs and constants}

The substrate inputs used in the four derivations:

\begin{itemize}
\item $\hbar$ (Planck's constant). Substrate input from Chapter 1's primitive layer.
\item $c$ (speed of light). Substrate input.
\item $H_0$ (Hubble rate). Substrate input characterizing the cosmic horizon's distance.
\item $\ell_P$ (Planck length / substrate length scale). Identified with the standard Planck length through T19's Newton-recovery argument; this identification is \textit{derived}, not postulated.
\end{itemize}

The substrate primitives consumed:
\begin{itemize}
\item The participation-density primitives (load-bearing for cumulative-strain mechanism and cosmic-horizon participation density).
\item Substrate locality (load-bearing for holographic participation-count bound on boundary surfaces).
\item DCGT (Chapter 3) consolidates substrate-gravity content into a modified Poisson equation in flat-background regime.
\item Theorem 18 (Chapter 4) supplies the kernel-level arrow ensuring forward-only causal structure for substrate dynamics.
\end{itemize}
...
\begin{enumerate}
\item \textbf{Compare with Newton's law.} Newton's law is $a = GM/R^2$. Comparing:
\end{enumerate}
\[
...
\subsection{11.4.2 The Planck-length identification}

The substrate length scale $\ell_P$ is identified with the standard Planck length through Newton-recovery. The framework does \textit{not} assume $\ell_P = \sqrt{\hbar G/c^3}$ at the substrate-ontological level; that identification follows from solving for $\ell_P$ given the empirical value of $G$:
...
\subsection{11.4.3 What T19 does and does not deliver}

T19 \textbf{delivers}:
\begin{itemize}
\item Newton's gravitational constant $G$ as a relationship among substrate constants $c$, $\ell_P$, $\hbar$.
\item The identification of the substrate length scale with the standard Planck length as a derived consequence of Newton-recovery.
\item The substrate-level structural origin of inverse-square Newton's law via cumulative-strain mechanism plus holographic participation-count bound.
\end{itemize}

T19 \textbf{does not deliver}:
\begin{itemize}
\item A closed-form first-principles derivation of $\ell_P$'s specific value. Inherited from substrate constants.
\item A substrate-level account of strong-field gravity (extreme curvature regimes). Chapter 12 develops the curvature-emergence extension at acoustic-metric scalar-tensor level; full GR is not derived.
\item A structural account of cluster-scale or cosmological-scale gravity. Out of scope.
\end{itemize}

\section{11.5 Theorem T20: The MOND Transition Acceleration}

\subsection{11.5.1 The structural question}
...
\subsection{11.5.2 The structural derivation}

Theorem T20 derives $a_0$ from a substrate-native dipole-projection mechanism:

\begin{enumerate}
\item \textbf{A chain at rest in flat substrate} has its participation environment in three-dimensional symmetry. There is no preferred direction; the chain's substrate environment is isotropic.
\item \textbf{A chain that is accelerating breaks that symmetry.} There is a "forward" direction (the direction of acceleration) and a "backward" direction. The chain's participation environment becomes anisotropic with a privileged axis along the acceleration direction. This is the substrate-level analogue of the Rindler-frame anisotropy in standard physics.
\item \textbf{The chain has an acceleration-induced decoupling surface} at distance $c^2/a$ behind it. Beyond this surface, micro-events cannot reach the chain in finite proper time. The substrate-level reading: the chain's environment has a participation-density boundary set by its acceleration.
\item \textbf{The cosmic horizon contributes to the chain's stability landscape.} But because the chain's environment is anisotropic, the cosmic horizon's contribution is not the isotropic full-sphere integration. It is a \textit{directional projection} along the chain's acceleration axis.
\item \textbf{Decompose this projection in spherical harmonics.} The leading anisotropic mode is the dipole — $\ell = 1$, $m = 0$. The dipole has a specific azimuthal symmetry around the acceleration axis: its only spatial periodicity is the $2\pi$ azimuthal cycle.
\item \textbf{The chain integrates the cosmic horizon's contribution through the azimuthal mode.} The effective rate at which it experiences cosmic-horizon fluctuations is the cosmic angular rate $H_0$ divided by the dipole's azimuthal period $2\pi$:
\end{enumerate}
\[
\gamma_\mathrm{cosmic}^\mathrm{eff} = \frac{H_0}{2\pi}.
\]
\begin{enumerate}
\item \textbf{The chain's own acceleration-induced decoupling surface refreshes at rate $a/c$.} The transition between local-decoupling-dominated dynamics (Newton regime) and cosmic-decoupling-dominated dynamics (MOND regime) occurs where these two rates match:
\end{enumerate}
\[
...
\subsection{11.5.3 The empirical match}
...
\subsection{11.5.4 The structural significance of the $2\pi$}
...
\subsection{11.5.5 What T20 does and does not deliver}

T20 \textbf{delivers}:
\begin{itemize}
\item The MOND transition acceleration $a_0 = c\,H_0/(2\pi)$ from substrate-native dipole-projection mechanism.
\item A structural account of the suspicious $2\pi$ in the empirical $a_0$ relationship.
\item A parameter-free numerical match within ~10\% of the empirical MOND constant (within Hubble-tension uncertainty).
\end{itemize}

T20 \textbf{does not deliver}:
\begin{itemize}
\item A closed-form first-principles prediction of the specific Hubble rate $H_0$. The Hubble rate is a substrate input, treated as empirical.
\item A complete substrate-level account of cosmic-horizon participation density. Arc COSMO (Chapter 15, OPEN status) is the natural future arc that would extend T20's content to cosmological-scale dynamics including dark-energy-class phenomena.
\item A substrate-level resolution of the Hubble tension itself. The 15\% prediction band reflects empirical uncertainty in $H_0$, not a substrate-level prediction.
\end{itemize}

\section{11.6 The ED Combination Rule (ECR)}

\subsection{11.6.1 The structural question}
...
\subsection{11.6.2 The substrate-level derivation}

In the joint weak-gradient regime — where neither Newton-class strain nor cosmic-horizon-class strain dominates the chain's accessible region — the chain's stability landscape acquires a \textit{logarithmic cross-term}:

\[
\Sigma_\mathrm{cross}(R) = \sqrt{G\,M\,a_0}\cdot\log(R/R_0) + \mathrm{const.}
\]

The coefficient $\sqrt{G\,M\,a_0}$ is the \textit{geometric mean} of the two substrate strain scales, not their sum. This is multiplicative participation between local-mass and cosmic-horizon contributions, baked into the substrate's two-scale structure.

The cross-term is \textit{not} a perturbative correction; it is the substrate's structural response to the joint regime. Standard perturbation theory would produce additive corrections (Newton plus MOND in some weighted average). The substrate's structural response is multiplicative — the geometric mean of the two scales — because the two scales characterize different substrate channels (local-mass strain in the Newton-class participation channel; cosmic-horizon strain in the dipole-projection channel) and their joint contribution to the stability landscape is the product of the two channel-specific strains.
...
\subsection{11.6.3 The structural significance}
...
\subsection{11.6.4 ECR is a derived rule, not a primitive}

The ED Combination Rule is \textit{not} a substrate primitive. The framework's nomenclature distinguishes:
\begin{itemize}
\item \textbf{P11} is Primitive 11 (commitment-irreversibility, Chapter 1).
\item \textbf{ECR} is the ED Combination Rule (this section's substrate-derived multiplicative-participation rule).
\end{itemize}
...
\section{11.7 Theorem T21: The Slope-4 Baryonic Tully–Fisher Relation}

\subsection{11.7.1 The structural derivation}

Theorem T21 follows from T19, T20, and ECR through orbital balance:

\begin{enumerate}
\item \textbf{A test particle in a circular orbit at radius $R$ around a galactic mass $M$} has centripetal acceleration $v^2/R$ in the orbital frame.
\item \textbf{Set the centripetal acceleration equal to the deep-MOND effective acceleration} from ECR:
\end{enumerate}
\[
\frac{v^2}{R} = \frac{\sqrt{G\,M\,a_0}}{R}.
\]
\begin{enumerate}
\item \textbf{The right-hand side is independent of $R$.} Solving:
\end{enumerate}
\[
v^2 = \sqrt{G\,M\,a_0}.
\]
\begin{enumerate}
\item \textbf{The orbital speed asymptotes to a finite, mass-determined value at large radius.} This is \textit{flat rotation curves}, derived from substrate primitives.
\item \textbf{Squaring:}
\end{enumerate}
\[
v^4 = G\,M\,a_0.
...
\subsection{11.7.2 The robustness audit}
...
\begin{itemize}
\item \textbf{Necessary structural features:} (i) the cumulative-strain mechanism producing inverse-square Newton; (ii) the dipole-projection mechanism producing the $a_0$ scale; (iii) the geometric-mean ECR producing multiplicative cross-term composition; (iv) circular-orbit centripetal balance.
\item \textbf{Admissible variations:} different specific shapes of the substrate cumulative-strain kernel; different specific shapes of the cosmic-horizon dipole-projection profile; different specific shapes of the ECR transition function between Newton-dominated and deep-MOND regimes.
\end{itemize}
...
\subsection{11.7.3 Zero intrinsic scatter prediction}

A direct consequence of T21's robustness: the framework predicts \textit{zero intrinsic scatter} in the asymptotic deep-MOND regime. Scatter in empirical Tully–Fisher data should be dominated by baryonic-mass measurement uncertainty rather than by intrinsic physics scatter.
...
\subsection{11.7.4 What T21 does and does not deliver}

T21 \textbf{delivers}:
\begin{itemize}
\item The slope-4 baryonic Tully–Fisher relation $v^4 = G\,M\,a_0$ from substrate primitives.
\item The proportionality constant $G\,a_0$ in fundamental substrate quantities.
\item Flat rotation curves as a corollary.
\item Zero intrinsic scatter prediction in the asymptotic deep-MOND regime.
\item Robustness under admissible substrate-kernel variations.
\end{itemize}

T21 \textbf{does not deliver}:
\begin{itemize}
\item A constructive derivation of any specific galaxy's rotation curve. The framework supplies the structural relation; specific galactic dynamics depend on baryonic mass distribution.
\item A complete theory of galactic dynamics including disk dynamics, spiral-arm structure, halo substructure, etc.
\item An account of cluster-scale dynamics. Out of scope.
\end{itemize}

\section{11.8 Consolidation Under DCGT: The Modified Poisson Equation}

\subsection{11.8.1 DCGT applied to substrate gravity}
...
\begin{itemize}
\item $\mu(x) \to 1$ for $x \gg 1$ (high-acceleration regime, recovering Newton's law).
\item $\mu(x) \to x$ for $x \ll 1$ (low-acceleration regime, recovering ECR's geometric-mean composition).
\end{itemize}

The interpolation function $\mu$ is \textit{constrained} in its asymptotic behavior by the substrate-level ingredients (Newton recovery at high acceleration; ECR at low acceleration); its specific functional form between the two limits is INHERITED from substrate-cumulative-strain-kernel details.

\subsection{11.8.2 The structural status of the modified Poisson equation}
...
The equation is \textit{not} general relativity. It is a flat-spacetime / weak-field equation. Curvature emergence (Chapter 12) extends substrate gravity to acoustic-metric scalar-tensor covariantization; full GR is not derived.

\subsection{11.8.3 The framework's substrate-gravity scope}

The substrate-gravity sector covers:
\begin{itemize}
\item \textbf{Newton's law in the solar system} (T19, recovered in the high-acceleration regime).
\item \textbf{The MOND transition acceleration} $a_0$ (T20, derived from cosmic-horizon dipole-projection).
\item \textbf{The deep-galactic combination rule} (ECR, derived from substrate's two-scale logarithmic stability landscape).
\item \textbf{The slope-4 baryonic Tully–Fisher relation} (T21, derived from T19+T20+ECR with orbital balance).
\item \textbf{Flat rotation curves} (corollary of T21).
\item \textbf{The modified Poisson equation} in flat-background regime (DCGT consolidation).
\end{itemize}

The framework's scope explicitly does not cover:
\begin{itemize}
\item \textbf{Cluster-scale dynamics.} Where MOND has known difficulties; future work.
\item \textbf{Strong-field gravity.} Extreme curvature regimes; addressed in Chapter 12 at acoustic-metric class but not full GR.
\item \textbf{Cosmological-scale dynamics.} Dark-energy / cosmological-constant questions, structure formation, CMB acoustic peaks. Arc COSMO (Chapter 15, OPEN) is the natural future arc.
\item \textbf{Dark-matter component of the universe's mass-energy budget at non-galactic scales.} Out of scope.
\end{itemize}
...
\section{11.9 The Empirical Anchor: Dwarf-Galaxy Outer-Radius Mass Discrepancy}

\subsection{11.9.1 The PASSED-status anchor}
...
The structural prediction: dynamically \textit{active} dwarf galaxies (recent star formation, mergers, AGN feedback, bar-driven inflow) should exhibit larger outer-radius mass discrepancies than dynamically \textit{quiet} dwarfs at fixed baryonic mass. The substrate-level reason: sustained internal activity generates ED gradients that diffuse into smooth temporal halos, increasing the apparent outer-radius mass discrepancy in active systems.

The empirical test: SPARC dataset analysis of 46 dwarf galaxies, classified into Quiet and Active groups. Results:
\begin{itemize}
\item Quiet dwarfs: $\langle D_\mathrm{outer}\rangle \approx 3.94$.
\item Active dwarfs: $\langle D_\mathrm{outer}\rangle \approx 6.01$.
\item Ratio: $D_\mathrm{Active}/D_\mathrm{Quiet} \approx 1.53$.
\end{itemize}
...
\subsection{11.9.2 What the PASSED status means}

The PASSED status reflects:
\begin{itemize}
\item \textbf{The structural prediction is empirically confirmed.} Active > Quiet outer-radius mass discrepancy is observed at the predicted ~50\% level.
\item \textbf{The structural mechanism is consistent with the data.} Activity-dependent halo strength is the substrate-level reading; the empirical pattern matches.
\item \textbf{First completed astrophysical test of the framework.} The dwarf-galaxy result is the first structural prediction of the framework that has been empirically tested with published data.
\end{itemize}

The PASSED status does \textit{not} mean:
\begin{itemize}
\item \textbf{The framework is empirically validated as a complete theory.} PASSED status is for one specific structural prediction; other predictions (cluster dynamics, cosmological observables) are out of scope or open.
\item \textbf{Dark matter is refuted.} The framework's claim is structural rather than empirical-discriminating; both dark matter and the substrate-gravity reading can fit galactic rotation curves, and the framework derives the rotation curves from substrate primitives without invoking dark matter at galactic scales.
\end{itemize}

\section{11.10 What This Changes (And What It Doesn't)}

\subsection{11.10.1 What does not change}
...
\subsection{11.10.2 What does change}

Three structural shifts:

\begin{itemize}
\item \textbf{Newton's gravitational constant becomes substrate-derived.} $G = c^3\ell_P^2/\hbar$, with the substrate length scale identified with the standard Planck length through Newton-recovery rather than postulated.
\item \textbf{The MOND transition acceleration becomes substrate-derived.} $a_0 = c\,H_0/(2\pi)$, with the suspicious $2\pi$ identified as the dipole's azimuthal period rather than as a phenomenological coincidence.
\item \textbf{The deep-galactic combination rule and slope-4 Tully–Fisher relation become substrate-derived.} ECR's geometric-mean form follows from the substrate's two-scale logarithmic stability landscape; T21's slope-4 follows from orbital balance applied to ECR.
\end{itemize}
...
\section{11.11 Form-FORCED vs Value-INHERITED at Substrate Gravity}

\subsection{11.11.1 What is form-FORCED}

\begin{itemize}
\item \textbf{The functional form $G = c^3\ell_P^2/\hbar$.} Newton's gravitational constant as a relationship among substrate constants.
\item \textbf{The functional form $a_0 = c\,H_0/(2\pi)$.} MOND transition acceleration with the $2\pi$ as dipole azimuthal period.
\item \textbf{The geometric-mean form of ECR.} $a = \sqrt{a_N\,a_0}$ in the deep-MOND regime as substrate's multiplicative-participation rule.
\item \textbf{The slope-4 form of Tully–Fisher.} $v^4 = G\,M\,a_0$ from T19+T20+ECR with orbital balance.
\item \textbf{Flat rotation curves.} Corollary of T21.
\item \textbf{The asymptotic constraints on the modified Poisson equation interpolation function.} $\mu \to 1$ at high acceleration (Newton recovered); $\mu \to x$ at low acceleration (ECR recovered).
\item \textbf{Robustness of slope-4 under admissible substrate-kernel variations.}
\item \textbf{Zero intrinsic scatter prediction in the asymptotic deep-MOND regime.}
\item \textbf{Activity-dependent halo strength prediction} (dwarf-galaxy PASSED anchor).
\end{itemize}

\subsection{11.11.2 What is value-INHERITED}

\begin{itemize}
\item \textbf{The numerical value of $\ell_P$ (and equivalently of $G$).} Inherited from substrate constants; calibrated against Newton-recovery.
\item \textbf{The numerical value of $H_0$.} Substrate input; subject to Hubble-tension empirical uncertainty (15\% prediction band on $a_0$).
\item \textbf{The specific shape of the modified Poisson interpolation function $\mu(x)$ between asymptotic limits.} Inherited from substrate-cumulative-strain-kernel details.
\item \textbf{Specific galaxy-by-galaxy rotation-curve details.} Depend on baryonic mass distribution; not a substrate-level prediction.
\end{itemize}

\subsection{11.11.3 What is open}

\begin{itemize}
\item \textbf{Cluster-scale dynamics.} Where MOND has known difficulties; future substrate-gravity work needed.
\item \textbf{Cosmological-scale dynamics.} Dark-energy questions, structure formation, CMB acoustic peaks. Arc COSMO is the natural future arc.
\item \textbf{Closed-form derivation of $\ell_P$ from substrate primitives without Newton-recovery calibration.} Open work.
\item \textbf{Closed-form derivation of $H_0$ from substrate primitives.} Out of current scope; tied to Arc COSMO.
\item \textbf{Strong-field gravity beyond acoustic-metric class.} Chapter 12 develops curvature emergence at acoustic-metric scalar-tensor; full GR is not derived.
\end{itemize}

\section{11.12 Dependencies}

\subsection{11.12.1 Upstream}

\begin{itemize}
\item \textbf{Chapter 1.} Substrate primitives, especially the participation primitives (load-bearing for cumulative-strain mechanism and cosmic-horizon participation density), the substrate-locality commitment (load-bearing for holographic participation-count bound).
\item \textbf{Chapter 2.} Load-bearing invariants. Cross-bandwidth $\Gamma_\mathrm{cross}$ enters the cosmic-horizon participation density treatment indirectly; participation density gradient enters the cumulative-strain mechanism.
\item \textbf{Chapter 3.} DCGT consolidates substrate-gravity content under the modified Poisson equation in flat-background regime. The substrate-to-continuum bridge for the cumulative-strain content uses DCGT's coarse-graining machinery.
\item \textbf{Chapter 4.} Theorem 18 kernel-level arrow. Supplies the substrate-level forward-cone-only causal structure that substrate gravity inherits at the kernel level.
\end{itemize}

\subsection{11.12.2 Downstream}

\begin{itemize}
\item \textbf{Chapter 12 (Curvature Emergence).} Extends the substrate-gravity content of this chapter to acoustic-metric scalar-tensor covariantization. The flat-background modified Poisson equation derived here is the weak-field limit of Chapter 12's covariant equation.
\item \textbf{Chapter 13 (Black-Hole Architecture).} The cosmic-horizon participation-boundary content of T20 connects structurally to Chapter 13's universal horizon mechanism; both use the substrate-level cross-bandwidth $\Gamma_\mathrm{cross}$ collapse content from Chapter 2.
\item \textbf{Chapter 14 (Cross-Platform Unifications).} The substrate-gravity sector's empirical content (especially the dwarf-galaxy PASSED anchor and the substrate-level reframing of $a_0$) is part of the program's cross-domain consistency signatures.
\item \textbf{Chapter 15 (Public Test Inventory).} The dwarf-galaxy outer-radius mass-discrepancy result (PASSED status), the SPARC slope-4 Tully–Fisher anchor, the halo-lag-in-cluster-collisions prediction (Active status), and other galactic-gravity predictions are catalogued in Chapter 15.
\end{itemize}

\section{11.13 Canonical Sources}

\begin{itemize}
\item \texttt{papers/Substrate\_Gravity\_Foundations/}
\item \texttt{papers/Substrate\_Gravity\_Foundations/ED\_substrate\_gravity\_foundations\_2026-04-28.\{md,tex,pdf\}} (detailed-derivation companion)
\end{itemize}

The Substrate\_Gravity\_Foundations paper presents the publication-grade architectural account of the substrate-gravity sector, including T19, T20, ECR, T21, and the consolidated modified Poisson equation under DCGT. The detailed-derivation companion paper (April 2026) presents the explicit substrate-level derivations of T19's cumulative-strain mechanism plus holographic participation-count bound, T20's cosmic-horizon dipole-projection mechanism, ECR's two-scale logarithmic stability-landscape derivation, and T21's robustness audit.
...
\subsection{12.2.2 What Arc ED-10 does and does not deliver}

Arc ED-10 \textbf{delivers}:
\begin{itemize}
\item A substrate-FORCED covariant generalization of the modified Poisson equation.
\item The acoustic-metric scalar-tensor structural form $\nabla_\mu[\mu(|\nabla\Phi|/a_0)\nabla^\mu\Phi] = 4\pi G T$.
\item Reduction to the flat-spacetime modified Poisson equation in the weak-field limit.
\item An OS-positive, ghost-free, gradient-stable scalar-tensor framework.
\item Identification of the substrate-level structural cost (deep-MOND superluminal scalar propagation) as inevitable given the substrate-derivation chain.
\end{itemize}

Arc ED-10 \textbf{does not deliver}:
\begin{itemize}
\item Einstein's general relativity. The acoustic metric is kinematic; the Einstein equation $G_{\mu\nu} = 8\pi G T_{\mu\nu}$ is not derived. Promotion to full GR remains SPECULATIVE in the program (Chapter 15's GR-4A item).
\item A dynamical metric content equivalent to GR's. The acoustic metric is a coarse-grained summary, not a dynamical degree of freedom in the GR sense.
\item A constructive-rigor proof of well-posedness for the covariant equation in arbitrary backgrounds. Conditional-positive verdict at structural-suggestive level.
\end{itemize}
...
\section{12.3 The Substrate Cumulative-Strain Four-Index Object}

\subsection{12.3.1 The load-bearing curvature degree of freedom}
...
At the substrate-level structural analysis, the cumulative-strain object is \textit{not} fundamentally a scalar. It is a four-index tensorial object — the substrate-level analog of a rank-four tensor that captures the full strain content of a chain's participation environment as it accumulates along the path. The four-index character is structural: substrate strain has both magnitude and directional structure, and the four-index object records both the local strain density and the directional dependence of how that strain accumulates in the four spacetime directions.

The four-index cumulative-strain object is the \textbf{load-bearing curvature degree of freedom} for Arc ED-10. Standard scalar-tensor MOND covariantizations introduce a scalar degree of freedom (the MOND potential) coupled to the spacetime metric; Arc ED-10's substrate-level analysis identifies the four-index cumulative-strain object as the structural origin of this scalar field, with the scalar field being one specific projection of the four-index content.

\subsection{12.3.2 What the four-index character buys}

Three structural consequences of identifying the cumulative-strain object as four-index:

\begin{itemize}
\item \textbf{The scalar field of the covariant equation is not a fundamental dynamical field.} It is a substrate-derived projection of the cumulative-strain four-index object. Its specific form-FORCED structure follows from the substrate analysis rather than from a phenomenological postulate.
...
\item \textbf{The acoustic-metric structure follows from the four-index object's coarse-graining.} Under DCGT, the four-index cumulative-strain object reduces at leading order to a scalar field plus a kinematic-summary metric structure. The kinematic-summary metric is the acoustic metric of Arc ED-10.
\end{itemize}
...
\subsection{12.3.3 Form-FORCED at the substrate level}
...
\section{12.4 The Acoustic Metric}

\subsection{12.4.1 Substrate-level definition}
...
The structural reading: the acoustic metric is \textit{kinematic}, not \textit{dynamical}. It is a summary of substrate participation density, computed from the substrate state, rather than an independent dynamical degree of freedom with its own equation of motion. This is the central distinction between Arc ED-10's acoustic metric and Einstein's general-relativistic metric. Einstein's metric is dynamical: it satisfies the Einstein equation $G_{\mu\nu} = 8\pi G T_{\mu\nu}$ and is an independent degree of freedom of the gravitational sector. Arc ED-10's acoustic metric is kinematic: it is determined by the substrate state, not by an independent equation of motion.

\subsection{12.4.2 The kinematic-class status}

The kinematic-class status of the acoustic metric has structural consequences:

\begin{itemize}
\item \textbf{The acoustic metric is computed from substrate state.} Given a substrate participation-density configuration, the acoustic metric follows by coarse-graining. There is no independent acoustic-metric equation analogous to the Einstein equation.
\item \textbf{Variations in the substrate state produce variations in the acoustic metric.} Strong substrate gradients produce strong acoustic-metric curvature; weak substrate gradients produce weak curvature. The relationship is determined by the substrate's coarse-graining structure.
\item \textbf{The acoustic metric does not propagate independently.} Standard general-relativistic content includes gravitational waves as independent dynamical degrees of freedom of the metric; Arc ED-10's acoustic-metric content does not include such an independent dynamical sector. Whether substrate-level analogues of gravitational-wave content can be developed within Arc ED-10's framework is open work.
\end{itemize}
...
\subsection{12.4.3 Why "acoustic" metric}
...
\section{12.5 The Acoustic-Metric Scalar-Tensor Covariantization}

\subsection{12.5.1 The covariant equation}
...
where:
\begin{itemize}
\item $\Phi$ is the substrate-derived gravitational scalar field (the leading-order coarse-grained projection of the four-index cumulative-strain object).
...
\item $\mu(x)$ is the interpolation function of Chapter 11, with asymptotic constraints $\mu(x) \to 1$ for $x \gg 1$ (Newton recovery at high acceleration) and $\mu(x) \to x$ for $x \ll 1$ (ECR recovery at low acceleration).
\item $T$ is the trace of the matter stress-energy tensor.
\item $a_0$ is the MOND transition acceleration of T20 (Chapter 11).
\item $G$ is Newton's gravitational constant of T19 (Chapter 11).
\end{itemize}
...
\subsection{12.5.2 The structural relation to RAQUAL-class MOND covariantizations}
...
\subsection{12.5.4 What is form-FORCED in the covariant equation}

\begin{itemize}
\item \textbf{The functional form of the equation} $\nabla_\mu[\mu(|\nabla\Phi|/a_0)\nabla^\mu\Phi] = 4\pi G T$.
\item \textbf{The asymptotic constraints on $\mu$}: $\mu \to 1$ at high acceleration; $\mu \to x$ at low acceleration.
\item \textbf{The acoustic-metric kinematic-class status}.
\item \textbf{The reduction to the flat-spacetime modified Poisson equation in the weak-field limit}.
\item \textbf{The use of $T$ (trace of stress-energy)} rather than $\rho_m$ as the source term in the relativistic regime.
\item \textbf{The non-introduction of additional dynamical fields} (no extra vector or tensor fields beyond the scalar $\Phi$ and the acoustic metric).
\end{itemize}

\subsection{12.5.5 What is value-INHERITED in the covariant equation}

\begin{itemize}
\item \textbf{The specific functional shape of $\mu(x)$ between asymptotic limits.} Inherited from substrate-cumulative-strain-kernel details.
\item \textbf{The numerical values of $G$ and $a_0$ entering the equation.} Inherited from T19 and T20 (Chapter 11).
\item \textbf{Specific solutions of the covariant equation in any given background.} Depend on matter distribution and chosen background; not substrate-level structural predictions.
\end{itemize}

\section{12.6 Reduction to Flat-Spacetime Substrate-Gravity}

\subsection{12.6.1 The weak-field limit}
...
\subsection{12.6.2 What this reduction establishes}
...
\subsection{12.6.3 The dwarf-galaxy PASSED anchor inheritance}
...
\section{12.7 The Structural Cost: Deep-MOND Superluminal Scalar Propagation}

\subsection{12.7.1 What deep-MOND superluminality is}
...
\subsection{12.7.2 Why the superluminality is structurally inevitable}

The deep-MOND superluminal scalar propagation is \textit{not} a phenomenological cost imposed on Arc ED-10. It is a structurally inevitable consequence of producing the substrate-derived MOND behavior without introducing additional dynamical fields:

\begin{itemize}
\item \textbf{The substrate-derivation chain produces the RAQUAL-class scalar-tensor structure} with $\mu(x)$ as the interpolation function.
\item \textbf{The asymptotic constraint $\mu(x) \to x$ at small $x$} is required for ECR recovery (Chapter 11) — this constraint is non-optional given T20 + ECR.
\item \textbf{Any RAQUAL-class theory satisfying $\mu(x) \to x$ at small $x$} has deep-MOND superluminal scalar propagation. This is a known property of RAQUAL-class theories in the relativistic-MOND literature.
\end{itemize}

The framework's claim: the deep-MOND superluminality is \textit{the only structural cost} of producing the substrate-derived MOND behavior covariantly. Avoidance routes — adding additional dynamical fields (vector or tensor) to bound the scalar propagation, or modifying the asymptotic structure of $\mu$ — are inadmissible because they would either violate the substrate-level four-index-object structural commitment (no-new-primitives) or violate the BTFR slope-4 robustness (Chapter 11, Section 11.7).

\subsection{12.7.3 The structural significance}
...
\begin{itemize}
\item \textbf{OS-positivity.} Osterwalder–Schrader positivity, the structural property required for a Wick-rotated theory to be a Wightman QFT. The covariant equation's substrate-derivation produces a structure consistent with OS-positivity at substrate-suggestive level.
\item \textbf{Ghost-free.} The theory has no negative-norm states (ghosts) that would render the theory unstable or non-unitary. RAQUAL-class theories with appropriately-chosen kinetic structure are ghost-free; Arc ED-10's substrate-derived form satisfies the ghost-free condition.
\item \textbf{Gradient-stable.} The theory's perturbations around stable backgrounds are not exponentially growing modes (gradient instabilities). Arc ED-10's covariant equation is gradient-stable in the relevant regimes (high-acceleration Newton-recovered regime; deep-MOND regime, with the caveat of superluminal but non-divergent propagation).
\end{itemize}
...
\subsection{12.8.2 The conditional-positive verdict}

The framework's verdict on Arc ED-10's covariantization is \textbf{conditional-positive at structural-suggestive level}:

\begin{itemize}
\item \textbf{Substrate-derivation produces a coherent scalar-tensor framework.} The four-index cumulative-strain object plus DCGT coarse-graining produces the RAQUAL-class form structurally.
\item \textbf{OS-positive, ghost-free, gradient-stable.} Stability-class audit passes at substrate-suggestive level.
\item \textbf{Recovers all flat-spacetime substrate-gravity results in the weak-field limit.} Internal consistency with Chapter 11.
\item \textbf{Deep-MOND superluminality is the only structural cost.} Acknowledged as structurally inevitable.
\item \textbf{Not a constructive-rigor proof of well-posedness for arbitrary backgrounds.} Conditional-positive at substrate-suggestive level rather than constructive-rigorous level.
\end{itemize}
...
\subsection{12.8.3 What the conditional-positive verdict means}

The verdict means:
\begin{itemize}
\item \textbf{The framework supplies real curvature-emergence content} at the substrate level: a substrate-derived covariant equation with stability-class properties that pass at substrate-suggestive level.
\item \textbf{The framework explicitly does not claim full GR derivation.} Promotion to GR remains SPECULATIVE.
\item \textbf{The framework is honest about the structural cost} (deep-MOND superluminality) and identifies the cost as inevitable given the substrate-derivation chain.
\item \textbf{The framework supplies an audit trail} from substrate primitives to the covariant equation, parallel to the audit trails for Clay-NS (Chapter 8) and Yang–Mills (Chapter 9).
\end{itemize}
...
\section{12.9 What This Does Not Derive: Einstein's General Relativity}

\subsection{12.9.1 The explicit non-derivation}

The framework explicitly does not derive Einstein's general relativity. Three structural reasons:

\begin{itemize}
\item \textbf{The acoustic metric is kinematic, not dynamical.} The acoustic metric is computed from substrate state under DCGT coarse-graining; it is not an independent dynamical degree of freedom with its own equation of motion. Einstein's metric is dynamical; the Einstein equation $G_{\mu\nu} = 8\pi G T_{\mu\nu}$ governs its evolution as an independent field.
\item \textbf{No Einstein equation is derived.} The substrate-level analysis produces the modified Poisson equation in flat regime (Chapter 11) and the scalar-tensor covariantization in this chapter. Neither produces $G_{\mu\nu} = 8\pi G T_{\mu\nu}$ as a substrate-level theorem.
...
\begin{itemize}
\item \textbf{GR-4A succeeds (long-horizon).} Future substrate-primitive work or a new derivation strategy produces full GR from substrate primitives. The framework becomes a gravity theory at field-equation level.
\item \textbf{GR-4A fails (kinematic-ceiling permanent).} The acoustic-metric kinematic-class reading is the structural ceiling for substrate-level gravity content. The framework remains at Arc ED-10's acoustic-metric scalar-tensor level for relativistic-gravity content, with full GR remaining a separate framework that the substrate ontology does not derive.
\end{itemize}
...
\subsection{12.9.3 Sharpening from Arc ED-10 closure}
...
\begin{itemize}
\item \textbf{The acoustic-metric reading is kinematic, not dynamical-fundamental.} Converting to genuine dynamical metric content is what GR-4A would require.
\item \textbf{The substrate's four-index cumulative-strain object} is the load-bearing curvature degree of freedom at substrate-suggestive level; whether this object can be promoted to a genuine dynamical metric content under additional substrate-level structural work is open.
\item \textbf{The substrate-level scope of dynamical content} is currently bounded by what DCGT and its non-Abelian generalization (Chapter 9) deliver; whether dynamical-metric content fits this scope or requires extension is open.
\end{itemize}
...
\section{12.10 What This Changes (And What It Doesn't)}

\subsection{12.10.1 What does not change}
...
\subsection{12.10.2 What does change}
...
\section{12.11 Form-FORCED vs Value-INHERITED at Curvature Emergence}

\subsection{12.11.1 What is form-FORCED}

\begin{itemize}
\item \textbf{The substrate cumulative-strain four-index object} as the load-bearing curvature degree of freedom.
\item \textbf{The acoustic-metric kinematic-class status} (computed from substrate state, not dynamical).
\item \textbf{The covariant equation form} $\nabla_\mu[\mu(|\nabla\Phi|/a_0)\nabla^\mu\Phi] = 4\pi G T$.
\item \textbf{The RAQUAL-class scalar-tensor structure} as the substrate-FORCED relativistic-MOND covariantization.
\item \textbf{The asymptotic constraints on $\mu$}: Newton recovery at high acceleration; ECR recovery at low acceleration.
\item \textbf{The non-introduction of additional dynamical fields} beyond the scalar $\Phi$ and the acoustic metric.
\item \textbf{Reduction to the flat-spacetime modified Poisson equation in the weak-field limit}.
\item \textbf{OS-positive, ghost-free, gradient-stable} structural properties at substrate-suggestive level.
\item \textbf{Deep-MOND superluminal scalar propagation} as the structurally-inevitable cost.
\item \textbf{The conditional-positive verdict} at structural-suggestive level.
\end{itemize}

\subsection{12.11.2 What is value-INHERITED}

\begin{itemize}
\item \textbf{The specific functional shape of $\mu(x)$} between asymptotic limits. Inherited from substrate-cumulative-strain-kernel details.
\item \textbf{Specific solutions of the covariant equation} in any given background. Depend on matter distribution and chosen background.
\item \textbf{Numerical values of $G$ and $a_0$} entering the covariant equation. Inherited from T19 and T20 (Chapter 11).
\item \textbf{The specific deep-MOND superluminal propagation speed} at any given acceleration. Inherited from the specific shape of $\mu$ in the deep-MOND regime.
\end{itemize}

\subsection{12.11.3 What is open}

\begin{itemize}
\item \textbf{GR-4A: full Einstein-equation emergence from substrate primitives.} SPECULATIVE-not-REFUTED. Arc ED-10 closure sharpens the obstruction (acoustic-metric kinematic-versus-dynamical) but does not close GR-4A.
\item \textbf{Substrate-level analogues of gravitational-wave content.} Whether substrate-level dynamical analogues of GR's gravitational-wave content can be developed within Arc ED-10's framework is open.
\item \textbf{Strong-field gravity beyond acoustic-metric class.} The covariant equation at extreme curvatures may have additional structural content not derived in the chapter.
\item \textbf{Cosmological-scale dynamics.} Arc COSMO (Chapter 15, OPEN status) is the natural future arc; cosmic expansion and dark-energy questions are out of current scope.
\item \textbf{Closed-form derivation of the deep-MOND interpolation function shape.} Inherited; closed-form is downstream open work.
\end{itemize}

\section{12.12 Dependencies}

\subsection{12.12.1 Upstream}

\begin{itemize}
\item \textbf{Chapter 1.} Substrate primitives, especially the participation primitives (load-bearing for cumulative-strain content) and substrate locality (load-bearing for the four-index character of cumulative strain).
\item \textbf{Chapter 2.} Load-bearing invariants. Participation density gradient enters the cumulative-strain content; cross-bandwidth $\Gamma_\mathrm{cross}$ enters the cosmic-horizon-related content indirectly.
\item \textbf{Chapter 3.} DCGT. The substrate-to-continuum bridge for the four-index cumulative-strain object's leading-order projection (the scalar field $\Phi$) and for the acoustic metric's kinematic-summary status.
\item \textbf{Chapter 11.} Substrate gravity at galactic scale. The flat-background modified Poisson equation is the weak-field limit of Arc ED-10's covariant equation. T19, T20, ECR, T21 supply the substrate-derived ingredients (Newton's $G$, MOND $a_0$, ECR combination rule, slope-4 Tully–Fisher) that the covariant equation inherits.
\end{itemize}

\subsection{12.12.2 Downstream}

\begin{itemize}
\item \textbf{Chapter 13 (Black-Hole Architecture).} The acoustic-metric kinematic content underlies the substrate-level reading of black-hole horizons (Chapter 13 develops the BH architecture using acoustic-metric content respecting ED-Phys-10 guardrails).
\item \textbf{Chapter 14 (Cross-Platform Unifications).} The form-FORCED methodology, including Arc ED-10's substrate-FORCED scalar-tensor structure, is part of the program's cross-domain consistency signatures.
\item \textbf{Chapter 15 (Public Test Inventory).} The dwarf-galaxy outer-radius mass-discrepancy result (PASSED status from Chapter 11) extends to the curvature-emergence sector by structural inheritance through the weak-field reduction. GR-4A is catalogued as SPECULATIVE-not-REFUTED.
\end{itemize}

\section{12.13 Canonical Sources}

\begin{itemize}
\item \texttt{papers/Substrate\_Gravity\_Foundations/} (curvature-emergence section in extended-scope paper)
\item Arc ED-10 memos in \texttt{theory/Substrate\_Gravity/}
\end{itemize}

The Substrate\_Gravity\_Foundations paper's curvature-emergence section presents the publication-grade Arc ED-10 closure with the substrate-FORCED covariant equation, the four-index cumulative-strain object identification, the acoustic-metric kinematic-class status, the OS-positive / ghost-free / gradient-stable stability-class audit, the deep-MOND superluminality structural-cost identification, and the conditional-positive verdict at structural-suggestive level. Arc ED-10 memos in \texttt{theory/Substrate\_Gravity/} (six memos covering the substrate-derivation chain in detail) develop each component in detail.
...
\section{13.2 The Standard Paradox Cluster}

\subsection{13.2.1 The four canonical paradoxes}
...
\begin{itemize}
\item \textbf{The information-loss paradox.} Stephen Hawking's 1974 result that black holes radiate thermally implies that information apparently disappears as the black hole evaporates: the radiation that emerges is featureless, regardless of what fell in. Standard quantum mechanics requires information preservation under unitary evolution; combining GR's classical evaporation with QM's information conservation produces the paradox.
\item \textbf{The firewall paradox.} Pushing hard on the demand that information escape evaporating black holes plus the standard rules of quantum entanglement plus the standard demand that an infalling observer experiences nothing strange at the horizon produces a contradiction (AMPS 2012). Saving quantum mechanics appears to require the horizon to be a wall of high-energy radiation that vaporizes anything entering it — the firewall.
\item \textbf{The complementarity contradiction.} The proposal that outside the horizon information returns in radiation and inside the horizon information falls toward the center, with both descriptions valid because no single observer can verify both at once. The proposal works as a partial fix until sharp questions are asked, at which point complementarity cracks. The firewall was, in part, the crack.
\item \textbf{The singularity problem.} General relativity, applied to a collapsing massive object, drives toward a point at which curvature is infinite, density is infinite, and the equations stop working. Every physicist agrees the singularity is not a feature of nature; it is a notice from the formalism that the formalism has run out. What replaces it has been an open question for sixty years.
\end{itemize}

\subsection{13.2.2 The four assumptions whose joint imposition generates the paradoxes}

Each of the four paradoxes arises from imposing a combination of four assumptions about reality:

\begin{itemize}
\item \textbf{Global unitarity.} The universe's quantum state evolves perfectly invertibly across all space and time.
\item \textbf{Global Cauchy data.} The entire state of the universe must be specifiable on a single connected spacelike slice.
\item \textbf{A sharp geometric boundary.} The horizon is a knife-edge surface, not a fade or a coarse-grained statistical feature.
\item \textbf{Monogamy at that boundary.} Entanglement between two regions cannot be shared with a third region, particularly enforced at the horizon.
\end{itemize}
...
\subsection{13.2.3 The Event Density structural position}

ED imposes none of the four assumptions at the substrate level:

\begin{itemize}
\item \textbf{No global unitarity requirement.} Unitarity is a coarse-grained continuum property of closed sub-systems in the QM-emergence sector (Phase-1, Chapter 5); it is not a global constraint on the substrate.
\item \textbf{No global Cauchy data.} ED commits events locally; there is no spacelike surface on which the entire universe's state must be specified.
\item \textbf{No sharp geometric boundary.} A horizon in ED is a coarse-grained statistical feature of cross-bandwidth $\Gamma_\mathrm{cross}$, not a knife-edge geometric locus.
\item \textbf{No monogamy-at-boundary requirement.} Without a sharp boundary, monogamy-of-entanglement enforcement at one is not a structural requirement.
\end{itemize}

The four standard paradoxes therefore do not arise. They are not "solved by clever ED machinery" — they are \textit{not generated in framework}. This is structurally distinct from any of the standard interpretive resolutions (Copenhagen, many-worlds, holography, ER=EPR, etc.); ED's substrate ontology does not impose the assumptions whose joint presence is what generates the paradox cluster, and so the cluster does not exist at substrate level.

\section{13.3 The Single Substrate Condition}

\subsection{13.3.1 The threshold condition}
...
\subsection{13.3.2 What the condition controls}

The single condition supports six derivations:

\begin{itemize}
\item \textbf{Horizon formation (BH-2).} Where the condition is met across a closed surface, $\Gamma_\mathrm{cross}$ collapses below hydrodynamic-window resolution and the surface becomes a substrate-level decoupling surface — the substrate reading of an event horizon.
\item \textbf{Interior saturation (BH-3).} Where the condition is met in a bulk volume, substrate gradients are at the saturation regime; the acoustic-metric reading fails; substrate dynamics remain finite but the continuum-spacetime description does not.
\item \textbf{Information blocking (BH-4).} Where the condition is met across the horizon surface, committed structure cannot cross the surface (participation-bandwidth prohibition); entanglement (uncommitted structure) can straddle.
\item \textbf{Participation-capacity saturation (BH-5).} Where the condition is met, the substrate's participation channels at the surface are at their saturated capacity; the area-law entropy form follows from counting capacity.
\item \textbf{Strong-curvature scattering (BH-6).} Near the surface, substrate gradients produce the strong-curvature region through which minimal ED-channels (gravitational waves) propagate; the BHPT phase shift is the path-integrated channel-action over this region.
\item \textbf{Verdict status (BH-7).} All six derivations rest on the same substrate condition; the architectural-level closure is internally consistent.
\end{itemize}
...
\subsection{13.3.3 Form-FORCED and value-INHERITED}
...
\section{13.4 Horizon as Decoupling Surface (BH-2)}

\subsection{13.4.1 The substrate-level horizon}
...
\subsection{13.4.2 Universal horizon mechanism}

The substrate-level horizon mechanism is \textit{universal} across four classes of horizons that standard physics treats as analogous but distinct:

\begin{itemize}
\item \textbf{Black-hole event horizons.} Strong gradients at high-mass concentrations produce the substrate condition.
\item \textbf{Rindler horizons.} Acceleration-induced gradients in accelerated frames produce the substrate condition.
\item \textbf{Cosmological horizons.} $H_0$-scale gradient set by cosmic-horizon participation density produces the substrate condition.
\item \textbf{Acoustic horizons.} Flow-induced gradients in fluid analog systems (the acoustic metric of analog gravity) produce the substrate condition.
\end{itemize}
...
\subsection{13.4.3 The geometric horizon as continuum shadow}
...
\subsection{13.4.4 Observer-independence}
...
\section{13.5 Singularity Replacement by Saturated Zone (BH-3)}

\subsection{13.5.1 The three substrate constraints}
...
\begin{itemize}
\item \textbf{(C1) Finite participation.} The substrate has a finite count of viable participation channels per coarse-graining cell. There is no substrate state in which infinitely many channels participate at one point. P04 (bandwidth update rule, Chapter 1) plus participation primitives (Chapter 2) supply the finite-participation commitment.
\item \textbf{(C2) Discrete micro-events.} Substrate events are discrete (P01, Chapter 1). Each commitment is a finite-amplitude occurrence at a finite-resolution patch. Continuum singularity statements that require unboundedly many infinitely small events at a single point are inadmissible at the substrate level.
\item \textbf{(C3) Gradient saturation.} $|\nabla\rho|$ has a substrate-imposed maximum: when $\sigma \to \beta_\mathrm{crit}$, the substrate's mobility-capacity bound caps further steepening. Gradients cannot run away to infinity. P04's mobility-capacity content (Chapter 2) supplies the saturation commitment.
\end{itemize}

Any one of (C1)–(C3) alone would suffice to forbid a continuum singularity; the three together overdetermine the result. The substrate ontology has \textit{no singularities}.

\subsection{13.5.2 Acoustic-metric breakdown before substrate}
...
\subsection{13.5.3 The replacement: finite-thickness saturated zone}

The substrate replacement for the singular endpoint is a \textit{finite-thickness saturated participation zone} characterized by:

\begin{itemize}
\item \textbf{Gradients at the substrate-imposed maximum.} $\sigma \to \beta_\mathrm{crit}$ throughout the zone.
\item \textbf{Cross-participation bandwidth collapsed in all directions.} Not just radial; the saturated zone is decoupled from all neighboring substrate regions in the cross-bandwidth sense.
\item \textbf{Only the commitment-order direction (P11) remaining structurally meaningful.} Spatial directions become structurally interchangeable when $\sigma$ saturates; only the temporal commitment-order direction remains as a structural direction.
\item \textbf{Acoustic metric degenerate or undefined.} The continuum kinematic-summary breaks down; no acoustic-metric reading is available within the saturated zone.
\item \textbf{Substrate dynamics finite.} Substrate-level evolution proceeds as saturated commitment cycles. Nothing diverges.
\end{itemize}
...
\subsection{13.5.4 No geodesic incompleteness at substrate level}
...
\section{13.6 Information Architecture and Evaporation (BH-4)}

\subsection{13.6.1 Committed vs uncommitted structure}

ED distinguishes two structurally different forms of correlation at the substrate level:

\begin{itemize}
\item \textbf{Committed structure (information).} P11-irreversible records of participation events. Transmission of committed structure between two regions requires nonzero $\Gamma_\mathrm{cross}$ between them. This is the substrate analog of "information" in the standard QC and QI literature.
\item \textbf{Uncommitted structure (entanglement).} Correlation potential between two regions, set up at pair-formation. Persistence of uncommitted structure does not require ongoing cross-bandwidth — it is set by the joint participation amplitude when the pair was created.
\end{itemize}
...
\subsection{13.6.2 Information cannot cross a horizon}
...
\begin{itemize}
\item \textbf{Committed structure transmission across the surface requires bandwidth that is structurally absent.} Information, in the substrate sense, does not pass through a horizon.
\item \textbf{This is not a causal prohibition.} ED has no fundamental light-cone primitive; the prohibition is a \textit{participation-bandwidth prohibition}. The substrate channels that would carry committed structure have their bandwidth suppressed below resolution.
\item \textbf{The substrate-level horizon-blocking-information statement} is structurally distinct from the GR-level "information cannot exit a black hole because nothing can travel faster than light." The substrate-level reading is bandwidth-mediated, not lightcone-mediated.
\end{itemize}

\subsection{13.6.3 Entanglement straddling}
...
\subsection{13.6.4 Evaporation as participation re-routing}

The substrate-level evaporation mechanism follows from the architecture above:

\begin{enumerate}
\item \textbf{Horizon blocks committed structure} (Section 13.6.2).
\item \textbf{Uncommitted pair structures can straddle the surface} (Section 13.6.3).
...
\item \textbf{Commitment events on these pairs commit one leg inward} (joining the BH-3 saturated zone) \textbf{and the other outward} (joining the radiation field).
\item \textbf{Joint correlation is preserved globally} across the inward + outward commitments.
\end{enumerate}

This is \textit{participation re-routing}, not information destruction. Hawking radiation, in the substrate-level reading, is the coarse-grained statistics of outward-committed legs of these pair-creation-class events. The thermal-spectrum derivation itself is the next natural arc (B4); the \textit{mechanism} of evaporation as participation re-routing is established in BH-4.

\subsection{13.6.5 No information paradox: not solved, not generated}
...
The framework's stance is structural: BH puzzles are \textit{not solved in framework} but \textit{not generated in framework}. The same pattern applies to the singularity (BH-3) and to the horizon (BH-2). Standard puzzles dissolve when the continuum-level assumptions that generate them are not imposed on a fundamentally local-and-irreversible substrate.

This is the framework's central rhetorical move on black-hole physics, stated structurally: \textbf{the information paradox, firewall paradox, complementarity contradiction, and singularity problem are not solved in ED — they simply never arise in its ontology.}

\section{13.7 Area-Law Entropy (BH-5)}

\subsection{13.7.1 Participation capacity at the decoupling surface}
...
\begin{itemize}
\item \textbf{Area-extensivity.} $\mathcal{C}_H$ scales with the integrated surface area, not with the enclosed volume, because the integrand is supported only on $\Sigma_H$.
\item \textbf{Saturation in the BH-relevant regime.} $\chi \to 1$ where gradient saturation holds (BH-3); the capacity reduces to $\mathcal{C}_H \approx A/\ell_P^2$.
\end{itemize}

The area-law form is \textit{derived} from substrate machinery: two-dimensional surface support + gradient saturation + per-patch finite alphabet.

\subsection{13.7.2 Entropy as log of viable commitment histories}
...
\subsection{13.7.3 The 1/4 coefficient: INHERITED, not derived}

The Bekenstein–Hawking value $S_\mathrm{BH} = A/(4\ell_P^2)$ corresponds to $\log g = 1/4$, i.e., $g = e^{1/4} \approx 1.284$. This is \textit{not an integer}. Naive per-patch-independent finite-alphabet counting cannot reproduce $1/4$: integer $g \geq 2$ all give $\log g \geq \ln 2 \approx 0.693$, well above $1/4$.
...
\textbf{Verdict on the 1/4 coefficient: INHERITED, not derived.} Form-FORCED + value-INHERITED methodology applied honestly. Same category as $a_0$ (Chapter 11), $G$ (Chapter 11), $\Lambda$, gauge couplings (Chapter 9), and the other substrate-determined dimensionless numbers in the program.

\subsection{13.7.4 Implications for motif counting and V1 correlations}

The closed-form $\log g$ derivation would require either:
\begin{itemize}
\item An explicit V1-kernel-counting argument analogous to a substrate-level partition function.
\item A derivation of the per-patch commitment alphabet from microscopic substrate dynamics.
\end{itemize}

Either route is a candidate future arc. Currently flagged as \textbf{OPEN item O2} in the program's priority list. The closed-form-substrate-constants program (combining O-QC-1 for $\mathcal{M}_\mathrm{crit}$, O2 for $\log g$, and E4 for $\kappa/|\hat{N}'|$) is the program's structurally-similar collection of closed-form open work.

\section{13.8 Wave–Black-Hole Scattering (BH-6)}

\subsection{13.8.1 Minimal ED-channel}

A minimal ED-channel — a gravitational wave in the continuum reading — is a substrate excitation characterized by:
\begin{itemize}
\item Low multiplicity (small number of participation degrees of freedom involved per coarse-graining cell).
\item Low anisotropy (anisotropy tensor magnitude small compared to substrate-saturation).
\item DCGT-coherent propagation (channel survives multi-scale expansion to a wave equation on the acoustic metric).
\end{itemize}
...
\subsection{13.8.2 Maximal ED-structure}
...
\subsection{13.8.3 Phase shift as global path-integrated invariant}
...
\subsection{13.8.4 S-matrix exponentiation}
...
\subsection{13.8.5 Helicity preservation and flip}

For axisymmetric, non-rotating backgrounds (Schwarzschild-like): principal directions of the participation profile are aligned uniformly along the trajectory's symmetry plane. The channel's anisotropy tensor returns to itself up to an overall scalar phase under parallel transport. \textbf{Helicity is preserved}; phase shifts act identically on both helicity states.
...
\textbf{Helicity flip channels open}, with mixing strength tied to spin parameter via $\omega_\mathrm{FD}$.

\subsection{13.8.6 Kerr twist and superradiance}
...
\textbf{Superradiance} (energy extraction by certain scattering channels) is structurally compatible with this picture: channels traversing the rotating-vorticity exterior can net positive energy transfer from background to channel via the participation-flow vorticity. No committed structure is extracted from the interior; the energy transfer is mediated by exterior participation flow. Full amplitude derivation is \textbf{OPEN item O1}.

\section{13.9 The Conditional-Positive Verdict (BH-7)}

\subsection{13.9.1 Architectural-level structural closure}
...
\subsection{13.9.2 What the closure delivers}

\begin{itemize}
\item \textbf{The single substrate condition} unifying horizon formation, interior saturation, information blocking, participation-capacity saturation, and strong-curvature scattering region.
\item \textbf{Universal horizon mechanism} across BH / Rindler / cosmic / acoustic horizons.
\item \textbf{No singularities; finite-thickness saturated participation zone replacement.}
\item \textbf{Information / firewall / complementarity / singularity paradoxes not generated in framework.}
\item \textbf{Area-law entropy form derived; 1/4 coefficient INHERITED.}
\item \textbf{BHPT phase-shift structure derived as global path-integrated invariant.}
\item \textbf{Helicity preservation/flip and Kerr twist} from anisotropy-basis transport plus substrate-level vorticity.
\item \textbf{Conditional-positive structural closure} at architectural level.
\end{itemize}

\subsection{13.9.3 What the closure does not deliver}

\begin{itemize}
\item \textbf{Hawking spectrum derivation} (the next natural arc, B4). The mechanism is in hand from BH-4 (asymmetric participation flow at saturated decoupling surface produces outward-radiation statistics); the explicit V5 cross-chain correlation calculation that produces the spectrum is the remaining work.
\item \textbf{Closed-form 1/4 coefficient.} OPEN item O2; closed-form $\log g$ requires either explicit V1-kernel-counting argument or substrate-level partition function.
\item \textbf{Superradiance amplitude derivation.} OPEN item O1; structurally compatible per BH-6, but full derivation requires extending BH-6 + Arc SG.
\item \textbf{Full Kerr interior audit.} OPEN item O3; BH-3 establishes saturated-zone interior structure for Schwarzschild-class BHs; rotating-BH interior architecture (ergosphere, Cauchy horizons, frame-dragging at saturation) requires extension.
\item \textbf{Constructive-rigor proofs} at any single component. The verdict is structural-positive at substrate-suggestive level, not constructive-rigor.
\end{itemize}

\subsection{13.9.4 The Hawking-spectrum next arc}

Hawking-spectrum derivation (B4) is the next natural concrete arc following Arc BH closure. The components in hand:
\begin{itemize}
\item \textbf{The mechanism} (BH-4, evaporation as participation re-routing through pair-creation events at the saturated decoupling surface).
\item \textbf{The substrate-level structural framework} for the spectrum derivation (BH-2's universal horizon mechanism plus BH-4's information-architecture content).
\item \textbf{The V5 cross-chain memory kernel} (Chapter 1, Chapter 2) as the substrate-level object whose cross-chain correlation calculation produces the spectrum.
\end{itemize}
...
\section{13.10 What This Changes (And What It Doesn't)}

\subsection{13.10.1 What does not change}
...
\subsection{13.10.2 What does change}

Three structural shifts:

\begin{itemize}
\item \textbf{The standard paradox cluster does not arise.} Information-loss, firewall, complementarity, and singularity paradoxes are not solved in framework but not generated in framework. The four assumptions whose joint imposition produces them are not imposed at substrate level.
\item \textbf{Horizons become coarse-grained statistical features} of $\Gamma_\mathrm{cross}$ collapse, not knife-edge geometric loci. The decoupling-surface mechanism is universal across BH / Rindler / cosmic / acoustic horizons.
\item \textbf{Singularities do not exist} as physical objects. The acoustic-metric reading breaks down at $\sigma \gtrsim \beta_\mathrm{crit}$; substrate dynamics remain finite. The replacement is a finite-thickness saturated participation zone.
\end{itemize}
...
\section{13.11 Form-FORCED vs Value-INHERITED at Black-Hole Architecture}

\subsection{13.11.1 What is form-FORCED}

\begin{itemize}
\item \textbf{The single substrate condition} $\sigma \gtrsim \log(R_\mathrm{cg}/\ell_P)$ unifying six derivations.
\item \textbf{The horizon as decoupling surface} mechanism, universal across BH / Rindler / cosmic / acoustic horizons.
\item \textbf{The three substrate constraints (C1)–(C3)} jointly forbidding divergent curvature (no singularities).
\item \textbf{The finite-thickness saturated participation zone} as replacement for the singular interior.
\item \textbf{The committed-vs-uncommitted structure distinction} producing information blocking + entanglement straddling.
\item \textbf{The evaporation-as-participation-re-routing} mechanism through pair-creation events at the saturated surface.
\item \textbf{The non-generation of standard paradoxes} (information, firewall, complementarity, singularity).
\item \textbf{The area-law form} $S \propto A/\ell_P^2$ from two-dimensional surface support + gradient saturation + per-patch finite alphabet.
\item \textbf{The BHPT phase-shift structure} as global path-integrated invariant of minimal ED-channels.
\item \textbf{Helicity preservation/flip from anisotropy-basis transport.}
\item \textbf{Kerr twist from integrated frame-dragging vorticity.}
\item \textbf{The conditional-positive structural closure verdict.}
\end{itemize}

\subsection{13.11.2 What is value-INHERITED}

\begin{itemize}
\item \textbf{The 1/4 coefficient} in the area-law entropy. Corresponds to $\log g \approx 1/4$ from substrate motif counting; closed-form derivation is OPEN item O2.
\item \textbf{BHPT spectral details.} Specific values of $\delta_{\ell m}$ for given backgrounds; INHERITED from acoustic-metric structure (Chapter 12) and from background-specific content.
\item \textbf{$\beta_\mathrm{crit}$} in the substrate condition's right-hand side. Substrate-determined dimensionless number; precise prefactor depends on coarse-graining choices.
\item \textbf{V1 kernel width.} Set the per-patch substrate temporal width entering the motif alphabet $g$.
\item \textbf{Specific Hawking temperature for any given background.} Depends on V5 cross-chain correlation calculation; the next-arc B4 deliverable.
\item \textbf{Specific superradiance amplitudes.} OPEN item O1.
\item \textbf{Specific Kerr-interior architectural details} beyond Schwarzschild-class. OPEN item O3.
\end{itemize}

\subsection{13.11.3 What is open}

Three OPEN items carrying forward from Arc BH:

\begin{itemize}
\item \textbf{O1 — Superradiance amplitude derivation.} Structurally compatible; full amplitude derivation requires extending BH-6.
\item \textbf{O2 — Closed-form $\log g$ from substrate motif counting.} Closed-form $1/4$ coefficient; structurally similar in difficulty to O-QC-1 ($\mathcal{M}_\mathrm{crit}$) and E4 ($\kappa/|\hat{N}'|$).
\item \textbf{O3 — Full Kerr interior audit.} Rotating-BH interior architecture beyond Schwarzschild-class.
\end{itemize}

Plus the \textbf{B4 Hawking spectrum derivation} as the next natural concrete arc.

\section{13.12 Dependencies}

\subsection{13.12.1 Upstream}

\begin{itemize}
\item \textbf{Chapter 1.} Substrate primitives. Especially: P11 commitment-irreversibility (load-bearing for the committed-vs-uncommitted information architecture); P01 event discreteness (load-bearing for constraint C2 forbidding singularities); P04 bandwidth update rule (load-bearing for constraint C1 finite participation and constraint C3 gradient saturation); the finite-kernel commitment for V1 (load-bearing for the per-patch motif alphabet); substrate locality (load-bearing for the cross-bandwidth structure underlying horizon formation).
\item \textbf{Chapter 2.} Load-bearing invariants. Gradient sparsity $\sigma$ enters the single substrate condition directly; cross-bandwidth $\Gamma_\mathrm{cross}$ enters horizon formation through DCGT; multiplicity $\mathcal{M}$ enters the saturated-zone interior structure; V1 finite-width vacuum kernel enters the motif alphabet.
\item \textbf{Chapter 3.} DCGT. The substrate-to-continuum bridge for $\Gamma_\mathrm{cross}$ structure used in horizon formation; the multi-scale expansion underlying the threshold scale $\log(R_\mathrm{cg}/\ell_P)$.
\item \textbf{Chapter 12.} Arc ED-10 acoustic-metric scalar-tensor framework. The acoustic metric is the kinematic-summary structure that breaks down at $\sigma \gtrsim \beta_\mathrm{crit}$ in BH-3; the curvature-emergence content underlies the strong-curvature scattering region in BH-6.
\end{itemize}

\subsection{13.12.2 Downstream}

\begin{itemize}
\item \textbf{Chapter 14 (Cross-Platform Unifications).} The cross-domain $\Gamma_\mathrm{cross}$-collapse mechanism shared with QC condition (ii) failure (Chapter 7) at scales separated by ~50 orders of magnitude is one of the program's strongest cross-platform substrate-mechanism identities. Chapter 14 develops it as part of the program's cross-domain unification analysis.
\item \textbf{Chapter 15 (Public Test Inventory).} Hawking spectrum is the next natural arc (B4); BHPT phase-shift content is anchored against LIGO/Virgo quasinormal-mode catalogs; OPEN items O1, O2, O3 are catalogued.
\end{itemize}

\section{13.13 Canonical Sources}

\begin{itemize}
\item \texttt{papers/Black\_Hole\_Foundations/}
\item Arc BH memos in \texttt{theory/Black\_Holes/}
\end{itemize}

The Black\_Hole\_Foundations paper presents the publication-grade architectural account of the BH sector, including the single substrate condition, the universal horizon mechanism, the singularity replacement, the information non-paradox, the area-law form, the BHPT phase-shift structure, the conditional-positive verdict, and the explicit not-generated-in-framework framing for the four standard paradoxes. Arc BH memos in \texttt{theory/Black\_Holes/} (seven memos: Opening, Horizon-DCGT, Singularity-And-Strong-Curvature-Audit, Information-And-Evaporation, Area-Law-Entropy, Wave-BH-Scattering, Synthesis) develop each component in detail.
...
\section{13.14 Optional Figures}


\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_85_six_derivations_one_substrate_condition.pdf}
\caption*{Figure 13.1 --- The single substrate condition unifying six derivations. A central diagram with the substrate condition $\sigma \gtrsim \log(R_\mathrm{cg}/\ell_P)$ at the center and six radial spokes labeled BH-2 horizon formation, BH-3 interior saturation, BH-4 information blocking, BH-5 participation-capacity, BH-6 strong-curvature scattering, BH-7 verdict. Each spoke is annotated with its substrate-level consequence.}
\label{fig:fig_85_six_derivations_one_substrate_condition}
\end{figure}


\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_86_universal_horizon_four_classes.pdf}
\caption*{Figure 13.2 --- Universal horizon mechanism across four classes. A four-panel diagram showing four horizon classes (BH event horizons, Rindler horizons, cosmic horizons, acoustic horizons in analog gravity). Each panel shows the substrate-level decoupling surface produced by the same single substrate condition. A central note observes that the four classes share thermodynamic-style features at standard-physics level because they are projections of one substrate phenomenon at four different scales.}
\label{fig:fig_86_universal_horizon_four_classes}
\end{figure}


\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_87_singularity_replacement.pdf}
\caption*{Figure 13.3 --- Singularity replacement. A two-panel diagram. Left panel: standard GR singular interior (curvature divergence, geodesic incompleteness). Right panel: substrate-level finite-thickness saturated participation zone (gradient saturation, finite substrate dynamics, no singularity). A note observes that the acoustic-metric reading breaks down before the substrate does; substrate dynamics remain finite.}
\label{fig:fig_87_singularity_replacement}
\end{figure}


\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_88_paradox_cluster_assumptions.pdf}
\caption*{Figure 13.4 --- The four assumptions whose joint imposition generates the paradox cluster. A diagram showing the four assumptions (global unitarity, global Cauchy data, sharp geometric boundary, monogamy-at-boundary) connected to the four paradoxes (information loss, firewall, complementarity, singularity). A central note observes that ED imposes none of the four at substrate level — the paradoxes do not arise.}
\label{fig:fig_88_paradox_cluster_assumptions}
\end{figure}


\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_89_information_vs_entanglement_horizon.pdf}
\caption*{Figure 13.5 --- Information vs entanglement at the horizon. A schematic showing committed structure (information, P11-irreversible) blocked at the decoupling surface, and uncommitted structure (entanglement, joint amplitude) straddling the surface freely. A note observes that the blocking is participation-bandwidth, not causal.}
\label{fig:fig_89_information_vs_entanglement_horizon}
\end{figure}


\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_90_evaporation_as_rerouting.pdf}
\caption*{Figure 13.6 --- Evaporation as participation re-routing. A schematic showing pair-creation-class events at the saturated decoupling surface, with one leg committing inward (joining the saturated zone) and the other committing outward (joining the radiation field). Joint correlation is preserved globally; no committed structure is destroyed.}
\label{fig:fig_90_evaporation_as_rerouting}
\end{figure}


\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_91_area_law_form_inherited_coefficient.pdf}
\caption*{Figure 13.7 --- Area-law entropy form derived; 1/4 coefficient INHERITED. A two-panel diagram. Left panel: area-law form $S = A/\ell_P^2 \cdot \log g$ derived from substrate participation-capacity at the decoupling surface. Right panel: 1/4 coefficient corresponds to $\log g \approx 1/4$, requires substrate motif counting beyond per-patch independence; INHERITED, OPEN item O2.}
\label{fig:fig_91_area_law_form_inherited_coefficient}
\end{figure}


\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_92_bhpt_phase_shift.pdf}
\caption*{Figure 13.8 --- BHPT phase-shift structure. A schematic showing minimal ED-channel propagating through the strong-curvature exterior of a maximal ED-structure, accumulating phase shift $\delta_{\ell m} = \int_\mathrm{path}\Delta\mathcal{A}\,d\lambda$ as a global path-integrated invariant. Helicity preservation (axisymmetric backgrounds) and helicity flip (frame-dragging backgrounds) shown as anisotropy-basis transport.}
\label{fig:fig_92_bhpt_phase_shift}
\end{figure}


\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_93_gamma_cross_collapse_bh_echo.pdf}
\caption*{Figure 13.9 --- The cross-domain $\Gamma_\mathrm{cross}$-collapse echo. A length-scale diagram showing the same substrate-condition mechanism producing horizon formation at gravitational-collapse scales (~$10^{38}\,\ell_P$ for stellar-mass black holes) and condition (ii) failure in QC at engineered-system scales (~$10^{-9}$ m for Josephson junctions). Two empirical phenomena, one substrate identity, ~50 orders of magnitude scale separation.}
\label{fig:fig_93_gamma_cross_collapse_bh_echo}
\end{figure}


\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_94_bh_form_value.pdf}
\caption*{Figure 13.10 --- Form-FORCED vs Value-INHERITED at black-hole architecture. A two-column diagram. Left column ("Form-FORCED"): single substrate condition, universal horizon mechanism, no singularities, paradoxes not generated, area-law form, BHPT phase-shift structure, helicity behavior, Kerr twist. Right column ("Value-INHERITED"): 1/4 coefficient ($\log g$), BHPT spectral details, $\beta_\mathrm{crit}$, V1 kernel width, specific Hawking temperature, superradiance amplitudes, Kerr interior details.}
\label{fig:fig_94_bh_form_value}
\end{figure}

\newpage


\part{Part V — Empirical Synthesis}
...
\chapter{Cross-Platform Unifications: Matter-Wave ↔ Qubit, $\Gamma_\mathrm{cross}$ Collapse, Methodology}
\label{chap:14}

\section{14.1 Chapter Overview}
...
This chapter collects the \textbf{cross-domain unifications} that emerge from the closed sectors — places where the same substrate quantity, mechanism, or architectural principle produces empirically distinct phenomena at structurally distant scales. Three kinds of unification recur across the program: \textbf{same-substrate-quantity at different platforms} (the matter-wave quantum-classical boundary and qubit-system multiplicity walls share $\mathcal{M}_\mathrm{crit}$); \textbf{same-substrate-mechanism at different scales} ($\Gamma_\mathrm{cross}$ collapse produces both black-hole horizon formation and quantum-computing condition (ii) failure at scales separated by ~50 orders of magnitude); and \textbf{same-architectural-principle across content channels} (P04's mobility-capacity bound produces both the Universal Mobility Law and Class A QC; V1's finite-width kernel produces the kernel-level arrow, R1 substrate-cutoff regularization, the Yang–Mills mass-gap mechanism, and the BH area-law motif alphabet). The chapter also establishes the program's \textbf{form-FORCED / value-INHERITED methodology} as a cross-domain testable signature: closed-form derivation of any inherited substrate constant in one sector should produce mutually consistent values when projected to other sectors.
...
\section{14.2 Why Cross-Domain Unifications Matter Structurally}

\subsection{14.2.1 What unifications across domains demonstrate}

A substrate-level framework that produces continuum physics across multiple sectors is structurally significant only if the substrate machinery is \textit{consistent} across the sectors. A framework in which every sector's substrate-derivation invokes a different combination of primitives, with no overlap or shared structural content, would not constitute a substrate-level theory; it would be a collection of separate theories sharing a common vocabulary. The unifying claim of the Event Density program is that one substrate ontology produces all nine closed sectors, and the cross-domain unifications of this chapter are the structural evidence for that claim.

Three kinds of cross-domain content emerge from the program. Each is a different \textit{type} of unification:

\begin{itemize}
\item \textbf{Cross-platform identity} at the level of substrate quantities. Two empirically distinct phenomena (at different platforms) share a substrate-determined parameter; sharper measurement of one constrains the other. The matter-wave Q-C boundary and the qubit-system multiplicity walls (Section 14.3) are the canonical example.
\item \textbf{Cross-scale identity} at the level of substrate mechanisms. The same substrate machinery produces phenomena at scales separated by tens of orders of magnitude. The $\Gamma_\mathrm{cross}$-collapse mechanism (Section 14.4) is the canonical example.
\item \textbf{Cross-channel identity} at the level of architectural principles. The same substrate primitive appears in multiple closed sectors with structurally consistent consequences. P04's mobility-capacity bound (Section 14.5) and V1's finite-width kernel (Section 14.6) are canonical examples.
\end{itemize}
...
\subsection{14.2.2 What the chapter does and does not deliver}

The chapter \textbf{delivers}:
\begin{itemize}
\item A structural synthesis of the cross-domain unifications established in Chapters 1–13.
\item The matter-wave ↔ qubit-system cross-platform identity as the program's strongest empirical-anchor unification.
\item The $\Gamma_\mathrm{cross}$-collapse cross-scale identity as the program's strongest substrate-mechanism unification.
\item The P04 mobility-capacity bound and V1 finite-width kernel cross-channel identities as architectural-principle unifications.
\item The form-FORCED / value-INHERITED methodology as a testable cross-domain signature.
\item The structural distinction between substrate-level frameworks (which produce cross-domain unifications) and phenomenological assemblies (which do not).
\end{itemize}

The chapter \textbf{does not deliver}:
\begin{itemize}
\item New substrate primitives, theorems, or arcs. The chapter is synthetic, not constructive.
\item Closed-form derivations of any inherited substrate constants. The closed-form-substrate-constants program is downstream open work; this chapter establishes the program's cross-domain consistency requirements but does not close them.
\item Empirical predictions beyond what Chapters 7–13 establish. The chapter restates the cross-platform identity in unified form; it does not introduce new predictions.
\end{itemize}

\section{14.3 Cross-Platform Identity: Matter-Wave ↔ Qubit-System Multiplicity Walls}

\subsection{14.3.1 Two empirically distinct phenomena}

Two empirical phenomena have been studied in different research communities for decades:

\begin{itemize}
\item \textbf{The matter-wave quantum-classical boundary.} Matter-wave interferometry experiments (Vienna and Basel programs, broadly) probe the quantum-classical transition by sending increasingly massive molecules through interferometers. Below approximately 140 kDa molecular mass, fringes are observed cleanly. Above approximately 250 kDa, fringes wash out and the molecule behaves classically. The empirical boundary is one of the most robust findings in foundational quantum physics; it has been studied for ~20 years and the boundary mass is reproducible across experimental groups.
\item \textbf{The qubit-system multiplicity wall.} Quantum-computing platforms (superconducting qubits, trapped ions, photonic gate-model architectures) face structural ceilings on system-level effective coherence as the system scales. Each Class A platform has a substrate-determined wall at a specific platform $N_\star$ (system size); past the wall, pure-Class-A scaling cannot continue.
\end{itemize}
...
\subsection{14.3.2 The substrate-level identity}
...
\begin{itemize}
\item \textbf{The matter-wave Q-C boundary mass $M_\star$} is the molecular mass at which the molecule's intrinsic internal-DOF activated multiplicity crosses $\mathcal{M}_\mathrm{crit}$. Below $M_\star$, the molecule's effective system multiplicity is $\mathcal{M}_\mathrm{floor}(M_\mathrm{mol}) < \mathcal{M}_\mathrm{crit}$; the unresolved-rule regime can be sustained; fringes are observed. Above $M_\star$, $\mathcal{M}_\mathrm{floor}(M_\mathrm{mol}) > \mathcal{M}_\mathrm{crit}$; the unresolved regime cannot be sustained; fringes wash out.
...
\subsection{14.3.3 What this identity implies empirically}

\begin{itemize}
\item \textbf{Sharper measurement of one anchor constrains the other.} A more precise measurement of the matter-wave Q-C boundary mass directly constrains $\mathcal{M}_\mathrm{crit}$, which in turn constrains the qubit-system $N_\star$ values for any platform with a calibrated $f_\mathrm{sys}^{(A)}$. Conversely, a measurement of the qubit-system multiplicity wall constrains $\mathcal{M}_\mathrm{crit}$ and therefore constrains the matter-wave boundary mass.
\item \textbf{The two communities study the same substrate phenomenon.} Foundational matter-wave-interferometry research and commercial QC-scaling research are not separate research programs at the substrate level; they probe the same substrate boundary from two empirical directions.
\item \textbf{Cross-platform calibration is structurally valuable.} A research program coordinating measurements of both anchors produces consistency checks unavailable in either community alone. Inconsistent calibration of $\mathcal{M}_\mathrm{crit}$ between the two platforms would \textit{refute the substrate-level identity}; consistent calibration is the form-FORCED prediction.
\end{itemize}

\subsection{14.3.4 The strongest empirical-anchor unification}

The matter-wave ↔ qubit-system identity is the program's \textbf{strongest empirical-anchor cross-platform unification}. It is testable at the form-FORCED level and falsifiable by inconsistent calibration. The empirical content:

\begin{itemize}
\item The matter-wave boundary at 140–250 kDa is a PASSED-class observation in the program's empirical inventory (Chapter 15); it is the natural anchor for $\mathcal{M}_\mathrm{crit}$.
\item Qubit-system multiplicity walls are IN PROGRESS as empirical anchors; SC qubit platforms are approaching the substrate-determined wall as system-level effective coherence is measured at increasing scales.
\item Cross-platform calibration of $\mathcal{M}_\mathrm{crit}$ from both anchors is a structural test: if the two anchors yield consistent $\mathcal{M}_\mathrm{crit}$ values up to the architecture-specific scaling functions, the cross-platform identity is empirically supported. If they yield inconsistent values, the identity is refuted.
\end{itemize}
...
\section{14.4 Cross-Scale Identity: $\Gamma_\mathrm{cross}$ Collapse Across ~50 Orders of Magnitude}

\subsection{14.4.1 The same substrate mechanism at different scales}
...
\subsection{14.4.2 Scale separation: ~50 orders of magnitude}
...
\begin{itemize}
\item \textbf{Black-hole horizon scales.} A stellar-mass black hole has horizon radius $\sim 10^{4}$ m. The substrate-level decoupling surface is at this scale, and the integrated $\sigma$ along radial paths through the horizon region produces the $\Gamma_\mathrm{cross}$-collapse exponential.
\item \textbf{Josephson-junction barrier scales.} A typical JJ barrier has thickness $\sim 10^{-9}$ m. The integrated $\sigma$ along the engineered barrier produces the same DCGT-derived exponential, manifesting as the WKB MQT rate $\tau_\mathrm{MQT}^{-1} \sim \omega_0\exp[-\alpha\int_\mathrm{barrier}\sigma\,d\ell]$.
\end{itemize}
...
\subsection{14.4.3 The structural significance}
...
\subsection{14.4.4 The cross-scale unification across other phenomena}
...
\begin{itemize}
\item \textbf{Cosmological horizon participation density.} T20 (Chapter 11) uses the cosmic horizon's participation-boundary content; the substrate-level reading of the cosmic horizon as a $\Gamma_\mathrm{cross}$-collapse boundary is consistent with BH-2's universal horizon mechanism.
\item \textbf{Acoustic horizons in analog gravity.} Flow-induced gradients in fluid analog systems produce the same substrate condition and the same $\Gamma_\mathrm{cross}$ collapse.
\item \textbf{Rindler horizons in accelerated frames.} Acceleration-induced gradients produce the same substrate condition.
\end{itemize}
...
\section{14.5 Cross-Channel Identity: P04 Mobility-Capacity Bound}

\subsection{14.5.1 The same substrate primitive in multiple sectors}
...
\subsection{14.5.2 The structural significance of cross-channel application}
...
\begin{itemize}
\item \textbf{P11 commitment-irreversibility} appears in the kernel-level arrow of time (Chapter 4), the no-collapse measurement rule (Chapter 5), the BH-4 information architecture (Chapter 13), and the UR-1 commitment-injection failure mode (Chapter 7).
\item \textbf{P01 event discreteness} appears in substrate UV-finiteness (Chapter 6), the BH-3 no-singularity result (Chapter 13), and the multiple closed sectors that inherit the substrate's smallest length scale.
\end{itemize}
...
\subsection{14.5.3 The shared exponent class $\beta \approx 2$}
...
\section{14.6 Cross-Channel Identity: V1 Finite-Width Vacuum Kernel}

\subsection{14.6.1 The same substrate object in multiple sectors}
...
\begin{itemize}
\item \textbf{Kernel-level arrow of time (Chapter 4).} V1's finite-width plus chain-sourced character supplies the structural input to Theorem 18's forcing chain. V1 retardation produces the substrate-level arrow of time.
\item \textbf{R1 substrate-cutoff regularization in Navier–Stokes (Chapter 8).} V1's finite width produces, at first subleading order in DCGT, the hyperviscous correction term $-\kappa\mu_{\mathrm{V1}}\ell_P^2\nabla^4\mathbf{v}$. R1 supplies the Clay-NS-relevant regularizing mechanism in the framework's Intermediate Path C verdict.
\item \textbf{Yang–Mills mass-gap mechanism (Chapter 9).} V1's second-moment expansion plus non-Abelian quartic stabilization produces a substrate-level mass-gap. Survival of the gap is conditional on V1's kernel-profile rescaling under continuum-limit flow.
\item \textbf{BH area-law motif alphabet (Chapter 13).} V1's per-patch substrate temporal width sets the per-patch motif alphabet $g$ in the BH area-law entropy expression $S = (A/\ell_P^2)\log g$.
\end{itemize}
...
\subsection{14.6.2 What this cross-channel content demonstrates}
...
\subsection{14.6.3 The unification across kernels}

The same cross-channel pattern appears for V5 (the cross-chain memory kernel):

\begin{itemize}
\item \textbf{V5 → Maxwell viscoelasticity in soft-matter mobility (Chapter 10).} V5's first temporal moment is identified as the relaxation time $\tau_R$ in the Maxwell equation.
\item \textbf{V5 cross-chain correlations in BH evaporation (Chapter 13, next-arc B4).} V5's cross-chain correlation calculation is the load-bearing remaining work for the Hawking-spectrum derivation. The mechanism is in hand from BH-4; the explicit V5 calculation is what produces the spectrum.
\end{itemize}
...
\section{14.7 Form-FORCED / Value-INHERITED as Cross-Domain Methodology}

\subsection{14.7.1 The methodological pattern}
...
The cross-domain pattern: \textit{every closed sector is structurally fixed in form and numerically calibrated against empirical anchors}. The methodology is consistent across the program, and the consistency itself is a structural signature.

\subsection{14.7.2 The cross-domain testable signature}

The methodology is testable as a cross-domain signature: \textit{closed-form derivation of any inherited substrate constant in one sector should produce mutually consistent values when projected to other sectors}. Three specific closed-form-substrate-constants candidates have been identified across the program:

\begin{itemize}
\item \textbf{$\mathcal{M}_\mathrm{crit}$ in QC} (Chapter 7, OPEN item O-QC-1). Closed-form derivation from V1 kernel + ED-I-01 substrate constants. Empirical anchor: matter-wave Q-C boundary at 140–250 kDa.
\item \textbf{$\log g$ in BH area-law entropy} (Chapter 13, OPEN item O2). Closed-form derivation from substrate motif counting. Empirical anchor: Bekenstein–Hawking 1/4 coefficient.
\item \textbf{$\kappa/|\hat{N}'| \approx 0.001766$ in the ED-SC arc} (priority list E4). Closed-form derivation from substrate constants.
\end{itemize}

These three are structurally similar: each calibrates a substrate constant from empirical anchors; closed-form derivation in any one constrains the others by the cross-domain consistency requirement. The three are sometimes called the \textbf{closed-form-substrate-constants program}.
...
\section{14.8 The Three Unification Types Summarized}

\subsection{14.8.1 Same-substrate-quantity (matter-wave ↔ qubit-system)}
...
\subsection{14.8.2 Same-substrate-mechanism ($\Gamma_\mathrm{cross}$ collapse across scales)}
...
\subsection{14.8.3 Same-architectural-principle (P04, V1 across content channels)}
...
\subsection{14.8.4 The three unification types together}

The three unification types together demonstrate that the substrate ontology is \textit{one framework with consistent applications across multiple sectors}, not a collection of separate sector-specific theories sharing a common vocabulary. Each unification type is a different facet of cross-domain consistency:

\begin{itemize}
\item \textbf{Cross-platform identity} demonstrates that substrate parameters are platform-independent.
\item \textbf{Cross-scale identity} demonstrates that substrate mechanisms are scale-independent.
\item \textbf{Cross-channel identity} demonstrates that substrate primitives are channel-independent.
\end{itemize}
...
\section{14.9 What This Changes (And What It Doesn't)}

\subsection{14.9.1 What does not change}
...
\subsection{14.9.2 What does change}
...
\begin{itemize}
\item \textbf{Matter-wave interferometry and qubit-system scaling become connected.} The two empirical research programs are reframed as two probes of the same substrate-determined parameter. Cross-platform calibration becomes structurally valuable.
\item \textbf{Black-hole horizon formation and quantum-computing condition (ii) failure share a substrate mechanism.} The cross-scale identity at ~50 orders of magnitude is one of the program's strongest substrate-mechanism unifications.
\item \textbf{The form-FORCED / value-INHERITED methodology is identified as a cross-domain signature.} The cross-domain consistency requirement on closed-form-substrate-constants derivations becomes a testable structural prediction.
\end{itemize}
...
\section{14.10 Form-FORCED vs Value-INHERITED at Cross-Platform Unifications}

\subsection{14.10.1 What is form-FORCED}

\begin{itemize}
\item \textbf{The matter-wave ↔ qubit-system cross-platform identity} via shared $\mathcal{M}_\mathrm{crit}$.
\item \textbf{The $\Gamma_\mathrm{cross}$-collapse cross-scale identity} between BH horizon formation and QC condition (ii) failure (and the broader universal-horizon-mechanism class).
\item \textbf{The P04 mobility-capacity bound cross-channel identity} between Universal Mobility Law and Class A QC.
\item \textbf{The V1 finite-width kernel cross-channel identity} across kernel-level arrow, R1 substrate-cutoff, YM mass-gap, BH area-law motif alphabet.
\item \textbf{The form-FORCED / value-INHERITED methodology} as cross-domain testable signature.
\item \textbf{The cross-domain consistency requirement} on closed-form-substrate-constants derivations.
\end{itemize}

\subsection{14.10.2 What is value-INHERITED}

\begin{itemize}
\item \textbf{The specific value of $\mathcal{M}_\mathrm{crit}$.} Anchored empirically by matter-wave Q-C boundary at 140–250 kDa. Closed-form derivation is OPEN item O-QC-1 (Chapter 7).
\item \textbf{The specific value of $\log g$ in BH entropy.} Anchored empirically by Bekenstein–Hawking 1/4 coefficient. Closed-form derivation is OPEN item O2 (Chapter 13).
\item \textbf{The specific value of $\kappa/|\hat{N}'|$ in the ED-SC arc.} Anchored empirically. Closed-form derivation is OPEN item E4.
\item \textbf{Architecture-specific scaling functions} ($f_\mathrm{int}$ for matter-wave molecules, $f_\mathrm{xy}$ for SC qubit arrays, etc.). INHERITED from architecture-to-platform calibration (O-QC-2).
\item \textbf{Specific platform-level wall locations} ($N_\star$ for any given QC platform). Determined by the architecture-specific $f_\mathrm{sys}^{(A)}$ plus the substrate-shared $\mathcal{M}_\mathrm{crit}$.
\end{itemize}

\subsection{14.10.3 What is open}

\begin{itemize}
\item \textbf{The closed-form-substrate-constants program.} Three structurally similar open items (O-QC-1, O2, E4); closed-form derivation in any one constrains the others by the cross-domain consistency requirement.
\item \textbf{Architecture-to-platform calibration} (O-QC-2). Mapping $\mathcal{M}_\mathrm{floor}^{(A)}(\mathcal{S})$ for canonical qubit platforms; predicting $N_\star$ values via the cross-platform identity.
\item \textbf{Cross-platform calibration of $\mathcal{M}_\mathrm{crit}$} from both the matter-wave anchor and the qubit-system anchor. Empirical work.
\item \textbf{Empirical confirmation of $\Gamma_\mathrm{cross}$-collapse cross-scale identity} in additional sectors beyond BH horizons and QC condition (ii). Cosmic-horizon and Rindler-horizon empirical content provides additional anchors when available.
\end{itemize}

\section{14.11 Dependencies}

\subsection{14.11.1 Upstream}

\begin{itemize}
\item \textbf{All preceding chapters.} The cross-domain unifications synthesize results established in Chapters 1–13. No single chapter is the sole upstream dependency; the chapter integrates content across the entire monograph.
\item \textbf{Chapter 1 (substrate primitives).} The cross-channel identities of P04 and V1 rely on the primitive-level commitments established in Chapter 1.
\item \textbf{Chapter 2 (load-bearing invariants).} The cross-platform identity rests on multiplicity $\mathcal{M}$ and the cross-bandwidth $\Gamma_\mathrm{cross}$, both established as load-bearing invariants in Chapter 2.
\item \textbf{Chapter 3 (DCGT).} The $\Gamma_\mathrm{cross}$-collapse cross-scale identity uses the DCGT-derived exponential structure from Chapter 3.
\item \textbf{Chapter 4 (kernel-level arrow).} V1's cross-channel content includes Theorem 18's role in the kernel-level arrow.
\item \textbf{Chapter 7 (QC architecture).} UR-1 and the multiplicity-cap function $M$ supply the matter-wave ↔ qubit-system cross-platform identity.
\item \textbf{Chapter 13 (BH architecture).} The BH horizon-formation mechanism is one side of the $\Gamma_\mathrm{cross}$-collapse cross-scale identity.
\item \textbf{Chapter 10 (soft-matter mobility).} Universal Mobility Law is one side of the P04 cross-channel identity.
\end{itemize}

\subsection{14.11.2 Downstream}

\begin{itemize}
\item \textbf{Chapter 15 (Public Test Inventory).} Empirical anchors for the cross-platform unifications are catalogued in Chapter 15. The closed-form-substrate-constants program (O-QC-1, O2, E4) is part of the open-extensions inventory.
\end{itemize}
...
\section{14.12 Canonical Sources}

\begin{itemize}
\item \texttt{papers/ED\_QFT\_Overview/}
\item \texttt{papers/ED\_One\_Substrate\_Three\_Domains/}
\item Cross-references to all Foundations papers (SG, BH, QC) and the NS Synthesis Paper.
\end{itemize}

The ED\_QFT\_Overview paper (program-level synthesis) presents the cross-domain unifications in publication-grade form across the nine closed sectors. The ED\_One\_Substrate\_Three\_Domains paper supplies the program-overview / three-domains framing that organizes the cross-domain content. The Foundations papers (Substrate\_Gravity\_Foundations, Black\_Hole\_Foundations, Quantum\_Computing\_Foundations) and the NS Synthesis Paper are the sector-specific publication-grade sources whose cross-domain content this chapter integrates.
...
\label{fig:fig_103_substrate_vs_phenomenological}
\end{figure}

\newpage


\chapter{Public Test Inventory and Open Extensions}
\label{chap:15}

\section{15.1 Chapter Overview}

The Event Density framework's structural content across the preceding fourteen chapters produces empirical content as a \textit{consequence}: every closed sector generates falsifiable substrate-level predictions. This final chapter collects those predictions as a \textbf{public test inventory} and identifies the \textbf{open extensions} — the structural follow-on programs that the framework's current scope leaves open. The chapter operates entirely within the form-FORCED / value-INHERITED methodology established across the program: each test is a substrate-level structural claim with an explicit confirmation/refutation condition; each open extension is a named structural target with a defined scope and a relationship to existing closed-arc content.

The chapter establishes the \textbf{status taxonomy} used uniformly across sectors: \textbf{PASSED} (empirical confirmation in published data), \textbf{ANCHORED} (prediction calibrated to existing observation), \textbf{IN PROGRESS} (active experimental program awaiting result), \textbf{ACTIVE} (test specified, awaiting execution), and \textbf{OPEN} (prediction stated, technique requires development). The current inventory comprises three PASSED predictions, thirteen ANCHORED, three IN PROGRESS, seventeen ACTIVE, and four OPEN — totaling forty empirical claims across the nine closed sectors. The chapter does not introduce new tests; it synthesizes those produced by the closed sectors into a unified inventory readable as the program's empirical interface.

The chapter also names the program's open extensions. \textbf{B4 Hawking spectrum} is the next natural concrete arc — the mechanism is in hand from Chapter 13; the explicit V5 cross-chain correlation calculation that produces the spectrum is the remaining work. \textbf{The closed-form substrate-constants program} unifies three structurally similar open items: $\mathcal{M}_\mathrm{crit}$ (O-QC-1, calibrated against the matter-wave Q-C boundary), $\log g$ (O2 from Arc BH, anchored at the Bekenstein–Hawking 1/4 coefficient), and $\kappa/|\hat{N}'|$ (E4 from the ED-SC arc). \textbf{Architecture-to-platform calibration} (O-QC-2), \textbf{topology-stability theorem} (O-QC-4), and the four other O-QC items carry forward from Arc Q-COMPUTE. \textbf{Arc COSMO} (cosmic expansion / $H_0$ derivation from substrate) and \textbf{GR-4A} (Einstein-equation emergence) sit at speculative-not-refuted status. The chapter closes the monograph at the empirical-synthesis layer.

\section{15.2 Why a Public Test Inventory}

\subsection{15.2.1 The empirical posture of a substrate-level framework}
...
\subsection{15.2.2 What the inventory delivers}

\begin{itemize}
\item \textbf{A unified status taxonomy} applied across sectors. Each test has the same status labels (PASSED, ANCHORED, IN PROGRESS, ACTIVE, OPEN); cross-sector comparison is structurally meaningful.
\item \textbf{An explicit confirmation/refutation condition} for each test. Every test is falsifiable; the refutation condition is named.
\item \textbf{A map from sector content to empirical anchor.} Each closed-sector chapter produces specific tests; the inventory identifies which tests come from which chapter.
\item \textbf{A list of open extensions.} Future arcs and open structural problems are identified, with the relationship to current closed-arc content named explicitly.
\end{itemize}

\subsection{15.2.3 What the inventory does not deliver}

\begin{itemize}
\item \textbf{No new predictions} beyond what the closed sectors produce. The chapter is synthetic, not constructive.
\item \textbf{No commitment that any test will succeed.} The framework's empirical posture is honest: PASSED tests are confirmed; ANCHORED tests are calibrated against existing observation; IN PROGRESS, ACTIVE, and OPEN tests have not produced empirical results.
\item \textbf{No claim that the framework as a whole is empirically validated.} The inventory's PASSED count is three; full empirical validation across nine sectors would require dozens more PASSED-status results. The framework's posture is structural-positive at architectural level (each sector closed at architectural level with form-FORCED content) plus empirically PASSED at three specific anchors.
\end{itemize}

\section{15.3 The Status Taxonomy}

\subsection{15.3.1 The five status levels}

The framework uses a uniform five-level status taxonomy across all sectors:

\begin{itemize}
\item \textbf{PASSED} — Empirical confirmation in published data. The structural prediction has been tested against observation, and the observation matches the prediction within stated uncertainty.
\item \textbf{ANCHORED} — Prediction calibrated to existing observation. The substrate-level prediction matches existing empirical content in functional form or numerical range, but the prediction is treated as anchored against rather than fully tested by the observation (because the observation predates the prediction or because the structural prediction is a reframing rather than a discriminating-from-standard-physics claim).
\item \textbf{IN PROGRESS} — Active experimental program awaiting result. A test has been specified, an experimental program is currently running, and empirical content is expected within a defined window.
\item \textbf{ACTIVE} — Test specified, awaiting execution. A test has been specified at sufficient detail that an experimental program could execute it; no experimental program is currently running.
\item \textbf{OPEN} — Prediction stated, technique requires development. The substrate-level analysis identifies a prediction, but the experimental technique to test it does not currently exist or requires substantial development beyond current capability.
\end{itemize}
...
\subsection{15.3.2 Why this five-level taxonomy}

The taxonomy distinguishes the structurally distinct empirical-status conditions that arise across the program:

\begin{itemize}
\item \textbf{PASSED vs ANCHORED.} Both involve empirical content matching the prediction. PASSED status requires that the empirical observation be a direct test of the substrate-level prediction (the observation was made or analyzed because of the prediction, or the prediction is sharp enough that the observation discriminates). ANCHORED status applies when the empirical observation predates the prediction or when the prediction is a reframing of existing empirical content rather than a sharp discriminating test.
\item \textbf{IN PROGRESS vs ACTIVE.} Both involve specified tests without empirical results yet. IN PROGRESS status requires an active experimental program currently running. ACTIVE status applies when the test is specified at adequate detail but no experimental program is currently running.
\item \textbf{ACTIVE vs OPEN.} Both involve specified predictions without current experimental programs. ACTIVE status applies when the experimental technique exists; OPEN status applies when the technique requires development beyond current capability.
\end{itemize}
...
\subsection{15.3.3 What the status counts represent}
...
\section{15.4 PASSED Predictions}

\subsection{15.4.1 The three PASSED-status anchors}

Three predictions have status PASSED — empirical confirmation in published data:

\begin{itemize}
\item \textbf{Dwarf galaxy outer-radius mass discrepancy: Active > Quiet} (Chapter 11, substrate-gravity sector). SPARC dataset analysis of 46 dwarf galaxies showed $\langle D_\mathrm{outer}\rangle \approx 6.01$ for Active dwarfs versus $\approx 3.94$ for Quiet dwarfs at fixed baryonic mass — a 53\% higher outer-radius mass discrepancy in Active systems. The substrate-level prediction (sustained internal activity generates ED gradients producing smooth temporal halos with enhanced outer-radius discrepancies) is empirically confirmed. This is the framework's first completed astrophysical test.
\item \textbf{Multi-timescale FPM relaxation (Hafezi 2025)} (Chapter 7, QC sector). The University of Maryland multi-timescale photonic resonator-lattice work demonstrated that adding a second timescale to a single-ring resonator dramatically relaxes frequency-phase matching, broadens harmonic generation, and produces 100\% device yield with disorder tolerance. The substrate-level prediction (multi-timescale lattices are high-multiplicity ED-objects with redundancy expansion of viable participation pathways) is empirically confirmed. This is the canonical Class C empirical demonstration in the QC architectural taxonomy.
\item \textbf{Universal Mobility Law $\beta \approx 1.72$ across ten systems} (Chapter 10, soft-matter sector). Empirical validation across ten chemically unrelated soft-matter systems (PMMA colloids, hard-sphere colloids, sucrose-water, glycerol-water, BSA, lysozyme, PEG-water, dextran, casein micelles, Ludox silica) yielded $\beta = 1.72 \pm 0.37$ with $R^2 > 0.986$. The substrate-level prediction (P04 mobility-capacity bound produces functional form $M(\rho) = M_0(1-\rho/\rho_\mathrm{max})^\beta$ with substrate-natural $\beta = 2$) is empirically confirmed within one standard deviation, with mechanism-clustered scatter consistent with the substrate-level account. This is the program's most quantitatively-tested structural prediction.
\end{itemize}

\subsection{15.4.2 What PASSED status delivers and what it does not deliver}

PASSED status delivers:
\begin{itemize}
\item \textbf{Empirical confirmation of a substrate-level structural prediction.} The empirical content matches the prediction within stated uncertainty.
\item \textbf{Cross-sector visibility of empirical anchoring.} PASSED tests in different sectors (gravity, QC, soft-matter) demonstrate that the framework's empirical content reaches multiple sectors.
\item \textbf{Evidence at architectural level.} The PASSED tests confirm form-FORCED content (the dwarf-galaxy active-versus-quiet ratio; the multi-timescale FPM relaxation behavior; the Universal Mobility Law functional shape).
\end{itemize}

PASSED status does \textit{not} deliver:
\begin{itemize}
\item \textbf{Full empirical validation of the framework.} Three PASSED tests across nine sectors is not full empirical validation.
\item \textbf{Discrimination from all alternative theories.} Each PASSED test discriminates between the substrate-level prediction and specific alternatives, but not all alternatives.
\item \textbf{Closed-form derivation of inherited values.} PASSED tests confirm the form-FORCED structure; they do not derive the value-INHERITED specific numerical content (the precise empirical $\beta$ value; the precise dwarf-galaxy ratio).
\end{itemize}

\section{15.5 ANCHORED Predictions}

\subsection{15.5.1 The thirteen ANCHORED predictions}

Thirteen predictions have status ANCHORED — calibrated to existing observation:

\begin{itemize}
\item \textbf{Matter-wave Q-C boundary as multiplicity wall} (Chapter 7). Empirical 140–250 kDa boundary is the natural anchor for $\mathcal{M}_\mathrm{crit}$.
\item \textbf{Newton's $G$ from substrate} (Chapter 11). $G = c^3\ell_P^2/\hbar$ identifies the substrate length scale with the Planck length through Newton-recovery; numerical content is empirically anchored.
\item \textbf{MOND $a_0$ within 10\%} (Chapter 11). $a_0 = c\,H_0/(2\pi)$ matches empirical MOND constant within ~10\% parameter-free; Hubble-tension band 67–73 km/s/Mpc translates to ~15\% prediction band on $a_0$.
\item \textbf{Slope-4 Tully–Fisher relation} (Chapter 11). $v^4 = G\,M\,a_0$ derived from substrate primitives; empirically anchored against SPARC galaxy-rotation-curve catalog (~150 galaxies, sub-0.1-dex scatter dominated by baryonic-mass measurement uncertainty).
\item \textbf{Hyper-coherence in ultra-symmetric superconductors} (Chapter 7, ED-I-01). Bulk superconductors below $T_c$ exhibit zero resistance, Meissner effect, and flux quantization, all reproduced under Class A substrate-level reading.
\item \textbf{Josephson MQT WKB structure recovered from DCGT} (Chapter 7, Chapter 8). Devoret-Martinis-Clarke 1985 measurements of MQT-rate exponential dependence on barrier parameters reproduced as DCGT $\kappa$-function evaluation at the engineered $\sigma$-profile.
\item \textbf{Krieger–Dougherty divergence} (Chapter 10). Empirical viscosity divergence near suspension packing limit reproduced as substrate-level mobility-capacity bound applied to volume fraction.
\item \textbf{Discontinuous shear-thickening} (Chapter 10). Empirical DST reproduced as P04 mobility-capacity bound applied to strain rate at the saturation regime.
\item \textbf{BH area-law entropy form} (Chapter 13). Bekenstein-Hawking $S \propto A/\ell_P^2$ reproduced as substrate-level participation-capacity at the decoupling surface; 1/4 coefficient inherited at $\log g \approx 1/4$.
\item \textbf{BHPT phase-shift structure} (Chapter 13). Standard black-hole-perturbation-theory phase shifts reproduced as global path-integrated invariants of minimal ED-channels through the strong-curvature region.
\item \textbf{Universal horizon mechanism} (Chapter 13). Thermodynamic-style features shared across BH / Rindler / cosmic / acoustic horizons reproduced as projections of one substrate decoupling-surface phenomenon.
\item \textbf{High-Tc superconductors as intrinsically low-multiplicity} (Chapter 7, ED-I-01). Three predicted features (short coherence lengths, unconventional pairing symmetries, complex phase diagrams) match observed properties of high-Tc materials.
\item \textbf{Cross-domain $\Gamma_\mathrm{cross}$ collapse} (Chapter 14). Same DCGT-derived exponential structure unifies BH-2 horizon formation and QC condition (ii) failure across ~50 orders of magnitude.
\item \textbf{Form-FORCED / value-INHERITED methodology} (Chapter 14). Cross-domain consistency requirement on closed-form-substrate-constants derivations.
\end{itemize}

\subsection{15.5.2 What ANCHORED status delivers and what it does not deliver}

ANCHORED status delivers:
\begin{itemize}
\item \textbf{Calibration against existing empirical content.} The substrate-level prediction matches existing observation in functional form or numerical range.
\item \textbf{Reframing of standard physics phenomena under substrate-level analysis.} Each ANCHORED prediction reproduces a standard-physics phenomenon as a consequence of substrate primitives plus closed-arc machinery.
\item \textbf{Empirical anchoring without discriminating-from-standard-physics claims.} ANCHORED predictions are typically not sharp discriminating tests against alternative theories; they are reframings that preserve the empirical content.
\end{itemize}

ANCHORED status does \textit{not} deliver:
\begin{itemize}
\item \textbf{Discriminating-from-standard-physics empirical content.} ANCHORED predictions match standard physics where standard physics already produces correct empirical content; they do not discriminate.
\item \textbf{Full empirical confirmation at the substrate-level mechanism layer.} The mechanism is reframed; the empirical content is anchored; discriminating tests at the substrate-level layer are typically ACTIVE or OPEN.
\end{itemize}

\section{15.6 IN PROGRESS Predictions}

\subsection{15.6.1 The three IN PROGRESS predictions}

Three predictions have status IN PROGRESS — active experimental program awaiting result:

\begin{itemize}
\item \textbf{SC-qubit system-multiplicity wall} (Chapter 7). Multiple SC-qubit programs are scaling toward the substrate-determined wall at platform-specific $N_\star$. As $\tau_\mathrm{QC}^\mathrm{logical}$ is measured at increasing physical-qubit counts at fixed code distance, the substrate-level prediction is that pure-Class-A scaling will plateau at $N_\star$. The empirical content is being generated by ongoing scaling efforts; the prediction will be tested as scaling continues.
\item \textbf{FRAP $t^{1/6}$ scaling at high BSA} (Chapter 10). Pre-registered FRAP measurement at high BSA concentration tests the substrate-level prediction of non-Fickian recovery with $R(t) \sim t^{1/6}$ scaling at deep concentrations, distinct from standard Fickian $t^{1/2}$. Quote received April 2026 from contract lab; technician-team review pending. Empirical content expected within defined window.
\item \textbf{Quantum error correction as Class A + Class C composition} (Chapter 7). Surface-code-based fault-tolerant QC programs are testing whether logical-qubit lifetimes scale as substrate-level meta-architectural composition predicts. The empirical content is being generated by ongoing fault-tolerant QC efforts.
\end{itemize}

\subsection{15.6.2 What IN PROGRESS status delivers}

IN PROGRESS status delivers:
\begin{itemize}
\item \textbf{Predicted empirical content within a defined window.} The experimental program is running; results are expected.
\item \textbf{Direct testability of substrate-level predictions.} Each IN PROGRESS test has explicit confirmation/refutation conditions defined in advance.
\item \textbf{Forward-looking empirical anchoring.} IN PROGRESS tests will become PASSED or REFUTED upon experimental completion; the empirical posture sharpens with time.
\end{itemize}

\section{15.7 ACTIVE Predictions}

\subsection{15.7.1 The seventeen ACTIVE predictions}
...
\textbf{Quantum sector (Chapter 7):}
\begin{itemize}
\item Mass-matched isomer matter-wave test (mass-matched isomers with different DOF structure should show different fringe behavior at fixed mass).
\item Cross-platform $\mathcal{M}_\mathrm{crit}$ calibration (consistency check between matter-wave and qubit-system anchors).
\item Class C correlation-budget plateau (multi-axis photonic platforms beyond ~3-5 axes; bosonic codes beyond a code-distance threshold).
\item Bell correlation degradation with multiplicity (entanglement-based correlations should degrade with system multiplicity load even at fixed environmental coupling).
\end{itemize}

\textbf{Gravity sector (Chapter 11):}
\begin{itemize}
\item Halo lag in cluster collisions (ED predicts lagging, sheared halo in Bullet-Cluster-class collisions).
\item Activity-dependent halo strength (galaxies with same baryonic mass but different dynamical activity should exhibit different halo strengths).
\item Post-starburst hysteresis (post-starburst galaxies should retain enhanced halos for relaxation timescales after activity subsides).
\item Reduced small-scale substructure (smoother halo profiles, fewer dwarf satellites, weaker strong-lensing flux anomalies than CDM predicts).
\end{itemize}

\textbf{Condensed matter and photonics sector:}
\begin{itemize}
\item Mesoscopic transport threshold (non-analytic kink in $G(L)$ at substrate-determined critical channel length).
\item SC phase-stiffness saturation (early-onset plateau in $J(T)$ before $T_c$).
\item Casimir-gradient saturation (force plateau at sub-100 nm separations).
\item Microresonator linewidth asymmetry (asymmetric Lorentzian splitting at substrate-determined threshold).
\item Photonic crystal ED-gradient limit (saturation of field confinement at substrate-determined mode volume).
\item Multi-timescale comb formation threshold (sharp threshold transition for stable comb formation).
\item ED-limited soliton step (saturation of soliton step height at substrate gradient ceiling).
\end{itemize}

\textbf{Phononics sector:}
\begin{itemize}
\item Chiral-phonon ED-vorticity (orbital current scaling with ED-tension and phonon vorticity, not phonon population).
\item Orbital Seebeck (current scaling with ED-tension rather than phonon number).
\end{itemize}

\subsection{15.7.2 What ACTIVE status delivers}

ACTIVE status delivers:
\begin{itemize}
\item \textbf{Specified tests} at sufficient detail for experimental execution.
\item \textbf{Future empirical content} when experimental programs run the tests.
\item \textbf{Cross-sector coverage} of the framework's empirical predictive content.
\end{itemize}
...
\section{15.8 OPEN Predictions}

\subsection{15.8.1 The four OPEN predictions}
...
\item \textbf{Hawking spectrum} (Chapter 13). Match to $T_H = \kappa/(2\pi)$ as substrate-level structural recovery; deviation as falsifier. Empirical test of black-hole evaporation spectrum requires astronomical observations beyond current capability; analog-gravity laboratory experiments may provide partial empirical content.
\item \textbf{No-singularity interior observation} (Chapter 13). Substrate-level account predicts no singularities in BH interiors. Direct empirical test is not currently feasible; indirect tests (late-time evaporation tail behavior, gravitational-wave observations of BH-merger interiors, analog-gravity simulations) require development.
\item \textbf{ED-entrainment universality across rotating fields} (Chapter 9, Chapter 14). Any rotating ED-flow structure should entrain overlapping ED-flow loops. Empirical test using non-phononic rotating fields (optical vortex beams, exciton vortices, polariton vortices, acoustic vortices, mechanical micro-rotors) requires development of the experimental setups.
\end{itemize}

\subsection{15.8.2 What OPEN status delivers}

OPEN status delivers:
\begin{itemize}
\item \textbf{Stated predictions} that future technology may test.
\item \textbf{Long-horizon empirical content} for the framework.
\item \textbf{Targets for technique development.} The OPEN tests identify what experimental techniques would need to be developed to test specific substrate-level predictions.
\end{itemize}

\section{15.9 The Open Extensions}

\subsection{15.9.1 B4 Hawking spectrum — the next natural arc}

The next natural concrete arc following Arc BH closure (Chapter 13) is \textbf{B4 Hawking spectrum derivation}. The components in hand:

\begin{itemize}
\item \textbf{The mechanism} (Chapter 13, BH-4): evaporation as participation re-routing through pair-creation events at the saturated decoupling surface.
\item \textbf{The substrate-level structural framework} for the spectrum derivation (BH-2's universal horizon mechanism plus BH-4's information-architecture content).
\item \textbf{The V5 cross-chain memory kernel} (Chapter 1, Chapter 2) as the substrate-level object whose cross-chain correlation calculation produces the spectrum.
\end{itemize}
...
\subsection{15.9.2 The closed-form-substrate-constants program}

Three structurally similar open items form the \textbf{closed-form-substrate-constants program}:

\begin{itemize}
\item \textbf{O-QC-1: closed-form $\mathcal{M}_\mathrm{crit}$.} Anchored against the matter-wave Q-C boundary at 140–250 kDa molecular mass. Closed-form derivation requires V1 kernel + ED-I-01 substrate constants combined into a structural argument that yields the specific value of $\mathcal{M}_\mathrm{crit}$.
\item \textbf{O2: closed-form $\log g$.} Anchored against the Bekenstein-Hawking $1/4$ coefficient. Closed-form derivation requires substrate-level motif counting beyond per-patch independence — either an explicit V1-kernel-counting argument or a substrate-level partition function.
\item \textbf{E4: closed-form $\kappa/|\hat{N}'| \approx 0.001766$.} Anchored against the ED-SC arc's empirical content. Closed-form derivation requires substrate constants from cross-scale invariance content.
\end{itemize}
...
\subsection{15.9.3 Architecture-to-platform calibration (O-QC-2)}
...
\begin{itemize}
\item \textbf{Map architecture-specific scaling functions} $f_\mathrm{sys}^{(A)}(\mathcal{S})$ for each platform.
\item \textbf{Predict $N_\star$ values} via $f_\mathrm{sys}^{(A)}(\mathcal{S}_\star) = \mathcal{M}_\mathrm{crit}$ at the substrate-shared $\mathcal{M}_\mathrm{crit}$.
\item \textbf{Provide cross-platform consistency checks} by comparing the calibration against scaling experiments at each platform.
\end{itemize}
...
\subsection{15.9.4 Topology-stability theorem for Class B (O-QC-4)}
...
\begin{itemize}
\item \textbf{Substrate-level account of $\tau_\mathrm{gap-stab}(\mathcal{T})$} for canonical topological structures.
\item \textbf{Connection to the ED-I-14 topological-effects content} (Chapter 6) at the substrate level.
\item \textbf{Predictive content for Class B platforms.} The theorem would supply the substrate-level structural prediction for topological-qubit coherence times beyond the gap-suppressed $\exp(\Delta_\mathrm{top}/T_\mathrm{eff})$ enhancement.
\end{itemize}
...
\subsection{15.9.5 Other O-QC items}

Three additional O-QC items carry forward from Arc Q-COMPUTE:

\begin{itemize}
\item \textbf{O-QC-3: closed-form $g(N), h(N), c(N)$ for Class C platforms.} Currently INHERITED; closed-form derivation requires extending DCGT to multi-axis redundancy structure.
\item \textbf{O-QC-5: surface-code logical-qubit recursive-overlay derivation.} Specific scaling of $\tau_\mathrm{QC}^\mathrm{logical}$ with code distance and threshold from the substrate-level error-correction-as-meta-architecture analysis.
\item \textbf{O-QC-6: hybrid-architecture handoff dynamics.} SC-photonic interconnects, topological-SC platforms, and other hybrid architectures' handoff boundaries as substrate-geometric transitions.
\end{itemize}

Each is a bounded structural extension of Chapter 7's QC architectural content. None requires new substrate primitives.

\subsection{15.9.6 Three O items from Arc BH}

Three O items carry forward from Arc BH:

\begin{itemize}
\item \textbf{O1: superradiance amplitude derivation.} Structurally compatible with BH-6's substrate-level reading; full amplitude derivation requires extending BH-6 + Arc SG.
\item \textbf{O2: closed-form $\log g$.} Part of the closed-form-substrate-constants program (Section 15.9.2).
\item \textbf{O3: full Kerr interior audit.} BH-3 establishes saturated-zone interior structure for Schwarzschild-class BHs; rotating-BH interior architecture (ergosphere, Cauchy horizons, frame-dragging at saturation) requires extension.
\end{itemize}

\subsection{15.9.7 Arc COSMO — speculative-not-refuted}

\textbf{Arc COSMO} is the cosmic-expansion / $H_0$-from-substrate program. The structural target: derive cosmic expansion at substrate level, including possible substrate-level Friedmann-class equations, $H_0$ derivation, expansion-history dynamics, $\Lambda$ value, Hubble tension.

The content:
\begin{itemize}
\item \textbf{Substrate-level cosmic-horizon dynamics.} Currently the cosmic horizon enters T20 (Chapter 11) as a substrate input characterizing distance $R_H = c/H_0$; Arc COSMO would derive the cosmic-horizon dynamics from substrate primitives.
\item \textbf{$H_0$ from substrate.} Currently the Hubble rate is a substrate input; Arc COSMO would derive $H_0$ from substrate constants.
\item \textbf{Hubble tension.} The 67-versus-73 km/s/Mpc empirical disagreement may have a substrate-level structural origin reachable from the framework's primitives.
\end{itemize}

Arc COSMO is \textbf{speculative-not-refuted}. The framework treats it as a long-horizon target; current substrate-level scope does not yet include the substrate-level cosmological-scale dynamics required to address it. Possible scope: 15–25 memos if pursued.

\subsection{15.9.8 GR-4A — Einstein-equation emergence (speculative-not-refuted)}
...
The status:
\begin{itemize}
\item \textbf{Speculative-not-refuted.} Promotion to full GR requires either a new substrate primitive or a derivation strategy not yet identified.
\item \textbf{If forced}, the framework becomes a gravity theory at field-equation level.
\item \textbf{If refuted}, the kinematic ceiling is permanent; the framework remains at Arc ED-10's acoustic-metric scalar-tensor level for relativistic-gravity content.
\end{itemize}
...
\section{15.10 The Overall Empirical Map}

\subsection{15.10.1 Forty empirical claims across nine sectors}
...
\begin{itemize}
\item \textbf{Quantum sector (Chapters 5–7):} approximately 10 empirical claims (matter-wave Q-C boundary, hyper-coherence in SC, Josephson MQT WKB, multi-timescale FPM relaxation, SC qubit wall, FRAP, QEC composition, mass-matched isomers, cross-platform calibration, Class C plateau, Bell correlation, Class B exponential, plus others).
\item \textbf{Gravity sector (Chapters 11–12):} approximately 10 empirical claims (Newton's $G$, MOND $a_0$, slope-4 Tully-Fisher, dwarf-galaxy Active>Quiet, halo lag, activity-dependent halo, hysteresis, reduced substructure, Hawking spectrum, no-singularity interior).
\item \textbf{Condensed-matter and photonics sector (Chapter 8 plus extensions):} approximately 10 empirical claims (mesoscopic transport, SC phase-stiffness, Casimir-gradient, microresonator linewidth, photonic-crystal limit, comb formation, soliton step, Krieger-Dougherty, DST, plus others).
\item \textbf{Soft-matter sector (Chapter 10):} approximately 3 empirical claims (Universal Mobility Law, FRAP $t^{1/6}$, Krieger-Dougherty + DST shared exponent).
\item \textbf{Phononics sector:} approximately 3 empirical claims (chiral-phonon vorticity, orbital Seebeck, ED-entrainment universality).
\item \textbf{Cross-domain sector (Chapter 14):} approximately 4 empirical claims (universal horizon mechanism, cross-domain $\Gamma_\mathrm{cross}$ collapse, form-FORCED methodology, BHPT structure).
\end{itemize}
...
\subsection{15.10.2 The empirical-status frontier}

The program's empirical-status frontier consists of:
\begin{itemize}
\item \textbf{Three PASSED claims} confirming form-FORCED content in three sectors.
\item \textbf{Thirteen ANCHORED claims} reframing existing empirical content under substrate-level analysis.
\item \textbf{Three IN PROGRESS claims} in active experimental programs.
\item \textbf{Seventeen ACTIVE claims} awaiting experimental execution.
\item \textbf{Four OPEN claims} awaiting technique development.
\end{itemize}
...
\subsection{15.10.3 What the inventory means for future work}
...
\begin{itemize}
\item \textbf{Empirical work on ACTIVE tests} would advance the framework's empirical anchoring.
\item \textbf{Engineering progress on OPEN tests} would convert OPEN status to ACTIVE.
\item \textbf{Closed-form derivation of inherited substrate constants} would complete the form-FORCED / value-INHERITED methodology at numerical-content level.
\end{itemize}

The inventory therefore serves as both an empirical interface for readers and a structural roadmap for future work.

\section{15.11 The Open-Extensions Map}

\subsection{15.11.1 Future architectural-level arcs}

The named open extensions identify future architectural-level arcs and their relationship to current closed-arc content:

\begin{itemize}
\item \textbf{B4 Hawking spectrum} — next natural concrete arc, builds directly on Chapter 13.
\item \textbf{Arc COSMO} — long-horizon arc, extends Chapter 11's substrate-gravity content to cosmological scales.
\item \textbf{GR-4A} — speculative long-horizon arc, would extend Chapter 12's curvature-emergence content to full GR.
\end{itemize}
...
\subsection{15.11.2 Cross-arc relationships}

The open extensions are structurally connected:

\begin{itemize}
\item \textbf{O-QC-1 (closed-form $\mathcal{M}_\mathrm{crit}$) and O2 (closed-form $\log g$) and E4 (closed-form $\kappa/|\hat{N}'|$)} are the closed-form-substrate-constants program. Cross-domain consistency constrains the three derivations to mutually consistent substrate-constant values.
\item \textbf{B4 (Hawking spectrum) and O1 (superradiance amplitude) and O3 (full Kerr interior)} are all extensions of Chapter 13's BH-architectural content; the three combine to produce a more complete BH sector.
\item \textbf{O-QC-2 (architecture-to-platform calibration) and O-QC-4 (topology-stability theorem)} are platform-level extensions of Chapter 7's QC architectural content.
\end{itemize}
...
\subsection{15.11.3 The program's overall future}

The program's overall future trajectory:

\begin{itemize}
\item \textbf{Short-horizon (1–2 years):} Empirical work on ACTIVE tests; experimental confirmation of IN PROGRESS tests; B4 Hawking spectrum derivation; closed-form-substrate-constants progress.
\item \textbf{Medium-horizon (3–5 years):} Architecture-to-platform calibration program (O-QC-2); topology-stability theorem (O-QC-4); other O-QC and O items; further empirical anchoring of the cross-platform identity.
\item \textbf{Long-horizon (5+ years):} Arc COSMO; GR-4A (if reachable); the program's substrate-level scope expanding to cosmological-scale dynamics.
\end{itemize}
...
\section{15.12 What This Chapter Changes (And What It Doesn't)}

\subsection{15.12.1 What does not change}
...
\subsection{15.12.2 What does change}

The chapter establishes:
\begin{itemize}
\item \textbf{The status taxonomy} as a uniform framework for empirical-status reporting across sectors.
\item \textbf{The empirical interface} to the program for readers who want the empirical content distilled.
\item \textbf{The open-extensions map} as a structural roadmap for future work.
\end{itemize}
...
\section{15.13 Form-FORCED vs Value-INHERITED at the Empirical Synthesis}

\subsection{15.13.1 What is form-FORCED at this layer}

\begin{itemize}
\item \textbf{The status taxonomy} (PASSED / ANCHORED / IN PROGRESS / ACTIVE / OPEN) as a uniform empirical-status framework across sectors.
\item \textbf{The cross-domain consistency requirement} on closed-form-substrate-constants derivations (form-FORCED by the form-FORCED / value-INHERITED methodology).
\item \textbf{The named open-extensions list} (B4, closed-form-substrate-constants program, architecture-to-platform calibration, topology-stability theorem, Arc COSMO, GR-4A, plus six O-QC and three O items) as the program's structural roadmap.
\item \textbf{The structural relationships} between open extensions (the closed-form-substrate-constants program's mutual constraints; the BH-extension cluster's structural connections; the QC-extension cluster's platform-level relationships).
\end{itemize}

\subsection{15.13.2 What is value-INHERITED at this layer}

\begin{itemize}
\item \textbf{The current status counts} (3 PASSED, 13 ANCHORED, 3 IN PROGRESS, 17 ACTIVE, 4 OPEN). These reflect the empirical state at time of writing; the counts will evolve as experimental programs run.
\item \textbf{The specific empirical anchors} for the ANCHORED predictions (matter-wave 140–250 kDa boundary; SPARC catalog; Devoret-Martinis-Clarke 1985; Hafezi 2025; ten-system UDM data; Bekenstein-Hawking 1/4 coefficient; etc.).
\item \textbf{The future trajectory} of any specific test (whether ACTIVE tests will succeed in becoming PASSED; whether OPEN tests will transition to ACTIVE).
\end{itemize}

\subsection{15.13.3 What is open}

\begin{itemize}
\item \textbf{All ACTIVE tests} await experimental execution. Future work will determine outcomes.
\item \textbf{All OPEN tests} await technique development. Long-horizon work.
\item \textbf{All open extensions} are downstream programs. The framework's current scope is closed at architectural level for the nine sectors; the open extensions identify future work.
\end{itemize}

\section{15.14 Dependencies}

\subsection{15.14.1 Upstream}

\begin{itemize}
\item \textbf{All preceding chapters.} The empirical content synthesized in this chapter is generated by Chapters 5–13. No single chapter is the sole upstream dependency.
\item \textbf{Chapter 14 (cross-platform unifications).} The form-FORCED / value-INHERITED methodology and the cross-domain consistency requirement on closed-form-substrate-constants derivations are established in Chapter 14 and applied here.
\end{itemize}

\subsection{15.14.2 Downstream}
...
\section{15.15 Canonical Sources}

\begin{itemize}
\item \texttt{Desktop/ED\_Public\_Test\_Inventory.md} (living test catalog)
\item \texttt{docs/Investigation\_Priority\_List.md} (active priority list with open extensions)
\item All Foundations papers and Synthesis papers for sector-specific predictions.
\end{itemize}

The Public Test Inventory is the program's living test catalog, updated as experimental programs run and new predictions are derived. The Investigation Priority List is the program's active priority list with open extensions identified. The Foundations papers (Substrate\_Gravity\_Foundations, Black\_Hole\_Foundations, Quantum\_Computing\_Foundations) and the NS Synthesis Paper supply sector-specific empirical content that this chapter synthesizes.
...
\section{15.16 Optional Figures}


\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_104_status_taxonomy.pdf}
\caption*{Figure 15.1 --- The status taxonomy. A five-row diagram. Each row corresponds to one status level (PASSED, ANCHORED, IN PROGRESS, ACTIVE, OPEN). Columns: status definition; structural meaning; example test from the inventory; current count. The figure makes visible the uniform taxonomy across sectors.}
\label{fig:fig_104_status_taxonomy}
\end{figure}


\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_105_status_counts_pie.pdf}
\caption*{Figure 15.2 --- The status counts pie chart. A pie chart showing the forty-claim distribution: 3 PASSED (green), 13 ANCHORED (blue), 3 IN PROGRESS (orange), 17 ACTIVE (yellow), 4 OPEN (red). The figure makes visible the program's empirical-status distribution at time of writing.}
\label{fig:fig_105_status_counts_pie}
\end{figure}


\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_106_empirical_claims_nine_sectors.pdf}
\caption*{Figure 15.3 --- Empirical claims across nine sectors. A grid: rows are the nine closed sectors (QM, QFT, QC, NS, MHD, YM, soft-matter, gravity, BH); columns are status levels. Cells contain the count of empirical claims per (sector, status). The figure makes visible the cross-sector distribution of empirical content.}
\label{fig:fig_106_empirical_claims_nine_sectors}
\end{figure}


\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_107_passed_anchors_summary.pdf}
\caption*{Figure 15.4 --- The PASSED-status anchors. A three-row diagram. Each row corresponds to one PASSED test (dwarf-galaxy Active > Quiet; multi-timescale FPM relaxation; Universal Mobility Law). Columns: chapter; sector; substrate-level prediction; empirical confirmation; first completed status. The figure makes visible the three foundational empirical anchors.}
\label{fig:fig_107_passed_anchors_summary}
\end{figure}


\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_108_closed_form_constants_program_v2.pdf}
\caption*{Figure 15.5 --- The closed-form-substrate-constants program. A three-node diagram with $\mathcal{M}_\mathrm{crit}$ (O-QC-1), $\log g$ (O2), and $\kappa/|\hat{N}'|$ (E4). Connecting lines indicate cross-domain consistency requirements. A central note observes that closed-form derivation in any one constrains the others.}
\label{fig:fig_108_closed_form_constants_program_v2}
\end{figure}


\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_109_open_extensions_roadmap.pdf}
\caption*{Figure 15.6 --- The open-extensions roadmap. A diagram showing future architectural-level arcs (B4, Arc COSMO, GR-4A) and bounded extensions (O-QC items, O items from Arc BH). Time-horizon labels: short (1-2 years), medium (3-5 years), long (5+ years). The figure makes visible the program's overall future trajectory.}
\label{fig:fig_109_open_extensions_roadmap}
\end{figure}


\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_110_empirical_form_value.pdf}
\caption*{Figure 15.7 --- Form-FORCED vs Value-INHERITED at empirical synthesis. A two-column diagram. Left column ("Form-FORCED"): status taxonomy uniform across sectors; cross-domain consistency requirement; named open-extensions list; structural relationships between open extensions. Right column ("Value-INHERITED"): current status counts; specific empirical anchors; specific future trajectories of individual tests.}
\label{fig:fig_110_empirical_form_value}
\end{figure}


\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_111_program_at_empirical_interface.pdf}
\caption*{Figure 15.8 --- The program at empirical interface. A diagram showing the program's empirical interface: nine closed sectors producing forty empirical claims at five status levels, with closed-form-substrate-constants program and open-extensions map identifying future work. A central note observes that the framework's empirical posture is sharp at structural form, calibrated at numerical value, with cross-domain consistency as a testable structural signature.}
\label{fig:fig_111_program_at_empirical_interface}
\end{figure}
...
\appendix
\chapter{Appendix A --- Theorem Provenance Map}
\label{app:A}

\section{A.1 Appendix Overview}
...
\textbf{Canonical source.} \texttt{papers/Gauge\_Fields\_Theorem\_17/}.
...
\section{A.6 T19 — Newton's Gravitational Constant from Substrate}

\textbf{Statement.} Newton's gravitational constant is fixed by substrate constants:
\[
G = \frac{c^3 \ell_P^2}{\hbar}.
\]
...
\textbf{Canonical source.} \texttt{papers/Substrate\_Gravity\_Foundations/} (T19 derivation).
...
\section{A.7 T20 — MOND Transition Acceleration from Cosmic-Horizon Dipole}

\textbf{Statement.} The MOND transition acceleration is fixed by substrate constants:
\[
a_0 = \frac{c\,H_0}{2\pi}.
\]
...
\textbf{Canonical source.} \texttt{papers/Substrate\_Gravity\_Foundations/} (T20 derivation).
...
\section{A.8 ECR — ED Combination Rule}

\textbf{Statement.} Newton-class accelerations $a_N$ and cosmic-horizon-class accelerations $a_0$ combine multiplicatively (geometric mean) in the deep-galactic regime:
\[
a = \sqrt{a_N \cdot a_0}.
\]
...
\textbf{Dependency chain.} \textbf{T19} + \textbf{T20} + substrate logarithmic stability landscape (substrate-level cross-term $\sqrt{G\,M\,a_0}\,\log(R/R_0)$ from the leading correction to Newton-class acceleration in the cosmic-horizon-modulated regime) → geometric-mean composition.

\textbf{Canonical source.} \texttt{papers/Substrate\_Gravity\_Foundations/} (ECR derivation).

\textbf{Used in chapters.} Chapter 11 (substrate gravity — ECR is the bridge between T19/T20 and T21); Chapter 12 (curvature emergence, where the modified Poisson equation $\nabla\cdot[\mu(|\nabla\Phi|/a_0)\nabla\Phi] = 4\pi G\rho_m$ contains ECR in the asymptotic deep-MOND limit).

\bigskip\hrule\bigskip

\section{A.9 T21 — Slope-4 Baryonic Tully–Fisher Relation}

\textbf{Statement.} The slope-4 baryonic Tully–Fisher relation is forced:
\[
...
\textbf{Dependency chain.} \textbf{T19} + \textbf{T20} + \textbf{ECR} + circular-orbit centripetal balance ($v^2/R = a$, with $a$ given by ECR in the deep-MOND asymptote) → $v^4 = G\,M\,a_0$.

\textbf{Canonical source.} \texttt{papers/Substrate\_Gravity\_Foundations/} (T21 derivation).

\textbf{Used in chapters.} Chapter 11 (substrate gravity — T21 is the empirical anchor of the galactic-scale gravity layer); Chapter 15 (public test inventory: BTFR slope-4 has PASSED; zero-intrinsic-scatter prediction is ANCHORED).

\bigskip\hrule\bigskip

\section{A.10 N1 — V1 Finite-Width Vacuum Kernel}

\textbf{Statement.} The V1 vacuum response kernel is finite-width and chain-sourced. The kernel mediates substrate-level participation events through an effective-vacuum response of finite temporal extent; its width is set by substrate constants and is not zero (point-instantaneous response is forbidden). Form-FORCED content includes: the existence of a finite-width vacuum kernel, its chain-sourced character, and its strict positivity on support. Value-INHERITED content includes: the precise width function and its first and second moments.

\textbf{Dependency chain.} Substrate primitives → effective-vacuum response factorization + finite-width temporal kernel via substrate event-discreteness (P01) and bandwidth update (P04).

\textbf{Canonical source.} Theorem N1 paper (V1 kernel inventory).
...
\section{A.11 GR1 — V1 Curved-Spacetime Extension (Phase-3)}

\textbf{Statement.} The V1 vacuum kernel extends to curved spacetime via Hadamard-parametrix causal-future restriction with the Synge world function. The curved-spacetime kernel inherits Theorem 18's forward-cone-only support and preserves P11 time orientation along curved geodesics. GR1 is structurally complete at the form level; full Einstein-equation emergence remains downstream work.

\textbf{Dependency chain.} \textbf{N1} + \textbf{T18} + Hadamard parametrix (the curved-spacetime analogue of the Minkowski Green's function) + Synge world function (geodesic interval substituting for $\sigma(x,y) = (x-y)^2/2$ on a curved background) → curved-spacetime V1 retarded.

\textbf{Canonical source.} Phase-3 GR1 derivation (paper under the substrate-gravity / curvature-emergence cluster).

\textbf{Used in chapters.} Chapter 12 (curvature emergence — GR1 is load-bearing); Chapter 13 (black-hole architecture — GR1 underwrites the curved-background V1 inside which decoupling surfaces form).

\bigskip\hrule\bigskip

\section{A.12 DCGT — Diffusion Coarse-Graining Theorem}

\textbf{Statement.} DCGT is the substrate-to-continuum bridge for canonical-ED dynamical content. Within the hydrodynamic-window scale separation
\[
\ell_P \ll R_\mathrm{cg} \ll L_\mathrm{flow},
\]
...
\textbf{Dependency chain.} Substrate primitives + hydrodynamic-window scale separation (Chapter 3) + multi-scale expansion → cross-bandwidth $\Gamma_\mathrm{cross}\sim\exp[-\alpha\sigma]$ + leading-order continuum content.

\textbf{Canonical sources.}
\begin{itemize}
\item Arc D memos in \texttt{theory/Arc\_D/}
\item \texttt{papers/Navier Stokes\_Synthesis\_Paper/} Appendix D (DCGT)
\item \texttt{papers/ED\_QFT\_Overview/} (program-level synthesis with DCGT central)
\end{itemize}
...
\section{A.13 UR-1 — Unresolved-Regime Characterization Theorem}

\textbf{Statement.} Three independently necessary substrate conditions characterize the unresolved-rule regime that quantum computation requires:

\begin{enumerate}
\item (i) bounded multiplicity ($\mathcal{M} \leq \mathcal{M}_\mathrm{crit}$),
\item (ii) sustained cross-endpoint connectivity ($\Gamma_\mathrm{cross} \geq \Gamma_\mathrm{min}$),
\item (iii) bounded commitment-injection ($\Lambda \leq \Lambda_\mathrm{V1}$).
\end{enumerate}
...
\textbf{Dependency chain.} Substrate primitives (especially \textbf{P11} commitment-irreversibility) + \textbf{DCGT} + \textbf{V1} + multiplicity-as-entropy reading (Chapter 2) → three-factor product form of $\mathcal{U}$ → three independent conditions.

\textbf{Canonical source.} \texttt{papers/Quantum\_Computing\_Foundations/}; Arc Q-COMPUTE Memo 2 in \texttt{theory/Quantum\_Computing/}.
...
\section{A.14 Provenance Graph (Topological Summary)}
...
\begin{itemize}
\item \textbf{P01–P13} → \textbf{N1} (V1 finite-width vacuum kernel)
\item \textbf{P01, P02, P04, P11, P13} + \textbf{Q.8 effective-vacuum factorization} + \textbf{N1} → \textbf{T18} (kernel-level arrow of time)
\item \textbf{P01–P13} + \textbf{Gleason 1957} + \textbf{Stone 1932} → \textbf{T1–T16} (Phase-1 closure of QM)
\item \textbf{P01–P13} + \textbf{DCGT} → \textbf{T17} (gauge-field-as-rule-type)
\item \textbf{P01–P13} + \textbf{hydrodynamic-window scale separation} → \textbf{DCGT}
\item \textbf{P01–P13} + \textbf{substrate cumulative-strain} + \textbf{holographic participation-count bound} → \textbf{T19} ($G$ from substrate)
\item \textbf{P01–P13} + \textbf{cosmic-horizon dipole projection} → \textbf{T20} ($a_0$ from cosmic horizon)
\item \textbf{T19} + \textbf{T20} + \textbf{substrate logarithmic stability landscape} → \textbf{ECR} (geometric-mean composition)
\item \textbf{T19} + \textbf{T20} + \textbf{ECR} + \textbf{circular-orbit balance} → \textbf{T21} (BTFR slope 4)
\item \textbf{N1} + \textbf{T18} + \textbf{Hadamard parametrix} + \textbf{Synge world function} → \textbf{GR1} (curved-background V1)
\item \textbf{P01–P13} (especially P11) + \textbf{DCGT} + \textbf{V1} + \textbf{multiplicity-as-entropy} → \textbf{UR-1} (three-factor unresolvedness)
\end{itemize}
...
\section{A.15 Canonical Sources (Aggregated)}
...
\item \texttt{papers/Time\_Arrow\_Theorem\_18/}
\item \texttt{papers/Substrate\_Gravity\_Foundations/}
\item \texttt{papers/Navier Stokes\_Synthesis\_Paper/} (Appendix D for DCGT)
\item \texttt{papers/Quantum\_Computing\_Foundations/}
\item \texttt{papers/ED\_QFT\_Overview/}
\item Theorem N1 paper (V1 kernel inventory)
\item Phase-3 GR1 derivation paper
\item \texttt{theory/Arc\_D/} (DCGT memos)
\item \texttt{theory/Quantum\_Computing/} (Arc Q-COMPUTE memos)
\item \texttt{theory/Substrate\_Gravity/} (substrate-gravity arc memos)
\item \texttt{arcs/arc-B/} (Arc B closure memos for T18)
\end{itemize}

\bigskip\hrule\bigskip

\section{A.16 Optional Figures}

The following diagrams are described for inclusion in the final monograph:

\begin{itemize}
\item \textbf{Figure A.1 — Provenance DAG.} Directed acyclic graph with nodes for each theorem (T1–T21, ECR, N1, GR1, DCGT, UR-1) and the substrate-primitive cluster {P01–P13}. Edges encode upstream dependencies. Color-coding: substrate primitives (P-cluster) in one shade; foundational theorems N1/GR1/DCGT/T18 in a second; QM-closure cluster T1–T16 in a third; substrate-gravity cluster T19/T20/ECR/T21 in a fourth; gauge/QC cluster T17/UR-1 in a fifth.
\item \textbf{Figure A.2 — Theorem-to-chapter incidence matrix.} Rows: theorems. Columns: chapters 1–15. Filled cells indicate "load-bearing." Used to verify the chapter dependency graph against the provenance graph.
\item \textbf{Table A.1 — Form-FORCED vs Value-INHERITED inventory.} For each theorem, two columns separating which content is form-FORCED and which is value-INHERITED, exactly as recorded in the entries above.
\end{itemize}

\bigskip\hrule\bigskip

\section{A.17 Dependency Section}
...
\chapter{Appendix B --- Notation Glossary}
\label{app:B}

\section{B.1 Appendix Overview}
...
\section{B.2 Substrate Quantities and Primitives}

\subsection{$\rho$ — Local participation density}
...
\subsection{$\Gamma_\mathrm{cross}$ — Substrate-mediated cross-bandwidth}
...
\subsection{$\mathcal{M}$ — Multiplicity}
...
\subsection{$\mathcal{U}(\mathcal{S},t) \in [0,1]$ — Participation-rule unresolvedness}
...
\subsection{$\Lambda$ — Local commitment-injection rate}
...
\subsection{V1, V5 — Finite-width vacuum kernel and cross-chain memory kernel}
...
\subsection{P1–P13 — The thirteen substrate primitives}

The irreducible substrate commitments. Notably P01 (event discreteness), P02 (chain worldline), P04 (bandwidth update), P11 (commitment-irreversibility), P13 (proper-time ordering); seven additional primitives covering participation, multiplicity, finite kernels, and substrate-locality conditions. Canonical inventory in \texttt{papers/Event\_Density\_Ontology\_and\_Axioms/}.

\subsection{R1 — Substrate-cutoff hyperviscous regularization}
...
\section{B.3 Length and Time Scales}

\subsection{$\ell_P$ — Planck length}
...
\subsection{$R_\mathrm{cg}$ — Coarse-graining length scale}
...
\subsection{$L_\mathrm{flow}$ — Continuum-flow length scale}
...
\subsection{$\tau_\mathrm{QC}$ — QC operating window}
...
\subsection{$\tau_{(\mathrm{i})}, \tau_{(\mathrm{ii})}, \tau_{(\mathrm{iii})}$ — UR-1 condition-failure timescales}
...
\subsection{$a_0$ — MOND transition acceleration (DERIVED)}
...
\subsection{$\alpha$ — DCGT prefactor}
...
\subsection{$\beta$ — Universal Mobility Law mobility exponent}
...
\section{B.5 UR-1 Thresholds (INHERITED)}
...
\subsection{$\mathcal{M}_\mathrm{crit}$ — Critical multiplicity threshold}
...
\subsection{$\Gamma_\mathrm{min}$ — Minimum cross-bandwidth}
...
\subsection{$\Lambda_\mathrm{V1}$ — V1 vacuum residual injection rate}
...
\subsection{$\beta_\mathrm{crit} \sim \log(R_\mathrm{cg}/\ell_P)$ — Critical gradient threshold}
...
\subsection{$N_\mathrm{corr}$ — Correlation budget}
...
\subsection{$\Delta_\mathrm{top}$ — Topological gap}
...
\subsection{$T_\mathrm{eff}$ — Substrate-equivalent perturbation temperature}
...
\section{B.6 UR-1 Functional Shapes (Form-FORCED, Shape INHERITED)}
...
\section{B.7 Multiplicity-Cap Function and Architectural Classes}

\subsection{$M(\mathcal{S}, K, \mathcal{E}, \mathcal{O})$ — Multiplicity-cap function}
...
\subsection{$K \in \{A, B, C\}$ — Architectural class}
...
\subsection{$\mathcal{O}$ — Meta-architectural overlay}
...
\subsection{$\mathcal{S}$ — Substrate-region system input}

The substrate-region characterization of the QC system itself.

\subsection{$\mathcal{E}$ — Environment input}

The substrate-region characterization of the QC system's environment.

\subsection{$\mathcal{M}_\mathrm{floor}, \gamma_\mathrm{floor}$ — Architectural floors}
...
\subsection{$A_S$ — Architectural restoring rate}

The class-dependent rate at which the architecture restores conditions (i)/(ii)/(iii) after perturbation.

\subsection{$f_\mathrm{int}, f_\mathrm{xy}, f_\mathrm{sys}^{(A)}$ — Architecture-specific scaling functions}

Dimensionless scaling functions in the multiplicity-cap projection for each architectural class.

\bigskip\hrule\bigskip

\section{B.8 Substrate-Gravity Quantities}

\subsection{$\Phi$ — Gravitational potential}
...
\subsection{$T$ — Trace of matter stress-energy}
...
\subsection{$A$ — Coarse-grained acoustic-metric area of $\Sigma_H$}
...
\subsection{$S$ — Black-hole entropy}
...
\subsection{$\tau_\mathrm{gap-stab}(\mathcal{T})$ — Class B topology-perturbation rate}
...
\section{B.10 Soft-Matter Mobility}

\subsection{$M(\rho)$ — Mobility (Universal Mobility Law)}
...
\subsection{$\rho_\mathrm{max}$ — Packing limit}

The maximum participation density at which mobility vanishes in UDM.

\subsection{$\tau_R$ — Maxwell viscoelastic relaxation time}
...
\section{B.11 Convention Notes}

\subsection{Symbol re-use across sectors}

Three symbols are re-used by convention in different sectors:

\begin{itemize}
\item \textbf{$\mu$}: appears as (i) the UR-1 multiplicity-headroom factor $\mu(x)$, (ii) the substrate-gravity interpolation function $\mu(|\nabla\Phi|/a_0)$, and (iii) the V1-amplitude prefactor $\mu_\mathrm{V1}$ in R1. Context disambiguates; sectorial reuse follows the source-paper canon.
\item \textbf{$M$}: appears as (i) the multiplicity-cap function $M(\mathcal{S}, K, \mathcal{E}, \mathcal{O})$, (ii) the soft-matter mobility $M(\rho)$, and (iii) the baryonic mass $M_b$. Context disambiguates.
\item \textbf{$\kappa$}: appears as (i) the UR-1 connectivity factor $\kappa(x)$, (ii) the BH surface gravity $\kappa$ in $T_H = \kappa/(2\pi)$, and (iii) the R1 prefactor $\kappa$ in $-\kappa\mu_\mathrm{V1}\ell_P^2\nabla^4\mathbf{v}$. Context disambiguates.
\end{itemize}

\subsection{Form-FORCED / Value-INHERITED Tagging}
...
\subsection{Repository Path Convention}

Source paths use \texttt{papers/...}, \texttt{theory/...}, and \texttt{arcs/...} exactly as in the chapter sources, with no prefix path component beyond the repository root \texttt{event-density/}.

\bigskip\hrule\bigskip

\section{B.12 Dependency Section}
...
\section{B.13 Canonical Sources}
...
\item \texttt{papers/Time\_Arrow\_Theorem\_18/}
\item \texttt{papers/Substrate\_Gravity\_Foundations/}
\item \texttt{papers/Navier Stokes\_Synthesis\_Paper/} (main + Appendices C/D/E)
\item \texttt{papers/Quantum\_Computing\_Foundations/}
\item \texttt{papers/Black\_Hole\_Foundations/}
\item \texttt{papers/Universal\_Mobility\_Law/}
\item \texttt{papers/P4\_NonNewtonian\_Paper\_Draft/}
...
\item \texttt{theory/Quantum\_Computing/} (Arc Q-COMPUTE memos)
\item \texttt{theory/Substrate\_Gravity/} (substrate-gravity arc memos)
\item \texttt{theory/Black\_Holes/} (Arc BH memos)
\item \texttt{theory/Yang\_Mills/} (Yang–Mills arc memos)
\item \texttt{arcs/arc-B/} (Arc B closure memos)
\end{itemize}

\bigskip\hrule\bigskip

\section{B.14 Optional Figures}

The following tables and diagrams are described for inclusion in the final monograph:

\begin{itemize}
\item \textbf{Table B.1 — Symbol-by-section incidence.} Rows: symbols. Columns: chapters 1–15. Filled cells indicate "used as load-bearing." Used to verify glossary completeness and chapter-by-chapter consistency.
\item \textbf{Table B.2 — Form-FORCED / Value-INHERITED tagging.} Two columns separating FORCED-structure entries from INHERITED-value entries. Highlights the closed-form-substrate-constants program targets ($\mathcal{M}_\mathrm{crit}$, $\log g$, $\alpha$, the UR-1 $\mu(x)$ and $\kappa(x)$ shapes).
\item \textbf{Figure B.1 — Symbol-reuse map.} Diagram showing the three convention-reused symbols ($\mu$, $M$, $\kappa$) and which sector each instance belongs to. Used to disambiguate notation under sectorial context.
\item \textbf{Table B.3 — Substrate-input vs derived-quantity inventory.} Two columns: substrate inputs ($\hbar$, $c$, $H_0$, $\ell_P$ at the program-level — note $\ell_P$ becomes derived under T19) versus derived quantities ($G$, $a_0$, $\Gamma_\mathrm{cross}$ form, $\mathcal{U}$ form, BTFR slope-4, area law form). Maps the FORCED/INHERITED tagging onto a clean substrate-input vs. derivation chart.
\end{itemize}

\newpage

\chapter{Appendix C --- Paper-to-Chapter Cross-Reference}
\label{app:C}

\section{C.1 Appendix Overview}
...
The cross-reference is consistent with Appendix A (theorem provenance map): every canonical-source path cited in Appendix A appears here under the chapter to which the source paper is assigned. It is consistent with Appendix B (notation glossary): every symbol defined in Appendix B has its canonical definition in a paper assigned here. It is consistent with Appendix D (substrate constants): every constant defined in Appendix D has its derivation or empirical anchor in a paper assigned here. Repository paths are stated relative to the repository root \texttt{event-density/}, exactly as in the chapter sources.

\bigskip\hrule\bigskip

\section{C.2 Part I — Substrate Foundations}

\subsection{Chapter 1 — The Substrate Ontology (P1–P13)}

\begin{itemize}
\item \textbf{\texttt{papers/Event\_Density\_Ontology\_and\_Axioms/}} — Canonical inventory of the thirteen substrate primitives and the axiomatic structure of the program. Every later result is reachable from a subset of P1–P13 plus mathematical structure.
\item \textbf{\texttt{papers/Foundations\_of\_Event\_Density/}} — Companion foundations collection: discusses the ontological commitments behind each primitive, the substrate-locality conditions, and the boundary between substrate ontology and emergent continuum physics.
\item \textbf{\texttt{papers/ED\_One\_Substrate\_Three\_Domains/}} — Program-overview / orientation context. Used in Chapter 1 only as a reading aid for situating the substrate ontology within the program's three-domain organization (substrate, continuum, gravity).
\end{itemize}

\subsection{Chapter 2 — Load-Bearing Invariants}

\begin{itemize}
\item \textbf{ED-I-01 (Superconductivity)} — Multiplicity-as-entropy reading. Establishes the structural identification between substrate multiplicity $\mathcal{M}$ and the entropic content of substrate participation pathways, anchored against superconducting condensate behavior.
\item \textbf{ED-I-23 (Josephson Junctions)} — Multiplicity at engineered low-$\mathcal{M}$ regions. Demonstrates the substrate-level mechanism by which engineered low-multiplicity regions sustain phase-coherent participation across a junction.
\item \textbf{ED-I-29 (Tunneling)} — Sparse-$\sigma$ regions and global rule reconfiguration. Connects the gradient-sparsity invariant $\sigma$ to the substrate-level mechanism of tunneling as a sparse-$\sigma$ rule reconfiguration rather than barrier penetration.
\item \textbf{ED-I-06 (Fields and Forces in Event Density)} — Ontological roof providing field/force vocabulary. The retroactive ontological frame that grounds the load-bearing invariants in a substrate-derived field/force taxonomy.
\end{itemize}

\subsection{Chapter 3 — The Coarse-Graining Bridge (DCGT)}

\begin{itemize}
\item \textbf{\texttt{papers/Navier Stokes\_Synthesis\_Paper/} Appendix D (DCGT)} — The canonical derivation of the Diffusion Coarse-Graining Theorem. Contains the multi-scale expansion, the hydrodynamic-window scale-separation conditions, and the leading-order continuum content (scalar diffusion, directional viscosity, V1→R1, V5→Maxwell, T17 minimal coupling).
\item \textbf{Arc D memos in \texttt{theory/Arc\_D/}} — Six-memo arc producing DCGT. Records the substrate-to-continuum bridge construction step by step.
\item \textbf{\texttt{papers/ED\_QFT\_Overview/}} — Program-level synthesis with DCGT central. Shows DCGT operating uniformly across the QM, QFT, NS, MHD, YM, soft-matter, and substrate-gravity sectors.
\end{itemize}

\subsection{Chapter 4 — Kernel-Level Arrow of Time (N1 + T18)}

\begin{itemize}
\item \textbf{\texttt{papers/Time\_Arrow\_Theorem\_18/}} — Canonical T18 derivation: V1 retardation forced at primitive level. Establishes the kernel-level arrow of time as a structural FORCED result rather than a postulated boundary condition.
\item \textbf{N1 derivation paper (V1 finite-width kernel)} — Canonical N1 derivation: existence and finite width of the V1 vacuum response kernel. Upstream of T18.
\item \textbf{\texttt{arcs/arc-B/}} — Arc B closure memos. Records the closure of the kernel-arrow arc; supplies the Q.8 effective-vacuum factorization that underwrites T18.
\end{itemize}

\bigskip\hrule\bigskip

\section{C.3 Part II — Quantum Sector}

\subsection{Chapter 5 — Phase-1 Closure of QM (T1–T16)}

\begin{itemize}
\item \textbf{\texttt{papers/Phase\_1/}} — Canonical Phase-1 closure paper: the sixteen forced theorems collectively deriving the four QM postulates from substrate primitives plus mathematical theorems (Gleason 1957, Stone 1932).
\item \textbf{\texttt{papers/QM\_Emergence\_Structural\_Completion/}} — Companion paper on the structural-completion methodology and the boundary between FORCED-form and INHERITED-value content in the QM emergence program.
\item \textbf{\texttt{papers/Born\_Gleason/}} — Detailed treatment of the Born-rule derivation via Gleason's theorem. The substrate-level argument for bandwidth-conservation across orthogonal decompositions, supplying Gleason's hypothesis.
\item \textbf{\texttt{papers/U1\_Participation\_Measure/}} — U1: the substrate-derived participation-measure structure underlying state-vector formalism.
\item \textbf{\texttt{papers/U2\_Inner\_Product/}} — U2: derivation of the inner-product structure on the participation-state space.
\item \textbf{\texttt{papers/U3\_Time\_Translation\_Schrodinger/}} — U3: time-translation symmetry plus Stone's theorem yielding the Schrödinger generator's linearity and first-order character.
\item \textbf{\texttt{papers/U4\_Hamiltonian\_Form/}} — U4: derivation of the Hamiltonian's substrate-level form.
\item \textbf{\texttt{papers/U5\_Translation\_Momentum/}} — U5: derivation of the canonical translation/momentum structure and the canonical commutator.
\end{itemize}

\subsection{Chapter 6 — Form-Level QFT and Quantum Information (T17, UV-FIN, ED-I-13)}

\begin{itemize}
\item \textbf{\texttt{papers/Gauge\_Fields\_Theorem\_17/}} — Canonical T17 derivation: gauge fields as participation measures of structural rule-types. Covers Abelian U(1) and non-Abelian compact-simple-group cases.
\item \textbf{ED-I-13 (Quantum Information: Channel Geometry)} — Quantum-information channel-geometry interpretation grounded in T17's interface-property reading.
\item \textbf{\texttt{papers/Arc\_Q/}} — Arc Q closure memos: gauge-field-as-rule-type arc.
\item \textbf{\texttt{papers/Arc\_R/}} — Arc R closure memos: substrate-level UV finiteness via V1 finite width.
\end{itemize}

\subsection{Chapter 7 — Quantum Computation (UR-1, $M$, A/B/C)}

\begin{itemize}
\item \textbf{\texttt{papers/Quantum\_Computing\_Foundations/}} — Canonical Q-COMPUTE Foundations Paper. Contains the UR-1 derivation, the multiplicity-cap function $M(\mathcal{S}, K, \mathcal{E}, \mathcal{O})$, the three-class architectural taxonomy A/B/C, and the closed-form-substrate-constants targets O-QC-1 through O-QC-6.
\item \textbf{Arc Q-COMPUTE memos in \texttt{theory/Quantum\_Computing/}} — Seven-memo arc producing the Q-COMPUTE Foundations Paper. Records the architectural-class derivation, the meta-architectural overlay structure, and the cross-domain echo with BH-2 ($\Gamma_\mathrm{cross}$ collapse).
\item \textbf{ED-I-14 (Topological Effects)} — Topological-effects interpretation underwriting the Class B (global-geometric-rigidity) projection of $M$.
\item \textbf{ED-I-18 (Multi-Timescale Photonics)} — Multi-timescale photonics interpretation; the Hafezi 2025 multi-timescale photonics anchor lives here as a PASSED prediction.
\item \textbf{ED-I-12 (Photonics)} — Photonics interpretation supplying the substrate-level reading of photonic platforms relevant to Class A and Class C realizations.
\end{itemize}

\bigskip\hrule\bigskip

\section{C.4 Part III — Continuum and Dynamics}

\subsection{Chapter 8 — Navier–Stokes Architectural Foundations}

\begin{itemize}
\item \textbf{\texttt{papers/Navier Stokes\_Synthesis\_Paper/} (main + Appendix C)} — The NS Synthesis Paper consolidates the full NS program: Path B-strong, the Intermediate Path C closure, the R1 substrate-cutoff regularization, the form-FORCED viscosity, and the vortex-stretching obstruction analysis. Appendix C covers the MHD extension (assigned to Chapter 9).
\item \textbf{Arc NS memos in \texttt{theory/Navier Stokes/}} — Records the NS-1, NS-2, NS-3, NS-Smoothness, NS-Turb, P4-NN, NS-Q, and NS-MHD arc closures.
\item \textbf{ED-I-06 (Fields and Forces)} — Ontological roof for the NS classification: identifies advection-non-ED and induction-kinematic-non-ED as transport-kinematic frame artifacts rather than substrate primitives.
\end{itemize}

\subsection{Chapter 9 — Magnetohydrodynamics and Yang–Mills}

\begin{itemize}
\item \textbf{\texttt{papers/Navier Stokes\_Synthesis\_Paper/} Appendix C (MHD)} — Full MHD treatment: the H1/H2/H3 hold conditions, the 6:2:3 classification of eleven MHD items into canonical-ED, transport-kinematic, and ED-coupled.
\item \textbf{\texttt{papers/Navier Stokes\_Synthesis\_Paper/} Appendix E (Yang–Mills)} — Full Yang–Mills treatment integrated into the NS Synthesis Paper. Contains the substrate-derived continuum YM equation $D_\mu F^{\mu\nu} = J^\nu$ via DCGT generalized to non-Abelian + T17, plus the mass-gap mechanism via V1 second-moment plus non-Abelian quartic stabilization.
\item \textbf{Arc YM memos in \texttt{theory/Yang\_Mills/}} — Six-memo arc YM-1 through YM-6 producing the YM closure. Records the structurally-positive Clay-relevance verdict and the OS-positivity audit.
\end{itemize}

\subsection{Chapter 10 — Soft-Matter Mobility and Non-Newtonian Rheology}

\begin{itemize}
\item \textbf{\texttt{papers/Universal\_Mobility\_Law/}} — Canonical Universal Mobility Law paper. Form-FORCED $M(\rho) \propto (1 - \rho/\rho_\mathrm{max})^\beta$ with canonical $\beta = 2$; empirical $\beta \approx 1.72 \pm 0.37$ across ten systems (PASSED).
\item \textbf{\texttt{papers/P4\_NonNewtonian\_Paper\_Draft/}} — P4-NN paper: substrate-derived Krieger-Dougherty form plus Maxwell viscoelastic memory via V5-DCGT.
\end{itemize}

\bigskip\hrule\bigskip

\section{C.5 Part IV — Gravity and Cosmology}

\subsection{Chapter 11 — Substrate Gravity at Galactic Scale (T19, T20, ECR, T21)}

\begin{itemize}
\item \textbf{\texttt{papers/Substrate\_Gravity\_Foundations/Substrate\_Gravity\_Foundations\_Paper.\{md,tex,pdf\}}} — Canonical Substrate Gravity Foundations Paper. Contains T19 ($G = c^3\ell_P^2/\hbar$), T20 ($a_0 = c\,H_0/(2\pi)$), the ED Combination Rule ($a = \sqrt{a_N\,a_0}$), and T21 (BTFR slope-4).
\item \textbf{\texttt{papers/Substrate\_Gravity\_Foundations/ED\_substrate\_gravity\_foundations\_2026-04-28.\{md,tex,pdf\}}} — Detailed-derivation companion paper. Records the dipole-projection derivation of $a_0$, the substrate-cumulative-strain mechanism for T19, and the substrate logarithmic stability landscape that produces ECR.
\end{itemize}

\subsection{Chapter 12 — Curvature Emergence (Arc ED-10)}

\begin{itemize}
\item \textbf{\texttt{papers/Substrate\_Gravity\_Foundations/}} — Curvature-emergence section in the extended-scope Substrate Gravity Foundations paper. Contains the substrate-FORCED scalar-tensor acoustic-metric covariantization $\nabla_\mu[\mu(\sqrt{g^{\alpha\beta}\nabla_\alpha\Phi\nabla_\beta\Phi}/a_0)\nabla^\mu\Phi] = 4\pi GT$, OS-positivity, ghost-freedom, and gradient-stability under (C1)-(C3).
\item \textbf{Arc ED-10 memos in \texttt{theory/Substrate\_Gravity/}} — Six-memo arc records the curvature-emergence closure; deep-MOND superluminality structurally FORCED as the only structural cost.
\end{itemize}

\subsection{Chapter 13 — Black-Hole Architecture (Arc BH)}

\begin{itemize}
\item \textbf{\texttt{papers/Black\_Hole\_Foundations/}} — Canonical Black Hole Foundations Paper. Contains the decoupling-surface construction, the area-law form $S = (A/\ell_P^2)\log g$, the BHPT phase-shift structure, the helicity behavior, and the Kerr-twist accumulated frame-dragging vorticity.
\item \textbf{Arc BH memos in \texttt{theory/Black\_Holes/} (BH-1 through BH-7)} — Seven-memo arc producing the BH Foundations Paper. Records the structural-positive verdicts on no-singularities, information blocking and entanglement straddling, evaporation as participation re-routing, and the open follow-ons (B4 Hawking spectrum, superradiance amplitude, closed-form $\log g$, full Kerr interior audit).
\end{itemize}

\bigskip\hrule\bigskip

\section{C.6 Part V — Empirical Synthesis}

\subsection{Chapter 14 — Cross-Platform Unifications and Methodology}

\begin{itemize}
\item \textbf{\texttt{papers/ED\_QFT\_Overview/}} — Program-level synthesis. Twelve sections integrating Phase-1 QM emergence + foundational theorems + DCGT + NS/MHD + Yang–Mills + substrate-gravity preview. The cross-platform unifications (matter-wave Q-C boundary ↔ qubit-system multiplicity wall via shared $\mathcal{M}_\mathrm{crit}$; $\Gamma_\mathrm{cross}$ collapse across BH-2 and QC condition (ii); P04 mobility-capacity bound across UDM and Class A QC; V1 across kernel-arrow, R1, YM mass-gap, and BH motif alphabet) are surveyed here.
\item \textbf{\texttt{papers/ED\_One\_Substrate\_Three\_Domains/}} — Program-overview / cross-domain context paper. Used in Chapter 14 to situate the cross-platform unifications within the program's three-domain organization.
\end{itemize}

\subsection{Chapter 15 — Public Test Inventory and Open Extensions}

\begin{itemize}
\item \textbf{\texttt{Desktop/ED\_Public\_Test\_Inventory.md}} — Living test catalog. Records the status taxonomy (PASSED / ANCHORED / IN PROGRESS / ACTIVE / OPEN) uniformly across sectors, with three PASSED predictions, thirteen ANCHORED predictions, three IN PROGRESS predictions, seventeen ACTIVE predictions, and four OPEN predictions.
\item \textbf{\texttt{docs/Investigation\_Priority\_List.md}} — Active priority list with open extensions. Records the current "what to work on next" priority list (twenty-three numbered items, easiest-to-hardest, with closed items archived).
\item \textbf{Various ED-I interpretation papers} — Sector-specific prediction sources. Each ED-I paper supplies the substrate-level reading for one sector and contributes the sector-specific predictions in the public test inventory.
\end{itemize}

\bigskip\hrule\bigskip

\section{C.7 Cross-Reference: ED-I Interpretation Papers}
...
\begin{itemize}
\item \textbf{ED-I-01 (Superconductivity)} → Chapter 2.
\item \textbf{ED-I-06 (Fields and Forces in Event Density)} → Chapter 2 (also load-bearing in Chapter 8).
\item \textbf{ED-I-12 (Photonics)} → Chapter 7.
\item \textbf{ED-I-13 (Quantum Information: Channel Geometry)} → Chapter 6.
\item \textbf{ED-I-14 (Topological Effects)} → Chapter 7.
\item \textbf{ED-I-18 (Multi-Timescale Photonics)} → Chapter 7.
\item \textbf{ED-I-23 (Josephson Junctions)} → Chapter 2.
\item \textbf{ED-I-29 (Tunneling)} → Chapter 2.
\end{itemize}
...
\section{C.8 Cross-Reference: Arc Memos and Closure Documents}
...
\begin{itemize}
\item \textbf{Arc B (Kernel-Arrow / V1 closure)} → Chapter 4. Memos in \texttt{arcs/arc-B/}.
\item \textbf{Arc D (DCGT)} → Chapter 3. Memos in \texttt{theory/Arc\_D/}. Six-memo arc producing the Diffusion Coarse-Graining Theorem.
\item \textbf{Arc Q (Gauge-Field-as-Rule-Type)} → Chapter 6. Memos in \texttt{papers/Arc\_Q/}.
\item \textbf{Arc R (Substrate UV Finiteness)} → Chapter 6. Memos in \texttt{papers/Arc\_R/}.
\item \textbf{Arc Q-COMPUTE} → Chapter 7. Seven-memo arc in \texttt{theory/Quantum\_Computing/}. Closed 2026-05-02.
\item \textbf{Arc NS (Navier–Stokes)} → Chapter 8. Multiple sub-arcs: NS-1, NS-2, NS-3 / NS-Smoothness, NS-Turb, P4-NN, NS-Q. Memos in \texttt{theory/Navier Stokes/}.
\item \textbf{Arc YM (Yang–Mills)} → Chapter 9. Six-memo arc in \texttt{theory/Yang\_Mills/}. Closed 2026-04-30.
\item \textbf{Arc SG (Substrate Gravity Extension)} → Chapter 11. Six-memo arc in \texttt{theory/Substrate\_Gravity/}. Closed 2026-04-30.
\item \textbf{Arc ED-10 (Curvature Emergence)} → Chapter 12. Six-memo arc in \texttt{theory/Substrate\_Gravity/}. Closed 2026-04-30.
\item \textbf{Arc BH (Black Holes)} → Chapter 13. Seven-memo arc (BH-1 through BH-7) in \texttt{theory/Black\_Holes/}. Closed 2026-05-01.
\end{itemize}

\bigskip\hrule\bigskip

\section{C.9 Dependency Section}
...
\item \texttt{papers/Universal\_Mobility\_Law/}
\item \texttt{papers/P4\_NonNewtonian\_Paper\_Draft/}
\item \texttt{papers/Substrate\_Gravity\_Foundations/Substrate\_Gravity\_Foundations\_Paper.\{md,tex,pdf\}}
\item \texttt{papers/Substrate\_Gravity\_Foundations/ED\_substrate\_gravity\_foundations\_2026-04-28.\{md,tex,pdf\}}
\item \texttt{papers/Black\_Hole\_Foundations/}
\item \texttt{papers/ED\_QFT\_Overview/}
\end{itemize}

\textbf{\texttt{theory/} cluster:}
\begin{itemize}
\item \texttt{theory/Arc\_D/}
\item \texttt{theory/Quantum\_Computing/}
\item \texttt{theory/Navier Stokes/}
\item \texttt{theory/Yang\_Mills/}
\item \texttt{theory/Substrate\_Gravity/}
\item \texttt{theory/Black\_Holes/}
\end{itemize}

\textbf{\texttt{arcs/} cluster:}
\begin{itemize}
\item \texttt{arcs/arc-B/}
\end{itemize}

\textbf{\texttt{docs/} and orientation:}
\begin{itemize}
\item \texttt{Desktop/ED\_Public\_Test\_Inventory.md}
\item \texttt{docs/Investigation\_Priority\_List.md}
\end{itemize}

\bigskip\hrule\bigskip

\section{C.11 Optional Figures}

The following tables and diagrams are described for inclusion in the final monograph:

\begin{itemize}
\item \textbf{Table C.1 — Paper-to-chapter assignment matrix.} Rows: every publication-grade paper in the program. Columns: chapters 1–15. Each row has exactly one filled cell (the chapter the paper is assigned to). Used to verify completeness of the cross-reference and prevent paper-orphaning.
\item \textbf{Table C.2 — ED-I interpretation paper inventory.} Rows: ED-I papers (numbered 01 through 29). Columns: primary chapter + secondary chapters + sectorial role. Records the full ED-I inventory and the interpretation-layer mapping into the chapter structure.
\item \textbf{Figure C.1 — Arc dependency graph.} Directed acyclic graph of arc closures: Arc B → T18 + N1; Arc D → DCGT; Arc Q → T17; Arc Q-COMPUTE → UR-1 + $M$; Arc NS → NS Synthesis; Arc YM → YM Synthesis; Arc SG → Substrate Gravity Foundations; Arc ED-10 → Curvature Emergence; Arc BH → BH Foundations. Edges encode "feeds into." Used to visualize how the arc-closure pipeline produces the publication-grade papers assigned in this appendix.
\item \textbf{Table C.3 — Repository-cluster summary.} Three rows (papers/, theory/, arcs/) with column counts of items per chapter. Used to verify that every chapter has at least one publication-grade source and at least one supporting arc/theory reference where applicable.
\end{itemize}

\newpage

\chapter{Appendix D --- Substrate Constants and Inherited Values}
\label{app:D}

\section{D.1 Appendix Overview}
...
\section{D.2 Substrate Inputs (Taken as Given)}
...
\textbf{Structural role.} Sets the absolute scale of canonical-conjugate uncertainty bounds (Heisenberg's $\hbar/2$ in T1–T16); enters T19's identification of $G$.

\textbf{Status.} Substrate input.

\textbf{Value.} $\hbar = 1.054 \times 10^{-34}$ J·s (CODATA).

\textbf{Used in.} Chapter 5 (Phase-1 closure of QM); Chapter 11 (substrate gravity, via T19).

\subsection{$c$ — Speed of light}

\textbf{Structural role.} The substrate-propagation speed for participation-rule connectivity; the asymptotic causal-cone speed for V1 retardation; enters T19 and T20 identifications.

\textbf{Status.} Substrate input.

\textbf{Value.} $c = 2.998 \times 10^8$ m/s (exact by SI definition).

\textbf{Used in.} Chapter 4 (kernel-arrow); Chapter 11 (substrate gravity, via T19 and T20); Chapter 12 (curvature emergence).

\subsection{$H_0$ — Hubble rate}

\textbf{Structural role.} Sets the cosmic-horizon participation density scale in T20.

\textbf{Status.} Substrate input.

\textbf{Value.} $H_0 \in [67, 73]$ km/s/Mpc (Hubble tension band). The Hubble-tension band translates to roughly a 15\% prediction band on $a_0$ via T20.

\textbf{Used in.} Chapter 11 (substrate gravity, via T20).

\subsection{$\ell_P$ — Planck length / substrate length scale}

\textbf{Structural role.} The substrate length scale: governs the dimensionless gradient sparsity $\sigma = |\nabla\rho|\,\ell_P^2/\rho_\mathrm{local}$, the BH area law $S = (A/\ell_P^2)\log g$, the substrate-cutoff hyperviscous prefactor in R1 ($-\kappa\mu_\mathrm{V1}\ell_P^2\nabla^4\mathbf{v}$), and the critical gradient threshold $\beta_\mathrm{crit}\sim\log(R_\mathrm{cg}/\ell_P)$.

\textbf{Status.} Identified with the Planck length via T19 Newton-recovery (derived, not postulated). Listed as a substrate input in this section because no still-more-fundamental substrate input fixes it; T19 fixes it by requiring the derived form $G = c^3\ell_P^2/\hbar$ to recover the empirically observed $G$.

\textbf{Value.} $\ell_P = (G\hbar/c^3)^{1/2} \approx 1.616 \times 10^{-35}$ m.

\textbf{Used in.} Chapters 2, 3, 7, 8, 9, 11, 12, 13.

\bigskip\hrule\bigskip

\section{D.3 Derived Substrate Constants (Form-FORCED, Value-DERIVED)}
...
\subsection{$G$ — Newton's gravitational constant}

\textbf{Structural role.} The proportionality constant in the inverse-square gravitational field. Enters every Newtonian-asymptote calculation in the substrate-gravity sector and the BTFR slope-4 relation.

\textbf{Status.} FORCED both in form (inverse-square) and in value (named combination of substrate inputs).

\textbf{Closed-form value.}
\[
G = \frac{c^3\,\ell_P^2}{\hbar} \approx 6.674 \times 10^{-11}\ \mathrm{m^3\,kg^{-1}\,s^{-2}}.
\]

\textbf{Source.} T19, in \texttt{papers/Substrate\_Gravity\_Foundations/}.

\textbf{Used in.} Chapter 11; Chapter 12; Chapter 13.

\subsection{$a_0$ — MOND transition acceleration}

\textbf{Structural role.} The deep-MOND-asymptote acceleration scale; the threshold below which the substrate-gravity interpolation function $\mu(|\nabla\Phi|/a_0)$ enters its deep-MOND limit; the proportionality constant in BTFR slope-4.

\textbf{Status.} FORCED both in form (cosmic-horizon dipole projection) and in value (named combination of substrate inputs).
...
\textbf{Source.} T20, in \texttt{papers/Substrate\_Gravity\_Foundations/}.

\textbf{Used in.} Chapter 11; Chapter 12; Chapter 15.

\bigskip\hrule\bigskip

\section{D.4 INHERITED Substrate Constants (Form-FORCED in Role; Specific Value Calibrated Empirically or Downstream)}
...
\textbf{Structural role.} The critical multiplicity above which the unresolved-rule regime fails; underwrites both the matter-wave Q-C boundary and the qubit-system multiplicity wall via the same substrate object. Form-FORCED by UR-1 condition (i).

\textbf{Status.} INHERITED. Value-anchored empirically by the matter-wave Q-C boundary at 140–250 kDa molecular mass (two-point Q-C mass extrapolation).

\textbf{Closed-form derivation status.} OPEN. Designated \textbf{O-QC-1} in the closed-form-substrate-constants program.

\textbf{Used in.} Chapter 7 (QC architectural taxonomy); Chapter 14 (cross-platform unifications: matter-wave ↔ qubit identity).

\subsection{$\Gamma_\mathrm{min}$ — Minimum cross-bandwidth}

\textbf{Structural role.} The minimum cross-bandwidth below which UR-1 condition (ii) fails; the threshold governing the Class C correlation-budget plateau and the BH decoupling-surface formation condition.

\textbf{Status.} INHERITED from V1-kernel + DCGT closed-form details.

\textbf{Used in.} Chapter 3 (DCGT); Chapter 7 (Class C correlation-budget plateau); Chapter 13 (decoupling-surface threshold).

\subsection{$\Lambda_\mathrm{V1}$ — V1 vacuum residual injection rate}

\textbf{Structural role.} The irreducible commitment-injection rate from the V1 vacuum kernel itself, present even in perfect environmental isolation. Bounds Class A's perfect-isolation ceiling: a Class A device cannot lower $\Lambda$ below $\Lambda_\mathrm{V1}$ regardless of engineering effort.

\textbf{Status.} INHERITED.

\textbf{Used in.} Chapter 7 (Class A perfect-isolation ceiling).

\subsection{$\beta_\mathrm{crit} \sim \log(R_\mathrm{cg}/\ell_P)$ — Critical gradient threshold}

\textbf{Structural role.} The critical sparsity threshold above which decoupling-surface formation becomes structurally favorable. Form-FORCED logarithmic dependence on the scale ratio $R_\mathrm{cg}/\ell_P$.

\textbf{Status.} INHERITED. Substrate-determined dimensionless number; precise prefactor depends on coarse-graining choices.

\textbf{Used in.} Chapter 7; Chapter 13.

\subsection{$N_\mathrm{corr}$ — Correlation budget for Class C platforms}

\textbf{Structural role.} The maximum number of correlated substrate channels available before Class C redundancy saturates. Sets the Class C correlation-budget plateau.

\textbf{Status.} INHERITED from substrate-coupling pattern across redundant pathways; platform-specific.

\textbf{Used in.} Chapter 7.

\subsection{$\Delta_\mathrm{top}^{\max}$ — Maximum stable topological gap (Class B)}

\textbf{Structural role.} The maximum topological gap that can be stably maintained against substrate-level perturbations in a Class B platform. Sets the upper bound on Class B coherent-computation duration.

\textbf{Status.} INHERITED from material/topology engineering. The Class B exponential gap-suppression prediction is governed by the rate at which $\Delta_\mathrm{top}$ closes under perturbation.

\textbf{Used in.} Chapter 7.

\subsection{$T_\mathrm{eff}^{\min}$ — Minimum substrate-equivalent perturbation temperature}

\textbf{Structural role.} The minimum effective substrate-equivalent temperature that environmental engineering can achieve in a given platform. Enters Class A and Class B failure routes through thermal commitment-injection.

\textbf{Status.} INHERITED from environmental engineering.

\textbf{Used in.} Chapter 7.

\subsection{$\log g$ — Black-hole area-law entropy coefficient}

\textbf{Structural role.} The coefficient in the form-FORCED BH area law $S = (A/\ell_P^2)\log g$. Determines the substrate-level analogue of the Bekenstein–Hawking $1/4$ coefficient.

\textbf{Status.} INHERITED from substrate motif counting. The empirical match $\log g \approx 1/4$ corresponds to $g \approx 1.28$, a non-integer substrate motif-count base.

\textbf{Closed-form derivation status.} OPEN. Designated \textbf{O2} in the Arc BH closed-form-substrate-constants targets.

\textbf{Used in.} Chapter 13.

\subsection{$\beta$ — Universal Mobility Law mobility exponent}

\textbf{Structural role.} The exponent in $M(\rho) = M_0(1 - \rho/\rho_\mathrm{max})^\beta$. The form-FORCED canonical substrate prediction is $\beta = 2$.

\textbf{Status.} Empirical anchor: $\beta = 1.72 \pm 0.37$ across ten systems with mechanism-clustered scatter (PASSED). The discrepancy from canonical 2 anchors a closed-form correction route as downstream work.

\textbf{Used in.} Chapter 10; Chapter 15.

\subsection{$\kappa/|\hat{N}'| \approx 0.001766$ — ED-SC anchor (cross-scale invariance)}

\textbf{Structural role.} The cross-scale invariance anchor of the ED-SC arc; central value 0.001766 with model band ±10.78\%. Records the empirically-measured cross-scale invariance ratio.

\textbf{Status.} INHERITED from cross-scale invariance.

\textbf{Closed-form derivation status.} OPEN. Designated \textbf{E4} in the ED-SC arc.

\textbf{Used in.} Cross-reference: ED-SC arc work, not within this monograph's nine sectors but flagged here for completeness because it is one of three structurally-similar INHERITED constants in the closed-form-substrate-constants program.

\bigskip\hrule\bigskip

\section{D.5 Class-Specific INHERITED Quantities}
...
\subsection{$\mu(x), \kappa(x)$ — UR-1 functional shapes}

\textbf{Structural role.} The multiplicity-headroom factor (monotone-decreasing from 1 to 0) and rule-spanning connectivity factor (monotone-increasing from 0 to 1) in the unresolvedness three-factor product. Form-FORCED to be monotone with specific limits.

\textbf{Status.} Specific shape (Boltzmann, rational, sigmoid, etc.) is INHERITED from V1-kernel + DCGT closed-form details.

\textbf{Used in.} Chapter 7.

\subsection{$g(N), h(N), c(N)$ — Class C redundancy modifier functions}

\textbf{Structural role.} The Class C scaling functions for redundancy, hybridization, and correlation. Form-FORCED in the Class-C projection of $M$.

\textbf{Status.} Specific shapes INHERITED.

\textbf{Used in.} Chapter 7.

\subsection{$f_\mathrm{int}(M_\mathrm{mol}), f_\mathrm{xy}(N_\mathrm{qubits}), f_\mathrm{sys}^{(A)}(\mathcal{S})$ — Architecture-specific scaling functions}

\textbf{Structural role.} Map system-size variables (molecular mass, qubit count, system-size descriptor) to effective multiplicity for each architectural class.

\textbf{Status.} INHERITED from architecture-to-platform calibration. Designated \textbf{O-QC-2} in the Q-COMPUTE open-extension list.

\textbf{Used in.} Chapter 7; Chapter 14.

\subsection{$\tau_\mathrm{gap-stab}(\mathcal{T})$ — Class B topology-perturbation rate}

\textbf{Structural role.} The rate at which the topological gap $\Delta_\mathrm{top}$ closes for topology label $\mathcal{T}$ under perturbation. Underwrites the Class B exponential gap-suppression prediction.

\textbf{Status.} Form named; topology-stability theorem (designated \textbf{O-QC-4}) is the natural follow-on for substrate-level derivation.

\textbf{Used in.} Chapter 7.

\bigskip\hrule\bigskip

\section{D.6 The Closed-Form-Substrate-Constants Program}
...
\section{D.7 Structural Roles Cross-Referenced by Sector}

The substrate constants in this appendix support the following sectorial structures:

\begin{itemize}
\item \textbf{Quantum sector (Chapters 5–7).} $\hbar$ in T1–T16 uncertainty bounds; $\mathcal{M}_\mathrm{crit}$ in QC architectural taxonomy and matter-wave Q-C boundary; $\Gamma_\mathrm{min}$, $\Lambda_\mathrm{V1}$, $\beta_\mathrm{crit}$, $N_\mathrm{corr}$, $\Delta_\mathrm{top}^{\max}$, $T_\mathrm{eff}^{\min}$ in UR-1 condition-failure timescales and class-specific projections of $M$.
\item \textbf{Continuum and dynamics (Chapters 8–10).} $\ell_P$ in the R1 substrate-cutoff hyperviscous prefactor; $\beta$ in the Universal Mobility Law; the V5-derived Maxwell relaxation time $\tau_R$ inheriting through DCGT.
\item \textbf{Gravity sector (Chapters 11–13).} $c$, $\hbar$, $H_0$, $\ell_P$ as substrate inputs; $G$ and $a_0$ as derived; $\log g$ in the BH area-law coefficient.
\item \textbf{Cross-platform and empirical synthesis (Chapters 14–15).} $\mathcal{M}_\mathrm{crit}$ unifying matter-wave Q-C and qubit-system multiplicity wall; $\Gamma_\mathrm{cross}$ collapse across BH-2 and QC condition (ii); $\beta$ as a PASSED empirical anchor; $a_0$ as the BTFR slope-4 anchor (PASSED).
\end{itemize}

\bigskip\hrule\bigskip

\section{D.8 Form-FORCED / Value-INHERITED Tagging Summary}
...
\section{D.9 Dependency Section}
...
\section{D.10 Canonical Sources}

\begin{itemize}
\item \textbf{Substrate inputs and T19 / T20 derivations:}
  - \texttt{papers/Substrate\_Gravity\_Foundations/Substrate\_Gravity\_Foundations\_Paper.\{md,tex,pdf\}}
  - \texttt{papers/Substrate\_Gravity\_Foundations/ED\_substrate\_gravity\_foundations\_2026-04-28.\{md,tex,pdf\}}
\item \textbf{UR-1 thresholds and Q-COMPUTE open-extension labels:}
  - \texttt{papers/Quantum\_Computing\_Foundations/}
  - Arc Q-COMPUTE memos in \texttt{theory/Quantum\_Computing/}
\item \textbf{BH area-law coefficient and O2:}
  - \texttt{papers/Black\_Hole\_Foundations/}
  - Arc BH memos in \texttt{theory/Black\_Holes/}
\item \textbf{Universal Mobility Law $\beta$ empirical anchor:}
  - \texttt{papers/Universal\_Mobility\_Law/}
  - \texttt{papers/P4\_NonNewtonian\_Paper\_Draft/}
\item \textbf{DCGT and V1/V5 closed-form details (governing $\Gamma_\mathrm{min}$, $\Lambda_\mathrm{V1}$, UR-1 functional shapes):}
  - \texttt{papers/Navier Stokes\_Synthesis\_Paper/} Appendix D (DCGT)
  - Arc D memos in \texttt{theory/Arc\_D/}
  - Theorem N1 paper (V1 finite-width kernel)
\item \textbf{ED-SC anchor and E4:}
  - ED-SC arc work (cross-reference; not within this monograph's nine sectors)
\end{itemize}

\bigskip\hrule\bigskip

\section{D.11 Optional Figures}

The following tables and diagrams are described for inclusion in the final monograph:

\begin{itemize}
\item \textbf{Table D.1 — Master substrate-constants inventory.} Rows: every constant in this appendix. Columns: structural role; status (substrate input / FORCED-DERIVED / FORCED-INHERITED / EMPIRICAL ANCHOR); current value; chapter(s) where load-bearing; closed-form derivation status / open-extension label. The complete tabular form of Sections D.2–D.5.
\item \textbf{Table D.2 — Form-FORCED vs Value-INHERITED tagging summary.} The summary table reproduced from Section D.8.
\item \textbf{Figure D.1 — Closed-form-substrate-constants program diagram.} Three nodes for $\mathcal{M}_\mathrm{crit}$, $\log g$, and $\kappa/|\hat{N}'|$, with their open-extension labels (O-QC-1, O2, E4) and empirical anchors. Edges indicate the cross-domain consistency requirement: closed-form derivation in any one constrains the others by substrate-level multiplicity counting.
\item \textbf{Figure D.2 — Substrate-input-to-derived-constants dependency tree.} Root nodes: $\hbar$, $c$, $H_0$, $\ell_P$. First-level derived: $G$ (via T19), $a_0$ (via T20). Second-level relationships: BTFR slope-4 (via $G$ and $a_0$), area law (via $\ell_P$ and $\log g$ INHERITED), Heisenberg bound (via $\hbar$), R1 prefactor (via $\ell_P$). Visualizes the program's value-derivation structure end-to-end.
\item \textbf{Table D.3 — Sectorial-incidence matrix.} Rows: constants. Columns: sectors (substrate ontology, load-bearing invariants, DCGT, kernel-arrow, QM, QFT, QC, NS, MHD, YM, soft-matter, substrate gravity, curvature, BH, cross-platform, public tests). Filled cells indicate "load-bearing." Used to verify that every constant has a sectorial home and every sector is grounded in the constants inventory.
\end{itemize}
...
Future work proceeds along three parallel tracks. The \textbf{closed-form substrate-constants program} (O-QC-1, O2 from Arc BH, E4 from ED-SC) attacks the INHERITED-constant boundary from below, deriving values currently anchored to empirical observation. The \textbf{next-arc program} opens new structural fronts (B4 Hawking spectrum as the immediate next concrete arc; Arc COSMO and GR-4A as long-horizon speculative arcs). The \textbf{chapter-expansion program} progressively replaces each shell summary with the full integrated treatment — a progression of multiple sessions per chapter, paced by which sector's derivation is most actively being clarified.
