...
\hypersetup{hidelinks,
  pdftitle={From Primitives to the 0.6 Problem},
  pdfauthor={Allen Proxmire}}
\setlength{\parindent}{0pt}
\setlength{\parskip}{6pt plus 2pt minus 1pt}

\title{\vspace{-1cm}\Large\bfseries From Primitives to the 0.6 Problem\\[6pt]
\large\mdseries A Walkthrough of the Event Density Dimensional-Dictionary Resolution}
...
\item The ``60\% speed mismatch'' is not a physical anomaly. It is a mismatch between an atlas-nondimensionalisation artifact and a physical invariant. The framework's parabolic substrate PDE has formally infinite signal speed anyway; $c_0 > c$ is a regime-boundary marker, not a physical prediction.
\item The structural identity $c_0/c = 1/(2 D_\mathrm{nd})$ is \emph{universal} across the framework's regimes (quantum, optomechanical, condensed-matter, galactic, cosmological). It is purely a channel-weight relationship.
\item At the equal-rates point $D_\mathrm{nd} = 1/2$ (where decoherence rate equals coherent-system rate), $c_0 = c$ \emph{exactly}. No free parameter, no fit. This is a clean structural selection rule: the regime where $D_\mathrm{nd} = 1/2$ is the regime where the dictionary's signal speed coincides with the physical speed of light.
\end{enumerate}

The chain has three structural moves:

\begin{enumerate}[noitemsep]
\item The dimensional dictionary construction $T_0 = L_0^{2} \cdot D_\mathrm{nd}/D_\mathrm{phys}$ is required for the nondimensionalisation to work --- rescaling $t \to t/T_0$, $x \to x/L_0$ must send the simulation's mobility coefficient to $D_\mathrm{nd}$ by construction.
\item Plugging in $L_0 = \hbar/(mc)$ (Compton length, Madelung-anchored), $D_\mathrm{phys} = \hbar/(2m)$ (Madelung diffusion coefficient), and the canonical quantum-regime convention $D_\mathrm{nd} = 0.3$ produces $T_0 = 0.6\,\hbar/(mc^{2})$ algebraically.
\item The five alternative derivation routes (damping discriminant, reversible-slice QFT, acoustic-metric curvature, PME similarity, $\zeta$-interpolation) all fail to reproduce the 0.6 from independent dynamics. The dictionary-construction route is the unique successful route.
\end{enumerate}

The structural payoff: the framework's most conspicuous unexplained number is reduced to a one-line algebraic identity that traces back to a single underlying convention --- the value of the quantum-regime channel weight $D_\mathrm{nd}$. The 0.6 is no longer a structural mystery; it is a structural \emph{bookmark} pointing to the genuinely open question of what fixes $D_\mathrm{nd}$ from first principles.

This walkthrough is shorter than Born or Schr\"odinger, because the math content is essentially one algebraic identity plus a five-route audit. But it serves a useful purpose: it documents how a conspicuous-looking constant in a substrate-level framework can be structurally clean once the construction is made explicit.

\section{The Primitives That Matter}

The 0.6 problem operates downstream of the framework's QM-emergence sector. The walkthrough uses the same substrate primitives that Born and Schr\"odinger used, plus three forced theorems and the framework's \emph{dimensional atlas} --- the catalog of regime-specific dimensional dictionaries.

\textbf{Micro-events (P01).} Discrete acts of becoming.

\textbf{Chains, bandwidth, polarity.} Standard substrate inventory.

\textbf{Substrate length scale $\ell_P$ (T19).} Newton-recovery identifies the substrate length scale as the Planck length.

\textbf{Madelung anchor.} The framework's QM emergence (U2 + U3) identifies the substrate's diffusion coefficient at the quantum regime as $D_\mathrm{phys} = \hbar/(2m)$ --- the Madelung diffusion coefficient that makes the substrate evolution equation the Schr\"odinger equation in disguise. This identification is FORCED by the U3 closure (Schr\"odinger walkthrough \S5).

\textbf{Compton length $L_0 = \hbar/(mc)$.} The reduced Compton wavelength of the chain, identified as the canonical quantum-regime length scale by inheriting from the substrate's relativistic structure (Klein-Gordon walkthrough plus Madelung).

\textbf{Dimensional atlas convention.} The framework's regime-specific dimensional dictionaries are catalogued in \texttt{papers/Dimensional\_Atlas/regimes/}. Each regime specifies $L_0$, $T_0$, $D_\mathrm{phys}$, and $D_\mathrm{nd}$. The quantum-regime dictionary fixes $D_\mathrm{nd} = 0.3$ as the canonical channel-weight value.
...
\section{The Dimensional Dictionary Construction}

The canonical quantum-regime dimensional dictionary specifies four quantities:
\[
\begin{aligned}
L_0 &= \hbar/(mc) \quad \text{(canonical length, reduced Compton wavelength)} \\
D_\mathrm{phys} &= \hbar/(2m) \quad \text{(canonical diffusion coefficient, Madelung)} \\
D_\mathrm{nd} &= 0.3 \quad \text{(canonical dimensionless channel weight)} \\
T_0 &= ? \quad \text{(canonical time, to be determined)}
\end{aligned}
\]
The first three are inputs. The fourth is \emph{derived} from a structural construction.
...
\]
This is the \textbf{dictionary construction}. It is not stipulative; it is required for the nondimensionalisation to produce the standard dimensionless simulation equation. If $T_0$ is chosen any other way, the rescaled equation has the wrong coefficient structure.

\subsection{Plugging in the canonical values}

Substituting $L_0 = \hbar/(mc)$ and $D_\mathrm{phys} = \hbar/(2m)$:
\[
T_0 = [\hbar/(mc)]^{2} \cdot D_\mathrm{nd} / [\hbar/(2m)] = [\hbar^{2}/(m^{2}c^{2})] \cdot D_\mathrm{nd} \cdot [2m/\hbar] = 2 D_\mathrm{nd} \cdot \hbar/(mc^{2})
\]
This is the algebraic relationship:
\[
\boxed{T_0 = 2 D_\mathrm{nd} \cdot \hbar/(mc^{2})}
\]
Substituting the canonical quantum-regime convention $D_\mathrm{nd} = 0.3$:
\[
T_0 = 2 \cdot 0.3 \cdot \hbar/(mc^{2}) = 0.6\, \hbar/(mc^{2})
\]
The 0.6 is \emph{algebraically} equal to $2 \cdot D_\mathrm{nd}(\mathrm{quantum})$. There is no freedom in this step.

\subsection{The signal-speed identity}
...
\subsection{Route 1: Damping discriminant}
...
\item At $\zeta = 0$: $D_\mathrm{crit} = 2\sqrt{2} - 2 \approx 0.828$. Not 0.3 or 0.6.
\item At $\zeta = 1/4$ (canonical): $D_\mathrm{crit} \approx 0.896$. Not 0.3 or 0.6.
\item At $\zeta = 1/2$: $D_\mathrm{crit} \approx 0.775$. Not 0.3 or 0.6.
\end{itemize}
...
\subsection{Route 2: Reversible-slice QFT normalisation}
...
The 0.6 never enters the reversible-slice QFT construction. The $c_s = c/0.6$ identification is applied \emph{after} the QFT construction, inherited from the dictionary.

\textbf{Route 2 fails.}

\subsection{Route 3: Acoustic-metric curvature scaling}
...
This is a suggestive numerical coincidence. But $\alpha$ is a \emph{temporal-decay exponent of the density}, not a velocity ratio. The Barenblatt similarity variable is $\xi = x \cdot t^{-\beta}$ with $\beta = 1/(d(m-1)+2) = 1/5$, giving a \emph{time-dependent} characteristic velocity $\propto t^{\beta-1}$, not a constant $L_0/T_0$ ratio.

The 3/5 does not sit at the right place in the dimensional hierarchy to fix $T_0/L_0$. \textbf{Route 4 is a false positive --- coincidentally the right number, structurally the wrong quantity.}

\subsection{Route 5: $\zeta$-dependence interpolation}
...
\begin{itemize}[noitemsep]
\item Solving $\sqrt{2-\zeta}/2 = 0.6$ gives $\zeta = 0.56$, not canonical.
\item Solving $1 - \sqrt{2-\zeta}/2 = 0.6$ gives $\zeta = 1.36$, outside the canonical range.
\end{itemize}

\textbf{Route 5 fails.}

\subsection{The audit summary}

\begin{center}
\begin{tabular}{lll}
\toprule
\textbf{Route} & \textbf{Test} & \textbf{Verdict} \\
\midrule
...
\midrule
\textbf{Dictionary construction} & $T_0 = L_0^{2} D_\mathrm{nd}/D_\mathrm{phys}$ & \textbf{Succeeds (algebraically forced)} \\
\bottomrule
\end{tabular}
\end{center}
...
\section{The Equal-Rates Selection Rule}
...
\subsection{Physical meaning of $D_\mathrm{nd} = 1/2$}
...
For $D_\mathrm{nd} = 1/2$: $\gamma_\mathrm{dec} = \omega_\mathrm{sys}$ --- \emph{equal rates}. Decoherence and coherent propagation operate at the same substrate-level rate. This is a structurally distinguished operating point --- the regime where the substrate operates symmetrically between commitment and propagation.

\subsection{The selection rule}

At the equal-rates point, the dictionary's canonical signal speed coincides exactly with the physical speed of light. This is a \emph{new structural statement} not previously articulated in the corpus:

\textbf{Equal-rates selection rule.} The substrate operates with $c_0 = c$ if and only if it sits at $D_\mathrm{nd} = 1/2$. Equivalently: rate-balance ($\gamma_\mathrm{dec} = \omega_\mathrm{sys}$) is the structurally distinguished regime where the dictionary's signal speed is the physical speed of light.

The canonical quantum-regime convention $D_\mathrm{nd} = 0.3$ sits \emph{off} this distinguished point. The dictionary's $c_0$ is therefore \emph{not} the physical speed of light at the canonical operating point --- it is an atlas-nondimensionalisation artifact, not a physical signal speed.

\subsection{Where ED-09.5 (Q-C transition) sits}
...
\subsection{The ``60\% speed mismatch'' is not a physical anomaly}

The framework's geometry-emergence and acoustic-analogue memos previously flagged $c_0 \approx 1.667 c$ as a mismatch requiring resolution. The reduction shows this is \emph{not} a physical anomaly. It is a mismatch between an atlas-nondimensionalisation artifact and a physical invariant. The framework's substrate PDE is parabolic and has formally infinite signal speed anyway; the dictionary's $c_0$ is a regime-boundary marker, not a physical prediction.
...
\subsection{The universal channel-weight scaling}
...
\begin{tabular}{lll}
\toprule
\textbf{Regime} & $D_\mathrm{nd}$ & $c_0/c$ \textbf{predicted} \\
\midrule
...
\subsection{The acoustic-analogue cross-regime consistency check}
...
The Madelung anchor $D_\mathrm{phys} = \hbar/(2m)$ is INHERITED from U3 closure (Schr\"odinger walkthrough \S5).
...
The closed-form derivation of $D_\mathrm{nd}(\mathrm{quantum})$ from substrate primitives is the load-bearing open question. The 0.6 problem reduces to this exactly: the work of deriving 0.6 \emph{is} the work of deriving $D_\mathrm{nd}(\mathrm{quantum})$.

The framework's \emph{rate-balance-template program} (\texttt{ED-Dimensional-01-Ext.md}) is the active attempt to apply rate-balance derivation \emph{within} the quantum regime to produce $D_\mathrm{nd}$ from first principles. Currently the template anchors $D_\mathrm{nd}$ for optomechanical, cavity-QED, condensed-matter, galactic, and cosmological regimes from independent rate identifications, but does not yet produce the quantum-regime value.
...
Substrate primitives + Madelung anchor (U3 closure $\to D_\mathrm{phys} = \hbar/(2m)$) + Compton length ($L_0 = \hbar/(mc)$) + dimensional-atlas convention ($D_\mathrm{nd}(\mathrm{quantum}) = 0.3$) $\to$ dictionary construction $T_0 = L_0^{2} \cdot D_\mathrm{nd}/D_\mathrm{phys}$ $\to$ algebraic identity $T_0 = 2 D_\mathrm{nd} \cdot \hbar/(mc^{2}) = 0.6\,\hbar/(mc^{2})$ $\to$ boxed identity $c_0/c = 1/(2 D_\mathrm{nd})$ $\to$ equal-rates selection rule ($c_0 = c$ at $D_\mathrm{nd} = 1/2$).
...
What's distinctive about this walkthrough's payload is its \emph{honesty}. The 0.6 is not derived to closed form; it is \emph{reduced} to a more fundamental open question (the $D_\mathrm{nd}$ anchoring problem). The reduction is partial. Three structural statements emerge that were not previously in the corpus:

\begin{enumerate}[noitemsep]
\item The ``superluminal'' $c_0 \approx 1.667 c$ is a nondimensionalisation artifact, not a physical anomaly. The framework's substrate PDE has formally infinite signal speed anyway; $c_0 > c$ is a regime-boundary marker.
\item The universal scaling $c_0/c = 1/(2 D_\mathrm{nd})$ is a falsifiable cross-regime prediction. Analogue platforms at differing $D_\mathrm{nd}$ should reproduce the scaling.
\item The equal-rates selection rule ($c_0 = c$ at $D_\mathrm{nd} = 1/2$) is a structurally distinguished operating point. Whether the framework's Q-C transition sits at this point is open.
\end{enumerate}
...
This walkthrough's payload is smaller than Born or Schr\"odinger because the math content is one algebraic identity. But it serves a useful structural purpose: it documents how a conspicuous-looking constant in the framework's dimensional construction can be made transparent without requiring closed-form first-principles derivation. The 0.6 problem is no longer a problem; it is a bookmark pointing to a specific tractable open question.

\section{References}

\begin{itemize}[noitemsep]
\item Bohm, D. ``A Suggested Interpretation of the Quantum Theory in Terms of `Hidden' Variables.'' \textit{Physical Review} \textbf{85}, 166--179 (1952).
\item Madelung, E. ``Quantentheorie in hydrodynamischer Form.'' \textit{Zeitschrift f\"ur Physik} \textbf{40}, 322--326 (1927).
\item V\'azquez, J. L. \textit{The Porous Medium Equation: Mathematical Theory.} Oxford University Press, 2007.
\item Visser, M. ``Acoustic black holes: horizons, ergospheres and Hawking radiation.'' \textit{Classical and Quantum Gravity} \textbf{15}, 1767--1791 (1998).
...
\item Proxmire, A. \textit{ED-Dimensional-01: Quantum Regime Dimensional Dictionary.} April 2026.
\item Proxmire, A. \textit{ED-Dimensional-01-Ext: Rate-Balance Template for $D_\mathrm{nd}$ Anchoring.} April 2026.
\item Proxmire, A. \textit{Walkthrough: From Primitives to the Schr\"odinger Equation.} April 2026.
...
\item Proxmire, A. \textit{Effective Acoustic Metric in Event Density.} April 2026.
\item Proxmire, A. \textit{Acoustic Analogue Experimental Program for Cross-Regime Signal-Speed Consistency.} April 2026.
