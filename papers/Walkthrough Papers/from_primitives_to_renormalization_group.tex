...
\hypersetup{hidelinks,
  pdftitle={From Primitives to the Renormalization-Group Three-Regime Structure},
  pdfauthor={Allen Proxmire}}
\setlength{\parindent}{0pt}
\setlength{\parskip}{6pt plus 2pt minus 1pt}

\title{\vspace{-1cm}\Large\bfseries From Primitives to the Renormalization-Group Three-Regime Structure\\[6pt]
\large\mdseries A Walkthrough of the Event Density Wilsonian RG Analysis}
...
The answer is more interesting than it might appear. Three structural results emerge from the Wilsonian analysis. First, the ED operator basis is \emph{form-closed} under coarse-graining --- the canonical ED PDE generates no operators not already present in its own ansatz, at tree level and at one loop. The operator structure of ED is a theorem rather than a guess. Second, ED is \emph{not} an RG fixed point at physical couplings --- the beta functions are non-trivial, and the couplings flow non-trivially under coarse-graining. ED-specific phenomenology is regime-dependent rather than scale-invariant. Third, the flow structure has \emph{three distinct regimes} --- a UV regime of free two-channel diffusion, an intermediate regime where the ED operator structure is fully active, and an IR regime of linear gapped diffusion. ED-specific predictions live in the intermediate regime only; outside it, the framework's effective PDE collapses to either free wave-like dispersion or trivial massive decay.

The structural payoff: cross-scale invariance in ED is \emph{operator-structure invariance within a bounded window}, not coupling-invariance across all scales. The framework's ``one PDE, three domains'' claim is a claim about the \emph{operator structure} surviving across the intermediate regime, not a claim that the couplings $(M_0, P_0, \zeta, \tau, H, D)$ take the same values at all scales. The window where ED-specific phenomenology applies is bounded by two crossover scales whose values are determined by the system's specific couplings.

This is the framework's renormalization-group result, and it serves a different purpose than the other walkthroughs in the series. The Born, Schr\"odinger, Heisenberg, and entanglement walkthroughs derive specific structural results --- particular equations or inequalities. The RG walkthrough establishes the \emph{self-consistency} of the framework: the canonical ED PDE is a closed structural object under the standard coarse-graining procedure of statistical physics, with a well-defined window of applicability and well-understood limits.

The chain has six structural moves:

\begin{enumerate}[noitemsep]
\item The canonical ED PDE has a two-channel structure: a density-perturbation field $\delta$ coupled to a participation mode $v$, with mobility, penalty, and inter-channel-coupling terms organized around a finite operator basis.
\item Wilsonian coarse-graining --- block-averaging plus rescaling --- produces a flow on the space of couplings. Applied to the canonical ED PDE, the flow generates no operators outside the canonical basis at tree level or at one loop. ED is form-closed.
\item The beta functions for the principal couplings, computed at dynamic exponent $z = 2$, give explicit RG flow trajectories: the linear mobility $M_0$ is marginal, the linear penalty $P_0$ and the participation friction $\zeta$ grow toward IR, the participation timescale $\tau$ shrinks toward IR.
\item The fixed-point catalog identifies a Gaussian fixed point (UV-stable only), two fixed lines (Wilson-Fisher at $\chi = 1$, nonlinear-mobility at $\chi = 0$), and a ``fixed point at infinity'' $(P_0 \to \infty, \zeta \to \infty, \tau \to 0)$ that is the universal IR sink.
\item Two crossover scales --- $\ell_v$ where participation slaving onsets, $\ell_\xi$ where the gap dominates --- bound the \emph{ED window} in which ED-specific phenomenology applies.
\item The three regimes are explicit: UV free two-channel diffusion ($\ell \to -\infty$), intermediate Model A universality ($\ell_v < \ell < \ell_\xi$), IR linear gapped diffusion ($\ell \to \infty$). ED-specific predictions --- quantum-classical transitions, nonlinear triad coupling, mobility-saturation phenomenology --- live in the intermediate regime only.
\end{enumerate}
...
The framework rests on substrate-level ontological commitments. The RG walkthrough operates on the \emph{canonical ED PDE} --- the continuum-level equation that DCGT produces from substrate primitives --- rather than on the substrate itself. The primitives behind the canonical PDE are the same set used by Born, Schr\"odinger, and the gauge-fields walkthrough.

\textbf{Micro-events, chains, bandwidth, polarity, ED gradient, commitment irreversibility, locality, rule-type.} The standard substrate inventory.

\textbf{The Diffusion Coarse-Graining Theorem (DCGT).} The substrate-to-continuum bridge that produces the canonical ED PDE as the leading-order coarse-grained continuum content of substrate dynamics. The RG analysis takes the output of DCGT as its starting point.
...
\[
M(\delta) = M_0 + M_2 \delta^{2}/2 + \cdots, \qquad P(\delta) = P_0 \delta + P_3 \delta^{3}/6 + \cdots
\]
...
\section{The Canonical ED PDE and Its Operator Basis}

Before the RG flow can be computed, the operator basis of the canonical PDE must be identified. The basis is the space of all operators consistent with the symmetries that \emph{might} be generated under coarse-graining.

\subsection{The reduced operator basis}

Restricting to $Z_2$-symmetric operators of dimension up to four derivatives (the truncation consistent with the tree-level scaling of \S4), the reduced basis has nine elements:

\begin{itemize}[noitemsep]
\item $O_0 = \delta$ \quad from linear penalty $P_0 \delta$
\item $O_2 = \nabla^{2}\delta$ \quad from linear mobility $M_0 \nabla^{2}\delta$
\item $O_{3,0} = \delta^{3}$ \quad from cubic penalty $P_3 \delta^{3}/6$
\item $O_{1,2} = \delta^{2}\nabla^{2}\delta$ \quad from even mobility $M_2 \delta^{2} \nabla^{2}\delta / 2$
\item $O_{2,2} = \delta|\nabla \delta|^{2}$ \quad from even mobility $M_2 \delta|\nabla \delta|^{2}$
\item $O_4 = \nabla^{4}\delta$ \quad higher-derivative (not in bare ED; trial)
\item $O_v = v$ \quad participation mode
\item $O_{v,t} = \dot v$ \quad participation kinetic
\item $O_{\delta v} = \delta v$ \quad two-channel mixing (implicit via $Hv$)
\end{itemize}
...
\subsection{What ``form-closure'' means}

The canonical PDE \emph{generates} certain operators from this basis: $O_0, O_2, O_{3,0}, O_{1,2}, O_{2,2}, O_v, O_{v,t}, O_{\delta v}$ are all present in the bare equation. $O_4$ (the four-derivative term $\nabla^4 \delta$) is not present in the bare equation but is present in the basis as a trial operator --- if the RG flow generates it, it must be added.

\textbf{Form-closure} means: the RG flow does not generate $O_4$ or any other operator outside the basis the bare equation already populates. The canonical ED PDE is closed under coarse-graining; its operator content does not expand under the flow.
...
\subsection{Why form-closure is the right diagnostic}

Wilsonian coarse-graining is a \emph{projection} onto larger length scales. If the projection produces the same operator structure that was already present, the structure is \emph{closed} under the projection. This is the strongest sense in which a PDE can be ``fundamental at the continuum level'': its operator content is preserved under the procedure that produces continuum content from microscopic content.
...
\section{Wilsonian Coarse-Graining and Form-Closure}
...
\subsection{The momentum-shell split}
...
\[
x' = x/b, \quad t' = t/b^z, \quad \delta'(x', t') = b^\chi \delta_<(bx', b^z t'), \quad v'(t') = b^{\chi_v} v_<(b^z t')
\]
Here $z$ is the \emph{dynamic exponent} (the scaling of time relative to space) and $(\chi, \chi_v)$ are the field scaling dimensions. The dimensionless choice $z = 2$ corresponds to diffusive dynamics, which is appropriate for the canonical ED PDE.

\subsection{The form-closure result}
...
\textbf{Tree level:} the bare operators $O_0, O_2, O_{3,0}, O_{1,2}, O_{2,2}, O_v, O_{v,t}, O_{\delta v}$, with coefficients rescaled by appropriate powers of $b$. No $O_4$ generated.

\textbf{One loop:} the bare operators with corrections proportional to coupling products. The corrections renormalize existing coefficients but do not generate new operators outside the basis.

The bare operator structure is preserved under the coarse-graining map. \textbf{The canonical ED PDE is form-closed.}

This is a non-trivial result. Many continuum field theories are \emph{not} form-closed at one loop --- they require the addition of operators absent from the bare equation. The Klein-Gordon equation requires the addition of $\phi^4$ at one loop in $d = 4$. The Navier-Stokes equation requires the addition of multiple operators when treated as a stochastic field theory. For ED, the operators present in the canonical PDE are sufficient to absorb the one-loop corrections; the structural ansatz is closed.

\subsection{The participation-channel structure survives}

The two-channel structure ($\delta$ coupled to $v$) is not destroyed by coarse-graining. The participation mode $v$ remains as a separate dynamical variable; its coupling to $\delta$ via $H$ is renormalized but not eliminated. The participation channel is \emph{intrinsic} to the structural form of the canonical PDE; it is not an artifact of the bare equation that washes out under coarse-graining.
...
\section{The Beta Functions and the Fixed-Point Catalog}
...
\subsection{The beta functions at $z = 2$}

At dynamic exponent $z = 2$, with field-scaling dimension $\chi$ chosen to make the linear mobility marginal:
\[
\begin{aligned}
\beta_{M_0} &= 0 \quad \text{(marginal)} \\
\beta_{P_0} &= 2 P_0 \quad \text{(relevant; $P_0$ grows toward IR)} \\
\beta_\zeta &= 2 \zeta \quad \text{(relevant; $\zeta$ grows toward IR)} \\
\beta_\tau &= -2 \tau \quad \text{(irrelevant; $\tau$ shrinks toward IR)} \\
\beta_{P_3} &= (2 - 2\chi) P_3 \\
\beta_{M_2} &= -2\chi M_2
\end{aligned}
\]

The linear mobility $M_0$ is marginal --- its scaling dimension is zero, so it does not flow under the leading-order RG. The penalty $P_0$ and participation friction $\zeta$ are \emph{relevant} --- both grow exponentially toward the IR limit. The participation timescale $\tau$ is \emph{irrelevant} --- it shrinks exponentially toward the IR limit.
...
\[
\begin{aligned}
P_0/\zeta &\quad \text{invariant (both scale as } e^{2\ell}\text{)} \\
P_0 \cdot \tau &\quad \text{invariant (one scales as } e^{2\ell}\text{, other as } e^{-2\ell}\text{)} \\
\zeta \cdot \tau &\quad \text{invariant (same combination)}
\end{aligned}
\]
...
\subsection{Fixed-point catalog}

A \emph{fixed point} of the RG flow is a coupling configuration $(M_0^*, P_0^*, \zeta^*, \tau^*, P_3^*, M_2^*)$ satisfying $\beta_g = 0$ for all $g$. The canonical ED PDE has the following fixed-point structure:

\textbf{Gaussian fixed point $G$:} $(M_0^*, 0, 0, \infty)$ with all interactions absent. Free two-channel diffusion at infinite participation timescale. UV-stable only --- it is the limit reached by running the RG flow backward to short distances.

\textbf{Wilson-Fisher fixed line at $\chi = 1$:} $\beta_{P_3} = 0$ at $\chi = 1$ makes the cubic penalty marginal. A line of fixed points parameterized by the value of $P_3$, structurally analogous to the Wilson-Fisher fixed point of $\phi^4$ theory in $d = 3$ near the upper critical dimension.

\textbf{Nonlinear-mobility fixed line at $\chi = 0$:} $\beta_{M_2} = 0$ at $\chi = 0$ makes the quadratic mobility marginal. A different line of fixed points, structurally analogous to fixed points of nonlinear-diffusion equations.

\textbf{``Fixed point at infinity'':} $(P_0 \to \infty, \zeta \to \infty, \tau \to 0)$ with the relevant couplings running to infinity and the irrelevant coupling running to zero. This is the universal IR sink --- every trajectory in the principal flow basin approaches this limit at sufficiently large $\ell$.
...
\subsection{ED is not a fixed point}
...
This is structurally important. ED is not a scale-invariant theory in the strict RG sense. Its couplings flow, its operator coefficients change with scale, and its phenomenology depends on what scale you analyze it at. What is preserved under the flow is the \emph{operator structure} --- the set of operators present in the equation --- not the \emph{coupling values}.

\section{The Two Crossover Scales and the ED Window}

The flow from initial physical couplings toward the fixed point at infinity passes through structurally distinct regimes separated by two crossover scales. These scales bound the \emph{ED window} --- the regime in which ED-specific phenomenology applies.

\subsection{The participation-slaving onset $\ell_v$}

The participation mode $v$ has a relaxation timescale governed by $\tau$. As $\tau$ shrinks toward IR (per $\beta_\tau = -2\tau$), the relaxation becomes faster and at some scale becomes faster than the dynamics of $\delta$. At that scale, $v$ ceases to be an independent dynamical degree of freedom --- its dynamics are \emph{slaved} to $\delta$ via the algebraic relation $v = F[\delta]/\zeta$ obtained by setting $\dot{v} = 0$ in the original two-channel equations.

The crossover scale is
\[
\ell_v = \tfrac{1}{2} \ln(\xi_v \Lambda)
\]
...
\subsection{The gap-dominated scale $\ell_\xi$}
...
\subsection{The ED window}

The ED window is the intermediate range $\ell_v < \ell < \ell_\xi$. Within this window:

\begin{itemize}[noitemsep]
\item Participation slaving has occurred: $v$ is no longer an independent dynamical variable, and the equation reduces to an effective single-channel form for $\delta$.
\item The gap has not yet dominated: the spatial gradient and nonlinear mobility terms are still active, and the system supports non-trivial spatial structure.
\item The full nonlinear ED structure --- quadratic mobility $M_2$, cubic penalty $P_3$, the divergence-form mobility flux --- is present and active.
\end{itemize}
...
\subsection{UV ($\ell \to -\infty$): free two-channel diffusion}
...
\subsection{Intermediate ($\ell_v < \ell < \ell_\xi$): Model A universality}
...
\[
\partial_t \delta = \Gamma_{\mathrm{eff}} \left[ M_0 \nabla^{2}\delta + (M_2/2) \delta^{2} \nabla^{2}\delta + M_2 \delta|\nabla \delta|^{2} - P_0 \delta - (P_3/6) \delta^{3} \right]
\]
This is \emph{Model A universality} --- the standard relaxational dynamics for a non-conserved scalar order parameter, with quadratic-mobility nonlinearities and cubic penalty. The full ED operator structure is present and active. The RG flow within this regime drives the system toward the fixed point at infinity but the trajectory passes through a regime where ED-specific phenomenology is rich and predictive.
...
\subsection{IR ($\ell \to \infty$): linear gapped diffusion}
...
\subsection{The full three-regime flow diagram}

\begin{center}
\begin{tabular}{p{0.31\textwidth} p{0.31\textwidth} p{0.31\textwidth}}
\hline
\textbf{UV: $\ell \to -\infty$} & \textbf{Intermediate: $\ell_v < \ell < \ell_\xi$} & \textbf{IR: $\ell \to \infty$} \\
\hline
Free two-channel diffusion & Model A universality, full ED operator structure & Linear gapped diffusion \\
$v$ unfrozen & Participation slaved ($v = F[\delta]/\zeta$) & Trivial massive endpoint $\delta \to 0$ \\
$P_0 \approx 0$ & Quadratic-mobility nonlinearities & Nonlinearities eliminated \\
$M_2, P_3 \approx 0$ & Cubic-penalty nonlinearities & \\
Wave-like dispersion & Saturating-penalty phenomenology & \\
& ED-specific predictive content (the ``ED window'') & \\
\hline
\end{tabular}
\end{center}
...
\section{Cross-Scale Invariance: What It Is and Isn't}
...
The \emph{values} of the couplings $(M_0, P_0, \zeta, \tau, M_2, P_3, H, D)$ at any specific scale are INHERITED. The framework establishes the flow equations; the numerical values of the couplings at any starting scale come from the substrate microscopic details plus the system being analyzed.

The \emph{positions} of the crossover scales $\ell_v$ and $\ell_\xi$ in any specific system are INHERITED from the system's couplings. The framework establishes that the crossover scales exist; their locations depend on the system.

The \emph{width} of the ED window is INHERITED from the ratio $\ell_\xi - \ell_v$, which depends on the couplings.

The \emph{specific predictions} in the intermediate window --- the Universal Mobility Law's saturating exponent, the matter-wave Q-C boundary's molecular-mass range, the triad-coupling phenomenology --- are INHERITED from the values of the couplings within the window. The framework establishes the structural form of these predictions; the numerical values come from the inherited couplings.
...
Substrate primitives (micro-events, chains, bandwidth, polarity, ED gradient, locality, rule-type, finite-width kernels) $\to$ DCGT (substrate-to-continuum bridge producing the canonical ED PDE) $\to$ canonical ED PDE with two-channel structure ($\delta$ coupled to $v$) and finite operator basis ($O_0, O_2, O_{3,0}, O_{1,2}, O_{2,2}, O_v, O_{v,t}, O_{\delta v}$) $\to$ Wilsonian coarse-graining (momentum-shell split + rescaling) $\to$ form-closure at tree level and one loop (no new operators generated) $\to$ beta functions at $z = 2$ ($M_0$ marginal, $P_0$ and $\zeta$ relevant, $\tau$ irrelevant) $\to$ fixed-point catalog (Gaussian, Wilson-Fisher fixed line, nonlinear-mobility fixed line, fixed point at infinity) $\to$ flow invariants ($P_0/\zeta$, $P_0\tau$, $\zeta\tau$) $\to$ two crossover scales ($\ell_v$ participation-slaving onset, $\ell_\xi$ gap-dominated) $\to$ ED window $\ell_v < \ell < \ell_\xi$ $\to$ three regimes (UV free two-channel, intermediate Model A, IR linear gapped) $\to$ cross-scale invariance as operator-structure invariance within the window.
...
\begin{itemize}[noitemsep]
\item Wilson, K. G. ``The Renormalization Group: Critical Phenomena and the Kondo Problem.'' \textit{Reviews of Modern Physics} 47, 773--840 (1975).
\item Wilson, K. G., Kogut, J. ``The Renormalization Group and the $\epsilon$ Expansion.'' \textit{Physics Reports} 12, 75--199 (1974).
\item Hohenberg, P. C., Halperin, B. I. ``Theory of Dynamic Critical Phenomena.'' \textit{Reviews of Modern Physics} 49, 435--479 (1977).
\item Forster, D., Nelson, D. R., Stephen, M. J. ``Large-Distance and Long-Time Properties of a Randomly Stirred Fluid.'' \textit{Physical Review A} 16, 732--749 (1977).
\item Goldenfeld, N. \textit{Lectures on Phase Transitions and the Renormalization Group.} Westview Press, 1992.
\item Cardy, J. \textit{Scaling and Renormalization in Statistical Physics.} Cambridge University Press, 1996.
\item Zinn-Justin, J. \textit{Quantum Field Theory and Critical Phenomena.} Oxford University Press, 4th edition, 2002.
\item Polchinski, J. ``Renormalization and Effective Lagrangians.'' \textit{Nuclear Physics B} 231, 269--295 (1984).
\item Proxmire, A. \textit{Wilsonian RG-Flow Analysis of the Canonical ED PDE: Operator-Basis Form-Closure and One-Loop Check.} April 2026.
\item Proxmire, A. \textit{ED RG Flow Geometry: Fixed-Point Catalog, Diagonal Stability, and the ED Window.} April 2026.
\item Proxmire, A. \textit{ED IR Effective PDE: Slaving Derivation and Two-Stage Nonlinearity Elimination.} April 2026.
\item Proxmire, A. \textit{ED UV PDE and the Three-Regime Flow.} April 2026.
\item Proxmire, A. \textit{The 0.6 Problem Resolution: Structural Identity from the Cross-Regime Nondimensional Invariant.} April 2026.
