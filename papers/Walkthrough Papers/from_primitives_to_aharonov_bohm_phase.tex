...
\hypersetup{hidelinks,
  pdftitle={From Primitives to the Aharonov-Bohm Phase},
  pdfauthor={Allen Proxmire}}

\setlength{\parindent}{0pt}
\setlength{\parskip}{6pt plus 2pt minus 1pt}

\title{\vspace{-1cm}\Large\bfseries From Primitives to the Aharonov--Bohm Phase\\[6pt]
...
In 1959, Yakir Aharonov and David Bohm proposed an experiment that should have been impossible according to the standard intuitions of classical electromagnetism. A charged particle is sent through a double-slit apparatus, but with a long thin solenoid placed \emph{between} the two paths, on the side away from the screen. The solenoid contains a magnetic field; the regions traversed by the particle do not. In classical mechanics, the particle should be entirely unaffected by the solenoid --- there is no force on it, no torque, no Lorentz coupling. The interference pattern on the screen should be unchanged whether the solenoid is on or off.

It isn't. When the solenoid is energized, the interference pattern shifts. The shift depends on the \emph{enclosed magnetic flux} through the solenoid:
\[
...
\begin{enumerate}[noitemsep]
  \item T17 establishes the gauge field $A_\mu$ as the substrate's rule-type connection: a substrate object that carries rule-type label information from point to point in the continuum.
  \item The substrate's participation graph supports multiply-connected topologies: substrate channels can have ``holes'' --- regions of substrate that are excluded from the chain's accessible pathway, but around which the chain's wavefunction can wind.
  \item Parallel transport of the participation measure along a path $\gamma$ multiplies the wavefunction by a phase factor $\exp\left[(iq/\hbar)\int_\gamma A_\mu dx^\mu\right]$.
  \item The phase difference between two paths $\gamma_1$ and $\gamma_2$ around a non-contractible loop is $\Delta\varphi = (q/\hbar)\oint A_\mu dx^\mu$, where the closed-loop integral is gauge-invariant and substrate-physically meaningful.
  \item Stokes' theorem identifies the closed-loop integral with the integrated field strength through any surface bounded by the loop: $\oint A_\mu dx^\mu = \int F_{\mu\nu} d\Sigma^{\mu\nu} = \Phi$, where $\Phi$ is the enclosed magnetic flux.
  \item The Aharonov--Bohm phase $\Delta\varphi = q\Phi/\hbar$ is therefore the substrate-level monodromy of the rule-type connection around the closed loop, observable through wavefunction interference and gauge-invariant by construction.
\end{enumerate}
...
\textbf{T17 (Gauge-field-as-rule-type connection).} Establishes the gauge field $A_\mu$ as the substrate's rule-type connection --- the parallel-transport infrastructure for the rule-type relabeling group at the continuum level. This is the load-bearing prerequisite for the AB derivation.
...
\section{The Substrate Reading of Multiple-Connectivity}
...
\subsection{Simply-connected vs. multiply-connected substrate channels}

A \emph{simply-connected} substrate channel is one whose accessible pathway has the property that any closed loop in the channel can be continuously deformed to a point without leaving the channel. The chain's wavefunction has only contractible loops to wind around; there are no topologically non-trivial cycles.

A \emph{multiply-connected} substrate channel has accessible pathways that exclude certain regions --- substrate ``holes'' --- that the chain cannot enter. Around such a hole, closed loops are non-contractible: a loop encircling the hole cannot be continuously deformed to a point without crossing the excluded region.
...
\subsection{What makes a substrate channel multiply-connected}

The substrate channel's topology is determined by what regions of substrate are accessible to the chain. In the AB experiment, the chain is excluded from the solenoid's interior because the solenoid contains a region of high participation density --- a coil of conducting wire carrying current. The chain, propagating as a low-multiplicity participation rule, cannot enter this region without massive substrate disruption that would force individuation. The chain's accessible substrate is the region \emph{outside} the solenoid, which is multiply-connected.

This is structurally analogous to other multiply-connected substrate situations:

\begin{itemize}[noitemsep]
  \item A particle propagating around a region containing a black-hole interior (the chain is excluded from the saturated participation zone inside the horizon).
  \item A flux quantum penetrating a superconducting ring (the chain --- a Cooper pair --- propagates around but not through the magnetic-flux region).
  \item A ground-state electron orbital around a nucleus (the chain is excluded from a small region around the nuclear charge density).
\end{itemize}
...
\subsection{The fiber bundle reading}
...
\section{Parallel Transport on the Rule-Type Bundle}
...
\subsection{The connection's transport rule}
...
\[
\Psi_{\mathrm{transported}}(x_1) = \exp\!\left[(iq/\hbar) \int_\gamma A_\mu \, dx^\mu\right] \cdot \Psi (x_0)
\]
The exponent is the \emph{holonomy} of the connection along the path. The factor $q/\hbar$ comes from the gauge coupling --- specifically, the rule-type-coupling charge $q$ that T17 derives as the substrate-level coupling strength of the rule-type's relabeling, scaled by $\hbar$ from the U2-derived inner-product normalization.

\subsection{What parallel transport means at the substrate level}
...
\subsection{Path-dependence in multiply-connected regions}
...
For a multiply-connected accessible substrate, paths between the same endpoints can have different integrals, \emph{because} the difference between two paths bounds a surface that may pass through the inaccessible region (the ``hole''). If $F_{\mu\nu}$ is non-zero in the hole, the path-difference integral does not vanish.
...
\[
\Psi_{\mathrm{detector}} \propto \left[\exp\!\left((iq/\hbar) \int_{\gamma_1} A_\mu \, dx^\mu\right) + \exp\!\left((iq/\hbar) \int_{\gamma_2} A_\mu \, dx^\mu\right)\right] \cdot \Psi_{\mathrm{source}}
\]
up to amplitude factors that are common to both paths and do not affect the phase analysis.

\subsection{The relative phase}

Factoring out the path-1 phase, the superposition becomes:
\[
\Psi_{\mathrm{detector}} \propto \left[1 + \exp\!\left((iq/\hbar) \left(\int_{\gamma_2} - \int_{\gamma_1}\right) A_\mu \, dx^\mu\right)\right] \cdot \exp\!\left((iq/\hbar) \int_{\gamma_1} A_\mu \, dx^\mu\right) \cdot \Psi_{\mathrm{source}}
\]
The relative phase between the two paths is:
\[
\Delta \varphi = (q/\hbar) \left(\int_{\gamma_2} - \int_{\gamma_1}\right) A_\mu \, dx^\mu = (q/\hbar) \oint_\gamma A_\mu \, dx^\mu
\]
...
\subsection{The interference pattern}
...
\]
The interference pattern is determined by $\Delta\varphi$. As $\Delta\varphi$ varies from 0 to $2\pi$ --- for example, by changing the flux through the solenoid --- the interference pattern shifts: maxima become minima and vice versa.
...
\section{Stokes' Theorem and the Topological Identity}
...
\[
\oint_\gamma \mathbf{A} \cdot d\mathbf{x} = \int_\Sigma \mathbf{B} \cdot d\mathbf{A} = \Phi
\]
where $\Phi$ is the magnetic flux through any surface bounded by $\gamma$.

\subsection{Why the surface integral is well-defined}

Stokes' theorem requires the surface $\Sigma$ to lie in a region where $A_\mu$ is defined and $F_{\mu\nu}$ is the curl of $A_\mu$. In the AB setup, the chain's accessible substrate excludes the solenoid's interior, but the surface $\Sigma$ --- bounded by $\gamma$ outside the solenoid --- \emph{passes through} the solenoid's interior, where $F_{\mu\nu} \neq 0$ (the magnetic field is concentrated there).
...
\subsection{Independence of surface choice}
...
\subsection{Independence of gauge}

Different gauge choices $A_\mu \to A_\mu + \partial_\mu \alpha$ change the line integral of $A_\mu$ along open paths (by $\alpha(x_1) - \alpha(x_0)$), but they do \emph{not} change the closed-loop integral: $\oint \partial_\mu \alpha \, dx^\mu = \alpha(x) - \alpha(x) = 0$. The AB phase is gauge-invariant by this elementary argument.
...
\subsection{The AB phase formula}
...
\begin{itemize}[noitemsep]
  \item \emph{Topological}: depends only on the homotopy class of the loop $\gamma$ around the flux region, not on the loop's specific shape.
  \item \emph{Gauge-invariant}: independent of the choice of $A_\mu$ within its gauge equivalence class.
  \item \emph{Universal} (in the sense of multivaluedness): defined modulo $2\pi$, since adding $2\pi$ to $\Delta\varphi$ produces an indistinguishable interference pattern. This is consistent with $\Phi/\Phi_0$ being defined modulo 1, where $\Phi_0 = 2\pi\hbar/q$ is the fundamental flux quantum.
\end{itemize}

\subsection{The flux quantum}

The phase $\Delta\varphi = q\Phi/\hbar$ becomes $2\pi$ when $\Phi = 2\pi\hbar/q$. This is the \emph{flux quantum} $\Phi_0 = 2\pi\hbar/q = h/q$. For an electron ($q = e$), $\Phi_0 = h/e \approx 4.14 \times 10^{-15}$ Wb. For a Cooper pair ($q = 2e$), $\Phi_0 = h/(2e) \approx 2.07 \times 10^{-15}$ Wb --- the famous superconducting flux quantum.

The flux quantum's appearance in the AB phase formula is structurally meaningful: it is the amount of flux that produces a full $2\pi$ phase rotation, equivalent to no observable effect. In a multiply-connected substrate channel, only the flux \emph{modulo} $\Phi_0$ is physically relevant; integer multiples of $\Phi_0$ are invisible to the AB measurement.

\section{The Empirical Setup and Observable Consequences}
...
The interference pattern \emph{does} shift when the solenoid is energized. The shift is sinusoidal in $\Phi$, with one full period corresponding to one flux quantum $\Phi_0$. The functional form is exactly $\Delta\varphi = q\Phi/\hbar$ as predicted.
...
\subsection{Substrate-level reading of the empirics}
...
\subsection{Generalizations and analogues}

The AB structure generalizes beyond the magnetic case:

\begin{itemize}[noitemsep]
  \item \textbf{Electric AB.} A line of electric scalar potential through a region the chain does not access produces the same kind of phase shift, integrated as $\int q \phi \, dt$ along the chain's worldline.
  \item \textbf{Aharonov--Casher.} A neutral particle with magnetic moment moving around a line of charge picks up a phase analogous to AB but with role of charge and moment swapped.
  \item \textbf{Berry phase.} When the chain's parameters undergo a closed cyclic evolution, the geometric phase accumulated is the holonomy of an analogous bundle connection on parameter space.
\end{itemize}
...
\section{The Gauge-Reality Question Resolved}
...
But this leaves a question hanging: \emph{why} should the gauge potential have physical reality? The answer, in standard physics, is roughly ``because experiments show it does.'' This is empirically correct but ontologically unsatisfying.

\subsection{The substrate ontology's answer}
...
\subsection{What the AB phase reveals}
...
These framings are not in conflict. The substrate ontology supplies the structural account of \emph{why} the gauge potential has reality, namely because it is the continuum reading of a primitive substrate object.

\subsection{Gauge-redundancy and physical content}

The puzzle of \emph{gauge redundancy} --- different $A_\mu$ giving the same physics --- is structurally clear in the substrate ontology. Gauge transformations $A_\mu \to A_\mu - \partial_\mu \alpha$ correspond to local rule-type relabelings. Rule-type \emph{labels} are not physical (the substrate's rule-type \emph{content} is what's physical, not the choice of label). Different choices of label correspond to different gauge representations of the same substrate connection.
...
The AB phase formula $\Delta\varphi = q\Phi/\hbar$ is forced as a consequence of T17 (gauge field as substrate connection) plus parallel transport along a non-contractible loop.
...
Substrate primitives (micro-events, chains, bandwidth, polarity, ED gradient, locality, rule-type, substrate channel topology) $\to$ T14 (participation measure form forced) $\to$ U2 (inner product on participation-measure space forced) $\to$ T17 (gauge field as substrate rule-type connection forced) $\to$ DCGT (substrate-to-continuum bridge) $\to$ multiply-connected substrate channels (substrate-level topology) $\to$ parallel transport of participation measure along a path multiplies wavefunction by phase factor $\exp[(iq/\hbar)\int_\gamma A_\mu \, dx^\mu]$ $\to$ closed-loop integral around non-contractible loop is gauge-invariant phase $(q/\hbar)\oint A_\mu \, dx^\mu$ $\to$ Stokes' theorem identifies closed-loop integral with enclosed flux $\to$ AB phase $\Delta\varphi = q\Phi/\hbar$ observable through wavefunction interference.
...
\begin{itemize}[leftmargin=*,itemsep=2pt]
  \item Aharonov, Y., Bohm, D. ``Significance of Electromagnetic Potentials in the Quantum Theory.'' \emph{Physical Review} 115, 485--491 (1959).
  \item Chambers, R. G. ``Shift of an Electron Interference Pattern by Enclosed Magnetic Flux.'' \emph{Physical Review Letters} 5, 3--5 (1960).
  \item Tonomura, A., Osakabe, N., Matsuda, T., et al. ``Evidence for Aharonov--Bohm Effect with Magnetic Field Completely Shielded from Electron Wave.'' \emph{Physical Review Letters} 56, 792--795 (1986).
  \item Aharonov, Y., Casher, A. ``Topological Quantum Effects for Neutral Particles.'' \emph{Physical Review Letters} 53, 319--321 (1984).
  \item Berry, M. V. ``Quantal Phase Factors Accompanying Adiabatic Changes.'' \emph{Proceedings of the Royal Society A} 392, 45--57 (1984).
  \item Wu, T. T., Yang, C. N. ``Concept of Nonintegrable Phase Factors and Global Formulation of Gauge Fields.'' \emph{Physical Review D} 12, 3845--3857 (1975).
  \item Dirac, P. A. M. ``Quantised Singularities in the Electromagnetic Field.'' \emph{Proceedings of the Royal Society A} 133, 60--72 (1931).
  \item Nakahara, M. \emph{Geometry, Topology and Physics.} Institute of Physics Publishing, 2nd edition, 2003.
  ...
  \item Proxmire, A. \emph{ED-I-14: Topological Effects --- Structure of Phase Without Force.} February 2026.
  ...
  \item Peshkin, M., Tonomura, A. \emph{The Aharonov--Bohm Effect.} Springer Lecture Notes in Physics 340, 1989.
\end{itemize}
