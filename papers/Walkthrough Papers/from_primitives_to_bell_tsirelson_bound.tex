...
\hypersetup{hidelinks,
  pdftitle={From Primitives to the Bell-Tsirelson Bound},
  pdfauthor={Allen Proxmire}}

\setlength{\parindent}{0pt}
\setlength{\parskip}{6pt plus 2pt minus 1pt}

\title{\vspace{-1cm}\Large\bfseries From Primitives to the Bell--Tsirelson Bound\\[6pt]
...
In 1980, Boris Tsirelson proved that $2\sqrt{2}$ is not just one quantum prediction --- it is the \emph{maximum} value that any quantum correlation can produce. No quantum state, no measurement scheme, no clever protocol can push $|S|$ beyond $2\sqrt{2}$. The bound is sharp, the value is exact, and it has been measured in laboratories around the world to within experimental precision.
...
The structural shape of this derivation is different from Born or Schr\"odinger. Born had T14's Cauchy argument as a substantive new derivation --- the framework genuinely earns the squared exponent through a primitive-level argument. Schr\"odinger had the Galilean integration producing the factor of $1/(2m)$ --- again a substantive new derivation step. Bell--Tsirelson is different. The framework's contribution here is upstream: it forces the Hilbert-space structure that Tsirelson's argument operates on. The argument itself, including the operator-norm bound that produces $2\sqrt{2}$, is mathematical physics that's already in the literature. What changes is the foundational status of the structures that argument requires.
...
\begin{enumerate}[noitemsep]
  \item The participation measure form is forced by T14, giving the complex-valued structure on which Hermitian operators act.
  \item The inner product is forced by U2, giving the participation-measure space its Hilbert-space structure.
  \item The bipartite tensor-product structure on $H_A \otimes H_B$ is forced by extending U2 to multi-system arrangements --- same primitive-level arguments, applied to the joint participation graph of separated chains.
  \item With the Hilbert-space structure forced, Tsirelson's argument applies: Cauchy--Schwarz on the inner product plus operator-norm bounds on the CHSH operator produces $|S| \leq 2\sqrt{2}$ as the maximum quantum correlation.
\end{enumerate}
...
The framework rests on substrate-level ontological commitments. The Bell--Tsirelson walkthrough uses the same working subset that Born and Schr\"odinger used:

\textbf{Micro-events.} Discrete acts of becoming, vertices in a graph spanning the event manifold.

\textbf{Participation.} The relation connecting micro-events. The participation graph's edges encode this relation. Participation is homogeneous --- no vertex is privileged at the primitive level.

\textbf{Channels.} Stable subgraphs along which a chain can repeatedly instantiate its update rule. Channels are primitive ontological objects --- their identity is intrinsic to the graph, not basis-relative.

\textbf{Bandwidth.} The graded measure of participation, supplied as a non-negative real edge weight. Bandwidth has a four-band orthogonal decomposition with conservation along chains.
...
\section{Forcing the Bipartite Hilbert Space}
...
\subsection{The bipartite participation graph}
...
\subsection{The bipartite inner product}
...
\textbf{Linearity.} $P_{AB}$ is a complex vector space because the joint participation measures take values in $\mathbb{C}$, and complex-valued function spaces inherit complex-vector-space structure pointwise. Same argument as the single-system case.

\textbf{Sesquilinearity.} The diagonal-equals-bandwidth constraint forces the diagonal pointwise structure to be the squared modulus on the joint slot --- same structural fact as the single-system case applied to the joint slot. The $U(1)$ invariance argument eliminates complex-bilinear and reduces real-bilinear to a strictly weaker form, leaving sesquilinearity as the unique structure that survives. Band additivity confirms slot-wise additivity. All four sesquilinearity properties are forced by primitive-level inputs on the joint system.

\textbf{Specific form.} The aggregation arguments transfer:
...
\subsection{The tensor-product structure}
...
\section{The CHSH Operator and the Bell Inequality}
...
\subsection{The CHSH operator in quantum mechanics}
...
\subsection{What the bound has to be}
...
\section{Tsirelson's Bound: The Operator Argument}
...
\subsection{Squaring the CHSH operator}
...
\subsection{Bounding the commutator term}
...
\begin{itemize}[noitemsep]
  \item The bipartite Hilbert space $H_{AB}$ with its tensor-product structure (Section 3).
  \item The inner product $\langle\psi|\hat{S}|\psi\rangle$ that lets us compute expectation values (U2).
  \item The operator norm $\|\hat{A}\|$ on each Hilbert space, which the inner product induces.
  \item The spectral theorem for self-adjoint operators with eigenvalues $\pm 1$, which gives the squaring identity $\hat{A}^2 = I$.
  \item The tensor-product algebra that lets us compute $(\hat{A} \otimes \hat{B})^2 = \hat{A}^2 \otimes \hat{B}^2$ and $\|\hat{A} \otimes \hat{B}\| = \|\hat{A}\| \cdot \|\hat{B}\|$.
\end{itemize}
...
This is a smaller payload than what Born or Schr\"odinger's walkthroughs delivered. Born had T14's Cauchy argument as substantive new structural content --- the framework genuinely earned the squared exponent through a primitive-level argument. Schr\"odinger had the Galilean integration producing the factor of $1/(2m)$ --- again substantive new structural content. Bell--Tsirelson doesn't have a comparable hinge. The framework's contribution here is upstream: it forces the Hilbert-space structure that Tsirelson's argument requires.
...
The Born walkthrough established the probability rule of quantum mechanics from substrate primitives. The Schr\"odinger walkthrough established the dynamical evolution rule. Both involved substantive new derivation steps --- Cauchy on bandwidth additivity in Born, Galilean integration in Schr\"odinger. They demonstrated that the framework can derive structural content rather than just rearranging it.
...
Together, Born, Schr\"odinger, Bell--Tsirelson, and Heisenberg cover all four foundational postulates of non-relativistic single-particle quantum mechanics. With this walkthrough, three of the four are in standalone form. The fourth (Heisenberg) follows the same pattern.

\section{What This Argument Establishes}

The chain runs:

Primitives (micro-events, participation, channels, bandwidth, polarity, commitment) $\to$ T14 (participation measure form forced) $\to$ U2 (inner product on single-system space forced) $\to$ bipartite extension of U2 (tensor-product Hilbert space $H_{AB}$ forced) $\to$ CHSH operator construction on $H_{AB}$ (standard quantum mechanics) $\to$ Tsirelson's argument: $\hat{S}^2 = 4\, I + (\text{commutator term})$, operator-norm bound $2$ on commutators, squared norm bound $8$ on $\hat{S}^2$, square root gives $\|\hat{S}\| \leq 2\sqrt{2}$.
...
Whether the substrate commitments themselves are right is a separate question, and as with Born and Schr\"odinger, it's the load-bearing one. The framework stands or falls on whether participation, bandwidth, channels, polarity, and the rest are the correct foundational concepts. The empirical exposure of the framework lives elsewhere --- in the soft-matter mobility law's prediction of sub-Fickian recovery in concentrated BSA, in the substrate-gravity prediction of MOND's transition acceleration, in other channels where the framework makes predictions that depart from standard physics in regimes not yet tested.
...
Combined with Born and Schr\"odinger, three of the four foundational postulates of non-relativistic single-particle quantum mechanics are now derived. The fourth --- Heisenberg uncertainty --- follows the same pattern and would close the set.

\section*{References}

\begin{itemize}[leftmargin=*,itemsep=2pt]
  \item Bell, J. S. ``On the Einstein Podolsky Rosen Paradox.'' \emph{Physics Physique Fizika} 1, 195--200 (1964).
  \item Clauser, J. F., Horne, M. A., Shimony, A., and Holt, R. A. ``Proposed Experiment to Test Local Hidden-Variable Theories.'' \emph{Physical Review Letters} 23, 880--884 (1969).
  \item Tsirelson, B. S. ``Quantum generalizations of Bell's inequality.'' \emph{Letters in Mathematical Physics} 4, 93--100 (1980).
  \item Aspect, A., Grangier, P., and Roger, G. ``Experimental Tests of Realistic Local Theories via Bell's Theorem.'' \emph{Physical Review Letters} 47, 460--463 (1981).
  \item Hensen, B., et al. ``Loophole-free Bell inequality violation using electron spins separated by 1.3 kilometres.'' \emph{Nature} 526, 682--686 (2015).
