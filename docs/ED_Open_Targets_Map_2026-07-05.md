...
**Tier-1b — canonical-vs-archived structure dependency (found 2026-07-05):**
- **The four-band P04 partition (internal/adjacency/environmental/commitment-reserve) is NON-canonical.** Canonical P04 (`Paper_087`) defines bandwidth as a bare non-negative additive scalar; the four-band split comes from `papers/Generative Papers/paper_M2_primitive_forcing_P04_four_band_partition.md`, **ARCHIVED 2026-05-13** ("do not build new work on its framework"). Yet **Arc M's σ_τ mass functional sums over the four bands** and the **Arc Q Higgs scoping uses b^env/b^adj** — live results with a dependency on a retracted structure. The `position-paper` 13-primitive doc also propagates it as "P04 §1.5." **Check:** does Arc M's four-band σ_τ stand as an independent Arc-M modeling choice, or does it inherit the archived M-2 "forcing"? If the latter, the whole mass/σ_τ sector's primitive grounding needs a rewrite. (Surfaced by a retracted dwell-mass argument; see `theory/Higgs_Emergence/Dwell_Question_Answer.md`.)
