...
4. **Chiral SM matter sector — anomaly cancellation. SCOPED 2026-07-05 (was "untouched").** `Paper_MS-II` names it "the deepest consistency requirement of a chiral gauge theory." Scoping (`theory/Anomaly_Cancellation/Anomaly_As_Spectral_Flow_Scoping.md`) reframed it: **ED's chirality IS the anomaly** — the arrow's non-Hermitian point-gap winding (MS-II §4.2) is spectral flow, and the anomaly *is* spectral flow (Atiyah-Singer); this is also ED's Nielsen-Ninomiya escape. ED already holds both faces of anomaly-freedom: the chirality-flow (§4.2) and B4's **exact integral Gauss law** (= exact gauge-charge conservation = anomaly-free, measured). Real open Q: do they survive together (→ ED would FORCE anomaly-freedom as a *constraint*). **Blocked by gauge-sector Σ-blindness** (same wall as the H1 mass), so it's analytic (V1 kernel `Paper_089`), not a certified-sim build; bounded next step = compute the V1-kernel point-gap winding. **The fermion CONTENT (hypercharges) stays a WALL**, downstream of the open channel-topology→rep classification. Alongside (still open): weak-chirality assignment (ACCOUNT) + the "why 3" linking premise.
