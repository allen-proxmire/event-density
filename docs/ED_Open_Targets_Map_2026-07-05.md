...
- **GRF-Gaussianity — TEST RUN 2026-07-05, verdict NEGATIVE below ~32 cells** (`theory/GRF_Gaussianity/`): the certified deposit field has **white covariance (ξ=1 cell) but scale-invariant skewness** (g1≈−0.9 flat through B=8, z −11 to −15, defying the iid-CLT 1/B decay by ~8×), Gaussianizing only at ~16–32-cell aggregation. The structure is phase-carried, invisible to the power spectrum. The S1 sub-arc's GRF regime hypothesis fails at the filter scales it plausibly uses (ξ_canonical ≈ 1.76 lu); bounded rescue only ≳32 cells. **Gate on fold-back: the lu↔cell unit mapping vs the ed-lab pipeline.** Supplies the structural reason for r*'s already-measured negative R². Positive residue: the cleanest measurement of where layer-2 begins.
