...
*Draft v1, 2026-07-10. Tier: grounded / synthesis (the individual jobs are each at their own section's tier; the claim that it is one arrow is a convergent synthesis). Provenance to verify at finalization: `philosophy_arrow_as_keystone_primitive`, `foundations/PrimitiveReduction_Audit_ArrowAsProcess.md`, `Paper_087` (canonical primitives), `reference_ed_primitive_corrections` (arrow = P11+P13+V1, T18), plus each job's home paper (cited inline as [→ §N]). This is the report's spine: everything after it is the arrow, again.*
