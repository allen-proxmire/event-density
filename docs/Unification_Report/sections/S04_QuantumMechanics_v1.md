...
- The load-bearing residual is the **technical Solèr lattice rigor** — the exchange-property geometry Piron's projective step needs, and the infinite-dimensional subtleties. It rides underneath the whole reconstruction, *including* under the orthogonality reduction (which is proven inside the representation it assumes). This is the one genuinely open joint (#8b), and the report carries it as open.
...
- *Tier discipline: every result stated at the keystone paper's tier (reduced / selected / candidate-grounded / open residual). Do not upgrade "reconstruction" to "derivation" anywhere. The Solèr residual must stay loud — it is the scorecard's ⚠️ for this box.*
