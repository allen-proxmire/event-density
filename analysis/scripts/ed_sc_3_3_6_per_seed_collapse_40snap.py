...
Pre-registered in theory/ED_SC_3_3_6_PerSeedCollapse_40Snapshot.md.

Repeats ED-SC 3.3.5 (per-seed collapse at L_ray_i = 1.08·ξ_i) with
40-snapshot pooling per seed, matching the canonical ξ-measurement
cadence (burn-in 100, snap every 10). Raises per-seed motif count
from ~3 (single-snapshot) to a target ~130–200 per seed so that
per-seed bootstrap S1/S2/S3 are statistically meaningful.
...
Per seed i:
  ...
  N_req       = 4     (canonical)
  snapshots   = 40 per seed (burn-in 100, snap every 10)

Resonance-window exclusions hard-fail at launch if any L_ray_i ∈
[2.50, 2.80] or [3.50, 3.90].

Writes:
  outputs/ed_sc_3_3_6/per_seed_40snap_pools.csv
  outputs/ed_sc_3_3_6/per_seed_40snap_summary.json
...
OUT_DIR = os.path.join(HERE, "..", "..", "outputs", "ed_sc_3_3_6")
...
# ---------------------------------------------------------------------------
# Motif extraction on a single snapshot
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Per-seed evolution + 40-snapshot motif pool at per-seed L_ray
# ---------------------------------------------------------------------------
def evolve_and_collect_40snap(seed, L_ray):
    """Evolve the canonical ED-SIM mobility engine for STEPS=500, recording
    motif-ratio pools at 40 snapshots (burn-in 100, snap every 10). Returns
    (pool_ratios, per_motif_list, N_per_snap, snap_stats_per_snap)."""
    ...
    for step in range(fr.STEPS):
        ...
        if step >= XI_BURN_IN and (step - XI_BURN_IN) % XI_SNAP_EVERY == 0:
            ratios, per_motif = collect_motifs_snapshot(p, L_ray)
            pool.extend(ratios)
            for m in per_motif:
                m["snap_index"] = snap_index
                per_motif_all.append(m)
            N_per_snap.append(len(ratios))
            if len(ratios) >= 2:
                snap_stats.append({
                    "snap": snap_index,
                    "N": len(ratios),
                    "median": float(np.median(ratios)),
                    "iqr": float(np.quantile(ratios, 0.75)
                                  - np.quantile(ratios, 0.25)),
                })
            else:
                snap_stats.append({
                    "snap": snap_index, "N": len(ratios),
                    "median": None, "iqr": None,
                })
            snap_index += 1
    return pool, per_motif_all, N_per_snap, snap_stats
...
def main():
    ...
    # -- 4. Per-seed 40-snapshot motif collection --------------------------
    per_seed_rows = []
    pool_rows = []
    for seed, L_i in seed_L_table:
        ts = time.time()
        pool, per_motif, N_per_snap, snap_stats = evolve_and_collect_40snap(
            seed, L_i)
        bs = bootstrap_median_iqr(pool)
        s3 = tail_log_slope(pool)
        row = {
            "seed": seed,
            "xi_i": xi_per_seed[seed],
            "L_ray_i": L_i,
            "N_i": int(bs["n"]),
            "N_i_per_snap": N_per_snap,
            "N_i_snap_mean": float(np.mean(N_per_snap)) if N_per_snap else None,
            "S1_i": bs["median"],
        ...
        print(f"  seed={seed:>4}  ξ={xi_per_seed[seed]:.4f}  "
              f"L_ray={L_i:.4f}  N={bs['n']:>4}  N̄/snap={row['N_i_snap_mean']:.1f}  "
    ...
    csv_path = os.path.join(OUT_DIR, "per_seed_40snap_pools.csv")
    with open(csv_path, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["seed", "xi_seed", "L_ray_seed", "snap_index",
                    "i", "j", "lam_neg", "lam_pos", "ratio"])
        for row in pool_rows:
            w.writerow([row["seed"], row["xi_seed"], row["L_ray_seed"],
                        row["snap_index"], row["i"], row["j"],
                        row["lam_neg"], row["lam_pos"], row["ratio"]])

    # -- 9. Write master summary JSON --------------------------------------
    out = {
        "method": ("ED-SC 3.3.6 per-seed distributional-collapse test "
                   "with 40-snapshot pooling per seed; per-seed "
                   "L_ray = 1.08·ξ_i; burn-in 100, snap every 10"),
        "simulator_of_record":
            "r2_grf_falsifier_tests.py + ED_Update_Rule.ed_step_mobility",
        ...
        "snapshot_schedule": {
            ...
            "total_steps": fr.STEPS, "snapshots_per_seed": 40,
        },
        "canonical_params": {
    ...
    json_path = os.path.join(OUT_DIR, "per_seed_40snap_summary.json")
