...
OUT = os.path.join(
    ...
    "docs", "figures", "ED-Math-Pipeline_v2.png",
)
...
def build():
    ...
    ax_x, ax_y, ax_w, ax_h = 0.4, 17.55, 21.2, 1.80
    ...
    cg_x, cg_y, cg_w, cg_h = 0.4, 15.25, 21.2, 1.95
    ...
    cr_x, cr_y, cr_w, cr_h = 0.4, 13.05, 21.2, 1.70
    ...
    cn_x, cn_y, cn_w, cn_h = 0.4, 9.20, 21.2, 3.35
    ...
    pgrid = [
        ...
        ("P6", "damping discriminant",
         r"$\Delta = D + 2\zeta; \; D_{\rm crit} = 0.5$ (sharp)"),
        ("P7", "nonlinear triad",
    ...
    # Row 5 — Unified PDE --------------------------------------------------
    pde_x, pde_y, pde_w, pde_h = 2.0, 6.70, 18.0, 2.25
    ...
    forks = [
        ...
        ("sharp bifurcation",
         r"$D + 2\zeta = 1 \Rightarrow D_{\rm crit}=0.5$",
         "ED-09.5 Q-C prediction"),
    ...
    for i, (name, eq, note) in enumerate(forks):
        ...
        txt(ax, xi + cell_w_f / 2, fk_y + fk_h - 0.38, name,
            fontsize=9.3, bold=True, color="#6b2e45")
        txt(ax, xi + cell_w_f / 2, fk_y + fk_h - 0.62, eq, fontsize=9.6)
        txt(ax, xi + cell_w_f / 2, fk_y + 0.14, note, fontsize=8.0,
