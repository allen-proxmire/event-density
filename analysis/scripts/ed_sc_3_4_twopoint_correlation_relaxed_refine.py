"""ED-SC 3.4 F4-alt-refine Two-Point Correlation driver (Δr = 0.25, small-r).

Pre-registered in
  theory/ED_SC_3_4_twopoint_FilterRelaxation_Refine.md        (scope)
  theory/ED_SC_3_4_twopoint_FilterRelaxation_Refine_Driver.md (driver spec)

Verbatim copy of analysis/scripts/ed_sc_3_4_twopoint_correlation_relaxed.py
(F4-alt driver) with exactly four localised changes:

  1. R_GRID replaced with the Δr = 0.25 refinement grid
     [0.25, 0.50, 0.75, 1.00, 1.25, 1.50, 1.75, 2.00].
  2. DR_BIN = 0.25 (edges 0.125 and 2.125 derive automatically).
  3. OUT_DIR redirected to outputs/ed_sc_3_4_twopoint_relaxed_refine/.
  4. Summary JSON adds a top-level `filter_relaxation_refinement`
     block per the driver spec §2.

No other changes — estimators, bootstrap, verdict logic, calibration
pre-pass, motif extraction, min-image distance, and all guards are
inherited verbatim. The six-way refined verdict taxonomy lives in the
integration memo and is applied post-hoc to this driver's raw output.

Simulator of record:
  r2_grf_falsifier_tests.py  +  ED_Update_Rule.ed_step_mobility

Seeds: {11, 22, 33, 44, 55, 66, 77, 88, 99, 111} (matches F2 /
canonical F4 / F4-alt). Filter: α_filt = 0.25, N_req = 2. Canonical
hinge: L_ray = 1.08·ξ_measured. 40-snapshot per seed, burn-in 100,
snap every 10.

Writes:
  outputs/ed_sc_3_4_twopoint_relaxed_refine/correlation_twopoint_table.csv
  outputs/ed_sc_3_4_twopoint_relaxed_refine/correlation_twopoint_per_seed.csv
...
# ---------------------------------------------------------------------------
# Canonical constants (inherited verbatim from F4-alt)
# ---------------------------------------------------------------------------
...
# r-grid for two-point correlation — REFINEMENT (Δr = 0.25 in small-r)
R_GRID = [0.25, 0.50, 0.75, 1.00, 1.25, 1.50, 1.75, 2.00]
DR_BIN = 0.25
R_GRID_MIN_EDGE = R_GRID[0] - DR_BIN / 2   # 0.125
R_GRID_MAX_EDGE = R_GRID[-1] + DR_BIN / 2  # 2.125
...
def main():
    ...
    out = {
        ...
        "filter_relaxation_refinement": {
            ...
            "motivation": (
                "F4-alt C(r) saturated at ~2; half-rise below 1.0 lu"),
            "scope_restriction": (
                "refinement of small-r region only"),
        },
