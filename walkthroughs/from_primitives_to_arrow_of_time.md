...
**Chains.** Stable sequences of commitment events along which a chain's update rule is repeatedly instantiated. Chains are the substrate-level objects the framework calls "particles." Each chain has a worldline $\gamma_K$ parameterized by proper time $\tau_K$.
...
**P11 — Commitment irreversibility.** Every commitment event is irreversible. Once a commitment is made, it is never unmade. There is no "un-commitment" operation in the substrate; commitment events along $\gamma_K$ are ordered forward along proper time, and the ordering admits no reversal.
...
**Relational timing.** Supplies the time axis along which chain proper time $\tau_K$ is parameterized. Combined with P11, this gives chain commitment events a forward-ordered structure: $\tau_K$ increases monotonically along $\gamma_K$, with later commitment events having larger $\tau_K$ than earlier ones.
...
The argument must show that P11 plus other primitives jointly force bandwidth to be forward-only — that the bandwidth content at commitment event $\tau_K$^(n) is determined by the chain's past history (commitment events with $\tau_K$^(m) ≤ $\tau_K$^(n)) and not by its future (commitment events with $\tau_K$^(m) > $\tau_K$^(n)).
...
**Step (i): Commitment events are ordered forward along $\gamma_K$.**
...
$$
\delta b_K(\tau_K^(n)) = \Sigma_{m \leq n} U_K(n, m) \cdot \delta b_K^{\mathrm{source}}(\tau_K^(m))
$$
...
$$
K̃_{\mathrm{vac}}(\omega , k) = c_0 for all \omega , k
$$
...
$$
K_{\mathrm{vac}}(x - x') = K_{\mathrm{vac}}^{\mathrm{prim}}((x - x') / \ell_ED)
$$
...
The Wightman function W(x, x') = $\langle 0|\phi ̂(x)\phi ̂(x')|0\rangle$ is the unperturbed correlator. It measures intrinsic vacuum correlations between points x and x', symmetric under exchange in the standard quantum field theory.
...
Let {$\gamma_K$}_{K=1}^N index the chains in the support of the effective vacuum at spacetime point x. Each chain $\gamma_K$ is a worldline parameterized by its proper time $\tau_K$, with commitment events at indices n = 1, 2, ... and proper-time values $\tau_K$^(n).

A chain perturbation δb_K($\tau_K$^(m)) at commitment index m on chain K produces a perturbation in the chain's bandwidth content at later commitment events n ≥ m via the chain's forward propagator (Section 3):
...
$$
\delta V_{\mathrm{vac}}^(K)(x) = \Sigma_n F_K(x; \tau_K^(n)) \cdot \delta b_K(\tau_K^(n))
$$
...
$$
K_{\mathrm{vac}}(x, x') = \Sigma_K \Sigma_{n \geq m} F_K(x; \tau_K^(n)) \cdot U_K(n, m) \cdot F_K^†(x'; \tau_K^(m))
$$

with the n ≥ m constraint enforced by U_K(n, m) = 0 for n < m.

The question: when does $\tau_K$^(n) ≥ $\tau_K$^(m) correspond to (x - x') being in the forward causal cone?
...
$$
K_{\mathrm{vac}}^{\mathrm{sym}}(x, x') = G(\sigma (x, x') / \ell_ED^{2})
$$
...
$$
K_{\mathrm{vac}}^{\mathrm{adv}}(x, x') = \theta (t' - t) \cdot G(\sigma / \ell_ED^{2})
$$
