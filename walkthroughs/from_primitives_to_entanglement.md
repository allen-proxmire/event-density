...
$$
Seg: P(H_A) \times P(H_B) \to P(H_A \otimes H_B), ([\psi_A], [\psi_B]) \mapsto[\psi_A \otimes \psi_B].
$$
...
$$
|\Psi \rangle= \sum ᵢ \sqrt\lambda ᵢ \cdot|i\rangle_A \otimes|i\rangle_B
$$
...
$$
\Gamma_{\mathrm{\max}}(A) := sup over partitionings of (\sum_k \Gamma_{A \to X_k})
$$

By the substrate-finiteness of $\mathcal{M}(A)$ and bounded V1-kernel coupling per channel,

$$
\Gamma_{\mathrm{\max}}(A) < \infty, \Gamma_{\mathrm{\max}}(A) \propto M(A).
$$
...
$$
Schmidt rank r_AB > 0 \iff \Gamma_AB > 0.
$$
...
$$
C_AB^{2} + C_AC^{2} \leq C_A(BC)^{2}
$$

is the bandwidth additivity (P04) at the probability level:

$$
\Gamma_AB / \Gamma_{\mathrm{\max}}(A) + \Gamma_AC / \Gamma_{\mathrm{\max}}(A) \leq 1,
$$
...
$$
p_B(b_j) = \sum ᵢ p(a_i, b_j) = \langle \Psi|(\sum ᵢ E_i^A \otimes F_j^B)|\Psi \rangle
= \langle \Psi|(I_A \otimes F_j^B)|\Psi \rangle
= Tr_B(F_j^B \cdot \rho_B)
$$
...
$$
S(\rho_A) = -k \cdot \sum ᵢ \lambda ᵢ \log \lambda ᵢ = -k \cdot Tr(\rho_A \log \rho_A)
$$
