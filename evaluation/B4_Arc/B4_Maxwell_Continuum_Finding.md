...
**So CGing ED gives Maxwell (coherent) + entropy (incoherent) — the inverted stack working correctly, not a wall. The earlier "no" summed the field and the entropy and called the sum non-Maxwell — a measurement error.**

**Bigger-box confirmation (L=121, N up to 384):** the coherent field deficit·r² = [0.285, 0.196, 0.169, 0.164, 0.153] at r=[4,8,16,32,48] now **tracks the Mod-B Maxwell reference** [0.301, 0.240, 0.245, 0.190, 0.151] closely (matching at the far edge, 0.153 vs 0.151), and cleaned up smoothly as N grew (N=32 messy → N=384 Coulomb-class). The incoherence (entropy) is **sharply localized at the source** (0.92 at r=4 → 0.14 at r=48). The coherence **plateaus at ~0.67** (stable across N=32/128/384), so the ~33% disorder is a *definite entropy fraction*, not finite-sample noise. What is nailed: **coherent field = the Maxwell field** (they match) and **incoherence = localized entropy**. (Honest caveat: neither the coherent field nor Mod-B reaches the asymptotic 0.126 on this box — both ~0.15 far-field, a finite-box/finite-source tail effect; the *equivalence* is the claim, and it holds.)

And **#3 (diffusion) and #5c (Gaussianity) are not yet re-checked this way** — the strong hypothesis is they flip the same way (signal+disorder summed). Read the rest of this doc (the §Result "no" and the committal-wall framing) as **superseded by this decomposition**, kept for the record.
