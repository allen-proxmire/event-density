"""
invariant_return_map.py
========================
Experiment / Analysis: Invariant Return-Map Structure

Scans all completed regime_D*_A*_Nm* runs, extracts the dominant two
modal amplitudes

    x(t) = |a_1(t)|,   y(t) = |a_2(t)|

and analyses the return-map trajectory (x(t), y(t)) → (x(t+Δ), y(t+Δ))
...
Usage:
    python experiments/invariant_return_map.py
...
STRIDE = 10             # Return-map stride Δ (in output steps)
N_ARROWS = 15           # Number of return-map arrows in Figure A
...
# ---------------------------------------------------------------------------
# Per-run analysis (streaming)
# ---------------------------------------------------------------------------
def analyse_run(run: dict) -> dict:
    """Extract dominant-mode attractor and convergence."""
    modal = run["modal"]
    t = run["t"]

    x = np.abs(modal[:, 1])   # |a_1(t)|
    y = np.abs(modal[:, 2])   # |a_2(t)|
...
def _fit_exponential(t: np.ndarray, arr: np.ndarray,
    ...
    n = len(t)
    start_idx = max(0, int((1.0 - FIT_FRAC) * n))
    t_fit = t[start_idx:]
    residual = np.abs(arr[start_idx:] - target)
...
# ---------------------------------------------------------------------------
# Figure A: Return Map Trajectory (representative run)
# ---------------------------------------------------------------------------
def figure_return_map(runs: list[dict], analyses: list[dict]):
    """Parametric (x(t), y(t)) with return-map arrows."""
    ...
    delta = STRIDE
    if n > 2 * delta:
        # Select evenly spaced arrow origins
        arrow_origins = np.linspace(0, n - delta - 1, N_ARROWS, dtype=int)
        for idx in arrow_origins:
            x0, y0 = x[idx], y[idx]
            x1, y1 = x[idx + delta], y[idx + delta]
            dx = x1 - x0
            dy = y1 - y0
    ...
    ax.plot(ana["x_star"], ana["y_star"], "*", color="#1b7837",
            ...
            label=rf"$(x^*, y^*) = ({ana['x_star']:.3e}, {ana['y_star']:.3e})$")
    ...
    fname = "return_map_trajectory.png"
...
# ---------------------------------------------------------------------------
# Figure B: Attractor Projection Scatter (all runs)
# ---------------------------------------------------------------------------
def figure_attractor_scatter(runs: list[dict], analyses: list[dict]):
    ...
    if len(all_points) > 1:
        ...
        ax.plot(
            ...
            label=rf"Mean $= ({mean_pt[0]:.3e},\;{mean_pt[1]:.3e})$",
        )
...
def print_summary(runs: list[dict], analyses: list[dict]):
    ...
    for label, arr_list in [("x* (|a₁|)", all_x), ("y* (|a₂|)", all_y)]:
...
def main():
    ...
    # --- Analyse all runs ---
    print("\nComputing return-map invariants...")
    analyses = []
    for i, run in enumerate(runs):
        ...
        if (i + 1) % 10 == 0 or i == len(runs) - 1:
            ...
            print(f"  [{i+1}/{len(runs)}] {run['name']}: "
                  f"x*={ana['x_star']:.3e} y*={ana['y_star']:.3e} "
                  f"(conv={c_str})")

    # --- Figures ---
    print("\n--- (A) Return Map Trajectory ---")
    figure_return_map(runs, analyses)

    print("\n--- (B) Attractor Projection Scatter ---")
