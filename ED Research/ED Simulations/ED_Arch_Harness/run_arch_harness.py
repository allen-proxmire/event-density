...
decay_collapsed = sum(1 for r in decay_predicted if r["mechanism_emp"] == "inward-collapse")
decay_preserved = decay_total - decay_collapsed

# Mechanism distribution inside DECAY region
...
# ---------------------------------------------------------------
    ...
    max_dev = max(r["deviation"] for r in records) if records else 0.0
    ...
    if outdir is not None:
        outdir_path = Path(outdir)
        outdir_path.mkdir(parents=True, exist_ok=True)
        with (outdir_path / "law_surface_engine_results.json").open("w", encoding="utf-8") as f:
            json.dump(summary, f, indent=2)

    # 4. Scatter plot
    chi_pred = [r["chi_pred"] for r in records]
    chi_emp = [r["chi_emp"] for r in records]
    gammas = [r["gamma"] for r in records]

    if chi_pred and chi_emp:
        chi_pred_arr = np.array(chi_pred, dtype=float)
        chi_emp_arr = np.array(chi_emp, dtype=float)
        gammas_arr = np.array(gammas, dtype=float)

        plt.figure(figsize=(6, 6))
        sc = plt.scatter(chi_pred_arr, chi_emp_arr, c=gammas_arr, cmap="viridis", s=20, alpha=0.8)
        lims = [
            ...
            serializable.append(
                {
                    ...
                    "max_deviation": float(getattr(r, "max_deviation", 0.0)),
...
def _plot_invariant_report(
    ...
    ids = [getattr(r, "id", "") for r in invariant_results]
...
def _save_harness_report(
    ...
    if law_surface_results is not None:
        lines.append("Law-Surface Validation")
        lines.append("----------------------")
    ...
    if engine_law_results is not None:
        lines.append("Engine-Integrated Law-Surface Validation")
        lines.append("----------------------------------------")
    ...
    # >>> PASTE THE ENGINE-LAW-SURFACE BLOCK HERE <<<

    if invariant_results is not None:
        lines.append("Invariant Suite")
        lines.append("---------------")
    ...
    if invariant_results is not None:
        ...
        for r in invariant_results:
            ...
            passed = bool(getattr(r, "passed", False))
            max_dev = float(getattr(r, "max_deviation", 0.0))
            lines.append(
                f"  {inv_id:8s}  {'PASS' if passed else 'FAIL'}  "
                f"max_dev={max_dev:.3f}  {title}"
            )
        lines.append("")
...
def main() -> None:
    ...
    if args.suite in ("law-surfaces", "all"):
        ...
        # No dedicated plot for DECAY window in Option C; summary is in report/JSON.
    ...
    _save_harness_report(
        ...
        invariant_results=invariant_results,
        engine_law_results=engine_law_results if args.engine_law_surfaces else None,
        outdir=outdir,
    )
